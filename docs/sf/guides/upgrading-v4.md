...
In V.4, these files are read automatically, without the `useDotEnv` property. `useDotenv` can still be set to a path (or array of paths) to load additional `.env` files from a custom location — useful for monorepos. See [Reference Environment Variables](./variables/env-vars.md) for details.

...
- `buildOptions`: The `buildOptions` property can be used to specify additional options to the `docker build` command. (See [Documentation](https://docs.docker.com/engine/reference/commandline/image_build/#options))
...
When `uri` is defined for an image, the parameters above cannot be defined.

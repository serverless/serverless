...
describe('EventBridgeEvents', () => {
  describe('using custom resources deployment pattern', () => {
    ...
    function getEventBridgeConfigById(resourceLogicalId) {
      const eventBridgeId =
        naming.getCustomResourceEventBridgeResourceLogicalId(
          resourceLogicalId,
          1,
        )
    ...
    it('should create the correct policy Statement', () => {
      ...
      expect(firstStatement.Resource['Fn::Join'][1]).to.deep.include(
        'event-bus/*',
      )
    ...
    it('should support inputPath configuration', () => {
      const eventBridgeConfig = getEventBridgeConfigById(
        'inputPathConfiguration',
      )
    ...
    it('should support $or pattern in event pattern', () => {
      ...
      expect(eventBridgeConfig.Pattern.$or[1]).to.deep.equal({
        source: ['aws.cloudformation'],
      })
    })
  ...
  describe('using native CloudFormation', () => {
    describe('when event bus is created as a part of the stack', () => {
      ...
      const getRuleResourceEndingWith = (resources, ending) =>
        Object.values(resources).find(
          (resource) =>
            ...
            resource.Properties.Name.endsWith(ending),
        )
      ...
      it('should correctly set ScheduleExpression on a created rule', () => {
        expect(ruleResource.Properties.ScheduleExpression).to.equal(
          'rate(10 minutes)',
        )
      })
      ...
      it('should correctly set InputPath on the target for the created rule', () => {
        const inputPathRuleResource = getRuleResourceEndingWith(
          cfResources,
          '2',
        )
      ...
      it('should correctly set InputTransformer on the target for the created rule', () => {
        const inputTransformerRuleResource = getRuleResourceEndingWith(
          cfResources,
          '3',
        )
        const inputTransformerRuleTarget =
          inputTransformerRuleResource.Properties.Targets[0]
        expect(
          inputTransformerRuleTarget.InputTransformer.InputPathsMap,
        ).to.deep.equal(inputTransformer.inputPathsMap)
        expect(
          inputTransformerRuleTarget.InputTransformer.InputTemplate,
        ).to.deep.equal(inputTransformer.inputTemplate)
      })

      it('should support retryPolicy configuration', () => {
        const retryPolicyRuleTarget = getRuleResourceEndingWith(
          cfResources,
          '6',
      ...
      it('should support deadLetterQueueArn configuration', () => {
        const deadLetterConfigRuleTarget = getRuleResourceEndingWith(
          cfResources,
          '7',
        ).Properties.Targets[0]
        expect(deadLetterConfigRuleTarget.DeadLetterConfig).to.have.property(
          'Arn',
        )
      })
      ...
      it('should create a rule that references correct function in target', () => {
        expect(ruleTarget.Arn['Fn::GetAtt'][0]).to.equal(
          naming.getLambdaLogicalId('basic'),
        )
      })
      ...
      it('should create a lambda permission resource that correctly references event bus in SourceArn', () => {
        const lambdaPermissionResource =
          cfResources[
            naming.getEventBridgeLambdaPermissionLogicalId('basic', 1)
          ]

        expect(
          lambdaPermissionResource.Properties.SourceArn['Fn::Join'][1][5][
            'Fn::Join'
          ][1][1],
      ...
      it('should correctly set ScheduleExpression on a created rule', () => {
        ...
        expect(descriptionResource.Properties.Description).to.equal(
          'My lambda description',
        )
      })
    })
    ...
    describe('when it references already existing EventBus or uses default one', () => {
      ...
      it('should not create an EventBus if it is provided or default', async () => {
        expect(
          Object.values(cfResources).some(
            (value) => value.Type === 'AWS::Events::EventBus',
          ),
        ).to.be.false
      })
      ...
      it('should create a lambda permission resource that correctly references CF event bus in SourceArn', () => {
        const lambdaPermissionResource =
          cfResources[
            naming.getEventBridgeLambdaPermissionLogicalId('basic', 2)
          ]
      ...
      it('should create a lambda permission resource that correctly references explicit default event bus in SourceArn', () => {
        const lambdaPermissionResource =
          cfResources[
            naming.getEventBridgeLambdaPermissionLogicalId('basic', 3)
          ]
      ...
      it('should create a lambda permission resource that correctly references implicit default event bus in SourceArn', () => {
        const lambdaPermissionResource =
          cfResources[
            naming.getEventBridgeLambdaPermissionLogicalId('basic', 4)
          ]
      ...
      it('should correctly set event name when set', () => {
        const eventBridgeResource =
          cfResources[
            naming.getEventBridgeRuleLogicalId('Eventbridgelambdarule5')
          ]

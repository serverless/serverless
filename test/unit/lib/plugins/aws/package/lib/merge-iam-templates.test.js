  ...
  describe('Default role', () => {
    describe('Defaults', () => {
      ...
      it('should configure LogGroup resources for functions', () => {
        const myFunctionWithRole =
          naming.getLogGroupLogicalId('myFunctionWithRole')
        const myCustomName = cfResources[myFunctionWithRole]

        expect(myCustomName.Type).to.be.equal('AWS::Logs::LogGroup')
        expect(myCustomName.Properties.LogGroupName).to.be.equal(
          '/aws/lambda/myCustomName',
        )
    ...
    describe('Provider properties', () => {
      ...
      it('should support `provider.iam.role.name`', async () => {
        ...
        expect(iamRoleLambdaResource.Properties.RoleName).to.be.eq(
          'custom-default-role',
        )
      })
      ...
      it('should support `provider.iam.role.path`', async () => {
        const iamRoleLambdaResource = cfResources[naming.getRoleLogicalId()]
        expect(iamRoleLambdaResource.Properties.Path).to.be.eq(
          '/custom-role-path/',
        )
      })
      ...
      it('should reject an invalid `provider.iam.role.path`', async () => {
        ...
        await expect(
          ...
          /'provider.iam.role.path': must match pattern/,
        )
      })
            ...
            provider: {
              iam: {
                role: {
                  permissionsBoundary: {
                    'Fn::Sub':
                      'arn:aws:iam::${AWS::AccountId}:policy/XCompanyBoundaries',
                  },
                },
              },
            },
      ...
      it('should support `provider.logRetentionInDays`', () => {
        ...
        expect(iamResource.Properties.LogGroupName).to.be.equal(
          `/aws/lambda/${service}-dev-basic`,
        )
      })
      ...
      it('should support `provider.logDataProtectionPolicy`', () => {
        const normalizedName = naming.getLogGroupLogicalId('basic')
        const iamResource = cfResources[normalizedName]
        expect(iamResource.Properties.DataProtectionPolicy.Name).to.equal(
      ...
      it('should not create default role when `provider.iam.role` defined with CF intrinsic functions', async () => {
        ...
        expect(cfTemplate.Resources[awsNaming.getRoleLogicalId()]).to.be
          .undefined
      })
    })
    ...
    describe('Function properties', () => {
      ...
      it('should support `functions[].disableLogs`', async () => {
        const functionName =
          serverless.service.getFunction('fnDisableLogs').name
      ...
      it('should support `functions[].logRetentionInDays`', async () => {
        const functionName = serverless.service.getFunction(
          'fnLogRetentionInDays',
        ).name
        const normalizedName = naming.getLogGroupLogicalId(
          'fnLogRetentionInDays',
        )
      ...
      it('should support `functions[].logDataProtectionPolicy`', async () => {
        ...
        expect(logResource.Type).to.be.equal('AWS::Logs::LogGroup')
        expect(logResource.Properties.DataProtectionPolicy.Name).to.equal(
          'data-protection-policy',
        )
        expect(logResource.Properties.LogGroupName).to.be.equal(
          naming.getLogGroupName(functionName),
      ...
      it('should not have allow rights to put logs for custom named function when disableLogs option is enabled', async () => {
        expect(
          cfResources[naming.getRoleLogicalId()].Properties.Policies[0]

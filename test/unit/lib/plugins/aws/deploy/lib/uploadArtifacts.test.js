...
describe('uploadArtifacts', () => {
  ...
  beforeEach(() => {
    ...
    awsDeploy.serverless.service.package.deploysDirectoryPrefix = 'somedir';
      ...
      it('should upload the CloudFormation file to the S3 bucket', () => {
        ...
        return awsDeploy.uploadCloudFormationFile().then(() => {
          ...
          expect(awsRequestStub).to.have.been.calledWithExactly('S3', 'upload', {
            ...
            Key: `${awsDeploy.serverless.service.package.deploysDirectoryPrefix}/${awsDeploy.serverless.service.package.timestamp}/compiled-cloudformation-template.json`,
    ...
    describe('#uploadZipFile()', () => {
      ...
      it('should upload the .zip file to the S3 bucket', () => {
        ...
        return awsDeploy.uploadZipFile(artifactFilePath).then(() => {
          ...
          expect(awsRequestStub).to.have.been.calledWithExactly('S3', 'upload', {
            Bucket: awsDeploy.bucketName,
            Key: `${awsDeploy.serverless.service.package.deploysDirectoryPrefix}/${awsDeploy.serverless.service.package.timestamp}/artifact.zip`,
    ...
    describe('#uploadCustomResources()', () => {
      ...
      it('should upload the custom resources .zip file to the S3 bucket', () => {
        ...
        return expect(awsDeploy.uploadCustomResources()).to.eventually.be.fulfilled.then(() => {
          ...
          expect(awsRequestStub).to.have.been.calledWithExactly('S3', 'upload', {
            Bucket: awsDeploy.bucketName,
            Key: `${awsDeploy.serverless.service.package.deploysDirectoryPrefix}/${awsDeploy.serverless.service.package.timestamp}/custom-resources.zip`,
  ...
  describe('when artifactsVersioningMode is hash', () => {
    beforeEach(() => {
      awsDeploy.serverless.service.provider.deploymentBucketObject = {
        artifactsVersioningMode: 'hash',
      };
    });
    ...
    describe('#uploadCloudFormationFile()', () => {
      ...
      it('should upload the CloudFormation file to the S3 bucket with timestamp path', () => {
        cryptoStub.createHash().update().digest.onCall(0).returns('local-hash-cf-template');

        return awsDeploy.uploadCloudFormationFile().then(() => {
          expect(awsRequestStub).to.have.been.calledOnce;
          const [, , { Key: s3Key }] = awsRequestStub.args.pop();
          expect(s3Key).to.equal('somedir/sometimestamp/compiled-cloudformation-template.json');
        });
      });
    });
    ...
    describe('#uploadZipFile()', () => {
      ...
      describe('when file does not exist on the S3', () => {
        ...
        it('should upload the .zip file to the S3 bucket', () => {
          ...
          const expectedKey = `${awsDeploy.serverless.service.package.deploysDirectoryPrefix}/code-artifacts/artifact/${expectedHash}.zip`;
      ...
      describe('when file already exists on the S3', () => {
        ...
        it('should log information about upload already done', () => {
          ...
          sinon.spy(awsDeploy.serverless.cli, 'log');
          return awsDeploy.uploadZipFile(artifactFilePath).then(() => {
            const expected = `Artifact ${expectedKey} already uploaded`;
            expect(awsDeploy.serverless.cli.log.calledWithExactly(expected)).to.be.equal(true);
          });
        });
      });
    ...
    describe('#uploadCustomResources()', () => {
      ...
      it('should upload the custom resources .zip file to the S3 bucket', () => {
        ...
        const expectedKey = `${awsDeploy.serverless.service.package.deploysDirectoryPrefix}/code-artifacts/custom-resources/${expectedHash}.zip`;

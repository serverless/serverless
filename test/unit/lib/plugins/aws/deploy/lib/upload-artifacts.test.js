...
describe('uploadArtifacts', () => {
    ...
    it('should upload the .zip file to the S3 bucket', async () => {
      ...
      return awsDeploy
        .uploadZipFile({
          ...
          s3KeyDirname:
            awsDeploy.serverless.service.package.artifactDirectoryName,
        })
        .then(() => {
          expect(uploadStub).to.have.been.calledOnce
          expect(uploadStub).to.have.been.calledWithExactly('S3', 'upload', {
            ...
            Body: sinon.match.object.and(
              sinon.match.has('path', artifactFilePath),
            ),
  ...
  describe('#uploadCustomResources()', () => {
    ...
    beforeEach(() => {
      ...
      customResourcesFilePath = path.join(
        serviceDirPath,
        '.serverless',
    ...
    it('should not attempt to upload a custom resources if the artifact does not exist', async () => {
      return expect(
        awsDeploy.uploadCustomResources(),
    ...
    it('should upload the custom resources .zip file to the S3 bucket', async () => {
      ...
      return expect(
        ...
        expect(uploadStub).to.have.been.calledWithExactly('S3', 'upload', {
          ...
          Body: sinon.match.object.and(
            sinon.match.has('path', customResourcesFilePath),
          ),
          ContentType: 'application/zip',
          Metadata: {
            filesha256: 'local-hash-zip-file',

...
const programmaticFixturesPath = path.resolve(
  __dirname,
...
describe('test/unit/scripts/serverless.test.js', () => {
  it('should display version when "--version" option', async () => {
    const output = String(
      (await spawn('node', [serverlessPath, '-v'])).stdoutBuffer,
    )
    ...
    const output = String(
      (
        await spawn('node', [serverlessBinPath, '--help'], {
          cwd: (
            await programmaticFixturesEngine.setup(
              'locally-installed-serverless',
            )
          ).servicePath,
        })
      ).stderrBuffer,
    ...
    expect(
      String(
        (
          await spawn('node', [serverlessPath, 'print'], {
            cwd: path.resolve(
              programmaticFixturesPath,
              'multi-service/service-a',
            ),
          })
        ).stdoutBuffer,
      ),
    ...
    expect(
      String(
        (
          await spawn(
            'node',
            [serverlessPath, 'print', '-c', 'serverless.custom.yml'],
            {
              cwd: path.resolve(
                programmaticFixturesPath,
  ...
  it('should support "-c" flag for "aws-service" commands', async () => {
    try {
      await spawn(
        'node',
        [serverlessPath, 'info', '-c', 'serverless.custom.yml'],
        {
      ...
      expect(error.code).to.equal(1)
      expect(String(error.stdoutBuffer)).to.include(
  ...
  it('should load env variables from dotenv files', async () => {
    const { servicePath: serviceDir } = await programmaticFixturesEngine.setup(
      'aws',
      {
        configExt: {
          useDotenv: true,
  ...
  it('should report "env" variables resolution conflicts with exception', async () => {
    ...
    try {
      ...
      expect(error.code).to.equal(1)
      expect(String(error.stdoutBuffer)).to.include(
        'Environment variable "FOO" which',
      )
    }
  })
  ...
  it('should throw meaningful error on unrecognized command for custom provider', async () => {
    try {
      await spawn('node', [serverlessPath, 'foo'], {
        cwd: (await programmaticFixturesEngine.setup('custom-provider'))
          .servicePath,
      })
      throw new Error('Unexpected')
    } catch (error) {
  ...
  it('should print general --help to stdout', async () => {
    const output = String(
      (await spawn('node', [serverlessPath, '--help'])).stdoutBuffer,
    )
    expect(output).to.include('serverless <command> <options>')
  })
  ...
  it('should print command --help to stdout', async () => {
    const output = String(
      (await spawn('node', [serverlessPath, 'deploy', '--help'])).stdoutBuffer,
    )
  ...
  it('should print not integrated command --help to stdout', async () => {
    const output = String(
      (await spawn('node', [serverlessPath, 'plugin', 'install', '--help']))
        .stdoutBuffer,
    )
  ...
  it('should print interactive setup help to stdout', async () => {
    const output = String(
      (await spawn('node', [serverlessPath, '--help-interactive']))
        .stdoutBuffer,
    )
  ...
  it('should crash in required option is missing', async () => {
    try {
      ...
      expect(error.code).to.equal(1)
      expect(String(error.stdoutBuffer)).to.include(
        'command "config credentials" requires',
      )
    }
  })
})

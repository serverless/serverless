...
describe('Serverless Framework Service - Domains REST API Base Path', () => {
  ...
  test('Validate Domain Works', async () => {
    ...
    while (attempt < maxRetries && !domainWorking) {
      ...
      try {
        let normalizedBasePath = basePath ? basePath.replace(/^\//, '') : ''
        if (normalizedBasePath && !normalizedBasePath.endsWith('/')) {
          normalizedBasePath += '/'
        }
        const testUrl = normalizedBasePath
          ? `https://${domainName}/${normalizedBasePath}`
          : `https://${domainName}/`
        console.log(

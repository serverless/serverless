...
describe('Serverless Framework Service - ESBuild Package Individually', () => {
  ...
  test('Validate Functions Packaged Individually', async () => {
    const functionsToCheck = Object.keys(service.functions).map(
      (functionName) => `${service.service}-${functionName}`,
    )
  ...
  test('Validate Functions Run', async () => {
    ...
    for (const functionName of functionsToCheck) {
      ...
      expect(payloadString.errorMessage).toBe(undefined)
    }
  })

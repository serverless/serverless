...
const deployAwsLambdaContainer = async ({
  ...
  const { awsLambdaClient, awsAlbClient, awsEcsClient } = awsClients
  ...
  if (
    ...
    /**
     * For new deployments, if no listener rule exists yet, create a default one so that the target group
     * is bound to the ALB. This prevents the ECS creation error due to an unassociated target group.
     */
    await awsAlbClient.createOrUpdateTargetGroupListenerRule({
      ...
      deprioritize: true, // Deprioritize the rule so it doesn't conflict with a Fargate ECS listener rule.
    })
    ...
    /**
     * Add a permission to the Lambda function to allow the ALB to invoke it.
     * This is necessary because the ALB needs to invoke the Lambda function
     * to route traffic to the container.
     */
    await awsLambdaClient.addPermission(
      functionArn,
      'lambda:InvokeFunction',
      'elasticloadbalancing.amazonaws.com',
      'lambda-alb-permission',
    )

    /**
     * Ensure the Lambda function is registered with the ALB target group.
     */
    await awsAlbClient.ensureLambdaTargetRegistered({
      targetGroupArn:
        state.state.containers[containerName].routing.awsAlb
          .targetGroupArnAwsLambda,
      functionArn,
    ...
    /**
     * Create or update the Lambda ALB listener rule not deprioritized.
     */
    await awsAlbClient.createOrUpdateTargetGroupListenerRule({
      listenerArn:
      ...
      pathPattern: containerConfig.routing?.pathPattern,
      hostHeader: domain ?? undefined,
      deprioritize: false, // Not deprioritized because it's the Lambda rule.
    })

...
export default {
  async ensureValidBucketExists() {
    ...
    try {
      ...
      // If there is a validation error with expected message, it means that
      // logical resource for S3 bucket does not exist and we want to proceed with handling that situation
      if (
    ...
    // By default, we assume that the stack exists because in most cases it does
    // unless the user is using legacy deployment bucket which we handle below
    ...
    mainProgress.notice('Ensuring that deployment bucket exists', {
      isMainEvent: true,
    })

    let getTemplateResult
    try {
      getTemplateResult = await this.provider.request(
        'CloudFormation',
        'getTemplate',
        {
          StackName: stackName,
      ...
      if (e.message.includes('does not exist')) {
        // Set the stackExists flag to false to create the stack below
        stackExists = false
      } else {
        throw e
      }
    ...
    mainProgress.notice('Deploying stack deployment bucket', {
      isMainEvent: true,
    })
    ...
    if (
      ...
      if (stackExists) {
        // If stack exists, we need to update it
        ...
        // If stack does not exist, we need to create it
        const params = this.getCreateStackParams({ templateBody })
      ...
      // Determine the change set type based on whether the stack exists
      const changeSetType = stackExists ? 'UPDATE' : 'CREATE'
      ...
      // Only if stack exists, we need to delete the previous change set
      if (stackExists) {

...
class AwsRollback {
  ...
  async setStackToUpdate() {
    ...
    // AWS Node SDK v3 S3 GetObject returns a stream-like body exposing
    // `transformToString()`; consuming it frees the underlying socket.
    // AWS Node SDK v2 returns Buffer/string. Normalize to string before JSON.parse
    // to support both, without unnecessary buffering on v3.
    let state = {}
    if (stateString) {
      const text =
        stateString && typeof stateString.transformToString === 'function'
          ? await stateString.transformToString()
          : Buffer.isBuffer(stateString)

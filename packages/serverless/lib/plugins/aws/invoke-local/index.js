 ...
 * If a filter function is provided, only files that pass the filter will be extracted.
 * Returns an array of file objects.
 *
 * @param {string} zipPath - The path to the ZIP archive.
 * @param {string} destination - The directory where the archive should be extracted.
 * @param {Object} [options] - Additional options.
 * @param {Function} [options.filter] - A filter function that receives a file object
 * and returns a boolean indicating whether the file should be extracted.
 *
 * @returns {Promise<{ path: string, isDirectory: boolean }[]>}
 * @throws {Error} - If the ZIP archive cannot be read or extracted.
 */
const decompress = async (zipPath, destination, options = {}) => {
  // Ensure the destination directory exists
  await fsp.mkdir(destination, { recursive: true })
  const zip = new AdmZip(zipPath)
  const entries = zip.getEntries()

  // If a filter function is provided, filter the entries BEFORE extraction
  let filteredEntries = entries
  if (typeof options.filter === 'function') {
    filteredEntries = entries.filter((entry) => {
      const fileObj = { path: entry.entryName, isDirectory: entry.isDirectory }
      return options.filter(fileObj)
    })
  }

  // Extract only the filtered entries
  for (const entry of filteredEntries) {
    zip.extractEntryTo(entry, destination, true, true, true)
  }

  // Return an array of file objects mimicking the original decompress output
...
/**
 * Downloads the file at the given URL and extracts its contents
 * into the specified destination directory.
 *
 * @param {string} url - The URL of the ZIP archive.
 * @param {string} destination - The directory where the archive should be extracted.
...
async function downloadAndExtract(url, destination) {
  ...
  // Create an AdmZip instance from the buffer and extract all contents.
  const zip = new AdmZip(buffer)
  zip.extractAllTo(destination, true, true)
}
...
class AwsInvokeLocal {
      ...
      (
        ...
        if (!exists) {
          ...
          if (!cacheExists) {
            ...
            await ensureDir(layerContentsCachePath)
  ...
  async invokeLocalDocker() {
    ...
    const bootstrapPath = path.join(artifactPath, 'bootstrap')
    const hasBootstrap = await fileExists(bootstrapPath)
    // provided.* images expect bootstrap at /var/runtime even though we mount the full artifact at /var/task
    const shouldMountRuntime =
      hasBootstrap && imageUri.startsWith('public.ecr.aws/lambda/provided')
    ...
    const containerName = this.getDockerContainerName(handler)
    const dockerArgs = [
      'run',
      '--rm',
      '-v',
      ...
      .concat(
        shouldMountRuntime
          ? ['-v', `${artifactPath}:/var/runtime:ro,delegated`]
          : [],
      )
    ...
    try {
      ...
      const { stdout } = await execP(`docker port ${containerName}`)
      ...
      if (splitLine.length === 2) {
        ...
        // Capture and display container logs (stdout/stderr from the function)
        // Write to stderr to keep stdout clean for piping the result
        try {
          const { stdout: logs } = await execP(`docker logs ${containerName}`)
          if (logs.trim()) {
            log.aside(logs.trimEnd())
          }
        } catch (logErr) {
        ...
        // Check for Lambda function errors using multiple signals:
        // 1. AWS Lambda error header (most reliable)
        // 2. Non-2xx HTTP status
        // 3. Response body contains errorType field (parsed from JSON)
        const lambdaErrorHeader = res.headers.get('X-Amz-Function-Error')

        if (lambdaErrorHeader) {
          // Lambda returned an error (Unhandled or custom error type)
          process.exitCode = 1
        } else if (!res.ok) {
          // Non-2xx status code
          process.exitCode = 1
        } else {
          // Try to parse JSON and check for errorType field
          try {
            const parsed = JSON.parse(body)
            if (parsed && typeof parsed === 'object' && 'errorType' in parsed) {
              process.exitCode = 1
            }
          } catch {
      ...
      try {
        await execP(`docker stop ${containerName}`)
      } catch (e) {
        /** ignore */
      }
    }
  ...
  getDockerContainerName(handler) {
    return `sls-docker-${this.normalizeDockerContainerName(handler)}`
  }

  normalizeDockerContainerName(value) {
    const sanitized = value
      .replace(/[^a-zA-Z0-9_.-]/g, '-')
      .replace(/^[^a-zA-Z0-9]+/, '')
    return sanitized.length > 0 ? sanitized : 'sls-docker-lambda'
  }
  ...
  async callJavaBridge(artifactPath, className, handlerName, input) {
    ...
    return new Promise((resolve, reject) => {
      ...
      java.child.stdout.on('data', (buf) => {
        writeText(buf.toString())
      })
      java.child.stderr.on('data', (buf) => {
        writeText(buf.toString())
      })
  ...
  async invokeLocalJava(
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        const mvn = spawnExt('mvn', [
          'package',
          '-f',
          path.join(javaBridgePath, 'pom.xml'),
        ])

        mvn.child.stderr.on('data', (buf) => {
          log.info(`mvn(stderr) - ${buf.toString()}`)
        })
        const chunk = []
        mvn.child.stdout.on('data', (buf) => chunk.push(buf))
  ...
  async invokeLocalRuby(runtime, handlerPath, handlerName, event, context) {
    ...
    return new Promise((resolve, reject) => {
      ...
      ruby.child.stdout.on('data', (buf) => {
        writeText(buf.toString())
      })
      ruby.child.stderr.on('data', (buf) => {
        writeText(buf.toString())
      })

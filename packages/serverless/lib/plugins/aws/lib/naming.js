...
export default {
  ...
  getLambdaVersionLogicalId(functionName, sha) {
    return `${this.getNormalizedFunctionName(
      functionName,
    )}LambdaVersion${sha.replace(/[^0-9a-z]/gi, '')}`
  },
  getLambdaCapacityProviderLogicalId(capacityProviderName) {
    return `${this.getNormalizedResourceName(
      capacityProviderName,
    )}LambdaCapacityProvider`
  },
  getLambdaCapacityProviderName(capacityProviderName) {
    return `${this.provider.serverless.service.service}-${this.provider.getStage()}-${capacityProviderName}`
  },
  getLambdaCapacityProviderOperatorRoleLogicalId() {
    return 'LambdaCapacityProviderOperatorRole'
  },
  ...
  getLambdaProvisionedConcurrencyAliasLogicalId(functionName, alias) {
    if (alias === this.getLambdaProvisionedConcurrencyAliasName()) {
      return `${this.getNormalizedFunctionName(functionName)}ProvConcLambdaAlias`
    }

    return `${this.getNormalizedFunctionName(functionName)}ProvConc${this.getNormalizedResourceName(alias)}LambdaAlias`
  },
  ...
  getWebsocketsApiName() {
    if (
      ...
      typeof this.provider.serverless.service.provider.websocketsApiName ===
        'string'
    ) {
  ...
  getWebsocketsAuthorizerLogicalId(functionName) {
    return `${this.getNormalizedAuthorizerName(
      functionName,
  ...
  getMethodLogicalId(resourceId, methodName) {
    return `ApiGatewayMethod${resourceId}${this.normalizeMethodName(
      methodName,
    )}`
  },
  ...
  getUsagePlanKeyLogicalId(usagePlanKeyNumber, usagePlanKeyName) {
    if (usagePlanKeyName) {
      return `ApiGatewayUsagePlanKey${this.normalizeName(
        usagePlanKeyName,
      )}${usagePlanKeyNumber}`
    }
  ...
  getTopicDLQPolicyLogicalId(functionName, topicName) {
    return `${this.normalizeTopicName(
      topicName,
    )}To${this.getNormalizedFunctionName(functionName)}DLQPolicy`
  },
  ...
  getScheduleLogicalId(functionName, scheduleIndex) {
    return `${this.getNormalizedFunctionName(
      functionName,
    )}EventsRuleSchedule${scheduleIndex}`
  },
  getSchedulerScheduleLogicalId(functionName, scheduleIndex) {
    return `${this.getNormalizedFunctionName(
      functionName,
    )}SchedulerSchedule${scheduleIndex}`
  },
  ...
  getStreamLogicalId(functionName, streamType, streamName) {
    return `${this.getNormalizedFunctionName(
      functionName,
    )}EventSourceMapping${this.normalizeName(
      streamType,
  ...
  getIotLogicalId(functionName, iotIndex) {
    return `${this.getNormalizedFunctionName(
      functionName,
    )}IotTopicRule${iotIndex}`
  },
  getLambdaIotPermissionLogicalId(functionName, iotIndex) {
    return `${this.getNormalizedFunctionName(
      functionName,
    )}LambdaPermissionIotTopicRule${iotIndex}`
  },
  ...
  getLambdaIotFleetProvisioningPermissionLogicalId(functionName) {
    return `${this.getNormalizedFunctionName(
      functionName,
    )}LambdaPermissionIotProvisioningTemplate`
  },
  ...
  getMSKEventLogicalId(functionName, clusterName, topicName) {
    ...
    const normalizedClusterName = this.normalizeNameToAlphaNumericOnly(
      clusterName,
    ).slice(0, 79)
    const normalizedTopicName = this.normalizeNameToAlphaNumericOnly(
      topicName,
    ).slice(0, 79)
  ...
  // Kafka
  /**
   * Build a Kafka EventSourceMapping logical ID.
   *
   * Default (legacy): returns a topic-based ID to preserve backward compatibility.
   * When identity details are provided (consumerGroupId and/or bootstrapServers),
   * appends a short, deterministic hash derived from the source identity to avoid collisions.
   *
   * Why: Historically, the ID depended only on the topic name, so multiple self-managed Kafka
   * mappings with the same topic (and often same consumer group) but different clusters
   * produced colliding logical IDs. The identity-based suffix avoids collisions while keeping
   * single-mapping deployments stable.
   *
   * Identity used for hashing: sorted bootstrapServers + topic + consumerGroupId
   *
   * Supported call signatures (backward compatible):
   * - getKafkaEventLogicalId(fn, topic)
   * - getKafkaEventLogicalId(fn, topic, consumerGroupId, bootstrapServers)
   */
  getKafkaEventLogicalId(
    functionName,
    topicName,
    consumerGroupId,
    bootstrapServers,
  ) {
    // Kafka logical ID tuning constants scoped to this function
    const KAFKA_HASH_SUFFIX_LENGTH = 8 // hex chars appended for disambiguation
    const KAFKA_TOPIC_TRIM_LEGACY = 158 // topic slice when no hash is appended
    const KAFKA_TOPIC_TRIM_WITH_HASH = 150 // topic slice when hash is appended

    const normalizedFunctionName = this.getNormalizedFunctionName(functionName)
    // Determine whether to use legacy (topic-only) logical ID
    const shouldUseLegacyId =
      consumerGroupId === undefined && bootstrapServers === undefined

    // TopicName is trimmed to avoid exceeding the 255-character logical ID limit.
    // When appending a hash, leave space for the hash suffix length.
    const maxTopicLen = shouldUseLegacyId
      ? KAFKA_TOPIC_TRIM_LEGACY
      : KAFKA_TOPIC_TRIM_WITH_HASH
    const normalizedTopicName = this.normalizeNameToAlphaNumericOnly(
      topicName,
    ).slice(0, maxTopicLen)
    const baseId = `${normalizedFunctionName}EventSourceMappingKafka${normalizedTopicName}`

    // Legacy behavior when only (fn, topic) are passed
    if (shouldUseLegacyId) return baseId

    const servers = (bootstrapServers || []).slice().sort()
    const identity = {
      topic: topicName,
    ...
    const hash = crypto
      ...
      .digest('hex')
      .slice(0, KAFKA_HASH_SUFFIX_LENGTH)
      .toUpperCase()
    return `${baseId}${hash}`
  },
  ...
  getActiveMQEventLogicalId(functionName, queueName) {
    ...
    const normalizedQueueName = this.normalizeNameToAlphaNumericOnly(
      queueName,
    ).slice(0, 150)
  ...
  generateAlbTargetGroupName(functionName, albId, multiValueHeaders) {
    const hash = crypto
      .createHash('md5')
      .update(
        this.getAlbTargetGroupNameTagValue(
          functionName,
          albId,
          multiValueHeaders,
        ),
  ...
  getLambdaSnsSubscriptionLogicalId(functionName, topicName) {
    return `${this.getNormalizedFunctionName(
      functionName,
    )}SnsSubscription${this.normalizeTopicName(topicName)}`
  },
  ...
  getLambdaFnUrlPermissionLogicalId(functionName) {
    return `${this.getNormalizedFunctionName(
      functionName,
    )}LambdaPermissionFnUrl`
  },
  getLambdaFnUrlInvokeFunctionPermissionLogicalId(functionName) {
    return `${this.getNormalizedFunctionName(
      functionName,
    )}LambdaPermissionFnUrlInvokeFunction`
  },
  getLambdaAuthorizerHttpApiPermissionLogicalId(authorizerName) {
    return `${this.getNormalizedResourceName(
      authorizerName,
    )}LambdaAuthorizerPermissionHttpApi`
  },
  ...
  getLambdaAlexaSmartHomePermissionLogicalId(
    functionName,
    alexaSmartHomeIndex,
  ) {
    return `${this.getNormalizedFunctionName(
  ...
  getLambdaRegisterTargetPermissionLogicalId(functionName) {
    return `${this.getNormalizedFunctionName(
      functionName,
    )}LambdaPermissionRegisterTarget`
  },
  ...
  getCustomResourceS3HandlerFunctionLogicalId() {
    return this.getLambdaLogicalId(
      `${this.getNormalizedFunctionName(
        this.getCustomResourceS3HandlerFunctionName(),
      )}`,
    )
  },
  ...
  getCustomResourceEventBridgeHandlerFunctionLogicalId() {
    return this.getLambdaLogicalId(
      `${this.getNormalizedFunctionName(
        this.getCustomResourceEventBridgeHandlerFunctionName(),
      )}`,
    )
  },
  ...
  getEventBridgeLambdaPermissionLogicalId(functionName, idx) {
    return `${this.getNormalizedFunctionName(
      functionName,
    )}EventBridgeLambdaPermission${idx}`
  },
  ...
  getCloudFrontCachePolicyLogicalId(cachePolicyName) {
    return `CloudFrontCachePolicy${this.getNormalizedFunctionName(
      cachePolicyName,
    )}`
  },

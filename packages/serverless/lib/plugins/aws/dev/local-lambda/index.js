...
class LocalLambda {
  constructor(config = {}) {
    ...
    this.environment = {
      ...
      ...(config.environment || {}),
      /**
       * We don't want to use the AWS_PROFILE environment variable when invoking local lambdas
       * because we get the "Multiple credential sources detected" error locally from the aws sdk
       * and it ends up using the local AWS_PROFILE instead of the lambda env creds
       */

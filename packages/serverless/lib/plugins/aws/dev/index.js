...
class AwsDev {
  ...
  async dev() {
    ...
    // After deploying the shim, clear any package artifacts from the in-memory service config
    // so that local dev builds (dev-build) do not skip building due to pre-set artifacts.
    // This only affects local behavior and does not change the already-deployed shim.
    await this.unsetArtifacts()
  ...
  /**
   * Clears service-level and per-function package artifacts in the in-memory configuration.
   *
   * Dev Mode deploys a shim zip for all functions. After that deploy completes, we clear
   * any `package.artifact` set on the service or functions in memory so that local dev builds
   * (dev-build) do not skip esbuild due to the presence of artifacts.
   *
   * This does not affect the already-deployed shim in AWS and only influences local build behavior.
   *
   * @async
   * @returns {Promise<void>} A promise that resolves once artifacts are cleared in memory.
   */
  async unsetArtifacts() {
    // Clear service-level artifact if present
    if (this.serverless?.service?.package?.artifact) {
      delete this.serverless.service.package.artifact
    }

    // Clear per-function artifacts if present
    const allFunctions = this.serverless.service.getAllFunctions()
    for (const functionName of allFunctions) {
      const functionConfig = this.serverless.service.getFunction(functionName)
      if (functionConfig?.package?.artifact) {
        delete functionConfig.package.artifact
      }
    }
  }

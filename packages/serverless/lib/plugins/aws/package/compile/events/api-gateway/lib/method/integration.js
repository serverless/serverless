...
export default {
  getMethodIntegration(http, { lambdaLogicalId, lambdaAliasName }) {
    ...
    const isResponseStreaming = responseTransferMode === 'STREAM'
    if (type === 'AWS' || type === 'AWS_PROXY') {
      const useStreamingInvokePath = isResponseStreaming && type === 'AWS_PROXY'
      const lambdaApiVersion = useStreamingInvokePath
        ? '2021-11-15'
        : '2015-03-31'
      const invokePath = useStreamingInvokePath
        ...
        Uri: {
          'Fn::Join': [
            ...
            [
              ...
              ':apigateway:',
              { Ref: 'AWS::Region' },
              `:lambda:path/${lambdaApiVersion}/functions/`,
              ...[],
      ...
      const integrationHttpMethod =
        (http.request && http.request.method) || http.method
      Object.assign(integration, {
    ...
    if (
      ...
      Object.assign(integration, {
        RequestParameters: this.getIntegrationRequestParameters(http, type),
      })
    }
    ...
    if (http.response) {
      ...
      if (http.cors) {
        ...
        if (http.cors.allowCredentials) {
          integrationResponseHeaders['Access-Control-Allow-Credentials'] =
            'true'
        }
      }
      ...
      Object.entries(http.response.statusCodes).forEach(
        ([statusCode, config]) => {
          ...
          const integrationResponse = {
            ...
            ResponseTemplates: {},
            ContentHandling: statusCode.startsWith('2')
              ? http.response.contentHandling
              : undefined,
          }
  ...
  getIntegrationRequestTemplates(http, useDefaults) {
    ...
    if (http.request && typeof http.request.template === 'object') {
      Object.entries(http.request.template).forEach(
  ...
  getIntegrationRequestParameters(http, integrationType) {
    const parameters = {}
    if (http.request && http.request.parameters) {
      Object.entries(http.request.parameters).forEach(([key, value]) => {
        if (
          integrationType === 'AWS_PROXY' &&
          key.toLowerCase() !== 'method.request.header.x-amz-tenant-id'
        ) {
          return
        }
        parameters[`integration.${key.substring('method.'.length)}`] =
          value.mappedValue || key
      })
    }

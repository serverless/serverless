...
export default {
        ...
        ;(functionObject.events || []).forEach((event) => {
          if (event.http) {
            ...
            if (http.response && http.response.transferMode) {
              http.response.transferMode = this.getResponseTransferMode(
            ...
            if (
              ...
              if (!http.request || !http.request.uri) {
                ...
                throw new ServerlessError(
                  errorMessage,
                  'API_GATEWAY_MISSING_REQUEST_URI',
                  { stack: false },
                )
              }
              ...
              if (
                ...
                throw new ServerlessError(
                  errorMessage,
                  'API_GATEWAY_MISSING_CONNECTION_ID',
                  { stack: false },
                )
              }
            }
            ...
            if (http.integration === 'AWS' || http.integration === 'HTTP') {
              ...
              if (
                http.integration === 'AWS' &&
                !Object.keys(http.response).length
              ) {
                http.response = {
              ...
              if (http.response) {
                const allowedResponseKeys = ['transferMode']
                const responseKeys = Object.keys(http.response)
                const disallowedResponseKeys = _.difference(
                  responseKeys,
                  allowedResponseKeys,
                )
                if (disallowedResponseKeys.length) {
                  log.warning(
                    [
                      `You're using the ${http.integration} in combination with response`,
                      ` configuration in your function "${functionName}". Only `,
                      allowedResponseKeys
                        .map((value) => `response.${value}`)
                        .join(', '),
                      ' are available in conjunction with proxy integrations.',
                      ' Serverless will remove other response configuration automatically',
                      ' before deployment.',
                    ].join(''),
                  )
                  for (const key of disallowedResponseKeys) {
                    delete http.response[key]
                  }
                }

                const remainingResponseKeys = Object.keys(http.response).filter(
                  (key) => allowedResponseKeys.includes(key),
                )
                if (remainingResponseKeys.length === 0) {
            ...
            if (http.response && http.response.transferMode) {
              const allowedStreamingIntegrations = ['AWS_PROXY', 'HTTP_PROXY']
              if (!allowedStreamingIntegrations.includes(http.integration)) {
                throw new ServerlessError(
                  'Property "response.transferMode" is supported only with AWS_PROXY (Lambda proxy) or HTTP_PROXY integrations.',
                  'API_GATEWAY_STREAMING_UNSUPPORTED_INTEGRATION',
                  { stack: false },
                )
              }
              if (http.response.transferMode === 'STREAM' && http.async) {
                throw new ServerlessError(
                  'Property "response.transferMode: STREAM" cannot be combined with "http.async"',
                  'API_GATEWAY_STREAMING_WITH_ASYNC',
                  { stack: false },
                )
              }
            }
  ...
  getAuthorizer(http) {
    ...
    if (typeof authorizer === 'string') {
      ...
      if (authorizer.type && authorizer.authorizerId) {
        ...
        authorizer.type &&
        authorizer.type.toUpperCase() === 'AWS_IAM'
      ) {
        type = 'AWS_IAM'
      ...
      resultTtlInSeconds = Number.isNaN(resultTtlInSeconds)
        ? 300
        : resultTtlInSeconds
      ...
      managedExternally =
        authorizer.managedExternally == null
          ? false
          : authorizer.managedExternally
    }

    if (authorizerFunctionName) {
      const authorizerFunctionObj = this.serverless.service.getFunction(
    ...
    if (
      ...
      throw new ServerlessError(
        errorMessage,
        'API_GATEWAY_INVALID_COGNITO_CLAIMS',
        { stack: false },
      )
    }
  ...
  getIntegration(http) {
    if (http.integration) {
      ...
      const normalizedIntegration = http.integration
        .toUpperCase()
        .replace('-', '_')
  ...
  getResponseTransferMode(responseTransferMode) {
    if (!responseTransferMode) return undefined
    if (typeof responseTransferMode === 'string') {
      return responseTransferMode.toUpperCase()
    }
  ...
  getResponse(http) {
    if (http.response) {
      ...
      if (response.statusCodes) {
        ...
        if (
          !Object.values(response.statusCodes).some(
            (code) => code.pattern === '',
          )
        ) {

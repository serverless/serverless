#!/usr/bin/env node
/**
 * Test script for the LangGraph Custom Browser agent.
 *
 * Usage:
...
async function invokeAgent(inputText) {
  ...
  if (body && typeof body[Symbol.asyncIterator] === 'function') {
    ...
    try {
      ...
      console.log(typeof parsed === 'string' ? JSON.parse(parsed) : parsed)
    } catch {
      console.log(text)
    }
...
async function main() {
  console.log('LangGraph Custom Browser Agent Test Suite')
  console.log('='.repeat(50) + '\n')

  console.log('Test 1: Browse with custom browser\n')
  await invokeAgent(
    'Browse to https://example.com and tell me the page title and content.',
  )

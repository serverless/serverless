 ...
 * Supports:
 * - ContainerConfiguration (containerImage)
...
export function compileRuntime(name, config, context, tags) {
  ...
  // Check if this is code deployment (entryPoint without s3.bucket or containerImage)
  const artifactConfig = config.artifact || {}
  const isCodeDeployment =
    artifactConfig.entryPoint &&
    !artifactConfig.s3?.bucket &&
    !artifactConfig.containerImage &&
    !artifactConfig.docker

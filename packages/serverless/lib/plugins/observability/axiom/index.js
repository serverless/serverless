...
class Axiom {
  constructor(serverless, options) {
    ...
    if (
      ...
      if (!this.serverless.accessKey || !this.serverless.orgId) {
        throw new ServerlessError(
          'Axiom integration requires organization details, which aren’t available right now. Please try again later.',
  ...
  async addEnvVarsAndLayer() {
    const { axiomToken } = await validateAndGetToken({
      ...
      frameworkVersion: this.serverless.version,
    })

...
import pluginAwsApiGatewayServiceProxy from '../plugins/aws/api-gateway-service-proxy/index.js'
...
import pluginAwsAlerts from '../plugins/aws/alerts/index.js'
...
const bundledPluginDefinitions = [
  {
    module: pluginAwsAlerts,
    externalNames: ['serverless-plugin-aws-alerts'],
    allowCommunityOverride: false,
  },
  {
    module: pluginPythonRequirements,
    externalNames: ['serverless-python-requirements'],
    allowCommunityOverride: true,
    shouldLoad: (serverless, context = {}) =>
      pluginPythonRequirements.shouldLoad
        ? pluginPythonRequirements.shouldLoad({
            serverless,
            ...context,
          })
  ...
  {
    module: pluginAwsApiGatewayServiceProxy,
    externalNames: ['serverless-apigateway-service-proxy'],
    allowCommunityOverride: false,
    shouldLoad: (serverless, context = {}) =>
...
const findBundledPluginByModule = (module) =>
  bundledPluginDefinitions.find(
    ({ module: definitionModule }) => definitionModule === module,
  )

const findBundledPluginByName = (name) =>
  bundledPluginDefinitions.find(({ externalNames = [] }) =>
    externalNames.includes(name),
  )
...
class PluginManager {
  constructor(serverless) {
    ...
    // Holds bundled plugin modules that should be skipped in favour of the community version
    this.bundledPluginOverrides = new Set()
    // Ensures we only warn once per legacy plugin entry during a single run
    this.warnedBundledPluginNames = new Set()
  }
  ...
  logBundledPluginNotice(pluginName) {
    if (this.warnedBundledPluginNames.has(pluginName)) return
    if (!findBundledPluginByName(pluginName)) return
    // Surface a single warning per legacy entry encountered in configuration parsing
    let errorMessage = `The "${pluginName}" plugin is now included by default in the Serverless Framework. You can remove it from the plugins list of your configuration file and from your package.json.`
    if (pluginName === 'serverless-python-requirements') {
      errorMessage +=
        ' Built-in integration activates whenever `custom.pythonRequirements` is present (even `{}`), so keep or add that block.\nIt ships with uv support for fast, repeatable installs.'
      ...
      errorMessage +=
        ' A built-in `serverless offline` command now ships with the Framework (HTTP API, REST, ALB, WebSocket, plus emulated S3 / SQS / SNS / EventBridge / Schedule). To migrate, see https://www.serverless.com/framework/docs/providers/aws/guide/offline. Removing it is optional — while it stays in `plugins:` the community version keeps handling the command.'
    }
    log.warning(errorMessage)
    log.blankLine()
    this.warnedBundledPluginNames.add(pluginName)
  }
  ...
  addPlugin(Plugin) {
    ...
    if (bundledPluginDefinition) {
      // Respect allow/deny policy for bundled plugins before instantiating anything
      if (!bundledPluginDefinition.allowCommunityOverride) {
        return null
      }
      if (!this.servicePluginNames.has(pluginName)) {
        return null
      }
    }
  ...
  async loadAllPlugins(servicePlugins) {
    // Cache the raw plugin names while we orchestrate internal vs external loading
    const pluginsObject = this.parsePluginsObject(servicePlugins)
    this.servicePluginNames = new Set(pluginsObject.modules ?? [])
    this.bundledPluginOverrides = new Set()

    for (const name of this.servicePluginNames) {
      if (findBundledPluginByName(name)) {
        this.logBundledPluginNotice(name)
      }
    }

    // Record which bundled plugins should yield to a community copy this run
    for (const definition of bundledPluginDefinitions) {
      if (!definition.allowCommunityOverride) continue
      if (
        definition.externalNames.some((name) =>
          this.servicePluginNames.has(name),
        )
      ) {
        this.bundledPluginOverrides.add(definition.module)
      }
    }

    // Load Internal Plugins
    isRegisteringExternalPlugins = false
    const bundledLog = log.get('sls:plugins:bundled')
    ...
    internalPlugins
      .filter((Plugin) => {
        ...
        // Skip the internal version when the service opts into a community override
        const shouldSkip = this.bundledPluginOverrides.has(definition.module)
        if (shouldSkip) {
          const legacyName = definition.externalNames?.[0] || Plugin.name
          bundledLog.debug(
            `Skipping bundled plugin "${legacyName}" in favour of community version`,
          )
        }
        if (shouldSkip) return false

        if (definition.shouldLoad) {
          const legacyName = definition.externalNames?.[0] || Plugin.name
          const shouldLoad = definition.shouldLoad(this.serverless, {
            log: bundledLog,
            pluginManager: this,
          })
          if (!shouldLoad) {
            bundledLog.debug(
              `Skipping bundled plugin "${legacyName}" (disabled by configuration)`,
            )
            return false
          }
        }
    ...
    isRegisteringExternalPlugins = true
    const resolvedServicePlugins = await this.resolveServicePlugins(
      servicePlugins,
      pluginsObject,
    )
  ...
  async requireServicePlugin(
    ...
    /**
     * Lastly, try to import the plugin as a TypeScript plugin
     */
    const tsPlugin = this.importTsPlugin(
      path.resolve(serviceDir, pluginListedName),
    )
    ...
    /**
     * If that fails, throw the not found error
     */
    throw new ServerlessError(
  ...
  importTsPlugin(pluginPath) {
    try {
      const configModule = tsxRequire(pluginPath, import.meta.url)
  ...
  async resolveServicePlugins(servicePlugs, preParsedPluginsObject) {
    const pluginsObject =
      preParsedPluginsObject ?? this.parsePluginsObject(servicePlugs)
    const serviceDir = this.serverless.serviceDir

    // Only attempt to resolve community plugins when the mapping says overrides are allowed
    const pluginNames = (pluginsObject.modules || []).filter((name) => {
      const definition = findBundledPluginByName(name)
      if (!definition) return true
      return definition.allowCommunityOverride
    })

...
class Serverless {
  ...
  init() {
    ...
    return this.service.load(this.processedInput.options)
      .then(() => {
        // set the awsProvider of the service (so that the PluginManager takes care to
        // execute the correct awsProvider specific plugins)
  ...
  getProvider(name) {
    if (this.providers[name]) return new this.providers[name](this);
    else return false
  }
}

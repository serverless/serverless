...
class Serverless {
  ...
  _execute(actionQueue, evt, config) {
    ...
    if (!_this._queue.chain) {

      _this._queue.chain = BbPromise.try(function() {
          ...
          // If programmatic and no options object, auto-set options
          if (!_this.cli && typeof evt.options === 'undefined') evt = { options: evt };

          // Init State
          _this.state = new _this.classes.State(_this);

          if (_this.config.projectPath) return _this.state.load();
        })
        .then(function() {

...
const debug = require('debug')('Serverless.js');
...
class Serverless {
  constructor(config) {
    ...
    debug(`Running Serverless v${this.version}`);
    ...
    debug(`Service Path: ${configObject.servicePath}`);
  ...
  init(spinner) {
    ...
    this.cli = new CLI(this, undefined, spinner);

    // get an array of commands and options that should be processed
    this.processedInput = this.cli.processInput();

    debug(`Processed Commands: ${util.inspect(this.processedInput.commands, false, null)}`);
    debug(`Processed Options: ${util.inspect(this.processedInput.options, false, null)}`);

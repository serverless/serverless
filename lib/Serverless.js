...
class Serverless {
  ...
  _setCredentials() {
      ...
      if (process.env.SERVERLESS_ADMIN_AWS_PROFILE && profiles[process.env.SERVERLESS_ADMIN_AWS_PROFILE]) {
        this._awsAdminKeyId = profiles[process.env.SERVERLESS_ADMIN_AWS_PROFILE]['aws_access_key_id'];
        this._awsAdminSecretKey = profiles[process.env.SERVERLESS_ADMIN_AWS_PROFILE]['aws_secret_access_key'];
        if(profiles[process.env.SERVERLESS_ADMIN_AWS_PROFILE]['aws_session_token']) {
          this._awsSessionToken = profiles[process.env.SERVERLESS_ADMIN_AWS_PROFILE]['aws_session_token']; // nodejs (i.e. aws-sdk)
        }
        if(profiles[process.env.SERVERLESS_ADMIN_AWS_PROFILE]['aws_security_token']) {
          this._awsSessionToken = profiles[process.env.SERVERLESS_ADMIN_AWS_PROFILE]['aws_security_token']; // python
        }
      } else {
        ...
        if(process.env.SERVERLESS_ADMIN_AWS_SESSION_TOKEN) {
          this._awsSessionToken = process.env.SERVERLESS_ADMIN_AWS_SESSION_TOKEN;
        }
      }
  }

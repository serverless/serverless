...
class Serverless {
  ...
  _setCredentials() {
    ...
    if (process.env.SERVERLESS_ADMIN_AWS_PROFILE) {
      if(!awsMisc.addProfileCredentials(this.config, process.env.SERVERLESS_ADMIN_AWS_PROFILE)) {
        // this seems a bit strange but trying to avoid changing/breaking the existing logic 
        awsMisc.addEnvironmentCredentials(this.config);
      }
    } else {
      awsMisc.addEnvironmentCredentials(this.config);
    }
  ...
  // TODO: Remove eventually. Many CI/CD systems are using this.
  _init() {
    return this.getProject().load();
  }

...
class Component extends SerializerFileSystem {
  ...
  toObjectPopulated(options) {
    ...
    // Merge templates
    let templates = _.merge(this.getProject().getTemplates().toObject(), this.getTemplates().toObject());
  ...
  fromObject(data) {
    ...
    if (data.templates) {
      let templates = new this._S.classes.Templates(this._S, {_filePath: this.getFilePath('s-template.json')});

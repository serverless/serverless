...
class Component extends SerializerFileSystem {
  ...
  constructor(S, project, name, runtime) {
    super(S);
    this._S        = S;
    this._class    = 'Component';
    this._project  = project;
    this._config.filePath = path.join(this._project.getRootPath(), name, 's-component.json');

    this.setRuntime(runtime || 'nodejs');

    // Default Properties
    this.name = name || 'component' + SUtils.generateShortId(6);
    this.custom = {};
    this.functions = {};
    // this.templates = {};
  ...
  toObjectPopulated(options) {
    ...
    return SUtils.populate(this.getProject().getVariables(), this.getTemplates(), obj, options.stage, options.region);
  }
  ...
  fromObject(data) {
    ...
    if (data.functions) {
      for (let f of Object.keys(data.functions)) {
        let functionClass = new this._S.classes.Function(this._S, this);
        this.setFunction(functionClass.fromObject(data.functions[f]));
      }
    }
    if (data.templates) {
      let templatesClass = new this._S.classes.Templates(this._S);

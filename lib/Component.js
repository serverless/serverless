...
class Component extends SerializerFileSystem {
  ...
  toObjectPopulated(options) {
    ...
    return SUtils.populate(this.getProject(), this.getTemplates(), obj, options.stage, options.region);
  }
  ...
  fromObject(data) {
    ...
    if (data.functions) {
      for (let f of Object.keys(data.functions)) {
        let functionClass = new this._S.classes.Function(this._S, this, data.functions[f], this.getFilePath());
        this.setFunction(functionClass.fromObject(data.functions[f]));
      }
    }

    if (data.templates) {
      this.setTemplates(new this._S.classes.Templates(this._S, data.templates));
    }
  ...
  getTemplates() {
    const projectTemplates   = this.getProject().getTemplates(),
          ownTemplates       = this.templates.toObject();

    return _.merge({}, projectTemplates, ownTemplates);
  }

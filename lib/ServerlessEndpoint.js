...
class ServerlessEndpoint {
  ...
  constructor(Serverless, config) {
    ...
    if (!config.component || !config.module || !config.endpointPath || !config.endpointMethod) throw new SError('Missing required config.component, config.module, config.endpointMethod, config.endpointPath');
  ...
  updateConfig(config) {
    ...
    if (config.component || config.module || config.function || config.endpointPath || config.endpointMethod) {
      ...
      this._config.sPath          = SUtils.buildSPath({
        ...
        function:       this._config.type === 'AWS' ? config.function : '_' + this._config.type,
  ...
  getPopulated(options) {
    ...
    // Return
    return SUtils.populate(_this._S, _this.get(), options.stage, options.region);
  }

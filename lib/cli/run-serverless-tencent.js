...
module.exports = async () => {
  ...
  if (!(await isStandaloneInstalled())) {
    ...
    try {
      ...
      /*
       * Store the binary installation telemetry into `homedir/.serverless-tencent/telemtry-cache`
       * Tencent CLI will automatically send the cache content while deploying
       */
      const storeBinaryTelemetry = async (err = null) => {
        try {
          const cacheAddr = `${os.homedir()}/.serverless-tencent/telemetry-cache`;
          await fse.ensureDir(cacheAddr);
          const payload = {
            event: 'components.command.install',
            timestamp: Date.now(),
            outcome: 'success',
          ...
          if (err) {
            ...
            payload.failure_reason = err.message;
            err.source = 'Serverless::Framework';
            err.step = '命令行安装';
            payload.err = err;
          }
          const id = uuid();
          await fse.writeJson(path.join(cacheAddr, id), {
            payload,
            timestamp: Date.now(),
          });
        } catch (e) {
      ...
      try {
        ...
        await storeBinaryTelemetry();
      } catch (error) {
        await storeBinaryTelemetry(error);

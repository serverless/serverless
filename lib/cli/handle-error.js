...
module.exports = async (exception, options = {}) => {
  ...
  if (!isTelemetryDisabled && hasTelemetryBeenReported === false) {
    ...
    if (commandSchema) {
      ...
      if (telemetryPayload.hasLocalCredentials) {
        // generateTelemetryPayload is explicitly kept as synchronous, but hasLocalCredentials() is asynchronous.
        // Therefore hasLocalCredentials() need be evaluated hered.
        telemetryPayload.hasLocalCredentials = await hasLocalCredentials();
      }

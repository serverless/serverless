...
const resolveEnterprisePluginVersion = async (isLocallyInstalled) => {
  if (isLocallyInstalled) {
    try {
      const pathToPlugin = (await resolve(process.cwd(), '@serverless/enterprise-plugin')).realPath;
      return require(path.join(path.dirname(pathToPlugin), 'package.json')).version;
    } catch {
      // Pass and return global version
    }
  }

  return require('@serverless/enterprise-plugin/package.json').version;
};

const resolveSdkVersion = async (isLocallyInstalled) => {
  if (isLocallyInstalled) {
    try {
      const pathToPlugin = (await resolve(process.cwd(), '@serverless/enterprise-plugin')).realPath;
      return require(pathToPlugin).sdkVersion;
    } catch {
      // Pass and return global version
    }
  }

  return require('@serverless/enterprise-plugin').sdkVersion;
};

const resolveSlsVersion = async (isLocallyInstalled) => {
  if (isLocallyInstalled) {
    try {
      const pathToServelress = await resolveLocalServerlessPath();
      return require(path.resolve(pathToServelress, '../../package.json')).version;
    } catch {
      // Pass and return global version
    }
  }
...
const resolveComponentsVersion = async (isLocallyInstalled) => {
  try {
    if (isLocallyInstalled) {
      try {
        const pathToComponents = (await resolve(process.cwd(), '@serverless/components')).realPath;
        return require(path.join(pathToComponents, 'package.json')).version;
      } catch {
        // Pass and try to return global version
      }
    }
...
module.exports = async (exception, options = {}) => {
  ...
  const slsVersion = await resolveSlsVersion(isLocallyInstalled);
  const sfeVersion = await resolveEnterprisePluginVersion(isLocallyInstalled);
  const sdkVersion = await resolveSdkVersion(isLocallyInstalled);
  const componentsVersion = await resolveComponentsVersion(isLocallyInstalled);

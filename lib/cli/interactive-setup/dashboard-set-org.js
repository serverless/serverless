...
const steps = {
  ...
  setOrgAppAndConsole: async (context, stepData) => {
    const { history, stepHistory, isConsole } = context;
    const { orgNames, isFullyPreconfigured, isDashboard, isMonitoringOverridenByCli } = stepData;
    ...
    if (isMonitoringOverridenByCli && isFullyPreconfigured) {
      const shouldOverrideDashboardConfig = await promptWithHistory({
        message:
          ...
          `to ${chalk.bold(
            `org: ${orgName}}${appName ? `, app: ${appName}` : ''}${
              isConsole ? ', console: true' : ''
            }${isConsole && isDashboard ? ', dashboard: true' : ''}`
          )}`,
        type: 'confirm',
      ...
      if (!shouldOverrideDashboardConfig) {
        ...
        delete context.configuration.dashboard;
        return;
      }
    }
    log.notice();
    ...
    await writeOrgAppAndConsole(orgName, appName, stepData, context);
    return;
  },
};
...
module.exports = {
  async isApplicable(context) {
    ...
    const isFullyPreconfigured =
      (!isConsole || isConsolePreconfigured) &&
      isMonitoringPreconfigured &&
  ...
  async run(context, stepData) {
    ...
    if (!stepData.isDashboard || (!stepData.appName && !stepData.newAppName)) {

...
const { writeOrgAppAndConsole, showOnboardingWelcome } = require('./utils');
const {
...
const steps = {
  ...
  setOrgAppAndConsole: async (
    ...
    await writeOrgAppAndConsole(orgName, appName, isConsole, context);
    return;
  },
};
...
module.exports = {
  async isApplicable(context) {
    ...
    const isConsole = options.console || Boolean(configuration.console && !configuration.dashboard);
    ...
    if (isConsole) {
      ...
      const isConsoleMonitoringPreconfigured = Boolean(configuration.org && configuration.console);
      const isConsoleMonitoringOverridenByCli =
        isConsoleMonitoringPreconfigured &&
      ...
      if (orgName && orgNames.has(orgName)) {
        ...
        if (options.org) {
          if (isConsoleMonitoringPreconfigured && !isConsoleMonitoringOverridenByCli) {
        ...
        return {
          ...
          orgName,
          isConsoleMonitoringPreconfigured,
          isConsoleMonitoringOverridenByCli,
          isConsole,
        };
      } else if (orgName) {
        if (options.org) {
  ...
  async run(context, stepData) {
    ...
    if (
      ...
      (options.console || configuration.console
        ? !context.initial.isConsoleEnabled
    ...
    if (
      !stepData.appName &&
      !stepData.newAppName &&
      (!stepData.isConsole || !configuration.dashboard)
    ) {
      delete configuration.app;
    }
    if (!options.org && !options.app) {
      if (
        stepData.isDashboardMonitoringPreconfigured ||
        stepData.isConsoleMonitoringPreconfigured
      ) {
        if (!stepData.orgName) {
          if (!(await orgUpdateConfirm(stepHistory))) return;
        } else if (stepData.newAppName && stepData.isDashboardMonitoringPreconfigured) {
    ...
    await steps.setOrgAppAndConsole(context, stepData);
  },
  steps,
  configuredQuestions: [
    'orgName',

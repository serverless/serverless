...
const projectTypeChoice = async ({ stepHistory, options }) => {
  const projectType = await promptWithHistory({
    message: 'What do you want to make?',
    type: 'list',
    name: 'projectType',
    choices: initializeProjectChoices.filter(
      // Temporarily filter out Python and SQS projects when console support is intended
      ({ value }) => !options.console || (!value.includes('python') && !value.includes('sqs'))
    ),
    pageSize: 13,
  ...
  async run(context) {
    ...
    if (context.options['template-path']) {
      ...
      if (context.options.template) {
        ...
        projectType = await projectTypeChoice(context);
      ...
      try {
        await downloadTemplateFromRepo(
          templateUrl,
          projectType,
          projectName,

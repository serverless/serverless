...
const doesServiceHaveLinkedProvider = async (orgName, appName, serviceName) => {
  const linkUid = getProviderLinkUid(appName, serviceName);
  ...
  let providers;
  try {
    providers = await sdk.getProvidersByLink(orgUid, linkType, linkUid);
  } catch (err) {
  ...
  return Boolean(providers.result.length);
};
...
module.exports = {
  async isApplicable(context) {
    const { configuration, history } = context;
    if (
    ...
    if (orgName && isUserLoggedIn()) {
      ...
      if (
        providers &&
        !history.has('service') &&
        (await doesServiceHaveLinkedProvider(orgName, configuration.app, configuration.service))
      ) {
        return false;
      }
    }
  ...
  async run(context) {
    ...
    const linked = await steps.linkProviderToService(
      context.configuration.app,
      context.configuration.service,
      context.configuration.org,

...
module.exports = {
  async isApplicable(context) {
    const { configuration, serviceDir, options, initial, devCommand } = context;
    if (!serviceDir) {
    ...
    if (devCommand) {
      context.inapplicabilityReasonCode = 'CONSOLE_DEV_COMMAND';
      return false;
    }
    ...
    if (new AWS.Config().credentials || context.awsAccountId) return true;

...
const jsonColors = {
  colors: {
    ...
    COLON: 'grey.bold',
    COMMA: 'grey',
    STRING_KEY: 'grey.bold',
    STRING_LITERAL: 'grey',
    NUMBER_LITERAL: 'grey',
    BOOLEAN_LITERAL: 'grey',
    NULL_LITERAL: 'grey',
  },
};
    ...
    for (const activity of sortedItems) {
      ...
      switch (activity.type) {
        case 'log':
          ...
          process.stdout.write(headerChalk(`${time} • ${resourceName} • Log\n`));
          try {
            ...
            process.stdout.write(
              chalk.grey.bold(`${activity.body}${activity.body.endsWith('\n') ? '' : '\n'}`)
            );
          }
          break;
        case 'span': {
          ...
          process.stdout.write(
            headerChalk(`${time} • ${resourceName} • Span • ${span.niceName}\n`)
          );
          break;
        }
        case 'aws-lambda-request':
          process.stdout.write(headerChalk(`${time} • ${resourceName} • Invocation Started\n`));
          break;
        case 'aws-lambda-response':
          consoleMonitoringCounter.responses += 1;
          process.stdout.write(headerChalk(`${time} • ${resourceName} • Invocation Ended\n`));
          break;
        case 'event': {
          consoleMonitoringCounter.events += 1;
          const { message, payload } = consoleUi.formatConsoleEvent(activity);
          const headerWriter = /ERROR •/.test(message) ? chalk.hex('#FD5750') : headerChalk;
          const options = /ERROR •/.test(message) ? jsonError : jsonColors;
          process.stdout.write(headerWriter(`${time} • ${resourceName} • ${message}\n`));
  ...
  new Promise((resolve) => {
    ...
    const connectionRefreshTimer = setInterval(async () => {
      const newConnection = await connectToWebSocket({
        functionName: context.targetFunctions,
...
module.exports = {
  async isApplicable(context) {
    const { isConsoleDevMode, shouldSkipDevMode, org } = context;
    ...
    if (shouldSkipDevMode) {
      context.inapplicabilityReasonCode = 'SHOULD_SKIP_DEV_MODE';
      return false;
    }

...
module.exports = {
  async isApplicable(context) {
    const { isConsole, isConsoleDevMode, shouldSkipDevMode } = context;
    ...
    if (isConsoleDevMode && shouldSkipDevMode) {
    ...
    try {
      ...
      const postMessage = isConsoleDevMode ? log.error : log.warning;
      postMessage(
        'Cannot integrate with Serverless Console: ' +
          'AWS account is already integrated with another org. ' +
          'You can use the --aws-profile option to use a different AWS Profile.'
      );
      context.shouldSkipDevMode = true;

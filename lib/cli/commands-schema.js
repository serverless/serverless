...
for (const commandSchema of commands.values()) {
  if (!commandSchema.serviceDependencyMode) continue;
  if (!commandSchema.options) commandSchema.options = {};
  Object.assign(commandSchema.options, serviceOptions);
  if (!commandSchema.hasAwsExtension) continue;
  Object.assign(commandSchema.options, awsServiceOptions);
}

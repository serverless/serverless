  ...
  if (unrecognizedOptions.length) {
    logDeprecation(
      ...
      `Detected unrecognized CLI options: "--${unrecognizedOptions.join('", "--')}".\n` +
        'Starting with the v3 release, Serverless Framework will report them with a thrown error.\nWith the v3.3.0 release, it is possible to replace unsupported options with parameters via CLI with `--param="<key>=<value>"` flag.',

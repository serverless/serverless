...
export default ({ command, options, commandSchema, commandsSchema }) => {
  ...
  if (!commandSchema) {
    ...
    if (command === 'uninstall') {
      errorMessage = 'The "install" command was deprecated in v4.';
    }

    if (command === 'create') {
      errorMessage = `The "create" command was deprecated in v4. To create a new project, you can now just run "serverless" instead.`;
    }
    ...
    if (command === 'config credentials') {
      errorMessage = `The "config credentials" command was deprecated in v4. To configure your AWS credentials, you can now just run "serverless" instead.`;
    }

    if (command === 'output get' || command === 'output list') {
      errorMessage = `This command was deprecated in v4. Please use the "info" command instead.`;
    }
    ...
    if (command === 'param get') {
      errorMessage = `This command was deprecated in v4. To get a param, please visit the Serverless Dashboard at https://app.serverless.com`;
    }

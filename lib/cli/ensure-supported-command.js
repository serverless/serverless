...
module.exports = ({
  ...
  for (const [optionName, optionValue] of Object.entries(options)) {
    ...
    if (schemaCommandOption.type === 'array') {
    ...
    else if (schemaCommandOption.type !== 'array' && typeof optionValue !== schemaCommandOption.type) {
      throw new ServerlessError(
        `Option "${optionName}" is of type "${typeof optionValue}" but expected type "${schemaCommandOption.type}".`,
        'INVALID_OPTION_TYPE'
      );
    }
  }

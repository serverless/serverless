    ...
    return SUtils.getFunctions(cwd, null)
      ...
      .spread(function(functions, endpoints) {
        ...
        functions.forEach(function(func){
          ...
          functionsAndEndponits.push(obj);
        });
        ...
        // Prepare endpoints choices
        let choices = [];
        for (let i = 0; i < functionsAndEndponits.length; i++) {

          let label = "function";
          // if endpoint, change label
          if (functionsAndEndponits[i].type === "endpoint") {
            label = functionsAndEndponits[i].obj.method + ' - ' + functionsAndEndponits[i].obj.path;
          }

          choices.push({
            key:        '  ',
            value:      functionsAndEndponits[i],
            label:      label
          });
        }

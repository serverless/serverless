...
class ServerlessPlugin {
  ...
  cliPromptSelectStage(message, stage, addLocalStage) {
    ...
    let stages = Object.keys(new _this.S.classes.Meta(_this.S).data.private.stages);
  ...
  cliPromptSelectRegion(message, addAllRegions, existing, region, stage) {
    ...
    let _this = this,
        Meta  = new _this.S.classes.Meta(_this.S);
    ...
    if (stage && Object.keys(Meta.data.private.stages[stage].regions).length === 1 && existing) {
      return BbPromise.resolve(Object.keys(Meta.data.private.stages[stage].regions)[0]);
    }
    ...
    if (stage){
      ...
      if (existing) {
        ...
        regionChoices = [];
        Object.keys(Meta.data.private.stages[stage].regions).forEach(function(region) {
          regionChoices.push(region)
        });
      } else {

        // Make sure there are regions left in stage
        if (Object.keys(Meta.data.private.stages[stage].regions).length === 4) {
        ...
        Object.keys(Meta.data.private.stages[stage].regions).forEach(function(regionInStage) {
  ...
  cliPromptSelectComponent(message, component) {
    ...
    let project = new _this.S.classes.Project(_this.S),
        componentChoices = Object.keys(project.data.components),
        choices = [];

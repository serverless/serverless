...
class ServerlessPlugin {
  ...
  cliPromptSelectStage(message, stage, addLocalStage) {
    ...
    let stages = Object.keys(new _this.S.classes.Meta(_this.S).data.stages);
  ...
  cliPromptSelectRegion(message, addAllRegions, existing, region, stage) {
    ...
    if (stage && Object.keys(Meta.data.stages[stage].regions).length === 1 && existing) {
      return BbPromise.resolve(Object.keys(Meta.data.stages[stage].regions)[0]);
    }
    ...
    if (stage){
      ...
      if (existing) {
        ...
        regionChoices = [];
        Object.keys(Meta.data.stages[stage].regions).forEach(function(region) {
          regionChoices.push(region)
        });
      } else {

        // Make sure there are regions left in stage
        if (Object.keys(Meta.data.stages[stage].regions).length === 4) {
        ...
        Object.keys(Meta.data.stages[stage].regions).forEach(function(regionInStage) {

...
describe('AwsLogs', () => {
  ...
  describe('#extendedValidate()', () => {
    ...
    it('it should set default options', () => awsLogs.extendedValidate().then(() => {
      ...
      expect(awsLogs.options.logGroupName).to.deep.equal(naming.getLogGroupName('customName'));
    }));
  });
  ...
  describe('#getLogStreams()', () => {
    beforeEach(() => {
      ...
      awsLogs.options = {
        ...
        region: 'us-east-1',
        function: 'first',
        logGroupName: naming.getLogGroupName('new-service-dev-first'),
      };
    });
      ...
      return awsLogs.getLogStreams()
        .then(logStreamNames => {
          ...
          expect(getLogStreamsStub.args[0][2].logGroupName)
            .to.be.equal(naming.getLogGroupName('new-service-dev-first'));

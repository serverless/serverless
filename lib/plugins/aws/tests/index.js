...
describe('AWS SDK', () => {
  ...
  describe('#getServerlessDeploymentBucketName', () => {
    ...
    it('should validate the region for the given S3 bucket', (done) => {
      const stage = 'test';
      ...
      class FakeS3 {
        ...
        getBucketLocation() {
          return {
            send(cb) {
              cb(undefined, { LocationConstraint: region });
            },
          };
        }
      }
      ...
      awsSdk.serverless.service.provider.bucketName = 'com.serverless.deploys';
      awsSdk.getServerlessDeploymentBucketName(stage, region)
        .then(() => done())
        .catch(done);
    });

    it('should reject an S3 bucket in the wrong region', (done) => {
      const stage = 'test';
      const region = 'us-east-1';
      ...
      class FakeS3 {
        ...
        getBucketLocation() {
          return {
            send(cb) {
              cb(undefined, { LocationConstraint: 'us-west-1' });
            },
          };
        }
      }

...
describe('updateStack', () => {
  ...
  describe('#createFallback()', () => {
    it('should create a stack with the CF template URL', () => {
      const createStackStub = sinon.stub(awsDeploy.provider, 'request').returns(BbPromise.resolve());
  ...
  describe('#update()', () => {
    ...
    beforeEach(() => {
      updateStackStub = sinon.stub(awsDeploy.provider, 'request').returns(BbPromise.resolve());
      sinon.stub(awsDeploy, 'monitorStack').returns(BbPromise.resolve());
    });

...
describe('#naming()', () => {
  ...
  beforeEach(() => {
    ...
    naming = new Naming(serverless);
  });

  it('should use the given Serverless instance', () => {
    expect(naming.serverless).to.equal(serverless);
  });

  it('#normalizeName()', () => {
    it('should capitalize the first letter', () => {
      expect(naming.normalizeName('name')).to.equal('Name');
    });
  ...
  it('#normalizeNameToAlphaNumericOnly()', () => {
    it('should strip non-alpha-numeric characters', () => {
      expect(naming.normalizeNameToAlphaNumericOnly('`!@#$%^&*()-={}|[]\\:";\'<>?,./')).to.be('');
    });
    it('should apply normalizeName to the remaining characters', () => {
      expect(naming.normalizeNameToAlphaNumericOnly('a-b-c')).to.be('Abc');
    });
  });
  it('#normalizeNameToCapitalAlphaNumbericOnly()', () => {
    it('converts `-` to `Dash`', () => {
      expect(naming.normalizePathtoCapitalAlphaNumbericOnlyWithReplacement(
        'a-path'
      )).to.equal('ADashpath');
    });
  ...
  it('#getServiceEndpointRegex()', () => {
    it('should match the prefix', () => {
      expect(naming.getServiceEndpointRegex().match('ServiceEndpoint')).to.be(true);
    });
    it('should not match a name without the prefix', () => {
      expect(naming.getServiceEndpointRegex().match('NotThePrefixServiceEndpoint')).to.be(true);
    });
    it('should match a name with the prefix', () => {
      expect(naming.getServiceEndpointRegex().match('ServiceEndpointForAService')).to.be(true);
    });
  });
  it('#getStackName()', () => {
    it('should use the service name and stage from the service and config', () => {
      serverless.service.service = 'myService';
      expect(naming.getStackName()).to.equal(`${serverless.service.service}-${options.stage}`);
    });
    it('should handle undefined service name', () => {
      serverless.service.service = 'myService';
      expect(naming.getStackName()).to.equal(`${undefined}-${options.stage}`);
    });
    it('should handle undefined stage', () => {
      serverless.service.service = 'myService';
      delete serverless.config.stage;
      expect(naming.getStackName()).to.equal(`${serverless.service.service}-${undefined}`);
    });
  });
  it('#getRolePath()', () => {
    it('should return `/`', () => {
      expect(naming.getRolePath()).to.equal('/');
    });
  });
  it('#getRoleName()', () => {
    it('uses the service name, stage, and region to generate a role name', () => {
      serverless.service.service = 'myService';
      expect(naming.getRoleName()).to.eql({
        'Fn::Join': [
          '-',
          [
  ...
  it('#getLogicalRoleName()', () => {
    it('should return the expected role name (IamRoleLambdaExecution)', () => {
      expect(naming.getLogicalRoleName()).to.equal('IamRoleLambdaExecution');
    });
  });
  it('#getPolicyName()', () => {
  ...
  it('#getLogicalPolicyName()', () => {
    it('should return the expected policy name (IamPolicyLambdaExecution)', () => {
      expect(naming.getLogicalPolicyName()).to.equal('IamPolicyLambdaExecution');
    });
  });
  it('#getLogGroupName()', () => {
    it('should add the function name to the log group name', () => {
      expect(naming.getLogGroupName('functionName')).to.equal('/aws/lambda/functionName');
    });
  });
  it('#getNormalizedLambdaName()', () => {
    it('should normalize the given functionName', () => {
      expect(naming.getNormalizedLambdaName('functionName')).to.equal('FunctionName');
    });
  });
  it('#extractLambdaNameFromArn()', () => {
    it('should extract everything after the last colon and dash', () => {
      ...
      expect(naming.extractLambdaNameFromArn(arn)).to.equal('lambda');
    });
  });
  it('#extractLambdaNameFromArn2()', () => {
    it('should extract everything after the last colon', () => {
      const arn = 'arn:aws:lambda:us-east-1:0123456789:my-dev-lambda';
      expect(naming.extractLambdaNameFromArn(arn)).to.equal('my-dev-lambda');
    });
  });
  it('#getLogicalLambdaName()', () => {
    it('should normalize the function name and add the logical suffix', () => {
      expect(naming.getNormalizedLambdaName('functionName')).to.equal('FunctionNameLambdaFunction');
    });
  });
  it('#getLogicalLambdaNameRegex()', () => {
    it('should match the suffix', () => {
      expect(naming.getLogicalLambdaNameRegex().match('LambdaFunction')).to.be(true);
    });
    it('should not match a name without the suffix', () => {
      expect(naming.getLogicalLambdaNameRegex().match('LambdaFunctionNotTheSuffix')).to.be(false);
    });
    it('should match a name with the suffix', () => {
      expect(naming.getLogicalLambdaNameRegex().match('AFunctionNameLambdaFunction')).to.be(true);
    });
  });
  it('#getLogicalLambdaArnName()', () => {
    it('should normalize the function name and add the logical arn suffix', () => {
      expect(
        naming.getLogicalLambdaArnName('functionName')
      ).to.equal('FunctionNameLambdaFunctionArn');
    });
  });
  it('#getLogicalLambdaArnNameRegex()', () => {
    it('should match the suffix', () => {
      expect(naming.getLogicalLambdaArnNameRegex().match('aLambdaFunctionArn')).to.be(true);
    });
    it('should not match a name without the suffix', () => {
      expect(naming.getLogicalLambdaArnNameRegex()
  ...
  it('#getApiGatewayName()', () => {
    it('should return the composition of stage and service name', () => {
      serverless.service.service = 'myService';
      expect(naming.getApiGatewayName()).to.equal(`${options.stage}-${serverless.service.service}`);
    });
    it('should handle undefined stage', () => {
      serverless.service.service = 'myService';
      delete serverless.config.stage;
      expect(naming.getApiGatewayName()).to.equal(`${undefined}-${serverless.service.service}`);
    });
    it('should handle undefined service name', () => {
      expect(naming.getApiGatewayName()).to.equal(`${options.stage}-${undefined}`);
    });
  });
  it('#getApiGatewayDeploymentId()', () => {
    it('should return ApiGatewayDeployment with a date based suffix', () => {
      expect(naming.getApiGatewayDeploymentId().match(/ApiGatewayDeployment(.*)/).length)
        .to.be.greaterThan(1);
    });
  });
  it('#getLogicalApiGatewayName()', () => {
    it('should return ApiGatewayRestApi', () => {
      expect(naming.getLogicalApiGatewayName()).to.equal('ApiGatewayRestApi');
    });
  });
  it('#getNormalizedAuthorizerName()', () => {
    it('normalize the authorizer name', () => {
      expect(naming.getNormalizedAuthorizerName('authorizerName')).to('AuthorizerName');
    });
  });
  ...
  it('#extractAuthorizerIdFromArn()', () => {
    it('should extract the authorizer name from an ARN', () => {
      const arn = 'arn:aws:lambda:us-east-1:0123456789:my-dev-lambda';
      expect(naming.extractAuthorizerIdFromArn(arn)).to.equal('lambda');
    });
  });
  ...
  it('#getNormalizedApiGatewayResourceName()', () => {
    it('should normalize each part of the resource path and remove non-alpha-numeric characters',
      () => {
        expect(naming.getNormalizedApiGatewayResourceName(
  ...
  it('#getLogicalApiGatewayResourceName()', () => {
    it('should normalize the resource and add the standard suffix', () => {
      expect(naming.getLogicalApiGatewayResourceName(
        'my/path/to/a-${var}-resource'
  ...
  it('#extractResourceId()', () => {
    it('should extract the normalized resource name', () => {
      expect(naming.extractResourceId(
  ...
  it('#getNormalizedApiGatewayMethodName()', () => {
    it('should capitalize the first letter and lowercase any other characters', () => {
      expect(naming.getNormalizedApiGatewayMethodName('gET')).to.equal('Get');
    });
  });
  it('#getLogicalApiGatewayMethodName()', () => {
    it('', () => {
      expect(naming.getLogicalApiGatewayMethodName(
        'ResourceId'
  ...
  it('#getLogicalApiGatewayApiKeyRegex()', () => {
    it('should match the prefix', () => {
      expect(naming.getLogicalApiGatewayApiKeyRegex().match('ApiGatewayApiKey')).to.be(true);
    });
    it('should not match a name without the prefix', () => {
      expect(naming.getLogicalApiGatewayApiKeyRegex()
        .match('NotThePrefixApiGatewayApiKey'))
        .to.be(false);
    });
    it('should match a name with the prefix', () => {
      expect(naming.getLogicalApiGatewayApiKeyRegex().match('ApiGatewayApiKeySuffix')).to.be(true);
    });
  });

  it('#getLogicalDeploymentBucketName()', () => {
    it('should return "ServerlessDeploymentBucket"', () => {
      expect(naming.getLogicalRoleName()).to.equal('ServerlessDeploymentBucket');
    });
  });
  it('#getLogicalDeploymentBucketOutputVariableName()', () => {
    it('should return "ServerlessDeploymentBucketName"', () => {
      expect(naming.getLogicalRoleName()).to.equal('ServerlessDeploymentBucketName');
    });
  });
  it('#getNormalizedBucketName()', () => {
    it('should remove all non-alpha-numeric characters and capitalize the first letter', () => {
      expect(naming.getNormalizedBucketName('b!u@c#k$e%t^N&a*m(e')).to.equal('BucketName');
    });
  });
  it('#getLogicalBucketName()', () => {
    it('should normalize the bucket name and add the standard prefix', () => {
      expect(naming.getLogicalBucketName('b!u@c#k$e%t^N&a*m(e')).to.equal('S3BucketBucketName');
    });
  });
  it('#getNormalizedSnsTopicName()', () => {
    it('should remove all non-alpha-numeric characters and capitalize the first letter', () => {
      expect(naming.getNormalizedSnsTopicName('t!o@p#i$c%N^a&m*e')).to.equal('TopicName');
    });
  });
  it('#getLogicalSnsTopicName()', () => {
    it('should remove all non-alpha-numeric characters and capitalize the first letter', () => {
      expect(naming.getLogicalSnsTopicName('t!o@p#i$c%N^a&m*e')).to.equal('SNSTopicTopicName');
    });
  });
  it('#getCloudWatchEventId()', () => {
    it('should add the standard suffix', () => {
      expect(naming.getCloudWatchEventId('functionName')).to.equal('functionNameSchedule');
    });
  });
  it('#getCloudWatchEventName()', () => {
    it('should normalize the function name and add the standard suffix including the index', () => {
      expect(naming.getCloudWatchEventName('functionName', 0))
  ...
  it('#getLambdaSnsTopicPermissionName()', () => {
    it('should normalize the function and topic names and add them as prefix and suffix to the ' +
      ...
      expect(naming.getLambdaSnsTopicPermissionName('functionName', 'topicName'))
  ...
  it('#getLambdaCloudWatchEventPermissionName()', () => {
    it('should normalize the function name and add the standard suffix including event index',
      () => {
        expect(naming.getLambdaCloudWatchEventPermissionName('functionName', 0))
  ...
  it('#getLambdaApiGatewayPermissionName()', () => {
    it('should normalize the function name and append the standard suffix', () => {
      expect(naming.getLambdaApiGatewayPermissionName('functionName'))

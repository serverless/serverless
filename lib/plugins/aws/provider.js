...
class AwsProvider {
              ...
              anyOf: [
                { $ref: '#/definitions/awsS3BucketName' },
                {
                  ...
                  properties: {
                    ...
                    artifactsVersioningMode: { type: 'string', enum: ['hash'] },
                  },
                  additionalProperties: false,
                },
              ],
  ...
  getCompiledTemplateS3Path() {
    const { deploysDirectoryPrefix, timestamp } = this.serverless.service.package;
    return `${deploysDirectoryPrefix}/${timestamp}/${this.naming.getCompiledTemplateS3Suffix()}`;
  }

  async getServiceArtifactS3Path(artifactFilePath) {
    const { deploysDirectoryPrefix, timestamp } = this.serverless.service.package;
    const { artifactsVersioningMode } =
      (this.serverless.service.provider &&
        this.serverless.service.provider.deploymentBucketObject) ||
      {};
    const { name: fileName, ext: extension } = path.parse(artifactFilePath);

    if (artifactsVersioningMode === 'hash') {
      const hash = await getHashForFilePath(artifactFilePath, 'hex');
      return `${deploysDirectoryPrefix}/code-artifacts/${fileName}/${hash}${extension}`;
    }
    return `${deploysDirectoryPrefix}/${timestamp}/${fileName}${extension}`;
  }
}

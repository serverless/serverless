...
class AwsProvider {
  constructor(serverless, options) {
    ...
    this.hooks = {
      initialize: () => {
        ...
        this.initS3DirectoryPaths(provider);
      },
    };
  ...
  initS3DirectoryPaths(provider) {
    const date = new Date();
    const serviceStage = `${this.serverless.service.service}/${this.getStage()}`;
    const dateString = `${date.getTime().toString()}-${date.toISOString()}`;
    const prefix = this.provider.getDeploymentPrefix();

    provider.s3CodeArtifactsDirectoryPath = `${prefix}/${serviceStage}/code-artifacts`;
    provider.s3DeploymentDirectoryPath = `${prefix}/${serviceStage}/${dateString}`;
  }

  get s3CodeArtifactsDirectoryPath() {
    const { provider } = this.serverless.service;
    return provider.s3CodeArtifactsDirectoryPath;
  }

  get s3DeploymentDirectoryPath() {
    const { provider } = this.serverless.service;
    return provider.s3DeploymentDirectoryPath;
  }

...
import { getOrCreateGlobalDeploymentBucket } from '@serverlessinc/sf-core/src/utils/globalBucket/index.js'
...
class AwsProvider {
  ...
  async getServerlessDeploymentBucketName() {
    ...
    if (this.serverless?.compose?.isWithinCompose || !existingStackBucket) {
      const { globalBucketName } = await getOrCreateGlobalDeploymentBucket({

...
Object.defineProperties(
  ...
  memoizeeMethods({
    ...
    resolveImageUriAndShaFromPath: d(
      async function (imageName, imagePath, imageFilename, buildArgs) {
        ...
        const buildArgsArr = Object.keys(buildArgs).map((key) => [
          '--build-arg',
          `${key}=${buildArgs[key]}`,
        ]);
        const buildArgsFlatArr = [].concat.apply([], buildArgsArr);

...
class AwsProvider {
  ...
  async request(service, method, params, options) {
    ...
    const shouldCache = _get(requestOptions, 'useCache', false);
    ...
    const serviceOptions = {
      ...
      params: {
        ...credentials,
        region: _get(requestOptions, 'region', this.getRegion()),
  ...
  getValues(source, objectPaths) {
    return objectPaths.map((objectPath) => ({
      path: objectPath,
      value: _get(source, objectPath.join('.')),
    }));
  }
  ...
  getCustomDeploymentRole() {
    ...
    return _get(provider, 'iam.deploymentRole', provider.cfnRole);
  }
  ...
  getCustomExecutionRole(functionObj) {
    ...
    const role = _get(provider, 'iam.role');
  ...
  memoizeeMethods({
    ...
    resolveImageUriAndSha: d(
      async function (functionName) {
        ...
        const imageDefinedInProvider = _get(
          this.serverless.service.provider,
          `ecr.images.${imageName}`
        );

        const imageScanDefinedInProvider = _get(
          this.serverless.service.provider,
          'ecr.scanOnPush',
          defaultScanOnPush
        );

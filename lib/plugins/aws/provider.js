...
class AwsProvider {
  ...
  async getServerlessDeploymentBucketName() {
    ...
    const bucketName = this.chooseBucket({
      isWithinCompose: this.serverless?.compose?.isWithinCompose,
      globalBucket: this.serverless?.globalDeploymentBucketName,
      providedBucket: this.serverless.service.provider.deploymentBucket,
      stackBucket: existingStackBucket,
    })

    return { existingStackBucket, bucketToUse: bucketName }
  }

  chooseBucket({ isWithinCompose, globalBucket, providedBucket, stackBucket }) {
    if (!isWithinCompose && stackBucket) {
      return providedBucket || stackBucket
    }
    if (isWithinCompose || (!isWithinCompose && !stackBucket)) {
      return providedBucket || globalBucket
    }
    if (providedBucket) {

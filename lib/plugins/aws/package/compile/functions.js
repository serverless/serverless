...
class AwsCompileFunctions {
  ...
  async compileFunction(functionName) {
    ...
    if (functionObject.handler) {
      ...
      const serverlessPackagedArtifact = artifactFilePath.split(path.sep).indexOf('.serverless');

      // Check if it's not a custom artifact
      if (serverlessPackagedArtifact > -1) {
        functionResource.Properties.Code.S3Key = `${
          this.serverless.service.package.artifactDirectoryName
        }/${artifactFilePath
          .split(path.sep)
          .splice(-1, `${serverlessPackagedArtifact}`)
          .join(path.sep)}`;
      } else {
        functionResource.Properties.Code.S3Key = `${this.serverless.service.package.artifactDirectoryName}/${artifactFilePath}`;
      }

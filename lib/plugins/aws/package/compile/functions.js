...
class AwsCompileFunctions {
  constructor(serverless, options, s3client) {
      ...
      'initialize': () => {
        if (
          ...
          this.serverless._logDeprecation(
            ...
            'Resolution of lambda version hashes with "20200924" algorithm is deprecated and it is highly recommend to migrate to new default algorithm. Please see the deprecation documentation for more details about the migration process.'
          );
        }
  ...
  async downloadPackageArtifact(functionName) {
    ...
    const body = (await this.s3client.send(new GetObjectCommand(s3Object))).Body;
    const readStream = stream.Readable.from(body);

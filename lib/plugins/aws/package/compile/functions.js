...
class AwsCompileFunctions {
  constructor(serverless, options) {
    ...
    this.hooks = {
      ...
      'package:compileFunctions': async () => {
        ...
        await this.downloadPackageArtifacts();
  ...
  setupRole() {
    ...
    this.serverless.service.getAllFunctions().forEach((functionName) => {
      ...
      const { name: resolvedFunctionName, disableLogs: areLogsDisabled } = functionObject;
      ...
      if (!areLogsDisabled) {
        ...
        const policyStatements = this.cfIamRolePolicyStatementsLogs();
        policyStatements[0].Resource.push({
          'Fn::Sub':
            'arn:${AWS::Partition}:logs:${AWS::Region}:${AWS::AccountId}' +
            `:log-group:${customFunctionNamelogGroupsPrefix}:*`,
        });
        policyStatements[1].Resource.push({
          'Fn::Sub':
            'arn:${AWS::Partition}:logs:${AWS::Region}:${AWS::AccountId}' +
            `:log-group:${customFunctionNamelogGroupsPrefix}:*:*`,
        });
        functionObject.iamConfig.policyStatements.push(...policyStatements);
      }
    });
    ...
    if (hasOneOrMoreCanonicallyNamedFunctions) {
      ...
      this.provider.iamConfig.policyStatements.push(...policyStatements);
    ...
    Object.values(this.serverless.service.functions).forEach((func) => {
      if (!func.vpc) {
        return;
      }

      // add managed iam policy to allow ENI management
      func.iamConfig.managedPolicies.push({
        'Fn::Join': [
          '',
          [
            'arn:',
  ...
  async compileFunction(functionName) {
    ...
    if (functionObject.onError) {
      ...
      if (typeof arn === 'string') {
        ...
        // Collect the PolicyDocument statements (if default policy is used)
    ...
    if (destinations) {
      ...
      if (destinations.onSuccess) {
        if (!hasAccessPoliciesHandledExternally) {
          this.ensureTargetExecutionPermission(destinations.onSuccess, { functionObject });
        }
      ...
      if (destinations.onFailure) {
        if (!hasAccessPoliciesHandledExternally) {
          this.ensureTargetExecutionPermission(destinations.onFailure, { functionObject });
        }
  ...
  ensureTargetExecutionPermission(functionAddress, { functionObject }) {
    const action = (() => {

...
class AwsCompileFunctions {
  ...
  async compileFunction(functionName) {
    ...
    if (functionObject.handler) {
      ...
      const runtimeManagement = this.provider.getRuntimeManagement(
        functionObject.runtimeManagement
      );
      let rawRuntimeManagement = runtimeManagement;
      let convertedRuntimeManagement;

      if (typeof runtimeManagement === 'object') {
        rawRuntimeManagement = runtimeManagement.mode;
      }

      if (rawRuntimeManagement === 'onFunctionUpdate') {
        convertedRuntimeManagement = 'Function update';
      } else {
        convertedRuntimeManagement =
          rawRuntimeManagement[0].toUpperCase() + rawRuntimeManagement.substr(1);
      }

      functionResource.Properties.RuntimeManagementConfig = {
        UpdateRuntimeOn: convertedRuntimeManagement,
      };
      if (rawRuntimeManagement === 'manual') {

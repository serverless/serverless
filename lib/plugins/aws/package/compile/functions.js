...
class AwsCompileFunctions {
    ...
    if (destinations) {
      ...
      if (destinations.onSuccess) {
        destinationConfig.OnSuccess = {
          Destination: this.getDesinationsArn(destinations.onSuccess),
        };
      ...
      if (destinations.onFailure) {
        destinationConfig.OnFailure = {
          Destination: this.getDesinationsArn(destinations.onFailure),
        };
  ...
  getDesinationsArn(destinationsProperty) {
    if (typeof destinationsProperty === 'object') {
      if (!destinationsProperty.arn) {
        throw new ServerlessError(
          'Missing required "arn" property in destinations.onSuccess(or onFailure). (needed if destinations.onSuccess(or onFailure) is not provided as plain ARN string)',
          'DESTINATIONS_MISSING_ARN'
        );
      }
  ...
  ensureTargetExecutionPermission(functionAddress) {
    ...
    const action = (() => {
      if (typeof functionAddress === 'object') {
        if (!functionAddress.type) {
          throw new ServerlessError(
            'Missing required "type" property in destinations.onSuccess(or onFailure). (needed if "destinations.onSuccess(or onFailure).arn" is provided.)',
            'DESTINATIONS_MISSING_TYPE'
          );
        }
        if (functionAddress.type === 'function') return 'lambda:InvokeFunction';
        if (functionAddress.type === 'sqs') return 'sqs:SendMessage';
        if (functionAddress.type === 'sns') return 'sns:Publish';
        if (functionAddress.type === 'eventBus') return 'events:PutEvents';
      }
    ...
    let ResourceArn;
    if (typeof functionAddress === 'object') {
      if (!functionAddress.arn) {
        throw new ServerlessError(
          'Missing required "arn" property in destinations.onSuccess(or onFailure). (needed if destinations.onSuccess(or onFailure) is not provided as plain ARN string)',
          'DESTINATIONS_MISSING_ARN'
        );
      }
      ResourceArn = functionAddress.arn;
    } else {
      ...
      ResourceArn = functionAddress.startsWith('arn:')
        ? functionAddress
        : {

...
class AwsCompileFunctions {
    ...
    if (destinations) {
      ...
      if (destinations.onSuccess) {
        ...
        destinationConfig.OnSuccess = {
          Destination:
            (typeof destinations.onSuccess === 'string' &&
              destinations.onSuccess.startsWith('arn:')) ||
            (typeof destinations.onSuccess === 'object' && 'Ref' in destinations.onSuccess)
      ...
      if (destinations.onFailure) {
        ...
        destinationConfig.OnFailure = {
          Destination:
            (typeof destinations.onFailure === 'string' &&
              destinations.onFailure.startsWith('arn:')) ||
            (typeof destinations.onFailure === 'object' && 'Ref' in destinations.onFailure)
  ...
  ensureTargetExecutionPermission(functionAddress) {
    ...
    const action = (() => {
      ...
      if (typeof functionAddress === 'object' && 'Ref' in functionAddress) {
        const type = this.serverless.service.resources.Resources[functionAddress.Ref].Type;
        if (type.includes('Function')) return 'lambda:InvokeFunction';
        if (type.includes('SQS')) return 'sqs:SendMessage';
        if (type.includes('SNS')) return 'sns:Publish';
        if (type.includes('EventBus')) return 'events:PutEvents';
      }
      throw new ServerlessError(
    ...
    iamPolicyStatements.push({
      ...
      Resource:
        (typeof functionAddress === 'string' && functionAddress.startsWith('arn:')) ||
        (typeof functionAddress === 'object' && 'Ref' in functionAddress)
          ? functionAddress
          : {

...
class AwsCompileFunctions {
  ...
  async compileFunction(functionName) {
    ...
    if (functionObject.handler) {
      ...
      if (artifactFilePath.includes(`.serverless${path.sep}`)) {
        functionResource.Properties.Code.S3Key = `${
          this.serverless.service.package.artifactDirectoryName
        }/${artifactFilePath.split(`.serverless${path.sep}`).pop()}`;
      } else {
        functionResource.Properties.Code.S3Key = `${
          this.serverless.service.package.artifactDirectoryName
        }/${artifactFilePath.split(path.sep).replace(path.parse(artifactFilePath).root, '')}`;
      }

  ...
  async compileFunction(functionName) {
    ...
    if (
      ...
      if (functionObject.provisionedConcurrency && functionObject.snapStart) {
        throw new ServerlessError(
          `Either "provisionedConcurrency" or "snapStart" property can be enabled on function "${functionName}".`,
      ...
      if (functionObject.snapStart !== undefined) {
        if (!shouldVersionFunction) delete versionResource.DeletionPolicy;

        const snapStartConfig = {
          ApplyOn: functionObject.snapStart ? 'PublishedVersions' : 'None',
        };

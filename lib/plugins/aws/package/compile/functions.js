...
class AwsCompileFunctions {
  ...
  async compileFunction(functionName) {
    ...
    let snapStartEnabled = false;
    if (functionObject.snapStart) {
      const snapStartConfig = {};
      if (_.isObject(functionObject.snapStart)) {
        if (functionObject.snapStart.applyOn) {
          snapStartConfig.ApplyOn = functionObject.snapStart.applyOn;
          if (snapStartConfig.ApplyOn === 'PublishedVersions') {
            snapStartEnabled = true;
          }
        }
      }
      functionResource.Properties.SnapStart = snapStartConfig;
    }

    if (shouldVersionFunction || functionObject.provisionedConcurrency || snapStartEnabled) {

...
describe('resolveDependsOn', () => {
  ...
  it('with ARN', () => {
    const funcRole = 'arn:aws:iam::xxxxxxxxxxxx:role/xxxx';
    const result = awsCompileMSKEvents.resolveDependsOn(funcRole);
  ...
  it('with string', () => {
    const funcRole = 'role';
    const result = awsCompileMSKEvents.resolveDependsOn(funcRole);
    expect(result).to.equal(funcRole);
  });

  it('with Fn::ImportValue', () => {
    const funcRole = { 'Fn::ImportValue': 'importvalue' };
    const result = awsCompileMSKEvents.resolveDependsOn(funcRole);
    expect(result).to.deep.equal([]);
  });

  it('with Ref', () => {
    const funcRole = { Ref: 'ReferencedResource' };
    const result = awsCompileMSKEvents.resolveDependsOn(funcRole);
    expect(result).to.deep.equal([]);
  });

  it('with Fn::GetAtt reference to role ARN', () => {
    const funcRole = { 'Fn::GetAtt': ['Resource', 'Arn'] };
    const result = awsCompileMSKEvents.resolveDependsOn(funcRole);

...
class HttpApiEvents {
  ...
  compileStage() {
    ...
    if (this.config.throttle.burstLimit) {
      properties.DefaultRouteSettings.ThrottlingBurstLimit = this.config.throttle.burstLimit;
    }

    if (this.config.throttle.rateLimit) {
      properties.DefaultRouteSettings.ThrottlingRateLimit = this.config.throttle.rateLimit;
    }
  ...
  memoizeeMethods({
    resolveConfiguration: d(function() {
      ...
      this.config = {
        ...
        throttle: Object.assign({ burstLimit: -1, rateLimit: -1 }, userConfig.throttle),
      };
      let cors = null;

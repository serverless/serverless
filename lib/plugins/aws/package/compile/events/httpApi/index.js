...
const resolveTargetConfig = memoizee(({
  functionLogicalId,
  functionAlias
}) => {
  const functionArnGetter = {
    'Fn::GetAtt': [functionLogicalId, 'Arn']
  };
  if (!functionAlias) return functionArnGetter;
  return {
    'Fn::Join': [':', [functionArnGetter, functionAlias.name]]
  };
});
    ...
    resolveConfiguration: d(function () {
      ...
      if (userConfig.timeout) {
        this.serverless._logDeprecation(
          ...
          'provider.httpApi.timeout is deprecated.HTTP API endpoints are configured to follow timeout setting as set for functions.'
        );
      }
    ...
    compileLambdaPermissions: d(function (routeTargetData) {
      this.cfTemplate.Resources[
        ...
        DependsOn: routeTargetData.functionAlias ?
          routeTargetData.functionAlias.logicalId : undefined,
      };
    }),
  })
);

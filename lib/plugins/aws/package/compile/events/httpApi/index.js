...
const methodPathPattern = new RegExp(
  `^(?:\\*|(?<method>${Array.from(allowedMethods).join('|')}) (?<path>\\/\\S*))$`,
  'i'
);
...
class HttpApiEvents {
  constructor(serverless) {
    ...
    this.serverless.configSchemaHandler.defineFunctionEvent('aws', 'httpApi', {
      anyOf: [
        { regexp: methodPathPattern.toString() },
        {
          type: 'object',
          properties: {
            method: { type: 'string' },
            ...
            authorizer: {
              ...
              properties: {
                id: { type: 'string' },
                name: { type: 'string' },
                authorizationScopes: { type: 'array', items: { type: 'string' } },
              },
        ...
        for (const event of functionData.events) {
          ...
          if (_.isObject(event.httpApi)) {
            ...
            if (methodPath === '*') {
              ...
              ({ method, path } = methodPath.match(methodPathPattern).groups);
            }
          }
          path = String(path);
          let routeKey;

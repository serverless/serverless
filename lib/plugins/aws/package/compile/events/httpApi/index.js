    ...
    this.hooks = {
      'initialize': () => {
        if (this.serverless.service.provider.name === 'aws') {
          ...
          const httpApiEvents = _.flatten(
            _.values(this.serverless.service.functions).map(({ events }) =>
              events.map(({ httpApi: eventObject }) => eventObject)
            )
          ).filter(Boolean);

          if (httpApiEvents.length) {
            if (!_.get(this.serverless.service.provider, ['httpApi', 'payload'])) {
              this.serverless._logDeprecation(
            ...
            if (httpApiEvents.some(eventObject => _.get(eventObject, 'timeout'))) {
              this.serverless._logDeprecation(
                'AWS_HTTP_API_TIMEOUT',

...
describe('AwsCompileSQSEvents #2', () => {
  describe('with provisioned concurrency', () => {
    ...
    it('should have correct batch size', () => {
      expect(JSON.stringify(eventSourceMappingResource.Properties.BatchSize)).to.equal('10');
    });

    it('should have correct batching window size', () => {
      expect(
        JSON.stringify(eventSourceMappingResource.Properties.MaximumBatchingWindowInSeconds)
      ).to.equal('100');
    });
  });
});

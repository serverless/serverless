      ...
      anyOf: [
        { $ref: '#/definitions/awsArn' },
        {
          ...
          properties: {
            ...
            maximumBatchingWindowInSeconds: { type: 'integer', minimum: 0 },
          },
          required: ['arn'],
      ...
      if (functionObj.events) {
        ...
        functionObj.events.forEach(event => {
          if (event.sqs) {
            ...
            let BatchSize = 10;
            let Enabled = true;
            let MaximumBatchingWindowInSeconds = 0;

            if (typeof event.sqs === 'object') {
              EventSourceArn = event.sqs.arn;
              BatchSize = event.sqs.batchSize || BatchSize;
              MaximumBatchingWindowInSeconds =
                event.sqs.maximumBatchingWindowInSeconds || MaximumBatchingWindowInSeconds;

...
function createUsagePlanResource(that, name) {
  const resourceTemplate = {
    ...
    Properties: {
      ...
      Description: `Usage plan "${name}" for ${that.serverless.service.getShortServiceName()} ${that.provider.getStage()} stage`,
      UsagePlanName: `${that.serverless.service.getShortServiceName()}-${name}-${that.provider.getStage()}`,
    },
  };
  ...
  if (name === 'default') {
    ...
    template.Properties.UsagePlanName = that.serverless.service.getBaseName();
    template.Properties.Description = `Usage plan for ${that.serverless.service.getShortServiceName()} ${that.provider.getStage()} stage`;
    // assign quota

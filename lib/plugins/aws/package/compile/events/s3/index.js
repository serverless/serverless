        ...
        functionObj.events.forEach(event => {
          if (event.s3) {
            ...
            if (typeof event.s3 === 'object') {
              if (!event.s3.bucket && !event.s3.resoureId) {
                const errorMessage = [
                  `Missing "bucket" or "resoureId" property for s3 event in function ${functionName}.`,
                  ' The correct syntax is: s3: bucketName OR an object with "bucket" or "resoureId" property.',
                  ' Please check the docs for more info.',
                ].join('');
                throw new this.serverless.classes.Error(errorMessage);
              }
              bucketName = event.s3.bucket;
              logicalId = event.s3.resoureId;
              if (event.s3.event) {
            ...
            if (logicalId) {
              if (!userResources[logicalId]) {
                const errorMessage = [
                  ...
                  ` Please check that you have a S3 bucket in your "Resources:" with logcal id "${logicalId}"`,
                ].join('');
                throw new this.serverless.classes.Error(errorMessage);
              }

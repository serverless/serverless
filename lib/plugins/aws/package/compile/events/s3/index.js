...
class AwsCompileS3Events {
  constructor(serverless, options) {
    ...
    this.allowedBucketProperties = _.mapKeys(
      {
        AccelerateConfiguration: 'object',
        AccessControl: 'string',
        AnalyticsConfigurations: 'array',
        BucketEncryption: 'object',
        BucketName: 'string',
        CorsConfiguration: 'object',
        InventoryConfigurations: 'array',
  ...
  newS3Buckets() {
          ...
          if (event.s3) {
            ...
            if (typeof event.s3 === 'object') {
              ...
              const errorMessage = [
                ...
                ' The correct syntax is: s3: bucketName/provider bucket id or an object with "bucket" property.',
                ' Please check the docs for more info.',
              ].join('');
              throw new this.serverless.classes.Error(errorMessage);
            }
            ...
            let bucketName;
            if (provderS3[bucket]) {
              bucketName = provderS3[bucket].name || bucket;
              logicalId = this.provider.naming.getBucketLogicalId(bucket);
            } else {
              bucketName = bucket;
            }

            if (logicalId) {
              bucketsLogicalIds[bucketName] = { logicalId, bucket };
            }
    ...
    _.forEach(bucketsLambdaConfigurations, (bucketLambdaConfiguration, bucketName) => {
      let bucketProps = null;
      if (bucketsLogicalIds[bucketName]) {
        const providerBucket = provderS3[bucketsLogicalIds[bucketName].bucket];
        if (providerBucket) {
          bucketProps = Object.keys(providerBucket).reduce((acc, key) => {
            if (key !== 'name') {
              const property = _.upperFirst(key);
              const allowedProperety = this.allowedBucketProperties[key];
              if (!allowedProperety) {
                const errorMessage = [
                  `"${key}" is not a valid bucket property.`,
                  'A bucket could only be configured with the following properties:\n',
                  ['name'].concat(_.keys(this.allowedBucketProperties)).join(', '),
                ].join('');
                throw new this.serverless.classes.Error(errorMessage);
              }
              const value = providerBucket[key];
              const type = Object.prototype.toString
                .call(value)
                .substring(8)
                .slice(0, -1)
                .toLowerCase();
              if (allowedProperety !== type) {
                throw new this.serverless.classes.Error(
                  `Bucket property "${key}" is not a "${allowedProperety}".`
                );
              }
              acc[property] = providerBucket[key];
            }
            return acc;
          }, {});
        }
      ...
      if (bucketProps && Object.keys(bucketProps).length > 0) {
        providedNotificationConfiguration = bucketProps.NotificationConfiguration;
        delete bucketProps.NotificationConfiguration;
      }
      ...
      _.forEach(dependsOnToCreate, item => {
        const lambdaPermissionLogicalId = this.provider.naming.getLambdaS3PermissionLogicalId(
          item.functionName,
          (bucketsLogicalIds[item.bucketName] && bucketsLogicalIds[item.bucketName].bucket) ||
            item.bucketName
        );
      ...
      const bucketLogicalId =
        (bucketsLogicalIds[bucketName] && bucketsLogicalIds[bucketName].logicalId) ||
    ...
    s3EnabledFunctions.forEach(s3EnabledFunction => {
      ...
      const lambdaPermissionLogicalId = this.provider.naming.getLambdaS3PermissionLogicalId(
        s3EnabledFunction.functionName,
        (bucketsLogicalIds[s3EnabledFunction.bucketName] &&
          bucketsLogicalIds[s3EnabledFunction.bucketName].bucket) ||

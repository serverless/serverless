      ...
      if (functionObj.events) {
        functionObj.events.forEach(event => {
          if (event.s3) {
            ...
            if (
              !new RegExp(this.serverless.configSchemaHandler.schema.definitions.awsS3BucketName.pattern).test(
                bucketName
              )
            ) {
              const errorMessage = [
                `${bucketName} - Bucket name must conform to pattern ${this.serverless.configSchema.definitions.awsS3BucketName.pattern}.`,

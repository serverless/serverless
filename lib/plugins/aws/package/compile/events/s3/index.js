        ...
        functionObj.events.forEach(event => {
          if (event.s3) {
            ...
            if (typeof event.s3 === 'object') {
              if (!event.s3.bucket && !event.s3.id) {
                const errorMessage = [
                  `Missing "${(!event.s3.bucket && 'bucket') ||
                    'id'}" property for s3 event in function ${functionName}.`,
            ...
            if (logicalId) {
              ...
              if (resources[logicalId].Type !== 'AWS::S3::Bucket') {
                throw new this.serverless.classes.Error(
                  `The resource "${logicalId}" is a "AWS::S3::Bucket" resource.`
                );
              }
              bucketName = resources[logicalId].Properties.BucketName;

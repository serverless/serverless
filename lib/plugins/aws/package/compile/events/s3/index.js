...
class AwsCompileS3Events {
  constructor(serverless, options) {
    ...
    this.allowedBucketProperties = _.keyBy([
  ...
  newS3Buckets() {
          ...
          if (event.s3) {
            ...
            if (typeof event.s3 === 'object') {
              ...
              const errorMessage = [
                ...
                ' The correct syntax is: "s3: bucketName/bucketRef" or an object with "bucket" property.',
                ' Please check the docs for more info.',
              ].join('');
              throw new this.serverless.classes.Error(errorMessage);
            }
    ...
    _.forEach(bucketsLambdaConfigurations, (bucketLambdaConfiguration, bucketName) => {
      ...
      if (bucketsMeta[bucketName]) {
        ...
        bucketConf = Object.keys(providerBucket).reduce((acc, key) => {
          if (key !== 'name') {
            const property = _.upperFirst(key);
            const allowedProperety = this.allowedBucketProperties[property];
            if (!allowedProperety) {
              const errorMessage = [

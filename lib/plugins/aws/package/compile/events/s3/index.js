...
class AwsCompileS3Events {
  ...
  newS3Buckets() {
        ...
        functionObj.events.forEach(event => {
          if (event.s3) {
            ...
            if (typeof event.s3 === 'object') {
              if (!event.s3.bucket && !event.s3.resourceId) {
                const errorMessage = [
                  `Missing "bucket" or "resourceId" property for s3 event in function ${functionName}.`,
                  ' The correct syntax is: s3: bucketName OR an object with "bucket" or "resourceId" property.',
                  ' Please check the docs for more info.',
                ].join('');
                throw new this.serverless.classes.Error(errorMessage);
              }
              bucketName = event.s3.bucket;
              logicalId = event.s3.resourceId;
              if (event.s3.event) {
              ...
              const errorMessage = [
                ...
                ' The correct syntax is: s3: bucketName OR an object with "bucket" or "id" property.',
                ' Please check the docs for more info.',
              ].join('');
              throw new this.serverless.classes.Error(errorMessage);
            }
            ...
            if (logicalId) {
              if (!userResources[logicalId]) {
                const errorMessage = [
                  `S3 event has id property "${logicalId}" but no resource with that logical id could be found.`,
                  ` Please check that you have a S3 bucket in your "Resources:" with logical id "${logicalId}"`,
                ].join('');
                throw new this.serverless.classes.Error(errorMessage);
              }
              if (userResources[logicalId].Type !== 'AWS::S3::Bucket') {
                throw new this.serverless.classes.Error(
                  `The resource "${logicalId}" is a "AWS::S3::Bucket" resource.`
                );
              }
              bucketName = userResources[logicalId].Properties.BucketName;
              bucketsLogicalIds[bucketName] = logicalId;
            }
    ...
    _.forEach(bucketsLambdaConfigurations, (bucketLambdaConfiguration, bucketName) => {
      ...
      _.forEach(dependsOnToCreate, item => {
        const lambdaPermissionLogicalId = this.provider.naming.getLambdaS3PermissionLogicalId(
          item.functionName,
          bucketsLogicalIds[item.bucketName] || item.bucketName
        );
      ...
      const bucketLogicalId =
        bucketsLogicalIds[bucketName] || this.provider.naming.getBucketLogicalId(bucketName);
    ...
    s3EnabledFunctions.forEach(s3EnabledFunction => {
      ...
      const lambdaPermissionLogicalId = this.provider.naming.getLambdaS3PermissionLogicalId(
        s3EnabledFunction.functionName,
        bucketsLogicalIds[s3EnabledFunction.bucketName] || s3EnabledFunction.bucketName
      );

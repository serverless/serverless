...
describe('AwsCompileS3Events', () => {
  ...
  describe('#constructor()', () => {
    ...
    it('should extend validation schema', () => {
      const slsInstanceForDefineFunctionEvent = new Serverless();
      const defineFunctionEventStub = sinon.stub(
        slsInstanceForDefineFunctionEvent.configSchemaHandler,
        'defineFunctionEvent'
      );
      new AwsCompileS3Events(slsInstanceForDefineFunctionEvent); // eslint-disable-line no-new
      expect(defineFunctionEventStub.called).to.be.true;
    });
  });

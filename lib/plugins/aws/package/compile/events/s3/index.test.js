...
describe('AwsCompileS3Events', () => {
  ...
  beforeEach(() => {
    ...
    const AwsCompileS3EventsProxyquire = proxyquire('./index', {
      '../../../../customResources': {
    ...
    awsCompileS3Events = new AwsCompileS3EventsProxyquire(serverless);
  ...
  describe('#constructor()', () => {
    ...
    describe('validation with extended schema', () => {
      let userConfig;
      let cliStub;

      beforeEach(() => {
        serverless.cli = new serverless.classes.CLI(serverless);
        new AwsCompileS3Events(serverless); // eslint-disable-line no-new
        cliStub = sinon.stub(serverless.cli, 'log');
      ...
      it('should not write error to console for sting s3 event', () => {
        serverless.configSchemaHandler.validateConfig(userConfig);
        expect(cliStub.called).to.be.false;
      });

      it('should write error to console for invalid config', () => {
        userConfig.functions.someFunc.events[0].s3 = false;
        serverless.configSchemaHandler.validateConfig(userConfig);
        expect(cliStub.called).to.be.true;
      });

      it('should not write error to console for object s3 event', () => {
        userConfig.functions.someFunc.events[0].s3 = {
          bucket: 'photos',

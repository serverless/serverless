...
describe('AwsCompileS3Events', () => {
  ...
  describe('#newS3Buckets()', () => {
    ...
    it('should throw an error if the "bucket" or "resourceId" property is not given', () => {
      awsCompileS3Events.serverless.service.functions = {
        first: {
          events: [
            {
    ...
    it('should use the custom resource logical id if resourceId property exists', () => {
      awsCompileS3Events.serverless.service.resources = {};
      ...
      expect(
        awsCompileS3Events.serverless.service.provider.compiledCloudFormationTemplate.Resources
          .BucketLogicalId.Type
      ).to.equal('AWS::S3::Bucket');
      expect(
        awsCompileS3Events.serverless.service.provider.compiledCloudFormationTemplate.Resources
          .FirstLambdaPermissionBucketLogicalIdS3.Type
      ).to.equal('AWS::Lambda::Permission');
      expect(
        awsCompileS3Events.serverless.service.provider.compiledCloudFormationTemplate.Resources
          .BucketLogicalId.DependsOn
      ).to.deep.equal(['FirstLambdaPermissionBucketLogicalIdS3']);
    });

    it('should throw an error if no bucket is found with the provided logical id', () => {
      awsCompileS3Events.serverless.service.functions = {
        first: {
          events: [
            {
    ...
    it('should throw an error if resouce with logical id is not an s3 bucket', () => {
      awsCompileS3Events.serverless.service.resources = {};
      awsCompileS3Events.serverless.service.resources.Resources = {
        TopicLogicalId: {
          Type: 'AWS::SNS::Topic',

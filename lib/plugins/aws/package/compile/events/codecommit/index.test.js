...
describe('AwsCompileCodeCommitEvents', () => {
	...
	describe('#compileCodeCommitEvents()', () => {
		...
		it('should create corresponding resources when CodeCommit events are given', () => {
			...
			expect(awsCompileCodeCommitEvents.serverless.service
				.provider.compiledCloudFormationTemplate.Resources.FirstLambdaPermissionRepo1GitRepository.Type
			).to.equal('AWS::Lambda::Permission');
			expect(awsCompileCodeCommitEvents.serverless.service
				.provider.compiledCloudFormationTemplate.Resources.FirstLambdaPermissionRepo2GitRepository.Type
			).to.equal('AWS::Lambda::Permission');
			expect(awsCompileCodeCommitEvents.serverless.service
				.provider.compiledCloudFormationTemplate.Resources.FirstLambdaPermissionRepo3GitRepository.Type
			).to.equal('AWS::Lambda::Permission');
			expect(awsCompileCodeCommitEvents.serverless.service
		...
		it('should create single CodeCommit repository when the same repository is referenced repeatedly', () => {
			awsCompileCodeCommitEvents.serverless.service.functions = {
				first: {
					events: [
						{
		...
		it('should not create CodeCommit repository when arn is given as a string', () => {
			...
			expect(awsCompileCodeCommitEvents.serverless.service
				.provider.compiledCloudFormationTemplate.Resources.FirstLambdaPermissionFooGitRepository.Type
			).to.equal('AWS::Lambda::Permission');
		});
		...
		it('should not create CodeCommit repository when arn, repositoryName are given as object properties', () => {
			awsCompileCodeCommitEvents.serverless.service.functions = {
				first: {
					events: [
						{
			...
			expect(awsCompileCodeCommitEvents.serverless.service
				.provider.compiledCloudFormationTemplate.Resources.FirstLambdaPermissionBarGitRepository.Type
			).to.equal('AWS::Lambda::Permission');
		});

		it('should not create CodeCommit repository when arn, repositoryName, triggers are given as object properties', () => {
			awsCompileCodeCommitEvents.serverless.service.functions = {
				first: {
					events: [
						{

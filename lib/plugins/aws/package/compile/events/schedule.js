      ...
      if (functionObj.events) {
        functionObj.events.forEach((event) => {
          if (event.schedule) {
            ...
            let Description;
            let RetryPolicy;
            let RetryPolicyMaximumRetryAttempts;
            let RetryPolicyMaximumEventAge;
            ...
            if (typeof event.schedule === 'object') {
              ...
              RetryPolicyMaximumRetryAttempts = RetryPolicy
                ? RetryPolicy.maximumRetryAttempts
                : null;
              RetryPolicyMaximumEventAge = RetryPolicy ? RetryPolicy.maximumEventAge : null;
            ...
            let retryPolicyTemplate;
            if (
              (RetryPolicyMaximumRetryAttempts &&
                typeof RetryPolicyMaximumRetryAttempts === 'number') ||
              (RetryPolicyMaximumEventAge && typeof RetryPolicyMaximumEventAge === 'number')
            ) {
              retryPolicyTemplate = `
                {
                  ${
                    RetryPolicyMaximumRetryAttempts
                      ? `"MaximumRetryAttempts": "${RetryPolicyMaximumRetryAttempts}",`
                      : ''
                  }
                  ${
                    RetryPolicyMaximumEventAge
                      ? `"MaximumEventAge": "${RetryPolicyMaximumEventAge}",`
                      : ''
                  }
                }
              `;
            ...
            const scheduleTemplate = `
              {
                ...
                "Properties": {
                  ...
                  "Targets": [{
                    ...
                    ${retryPolicyTemplate ? `"RetryPolicy": ${retryPolicyTemplate},` : ''}

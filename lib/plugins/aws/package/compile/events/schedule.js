            ...
            for (const ScheduleExpression of ScheduleExpressions) {
              ...
              if (method === METHOD_SCHEDULER) {
                if (!hasSchedulerEvents) {
                  ...
                  if (policyDocumentStatements) {
                    allowedInvokeResources = [];

                    policyDocumentStatements.push({
                      Effect: 'Allow',
                      Action: ['lambda:InvokeFunction'],
                      Resource: allowedInvokeResources,
                    });
                  }
                }

                if (allowedInvokeResources) {
                  const functionArnWithVars =
                    'arn:${AWS::Partition}:lambda:${AWS::Region}:${AWS::AccountId}' +
                    `:function:${functionObj.name}`;

                  if (
                    !allowedInvokeResources.find(
                      (resource) => resource['Fn::Sub'] === functionArnWithVars
                    )
                  ) {
                    allowedInvokeResources.push(
                      {
                ...
                if (dependsOn) {
                  Object.assign(properties, { DependsOn: dependsOn });
                }
                ...
                const newScheduleObject = {
                  [scheduleLogicalId]: schedule,
                };

                Object.assign(resources, newScheduleObject);
              } else {

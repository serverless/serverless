        ...
        functionObj.events.forEach((event) => {
          if (event.cloudwatchLog) {
            ...
            if (
              ...
              const errorMessage = [
                `Only ${CLOUDWATCHLOG_LOG_GROUP_EVENT_PER_FUNCTION_LIMIT} subscription filter can be configured per log group.`,
                ` For "${LogGroupName}" you're attempting to configure over limit.`,
              ].join('');
              throw new ServerlessError(
                errorMessage,
                'CLOUDWATCHLOG_LOG_GROUP_EVENT_PER_FUNCTION_LIMIT_EXCEED'
              );
            }

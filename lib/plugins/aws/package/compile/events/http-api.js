    ...
    this.hooks = {
      'initialize': () => {
        if (
          ...
          _get(this.serverless.service.provider.httpApi, 'useProviderTags')
        ) {
  ...
  compileAuthorizers() {
    for (const authorizer of this.config.authorizers.values()) {
      ...
      if (authorizer.type === 'request') {
        ...
        authorizerResource.DependsOn = _get(
          _get(authorizer.functionObject, 'targetAlias'),
          'logicalId'
        );
  ...
  memoizeeMethods({
    ...
    compileIntegration: d(function (routeTargetData) {
      ...
      this.cfTemplate.Resources[
        ...
        DependsOn: _get(routeTargetData.functionAlias, 'logicalId'),
        Properties: properties,
      };
    }),

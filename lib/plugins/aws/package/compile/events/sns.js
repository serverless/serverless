        ...
        functionObj.events.forEach((event) => {
          ...
          if (!topicArn) {
            ...
            if (event.sns.displayName) {
              snsTopicResource.Properties.DisplayName = event.sns.displayName;
            }

            cfTemplate.Resources[topicLogicalId] = snsTopicResource;

            if (event.sns.filterPolicy || redrivePolicy) {
              snsSubscriptionResource.Properties.TopicArn = {
                Ref: topicLogicalId,
              };

              cfTemplate.Resources[subscriptionLogicalId] = snsSubscriptionResource;
            } else {
              if (!cfTemplate.Resources[topicLogicalId].Properties.Subscription) {
                cfTemplate.Resources[topicLogicalId].Properties.Subscription = [];
              }
              cfTemplate.Resources[topicLogicalId].Properties.Subscription.push(subscription);
            }
          } else {
            snsSubscriptionResource.Properties.TopicArn = topicArn;
          ...
          cfTemplate.Resources[lambdaPermissionLogicalId] = {

...
describe('#getTargetId', () => {
  ...
  it('should return a reference to provisioned Alias Arn when provisionnedConcurrency is set', () => {
    serverless.service.functions.foo.targetAlias = { logicalId: 'FooProvConcLambdaAlias' };
    expect(awsCompileAlbEvents.getTargetId('foo')).to.deep.equal({ Ref: 'FooProvConcLambdaAlias' });
  });
});

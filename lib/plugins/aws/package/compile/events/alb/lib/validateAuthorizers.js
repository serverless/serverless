...
module.exports = {
  validateAuthorizers() {
    ...
    if (_.has(albAuthConfig.authorizers, 'authenticateCognito')) {
      authorizers.authenticateCognito = this.validateCognitoAuth(albAuthConfig.authorizers.authenticateCognito);
    }
    if (_.has(albAuthConfig.authorizers, 'authenticateOidc')) {
      authorizers.authenticateOidc = this.validateOidcAuth(albAuthConfig.authorizers.authenticateOidc);
    }
  ...
  validateCognitoAuth(authenticateCognito) {
    if (!_.isObject(authenticateCognito)) {
      const errorMessage = 'Invalid ALB authorizer "authenticateCognito" in provider. You must provide an object';
    ...
    if (authenticateCognito.allowUnauthenticated) {
      authenticateCognitoObj.onUnauthenticatedRequest = authenticateCognito.allowUnauthenticated === true ? 'allow' : 'deny';
    } else {
  ...
  validateOidcAuth(authenticateOidc) {
    if (!_.isObject(authenticateOidc)) {
      const errorMessage = 'Invalid ALB authorizer "authenticateOidc" in provider. You must provide an object';
      throw new this.serverless.classes.Error(errorMessage);
    }
    ...
    if (authenticateOidc.allowUnauthenticated) {
      authenticateOidcObj.onUnauthenticatedRequest = authenticateOidc.allowUnauthenticated === true ? 'allow' : 'deny';
    } else {

...
describe('#validate()', () => {
  ...
  it('throws an error when type in authorizer is not "cognito" or "oidc"', () => {
    ...
    expect(() => awsCompileAlbEvents.validate()).to.throw('Authorizer type "unknown_ting" not supported. Only "cognito" and "oidc" are supported');
  });

  it('throws an error when authorizer passed but no authorizers declared in provider', () => {
    awsCompileAlbEvents.validatedAuthorizers = null;
    awsCompileAlbEvents.serverless.service.functions = {
      first: {
        events: [
          {
    ...
    expect(() => awsCompileAlbEvents.validate()).to.throw(
      'authorizer passed in function "first" but no authorizers declared in provider'
    );
  });
  ...
  describe('#validateEventAuthorizer()', () => {
    it('returns valid authorizer array when string provided', () => {
      ...
      expect(awsCompileAlbEvents.validateEventAuthorizer(event, auths, '')).to.deep.equal(['myFirstAuth']);
    });
    ...
    it('returns valid authorizer array when array provided', () => {
      ...
      expect(awsCompileAlbEvents.validateEventAuthorizer(event, auths, '')).to.deep.equal(['myFirstAuth', 'mySecondAuth']);
    });
    ...
    it('throws an error when authorizer does not match any registered authorizers in provider', () => {
      ...
      expect(() => awsCompileAlbEvents.validateEventAuthorizer(event, auths, 'functionName')).to.throw(
  ...
  describe('#validateCognitoAuth()', () => {
    it('returns valid cognito authorizer when valid object provided', () => {
      const auth = {
        type: 'cognito',
        userPoolId: 'userPoolId'
      };
      expect(awsCompileAlbEvents.validateCognitoAuth(auth, 'authName')).to.deep.equal({
        type: 'cognito',
        userPoolId: 'userPoolId',
        onUnauthenticatedRequest: 'deny'
      });
    ...
    it('returns valid cognito authorizer when valid object provided with allowUnauthenticated', () => {
      const auth = {
        type: 'cognito',
        userPoolId: 'userPoolId',
        allowUnauthenticated: true
    ...
    it('throws an error when provided authorizer is not an object', () => {
      const auth = 'not_allowed';
      expect(() => awsCompileAlbEvents.validateCognitoAuth(auth, 'authName')).to.throw(
        'Invalid ALB authorizer "authName" in provider. You must provide an object'
      );;
    });
  });

  describe('#validateOidcAuth()', () => {
    it('returns valid oidc authorizer when valid object provided', () => {
      const auth = {
        type: 'oidc',
        clientId: 'clientId',
        clientSecret: 'i-am-secret'
      };
      expect(awsCompileAlbEvents.validateOidcAuth(auth, 'authName')).to.deep.equal({
        type: 'oidc',
        clientId: 'clientId',
        onUnauthenticatedRequest: 'deny',
        clientSecret: 'i-am-secret'
    ...
    it('returns valid oidc authorizer when valid object provided with allowUnauthenticated', () => {
      const auth = {
        type: 'oidc',
        clientSecret: 'i-am-secret',
        allowUnauthenticated: true,
    ...
    it('throws an error when provided authorizer is not an object', () => {
      const auth = 'not_allowed';
      expect(() => awsCompileAlbEvents.validateOidcAuth(auth, 'authName')).to.throw(
        'Invalid ALB authorizer "authName" in provider. You must provide an object'
      );;
    });
  });

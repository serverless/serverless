...
describe('#validate()', () => {
  ...
  describe('#validateEventAuthorizer()', () => {
    it('returns valid authorizer array when string provided', () => {
      ...
      expect(awsCompileAlbEvents.validateEventAuthorizer(event, '')).to.deep.equal([
        'authenticateCognito',
      ]);
    });
    ...
    it('throws an error when authorizer is not of type String or of type Array', () => {
      ...
      expect(() => awsCompileAlbEvents.validateEventAuthorizer(event, 'functionName')).to.throw(
        'Invalid ALB event "authorizer" attribute in function "functionName". Must be of type string or array of strings'
      );
    });
    ...
    it('throws an error when authorizer string value is not either "authenticateCognito" or "authenticateOidc"', () => {
      ...
      expect(() => awsCompileAlbEvents.validateEventAuthorizer(event, 'functionName')).to.throw(
        'Unknown ALB event "authorizer" attribute value in function "functionName". Only authenticateCognito or authenticateOidc are allowed'
      );
    });

...
describe('#validate()', () => {
  ...
  describe('#validateListenerArnAndExtractAlbId()', () => {
    ...
    it('throws an error if the listener ARN is missing', () => {
      ...
      listenerArns.forEach(listenerArn => {
  ...
  describe('#validateEventHealthCheck', () => {
    it('handles boolean a boolean healthCheck property value', () => {
      const event1 = {
        alb: {
          healthCheck: true,
        },
      ...
      expect(awsCompileAlbEvents.validateAlbHealthCheck(event1)).to.deep.equal({
        enabled: true,
      });
      expect(awsCompileAlbEvents.validateAlbHealthCheck(event2)).to.deep.equal({
        enabled: false,
      });
    });
  });

  it('returns a valid health check object if healthCheck child properties are set', () => {
    const event = {
      alb: {
        healthCheck: {
          intervalSeconds: 35,
    ...
    expect(awsCompileAlbEvents.validateAlbHealthCheck(event)).to.deep.equal({
      enabled: true,
      intervalSeconds: 35,
      path: '/health',
      timeoutSeconds: 30,

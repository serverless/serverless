...
describe('#validate()', () => {
  ...
  describe('#validateCognitoAuth()', () => {
    ...
    beforeEach(() => {
      baseValidEvent = { alb: { authenticateCognito: {
    ...
    it('throws an error when authenticateCognito is not of type Object', () => {
      ...
      expect(() => awsCompileAlbEvents.validateCognitoAuth(event, 'functionName')).to.throw('Invalid ALB event "authenticateCognito" attribute in function "functionName".\n You must provide a object.');
    });

    it('throws an error if the userPoolArn is missing', () => {
      delete baseValidEvent.alb.authenticateCognito.userPoolArn;
      expect(() => awsCompileAlbEvents.validateCognitoAuth(baseValidEvent, 'functionName')).to.throw('Missing userPoolArn for authenticateCognito in function "functionName".')
    });

    it('throws an error if the userPoolClientId is missing', () => {
      delete baseValidEvent.alb.authenticateCognito.userPoolClientId;
      expect(() => awsCompileAlbEvents.validateCognitoAuth(baseValidEvent, 'functionName')).to.throw('Missing userPoolClientId for authenticateCognito in function "functionName".')
    });

    it('throws an error if the userPoolDomain is missing', () => {
      delete baseValidEvent.alb.authenticateCognito.userPoolDomain;
      expect(() => awsCompileAlbEvents.validateCognitoAuth(baseValidEvent, 'functionName')).to.throw('Missing userPoolDomain for authenticateCognito in function "functionName".')
    });
    ...
    it('throws an error if provided requestExtraParams is not an object', () => {
      ...
      expect(() => awsCompileAlbEvents.validateCognitoAuth(baseValidEvent, 'functionName')).to.throw('Invalid requestExtraParams for authenticateCognito in function "functionName". Must be of type Object.');
    });
    ...
    it('throws an error if provided allowUnauthenticated is not a Boolean', () => {
      ...
      expect(() => awsCompileAlbEvents.validateCognitoAuth(baseValidEvent, 'functionName')).to.throw('Invalid allowUnauthenticated for authenticateCognito in function "functionName". Must be of type Boolean.');
    });

    it('throws an error if provided scope is not of type String', () => {
      baseValidEvent.alb.authenticateCognito.scope = {};
      expect(() => awsCompileAlbEvents.validateCognitoAuth(baseValidEvent, 'functionName')).to.throw('Invalid scope for authenticateCognito in function "functionName". Must be of type String.');
    });
    ...
    it('throws an error if provided sessionCookieName is not of type String', () => {
      ...
      expect(() => awsCompileAlbEvents.validateCognitoAuth(baseValidEvent, 'functionName')).to.throw('Invalid sessionCookieName for authenticateCognito in function "functionName". Must be of type String.');
    });

    it('throws an error if provided sessionTimeout is not of type Number', () => {
      baseValidEvent.alb.authenticateCognito.sessionTimeout = {};
      expect(() => awsCompileAlbEvents.validateCognitoAuth(baseValidEvent, 'functionName')).to.throw('Invalid sessionTimeout for authenticateCognito in function "functionName". Must be of type Number.');
    });
  });
  ...
  describe('#validateOidcAuth()', () => {
    ...
    beforeEach(() => {
      baseValidEvent = { alb: { authenticateOidc: {
    ...
    it('throws an error when authenticateOidc is not of type Object', () => {
      ...
      expect(() => awsCompileAlbEvents.validateOidcAuth(event, 'functionName')).to.throw('Invalid ALB event "authenticateOidc" attribute in function "functionName".\n You must provide a object.');
    });

    it('throws an error when authorizationEndpoint is missing', () => {
      delete baseValidEvent.alb.authenticateOidc.authorizationEndpoint;
      expect(() => awsCompileAlbEvents.validateOidcAuth(baseValidEvent, 'functionName')).to.throw('Missing authorizationEndpoint for authenticateOidc in function "functionName".');
    });

    it('throws an error when clientId is missing', () => {
      delete baseValidEvent.alb.authenticateOidc.clientId;
      expect(() => awsCompileAlbEvents.validateOidcAuth(baseValidEvent, 'functionName')).to.throw('Missing clientId for authenticateOidc in function "functionName".');
    });

    it('throws an error when both clientSecret and useExistingClientSecret are missing', () => {
      delete baseValidEvent.alb.authenticateOidc.clientSecret;
      expect(() => awsCompileAlbEvents.validateOidcAuth(baseValidEvent, 'functionName')).to.throw('Either clientSecret or useExistingClientSecret required for authenticateOidc in function "functionName".');
    });
    ...
    it('throws an error when clientSecret is missing and useExistingClientSecret equals false', () => {
      ...
      expect(() => awsCompileAlbEvents.validateOidcAuth(baseValidEvent, 'functionName')).to.throw('Invalid useExistingClientSecret for authenticateOidc in function "functionName". Must be set to true when clientSecret not provided.');
    });

    it('throws an error when both clientSecret and useExistingClientSecret are provided', () => {
      baseValidEvent.alb.authenticateOidc.useExistingClientSecret = false;
      expect(() => awsCompileAlbEvents.validateOidcAuth(baseValidEvent, 'functionName')).to.throw('Cannot have both clientSecret and useExistingClientSecret set in same function "functionName".');
    });

    it('throws an error when both clientSecret is not a string', () => {
      baseValidEvent.alb.authenticateOidc.clientSecret = [false];
      expect(() => awsCompileAlbEvents.validateOidcAuth(baseValidEvent, 'functionName')).to.throw('Invalid clientSecret for authenticateOidc in function "functionName". Must be valid string.');
    });

    it('throws an error when issuer is missing', () => {
      delete baseValidEvent.alb.authenticateOidc.issuer;
      expect(() => awsCompileAlbEvents.validateOidcAuth(baseValidEvent, 'functionName')).to.throw('Missing issuer for authenticateOidc in function "functionName".');
    });

    it('throws an error when tokenEndpoint is missing', () => {
      delete baseValidEvent.alb.authenticateOidc.tokenEndpoint;
      expect(() => awsCompileAlbEvents.validateOidcAuth(baseValidEvent, 'functionName')).to.throw('Missing tokenEndpoint for authenticateOidc in function "functionName".');
    });
  });
});

...
describe('#validate()', () => {
  ...
  beforeEach(() => {
    ...
    awsCompileAlbEvents.validatedAuthorizers = {
      authenticateCognito: {},
      authenticateOidc: {}
    }
  });
  ...
  it('throws an error when authorizer passed but no authorizers declared in provider', () => {
    ...
    expect(() => awsCompileAlbEvents.validate()).to.throw('authorizer passed in function "first" but no authorizers declared in provider');
  });
  ...
  describe('#validateEventAuthorizer()', () => {
    ...
    it('returns valid authorizer array when string provided', () => {
      const event = {
        alb: {
          authorizer: 'authenticateCognito',
        },
      };
      expect(awsCompileAlbEvents.validateEventAuthorizer(event, '')).to.deep.equal(['authenticateCognito']);
    });

    it('returns valid authorizer array when array provided', () => {
      const event = {
        alb: {
          authorizer: ['authenticateCognito', 'authenticateOidc'],
        },
      };
      expect(awsCompileAlbEvents.validateEventAuthorizer(event, '')).to.deep.equal(['authenticateCognito', 'authenticateOidc']);
    });

    it('throws an error when authorizer is not of type String or of type Array', () => {
      const event = {
        alb: {
          authorizer: {},
        },
      };
      expect(() => awsCompileAlbEvents.validateEventAuthorizer(event, 'functionName')).to.throw('Invalid ALB event "authorizer" attribute in function "functionName". Must be of type string or array of strings');
    });

    it('throws an error when authorizer string value is not either "authenticateCognito" or "authenticateOidc"', () => {
      const event = {
        alb: {
          authorizer: 'not_allowed',
        },
      };
      expect(() => awsCompileAlbEvents.validateEventAuthorizer(event, 'functionName')).to.throw('Unknown ALB event "authorizer" attribute value in function "functionName". Only authenticateCognito or authenticateOidc are allowed');
    });

    it('throws an error when value in authorizer array is not either "authenticateCognito" or "authenticateOidc"', () => {
      const event = {
        alb: {
          authorizer: ['authenticateCognito', 'not_allowed'],
        },
      };
      expect(() => awsCompileAlbEvents.validateEventAuthorizer(event, 'functionName')).to.throw('Unknown ALB event "authorizer" attribute value in function "functionName". Only authenticateCognito or authenticateOidc are allowed');
    });

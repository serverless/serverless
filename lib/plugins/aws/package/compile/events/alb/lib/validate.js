...
module.exports = {
  ...
  validateEventAuthorizer(event, authorizers, functionName) {
    const authorizer = Array.isArray(event.alb.authorizer)
  ...
  validateCognitoAuth(authenticateCognito, name) {
    if (!_.isObject(authenticateCognito)) {
      const errorMessage = `Invalid ALB authorizer "${name}" in provider. You must provide an object`;
      throw new this.serverless.classes.Error(errorMessage);
    }
    authenticateCognito.onUnauthenticatedRequest =
      authenticateCognito.allowUnauthenticated && authenticateCognito.allowUnauthenticated === true
        ? 'allow'
        : 'deny';
  ...
  validateOidcAuth(authenticateOidc, name) {
    ...
    authenticateOidc.onUnauthenticatedRequest =
      authenticateOidc.allowUnauthenticated && authenticateOidc.allowUnauthenticated === true
        ? 'allow'
        : 'deny';

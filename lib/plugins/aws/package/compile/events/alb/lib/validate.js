...
module.exports = {
          ...
          if (_.isObject(event.alb)) {
            ...
            if (event.alb.authorizer) {
              if (!this.validatedAuthorizers) {
                throw new this.serverless.classes.Error(`authorizer passed in function "${functionName}" but no authorizers declared in provider`);
              }
              albObj.authorizer = this.validateEventAuthorizer(event, functionName);
            }
            events.push(albObj);
          }
        }
  ...
  validateEventAuthorizer(event, functionName) {
    const authKeys = Object.keys(this.validatedAuthorizers);
    const { authorizer } = event.alb;
    if (!_.isArray(authorizer) && !_.isString(authorizer)) {
      const errorMessage = `Invalid ALB event "authorizer" attribute in function "${functionName}". Must be of type string or array of strings`;
      throw new this.serverless.classes.Error(errorMessage);
    }
    const errorMessage = `Unknown ALB event "authorizer" attribute value in function "${functionName}". Only ${authKeys.join(' or ')} are allowed`;
    if (_.isString(authorizer)) {
      if (authKeys.includes(authorizer)) {
        return [authorizer];
      } else {
        throw new this.serverless.classes.Error(errorMessage);
      }
    }
    if (_.isArray(authorizer)) {
      const auths = [];
      authorizer.forEach(auth => {

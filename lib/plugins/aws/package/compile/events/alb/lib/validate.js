...
module.exports = {
  ...
  validateCognitoAuth(event, functionName) {
    ...
    authenticateCognitoObj.userPoolArn = event.alb.authenticateCognito.userPoolArn;
    authenticateCognitoObj.userPoolClientId = event.alb.authenticateCognito.userPoolClientId;
    authenticateCognitoObj.userPoolDomain = event.alb.authenticateCognito.userPoolDomain;
    if (event.alb.authenticateCognito.allowUnauthenticated) {
      authenticateCognitoObj.onUnauthenticatedRequest = event.alb.authenticateCognito.allowUnauthenticated === true ? 'allow' : 'deny';
    } else {
      authenticateCognitoObj.onUnauthenticatedRequest = 'deny';
    }
    if (event.alb.authenticateCognito.requestExtraParams) {
      authenticateCognitoObj.requestExtraParams = event.alb.authenticateCognito.requestExtraParams;
    }
    if (event.alb.authenticateCognito.scope) {
      authenticateCognitoObj.scope = event.alb.authenticateCognito.scope;
    }
    if (event.alb.authenticateCognito.sessionCookieName) {
      authenticateCognitoObj.sessionCookieName = event.alb.authenticateCognito.sessionCookieName;
    }
    if (event.alb.authenticateCognito.sessionTimeout) {
      authenticateCognitoObj.sessionTimeout = event.alb.authenticateCognito.sessionTimeout;
    }
  ...
  validateOidcAuth(event, functionName) {
    ...
    authenticateOidcObj.authorizationEndpoint = event.alb.authenticateOidc.authorizationEndpoint;
    authenticateOidcObj.clientId = event.alb.authenticateOidc.clientId;
    authenticateOidcObj.issuer = event.alb.authenticateOidc.issuer;
    if (_.has(event.alb.authenticateOidc, 'clientSecret')) {
      authenticateOidcObj.clientSecret = event.alb.authenticateOidc.clientSecret;
    } else {
      authenticateOidcObj.useExistingClientSecret = event.alb.authenticateOidc.useExistingClientSecret;
    }
    authenticateOidcObj.tokenEndpoint = event.alb.authenticateOidc.tokenEndpoint;
    authenticateOidcObj.userInfoEndpoint = event.alb.authenticateOidc.userInfoEndpoint;
    if (event.alb.authenticateOidc.allowUnauthenticated) {
      authenticateOidcObj.onUnauthenticatedRequest = event.alb.authenticateOidc.allowUnauthenticated === true ? 'allow' : 'deny';
    } else {
      authenticateOidcObj.onUnauthenticatedRequest = 'deny';
    }
    if (event.alb.authenticateOidc.requestExtraParams) {
      authenticateOidcObj.requestExtraParams = event.alb.authenticateOidc.requestExtraParams;
    }
    if (event.alb.authenticateOidc.scope) {
      authenticateOidcObj.scope = event.alb.authenticateOidc.scope;
    }
    if (event.alb.authenticateOidc.sessionCookieName) {
      authenticateOidcObj.sessionCookieName = event.alb.authenticateOidc.sessionCookieName;
    }
    if (event.alb.authenticateOidc.sessionTimeout) {
      authenticateOidcObj.sessionTimeout = event.alb.authenticateOidc.sessionTimeout;
    }

...
module.exports = {
  ...
  validateCognitoAuthReqExtraParams(event, functionName) {
    if (!_.isObject(event.alb.authenticateCognito.requestExtraParams)) {
      throw new this.serverless.classes.Error(`Invalid requestExtraParams for authenticateCognito in function "${functionName}". Must be of type Object.`);
    }
  ...
  validateCognitoAuthAllowUnauthenticated(event, functionName) {
    if (event.alb.authenticateCognito.allowUnauthenticated) {
      if (!_.isBoolean(event.alb.authenticateCognito.allowUnauthenticated)) {
        throw new this.serverless.classes.Error(`Invalid allowUnauthenticated for authenticateCognito in function "${functionName}". Must be of type Boolean.`);
      }
  ...
  validateCognitoAuthScope(event, functionName) {
    if (!_.isString(event.alb.authenticateCognito.scope)) {
      throw new this.serverless.classes.Error(`Invalid scope for authenticateCognito in function "${functionName}". Must be of type String.`);
    }
  ...
  validateCognitoAuthSessionCookieName(event, functionName) {
    if (!_.isString(event.alb.authenticateCognito.sessionCookieName)) {
      throw new this.serverless.classes.Error(`Invalid sessionCookieName for authenticateCognito in function "${functionName}". Must be of type String.`);
    }
  ...
  validateCognitoAuthSessionTimeout(event, functionName) {
    if (!_.isNumber(event.alb.authenticateCognito.sessionTimeout)) {
      throw new this.serverless.classes.Error(`Invalid sessionTimeout for authenticateCognito in function "${functionName}". Must be of type Number.`);
    }
    ...
    if (!_.has(event.alb.authenticateOidc, 'clientSecret')) {
      if (!_.has(event.alb.authenticateOidc, 'useExistingClientSecret')) {
        ...
        if (event.alb.authenticateOidc.useExistingClientSecret !== true) {
          throw new this.serverless.classes.Error(`Invalid useExistingClientSecret for authenticateOidc in function "${functionName}". Must be set to true when clientSecret not provided.`);
        }
      ...
      throw new this.serverless.classes.Error(`Cannot have both clientSecret and useExistingClientSecret set in same function "${functionName}".`);
    }

...
module.exports = {
  ...
  validateCognitoAuth(event, functionName) {
    ...
    authenticateCognitoObj.userPoolClientId = this.validateCognitoAuthUserPoolClientId(
      event,
      functionName
    );
    authenticateCognitoObj.userPoolDomain = this.validateCognitoAuthUserPoolDomain(
      event,
      functionName
    );
    authenticateCognitoObj.onUnauthenticatedRequest = this.validateCognitoAuthAllowUnauthenticated(
      event,
      functionName
    );
    if (event.alb.authenticateCognito.requestExtraParams) {
      authenticateCognitoObj.requestExtraParams = this.validateCognitoAuthReqExtraParams(
        event,
        functionName
      );
    }
    ...
    if (event.alb.authenticateCognito.sessionCookieName) {
      authenticateCognitoObj.sessionCookieName = this.validateCognitoAuthSessionCookieName(
        event,
        functionName
      );
    }
    if (event.alb.authenticateCognito.sessionTimeout) {
      authenticateCognitoObj.sessionTimeout = this.validateCognitoAuthSessionTimeout(
        event,
        functionName
      );
    }
  ...
  validateCognitoAuthUserPoolArn(event, functionName) {
    if (!_.has(event.alb.authenticateCognito, 'userPoolArn')) {
      throw new this.serverless.classes.Error(
        `Missing userPoolArn for authenticateCognito in function "${functionName}".`
      );
    }
    ...
    if (_.isObject(userPoolArn)) {
      ...
      throw new this.serverless.classes.Error(
        `Invalid userPoolArn for authenticateCognito in function "${functionName}".`
      );
    }
  ...
  validateCognitoAuthUserPoolClientId(event, functionName) {
    if (!_.has(event.alb.authenticateCognito, 'userPoolClientId')) {
      throw new this.serverless.classes.Error(
        `Missing userPoolClientId for authenticateCognito in function "${functionName}".`
      );
    }
    ...
    if (_.isObject(userPoolClientId)) {
      ...
      throw new this.serverless.classes.Error(
        `Invalid userPoolClientId for authenticateCognito in function "${functionName}".`
      );
    }
    if (!_.isString(event.alb.authenticateCognito.userPoolClientId)) {
      throw new this.serverless.classes.Error(
        `userPoolClientId for authenticateCognito in function "${functionName}" must be of type String.`
      );
    }
  ...
  validateCognitoAuthUserPoolDomain(event, functionName) {
    if (!_.has(event.alb.authenticateCognito, 'userPoolDomain')) {
      throw new this.serverless.classes.Error(
        `Missing userPoolDomain for authenticateCognito in function "${functionName}".`
      );
    }
    ...
    if (_.isObject(userPoolDomain)) {
      ...
      throw new this.serverless.classes.Error(
        `Invalid userPoolDomain for authenticateCognito in function "${functionName}".`
      );
    }
    if (!_.isString(event.alb.authenticateCognito.userPoolDomain)) {
      throw new this.serverless.classes.Error(
        `userPoolDomain for authenticateCognito in function "${functionName}" must be of type String.`
      );
    }
  ...
  validateOidcAuth(event, functionName) {
    ...
    authenticateOidcObj.userInfoEndpoint = this.validateOidcAuthUserInfoEndpoint(
      event,
      functionName
    );
    authenticateOidcObj.onUnauthenticatedRequest = this.validateOidcAuthAllowUnauthenticated(
      event,
      functionName
    );
    if (event.alb.authenticateOidc.requestExtraParams) {
      authenticateOidcObj.requestExtraParams = this.validateOidcAuthReqExtraParams(
        event,
        functionName
      );
    }
    ...
    if (event.alb.authenticateOidc.sessionCookieName) {
      authenticateOidcObj.sessionCookieName = this.validateOidcAuthSessionCookieName(
        event,
        functionName
      );
    }
  ...
  validateOidcAuthEndpoint(event, functionName) {
    ...
    if (!this.validateUrl(event.alb.authenticateOidc.authorizationEndpoint)) {
      throw new this.serverless.classes.Error(
        `Invalid authorizationEndpoint for authenticateOidc in function "${functionName}". Must be valid url.`
      );
    } else {
  ...
  validateOidcAuthClientId(event, functionName) {
    ...
    if (!_.isString(event.alb.authenticateOidc.clientId)) {
      throw new this.serverless.classes.Error(
        `Invalid clientId for authenticateOidc in function "${functionName}". Must be valid string.`
      );
    } else {
  ...
  validateOidcClientSecret(event, functionName, obj) {
    ...
    } else if (_.has(event.alb.authenticateOidc, 'useExistingClientSecret')) {
      throw new this.serverless.classes.Error(
  ...
  validateOidcAuthIssuer(event, functionName) {
    ...
    if (!this.validateUrl(event.alb.authenticateOidc.issuer)) {
      throw new this.serverless.classes.Error(
        `Invalid issuer for authenticateOidc in function "${functionName}". Must be valid url.`
      );
    } else {
  ...
  validateOidcAuthTokenEndpoint(event, functionName) {
    ...
    if (!this.validateUrl(event.alb.authenticateOidc.tokenEndpoint)) {
      throw new this.serverless.classes.Error(
        `Invalid tokenEndpoint for authenticateOidc in function "${functionName}". Must be valid url.`
      );
    } else {
  ...
  validateOidcAuthUserInfoEndpoint(event, functionName) {
    if (!_.has(event.alb.authenticateOidc, 'userInfoEndpoint')) {
      throw new this.serverless.classes.Error(
        `Missing userInfoEndpoint for authenticateOidc in function "${functionName}".`
      );
    }
    if (!this.validateUrl(event.alb.authenticateOidc.userInfoEndpoint)) {
      throw new this.serverless.classes.Error(
        `Invalid userInfoEndpoint for authenticateOidc in function "${functionName}". Must be valid url.`
      );
    } else {
  ...
  validateOidcAuthReqExtraParams(event, functionName) {
    if (!_.isObject(event.alb.authenticateOidc.requestExtraParams)) {
      throw new this.serverless.classes.Error(
        `Invalid requestExtraParams for authenticateOidc in function "${functionName}". Must be of type Object.`
      );
    }
  ...
  validateOidcAuthAllowUnauthenticated(event, functionName) {
    if (event.alb.authenticateOidc.allowUnauthenticated) {
      if (!_.isBoolean(event.alb.authenticateOidc.allowUnauthenticated)) {
        throw new this.serverless.classes.Error(
          `allowUnauthenticated for authenticateOidc in function "${functionName}" must be of type Boolean.`
        );
      }
  ...
  validateOidcAuthScope(event, functionName) {
    if (!_.isString(event.alb.authenticateOidc.scope)) {
      throw new this.serverless.classes.Error(
        `scope for authenticateOidc in function "${functionName}" must be of type Boolean.`
      );
    }
  ...
  validateOidcAuthSessionCookieName(event, functionName) {
    if (!_.isString(event.alb.authenticateOidc.sessionCookieName)) {
      throw new this.serverless.classes.Error(
        `sessionCookieName for authenticateOidc in function "${functionName}" must be of type String.`
      );
    }
  ...
  validateOidcAuthSessionTimeout(event, functionName) {
    if (!_.isNumber(event.alb.authenticateOidc.sessionTimeout)) {
      throw new this.serverless.classes.Error(
        `sessionTimeout for authenticateOidc in function "${functionName}" must be of type Number.`
      );
    }

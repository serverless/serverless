...
const COGNITO_USER_POOL_PATTERN = /^arn:aws[\w-]*:cognito-idp:.+:userpool\/[\w-]+\/([\w-]+)\/([\w-]+)$/;
...
module.exports = {
  ...
  validateCognitoAuth(event, functionName) {
    ...
    authenticateCognitoObj.userPoolClientId = this.validateCognitoAuthUserPoolClientId(event, functionObject);
    ...
    if (event.alb.authenticateCognito.allowUnauthenticated)
      authenticateCognitoObj.allowUnauthenticated = this.validateCognitoAuthAllowUnauthenticated(event, functionName);
    if (event.alb.authenticateCognito.scope)
      authenticateCognitoObj.scope = this.validateCognitoAuthAllowUnauthenticated(event, functionName);
  ...
  validateCognitoAuthUserPoolClientId(event, functionName) {
    ...
    if (!event.alb.authenticateCognito.userPoolClientId instanceof String) {
  ...
  validateCognitoAuthUserPoolDomain(event, functionName) {
    ...
    if (!event.alb.authenticateCognito.userPoolDomain instanceof String) {
  ...
  validateCognitoAuthAllowUnauthenticated(event, functionName) {
    ...
    return event.alb.authenticateCognito.allowUnauthenticated;
  },

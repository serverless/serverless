...
module.exports = {
    ...
    if (albAuthConfig) {
      for (const [name, auth] of _.entries(albAuthConfig.authorizers)) {
        switch (auth.type) {
          case 'cognito':
          case 'oidc':
            authorizers[name] = this.validateAlbAuth(auth, name);
            break;
          default:
  ...
  validateAlbAuth(auth, authName) {
    ...
    const hasOnUnauthenticatedRequest = _.has(auth, ['onUnauthenticatedRequest']);
    const supportedValues = ['deny', 'allow', 'authenticate'];
    ...
    if (
      ...
      throw new this.serverless.classes.Error(
        `Invalid value ${
          auth.onUnauthenticatedRequest
        } for property "onUnauthenticatedRequest" in authorizer "${authName}". Supported values ${supportedValues.join(
          ','
        )} `
      );
    }
    ...
    if (hasAllowUnauthenticated) {
      logDeprecation(
        'AWS_ALB_EVENT_ALLOW_UNAUTHENTICATED',
    ...
    } else if (!hasOnUnauthenticatedRequest) {
      auth.onUnauthenticatedRequest = 'deny';
    }

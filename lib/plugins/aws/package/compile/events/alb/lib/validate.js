...
module.exports = {
  ...
  validateConditions(conditions, functionName) {
    ...
    // Conditions Values per rule cannot exceed 5
    // https://docs.aws.amazon.com/elasticloadbalancing/latest/application/load-balancer-limits.html

    const combinedValuesLength = this.getConditionsValuesLength(conditions);

    if (combinedValuesLength < 6) {
      return;
    }
    throw new ServerlessError(
      `Cannot setup "alb" event in "${functionName}" function: Maximum 5 conditions values must be defined. Found ${combinedValuesLength}`,
      'ALB_MAX_CONDITIONS_VALUES'
    );
  },
  ...
  getConditionsValuesLength(conditions) {
    let total = 0;

    if (conditions.path) {
      total += conditions.path.length;
    }

    if (conditions.method) {
      total += conditions.method.length;
    }

    if (conditions.host) {
      total += conditions.host.length;
    }

    if (conditions.ip) {
      total += conditions.ip.length;
    }

    if (conditions.header) {
      total += conditions.header.values.length;
    }

    if (conditions.query) {
      total += Object.values(conditions.query).reduce((accum, values) => {
        accum += values.length;
        return accum;
      }, 0);
    }

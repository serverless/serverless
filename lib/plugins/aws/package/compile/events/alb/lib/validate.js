...
module.exports = {
  validate() {
    ...
    if (albAuthConfig) {
      for (const name of Object.keys(albAuthConfig.authorizers)) {
        const auth = albAuthConfig.authorizers[name];
        switch (auth.type) {
          case 'cognito':
            authorizers[name] = this.validateCognitoAuth(auth, name);
            break;
          case 'oidc':
            authorizers[name] = this.validateOidcAuth(auth, name);
            break;
          default:
        ...
        if (_.has(event, 'alb')) {
          if (_.isObject(event.alb)) {
            ...
            if (event.alb.authorizer) {
              if (!this.validatedAuthorizers) {
                throw new this.serverless.classes.Error(
                  `authorizer passed in function "${functionName}" but no authorizers declared in provider`
                );
              }
              albObj.authorizer = this.validateEventAuthorizer(event, authorizers, functionName);
            }
            events.push(albObj);
          }
        }
  ...
  validateEventAuthorizer(event, authorizers, functionName) {
    const authorizer = Array.isArray(event.alb.authorizer) ? event.alb.authorizer : [event.alb.authorizer];
    for (const auth of authorizer) {
      if (!authorizers[auth]) {
        throw new this.serverless.classes.Error(`No match for "${auth}" in function "${functionName}" found in registered ALB authorizers`);
      }
    }
    return authorizer;
  },

  validateCognitoAuth(authenticateCognito, name) {
    if (!_.isObject(authenticateCognito)) {
      const errorMessage =
        `Invalid ALB authorizer "${name}" in provider. You must provide an object`;
      throw new this.serverless.classes.Error(errorMessage);
    }
    authenticateCognito.onUnauthenticatedRequest = authenticateCognito.allowUnauthenticated && authenticateCognito.allowUnauthenticated === true ? 'allow' : 'deny';

    return authenticateCognito;
  },

  validateOidcAuth(authenticateOidc, name) {
    if (!_.isObject(authenticateOidc)) {
      const errorMessage =
        `Invalid ALB authorizer "${name}" in provider. You must provide an object`;
      throw new this.serverless.classes.Error(errorMessage);
    }
    authenticateOidc.onUnauthenticatedRequest = authenticateOidc.allowUnauthenticated && authenticateOidc.allowUnauthenticated === true ? 'allow' : 'deny';

...
module.exports = {
  ...
  ALB_LISTENER_PATTERN: '^arn:aws[\\w-]*:elasticloadbalancing:.+:listener\\/app\\/[\\w-]+\\/([\\w-]+)\\/([\\w-]+)$',
        ...
        if (event.alb) {
          if (_.isObject(event.alb)) {
            ...
            if (event.alb.conditions.ip) {
              albObj.conditions.ip = event.alb.conditions.ip
            }
  ...
  validateListenerArn(listenerArn, functionName) {
    ...
    const matches = listenerArn.match(new RegExp(this.ALB_LISTENER_PATTERN));
    if (!matches) {

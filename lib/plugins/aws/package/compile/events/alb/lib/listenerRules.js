...
module.exports = {
  compileListenerRules() {
    const authorizers = this.validatedAuthorizers;
    this.validated.events.forEach(event => {
      ...
      const Actions = [];
      if (event.authorizer) {
        event.authorizer.forEach(authName => {
          if (authName === 'authenticateCognito') {
            const auth = authorizers.authenticateCognito;
            const config = {
            ...
            const auth = authorizers.authenticateOidc;
            const config = {

        ...
        conditions: {
          ...
          properties: {
            header: {
              type: 'object',
              properties: {
                name: { type: 'string', maxLength: 40 }, // The allowed characters are specified by RFC 7230
                values: { type: 'array', items: { type: 'string', maxLength: 128 } },
              },
              additionalProperties: false,
              required: ['name', 'values']
            },
            host: { type: 'string', pattern: '^[A-Za-z0-9*?.-]$', maxLength: 128 },
            ip: {
              type: 'array',
              items: { oneOf: [{ type: 'string', format: 'ipv4' }, { type: 'string', format: 'ipv6' }] }
            },
            method: { type: 'string', pattern: '^[A-Za-z_-]$', maxLength: 40 },
            path: { type: 'string', pattern: '^([A-Za-z0-9*?_.$/~"\'@:+-]|&amp;)+$', maxLength: 128 },
            query: {
              type: 'object',
              additionalProperties: { type: 'string', maxLenght: 128 },
              propertyNames: {type: 'string', maxLenght: 128}
            },
          },
          required: ['path'],
          additionalProperties: false,

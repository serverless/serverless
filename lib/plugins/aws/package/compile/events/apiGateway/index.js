      ...
      anyOf: [
        { regexp: { pattern: `^(${httpMethods.join('|')}) \\S+`, flags: 'i' } },
        {
            ...
            cors: {
              oneOf: [
                { type: 'boolean' },
                {
                  ...
                  oneOf: [
                    { properties: { origin: { type: 'string' } }, required: ['origin'] },
                    {
                      properties: {
                        origins: {
                          type: 'array',
          ...
          allOf: [
            ...
            {
              if: {
                properties: { integration: { enum: extendWithUppercase(['lambda']) } },
              },
              then: {
                properties: {
                  authorizer: {
            ...
            },
            {
              if: {
                properties: {
                  cognito_user_pools: {
                    properties: { type: { enum: extendWithUppercase(['cognito_user_pools']) } },
                  },
                },
              },
              then: {

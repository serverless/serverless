...
describe('#compileRestApi()', () => {
  ...
  it('should add model and validator to resources when schemas defined in provider. Default config', () => {
    awsCompileApigEvents.serverless.service.provider.apiGateway = {
      schemas: {
        'user-add-model': { foo: 'bar' }
      }
    }
    return awsCompileApigEvents.compileRestApi().then(() => {
      const resources =
      ...
      expect(resources.UserAddModel).to.deep.equal({
        ...
        Properties: {
          ...
          Description: 'Validation model for UserAddModel'
        },

      });

      expect(resources.UserAddModelValidator).to.deep.equal({
        ...
        Properties: {
          ...
          Name: 'UserAddModelValidator'
        }
      })
    })
  });

  it('should add model and validator to resources when schemas defined in provider. Model not in name config', () => {
    awsCompileApigEvents.serverless.service.provider.apiGateway = {
      schemas: {
        'user-add': { foo: 'bar' }
      }
    ...
    return awsCompileApigEvents.compileRestApi().then(() => {
      ...
      expect(resources.UserAddModel).to.deep.equal({
        ...
        Properties: {
          ...
          Name: 'UserAdd',
          ContentType: 'application/json',
          Description: 'Validation model for UserAdd'
        },
      ...
      expect(resources.UserAddModelValidator).to.deep.equal({
        ...
        Properties: {
          ...
          ValidateRequestBody: true,
          ValidateRequestParameters: true,
          Name: 'UserAddValidator'
        }
      })
    })
  });

  it('should add model and validator to resources when schemas defined in provider. Full config', () => {
    awsCompileApigEvents.serverless.service.provider.apiGateway = {
      schemas: {
        'user-add-model': {
          name: 'UserAddModel',
          schema: { foo: 'bar' },
          description: 'Model for adding users'
        }
      }
    }
    return awsCompileApigEvents.compileRestApi().then(() => {

...
describe('#validate()', () => {
  ...
  it('should accept API_KEY as authorizer', () => {
    awsCompileApigEvents.serverless.service.functions = {
      foo: {},
      first: {
        events: [
    ...
    const validated = awsCompileApigEvents.validate();
    expect(validated.events).to.be.an('Array').with.length(2);
    expect(validated.events[0].http.authorizer).to.equal(null);
    expect(validated.events[0].http.private).to.equal(true);
    expect(validated.events[1].http.authorizer).to.equal(null);
    expect(validated.events[1].http.private).to.equal(true);
  });

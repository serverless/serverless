...
module.exports = {
  ...
  compileResources() {
    ...
    _.keys(this.apiGatewayResources).forEach((path) => {
      ...
      const parentRef = this.getParentResourceId(resource.parent
  ...
  combineResourceTrees(trees) {
    function getNodePaths(result, node) {
      ...
      r[node.path] = node;
      if (!node.name) {
        r[node.path].name = this.provider.naming.normalizePath(node.path);
      }

      node.children.forEach(getNodePaths);
    }
    ...
    for (let i = sortedResourceNodes.length - 1; i >= 0; i--) {
      ...
      while (parent.parent) {
        if (parent.parent.hasMethod && !parent.parent.resourceId) {
          throw new Error(`Resource ID for path ${node.parent.path} is required`);
        }

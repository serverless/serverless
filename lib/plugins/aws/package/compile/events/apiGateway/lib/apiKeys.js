...
module.exports = {
  validateApiKeyInput(apiKey) {
    if (_.isObject(apiKey) && !_.isNil(apiKey.value) && apiKey.value.length < 20) {
      return false;
    } else if (_.isObject(apiKey) && (!_.isNil(apiKey.name) || !_.isNil(apiKey.value))) {
      return true;
          ...
          _.forEach(apiKeyDefinition[name], key => {
            if (!this.validateApiKeyInput(key)) {
              throw new this.serverless.classes.Error(
                'API Key must be a string or an object which contains name and/or value having at least 20 characters'
              );
            }
            keyNumber += 1;

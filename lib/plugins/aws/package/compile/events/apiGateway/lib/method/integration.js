...
module.exports = {
  ...
  getIntegrationResponses(http) {
    ...
    if (http.response) {
      ...
      _.forEach(http.response.statusCodes, (config, statusCode) => {
  ...
  getIntegrationRequestTemplates(http, useDefaults) {
    ...
    if (http.request && typeof http.request.template === 'object') {
      Object.keys(http.request.template).forEach(contentType => {
        if (http.request.template[contentType] === null) {
          delete integrationRequestTemplates[contentType];
        } else {
          integrationRequestTemplates[contentType] = http.request.template[contentType];
        }
      });
    }

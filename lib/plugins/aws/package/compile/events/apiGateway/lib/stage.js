    ...
    if (logs || TracingEnabled || Tags.length > 0) {
      ...
      if (logs) {
        ...
        _.merge(cfTemplate.Resources, {
          ...
          [logsRoleLogicalId]: logsIamRole
            ? getIamRoleResource(provider.logs.restApi.iamRole)
            : getIamRoleResource(service, stage),
...
/**
 *
 * @param  {...string} args ["service", "stage"] or ["roleName"] or ["roleArn"]
 */
function getIamRoleResource(...args) {
  const logsIamRoleRegEx = /^arn:aws:iam::[0-9]+:role\/[0-9a-zA-Z-_+=,.@]+$/;
  const roleName = [];
  if (args.length === 1) {
    if (logsIamRoleRegEx.test(args[0])) {
      // get roleName from ARN
      // ARN must be within the account
      roleName.push(args[0].split('/')[1]); // get roleName from ARN
    } else {

...
module.exports = {
  compileStage() {
    ...
    const logsIamRole = logs && provider.logs.restApi.iamRole;
    ...
    if (logs || TracingEnabled || Tags.length > 0) {
      ...
      if (logs) {
        ...
        if (logsIamRole && /^arn:/.test(provider.logs.restApi.iamRole)) {
          _.merge(cfTemplate.Resources, {
            [logGroupLogicalId]: getLogGroupResource(service, stage),
            [accountLogicalid]: getAccountResource(null, provider.logs.restApi.iamRole),
          });
        } else if (logsIamRole) {
          _.merge(cfTemplate.Resources, {
            [logGroupLogicalId]: getLogGroupResource(service, stage),
            [logsRoleLogicalId]: getIamRoleResource(provider.logs.restApi.iamRole),

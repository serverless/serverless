...
const logsIamRoleRegEx = /^arn:aws:iam::[0-9]+:role\/[0-9a-zA-Z-_+=,.@]+$/;
...
module.exports = {
  compileStage() {
    ...
    const logsIamRoleName = logs && provider.logs.restApi.iamRoleName;
    const logsIamRoleArn = logs && provider.logs.restApi.iamRoleArn;
      ...
      if (logs) {
        ...
        if (logsIamRoleArn) {
          _.merge(cfTemplate.Resources, {
            ...
            [accountLogicalid]: getAccountResource(null, provider.logs.restApi.iamRoleArn),
          });
        } else if (logsIamRoleName) {
          _.merge(cfTemplate.Resources, {
            [logGroupLogicalId]: getLogGroupResource(service, stage),
            [logsRoleLogicalId]: getIamRoleResource(provider.logs.restApi.iamRoleName),
...
/**
 *
 * @param  {...string} args ["service", "stage"] or ["roleName"]
 */
function getIamRoleResource(...args) {
  console.log('******************');
  console.log(JSON.stringify(args));
  const roleName = [];
  if (args.length === 1) {
    roleName.push(args[0]); // roleName
  }
  if (args.length === 2) {
    roleName.push(args[0]); // service
    roleName.push(args[1]); // stage
    roleName.push({
      Ref: 'AWS::Region',
    });
    roleName.push('apiGatewayLogsRole');
  }
...
/**
 *
 * @param {string} logsRoleLogicalId
 * @param {string} logsRoleArn Takes precedence
 */
function getAccountResource(logsRoleLogicalId = undefined, logsRoleArn = undefined) {
  console.log('******************');
  let cloudWatchRoleArn = {
    'Fn::GetAtt': [logsRoleLogicalId, 'Arn'],
  };
  if (logsRoleArn) {
    cloudWatchRoleArn = logsRoleArn;
  }
  console.log('cloudWatchRoleArn', cloudWatchRoleArn);
  console.log('******************');
  return {

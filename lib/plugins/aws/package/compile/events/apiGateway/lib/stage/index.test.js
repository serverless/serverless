...
describe('#compileStage()', () => {
  ...
  describe('logs with custom IAM role name', () => {
    beforeEach(() => {
      // setting up API Gateway logs
      awsCompileApigEvents.serverless.service.provider.logs = {
        restApi: {
      ...
      awsCompileApigEvents.compileStage().then(() => {
        ...
        expect(resources[logsRoleLogicalId]).to.deep.equal({
          ...
          Properties: {
            ...
            Path: '/',
            RoleName: {
              'Fn::Join': ['-', ['custom-api-gateway-logs-iam-role']],
            },
          },
        });
      }));
  ...
  describe('logs with custom IAM role ARN', () => {
    beforeEach(() => {
      // setting up API Gateway logs
      awsCompileApigEvents.serverless.service.provider.logs = {
        restApi: {
    ...
    it('should create an Account resource', () =>
      awsCompileApigEvents.compileStage().then(() => {
        ...
        expect(resources[accountLogicalid]).to.deep.equal({
          Type: 'AWS::ApiGateway::Account',
          Properties: {
            CloudWatchRoleArn: 'arn:aws:iam::123456789012:role/custom-api-gateway-logs-iam-role',
          },
        });
      }));
  });

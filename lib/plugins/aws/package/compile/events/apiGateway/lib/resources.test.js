...
describe('#compileResources()', () => {
  ...
  it('should throw error if parent of existing resources is required', () => {
    ...
    try {
      return awsCompileApigEvents.compileResources()
      .then(() => {
        throw new Error('Expected throwing error, got success');
      });
    } catch (e) {
      expect(e.message).to.equal('Resource ID for path users is required');
    }
  });
});

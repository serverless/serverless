      ...
      _.forEach(functionObject.events, (event) => {
        if (_.has(event, 'http')) {
          ...
          if (http.integration === 'AWS') {
            ...
            if (http.request) {
              let keys = Object.keys(http.request);
              if (!(keys.length === 1 && keys[0] === "parameters")) {
                const requestWarningMessage = [
                  'Warning! You\'re using the LAMBDA-PROXY in combination with request',
                  ` configuration in your function "${functionName}"  Only the 'request.parameters'.`,
                  ' options are available in conjunction with LAMBDA-PROXY.',

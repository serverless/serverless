  ...
  getAuthorizer(http, functionName) {
    ...
    if (typeof authorizer === 'string') {
      ...
      if (authorizer.type && authorizer.type.toUpperCase() === 'AWS_IAM') {
        type = 'AWS_IAM';
      } else if (authorizer.toUpperCase() === 'TOKEN') {
        // this is an alias for private: true in the event. If TOKEN is defined,
        // we return nothing and set the event to being private, so it will
        // require a valid api key further in the chain
        http.private = true;
        return null;

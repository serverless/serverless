  ...
  getAuthorizer(http, functionName) {
    ...
    if (typeof authorizer === 'string') {
      if (authorizer.toUpperCase() === 'AWS_IAM') {
        type = 'AWS_IAM';
      } else if (authorizer.toUpperCase() === 'API_KEY') {
        // this is an alias for private: true in the event. If API_KEY is
        // defined, we return nothing and set the event to being private, so it
        // will require a valid api key further in the chain
        http.private = true;  // eslint-disable-line no-param-reassign
        return null;

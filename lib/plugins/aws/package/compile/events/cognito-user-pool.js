...
class AwsCompileCognitoUserPoolEvents {
  ...
  existingCognitoUserPools() {
          ...
          if (event.cognitoUserPool && event.cognitoUserPool.existing) {
            ...
            if (numEventsForFunc === 1) {
              ...
              if (customSenderSources.includes(trigger)) {
                const previousKmsId = poolKmsId.get(currentPoolName);
                if (previousKmsId !== undefined && previousKmsId !== kmsKeyId) {
                  throw new ServerlessError(
                    `You cannot specify different "kmsKeyId" for same user pool "${currentPoolName}".`,
                    'FUNCTION_COGNITO_KMS_KEY_ID_NOT_SAME'
                  );
                }
                poolKmsId.set(currentPoolName, kmsKeyId);
    ...
    if (usesExistingCognitoUserPool) {
      ...
      // ability to invoke RemovePermission on removed lambda function
      const functionPrefix = `${this.serverless.service.service}-${this.provider.getStage()}`;
      iamRoleStatements.push({
        Effect: 'Allow',
        Resource: {
          'Fn::Sub': `arn:\${AWS::Partition}:lambda:*:*:function:${functionPrefix}-*`,
        },

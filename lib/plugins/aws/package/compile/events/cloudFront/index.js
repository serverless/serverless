...
const supportedRuntimes = ['nodejs6.10', 'nodejs8.10', 'nodejs10.x', 'python3.7'];
      ...
      if (functionObj.events) {
        functionObj.events.forEach(event => {
          if (event.cloudFront) {
            ...
            if (supportedRuntimes.indexOf(runtime) < 0) {
              throw new ServerlessError(
                `The function has a runtime that is not supported by Lambda@Edge: ${runtime}. Supported runtimes: [${supportedRuntimes.join(
                  ', '
                )}]`
              );
            }

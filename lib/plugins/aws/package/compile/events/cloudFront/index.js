          ...
          if (event.cloudFront) {
            ...
            if (event.cloudFront.defaultOrigin === true) {
              if (defaultOrigin !== undefined && defaultOrigin !== origin) {
                throw new Error('Found more than one cloudfront event with "defaultOrigin" set');
              }
    ...
    if (lambdaAtEdgeFunctions.length) {
      ...
      if (behaviors[0].PathPattern) {
        ...
        if (origin === undefined) {
          if (origins.length > 1) {
            throw new Error(
              'Found more than one origin but none of the cloudfront event has "defaultOrigin" set'
            );
          }
          origin = origins[0];
        }

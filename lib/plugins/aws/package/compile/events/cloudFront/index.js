  ...
  compileCloudFrontEvents() {
      ...
      if (functionObj.events) {
        functionObj.events.forEach(event => {
          if (event.cloudFront) {
            ...
            const existingBehaviour = _.find(
              behaviors,
              o =>
            ...
            lambdaAtEdgeFunctions.push(
              _.merge({}, functionObj, { functionName, lambdaVersionLogicalId })
            );
          }
        });
      }
    ...
    if (!_.isEmpty(lambdaAtEdgeFunctions)) {
      ...
      if (_.isUndefined(Resources.IamRoleLambdaExecution)) {
        ...
        const lambdaAssumeStatement = _.find(
          Resources.IamRoleLambdaExecution.Properties.AssumeRolePolicyDocument.Statement,
          statement => _.includes(statement.Principal.Service, 'lambda.amazonaws.com')
        );
        ...
        _.first(Resources.IamRoleLambdaExecution.Properties.Policies).PolicyDocument.Statement.push(
          {
            Effect: 'Allow',

        ...
        functionObj.events.forEach(event => {
          if (event.cloudFront) {
            ...
            if (event.cloudFront.behavior) {
              if (event.cloudFront.behavior.ForwardedValues) {
                this.serverless._logDeprecation(
                  'CLOUDFRONT_CACHE_BEHAVIOR_FORWARDED_VALUES',
                  'Cloudfront has deprecated the use of the ForwardedValues field to configure cache behavior. Please use `provider.cloudfront.cachePolicies` to define Cache Policies and reference it here with `cachePolicy.id` property.'
                );

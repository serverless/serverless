          ...
          if (event.cloudFront) {
            ...
            if (event.cloudFront.behavior) {
              if (
                ...
                event.cloudFront.behavior.MinTTL ||
                event.cloudFront.behavior.MaxTTL ||
                event.cloudFront.behavior.DefaultTTL
              ) {
                this.serverless._logDeprecation(
                  'CLOUDFRONT_CACHE_BEHAVIOR_FORWARDED_VALUES_AND_TTL',
                  'Cloudfront has deprecated the use of the ForwardedValues, MinTTL, MaxTTL and DefaultTTL field to configure cache behavior. Please use `provider.cloudfront.cachePolicies` to define Cache Policies and reference it here with `cachePolicy.name` property. You can also reference existing policies with `cachePolicy.id`.'
                );
            ...
            if (event.cloudFront.cachePolicy) {
              if (!shouldAssignCachePolicy) {
                throw new this.serverless.classes.Error(
                  `Both cachePolicy and deprecated fields ForwardedValues, MinTTL, MaxTTL and DefaultTTL found in function ${functionObj.name} configuration. Specifying a cachePolicy override those deprecated parameters. Please remove one the cache behavior definition.`,
                  'CACHE_POLICY_ID_AND_DEPRECATED_FIELDS_USED'
                );
              }
              const { id, name } = event.cloudFront.cachePolicy;
              if (name && !this.cachePolicies.has(name)) {
                throw new this.serverless.classes.Error(
                  `Event references not configured cache policy '${name}'`,
                  'UNRECOGNIZED_CLOUDFRONT_CACHE_POLICY'
                );
              }
              Object.assign(behavior.Properties, {
                CachePolicyId: id || {
                  Ref: this.provider.naming.getCloudFrontCachePolicyLogicalId(name),
                },
              });
              // Backward compatibilty - Assinging default cache policy only if none of the ForwardedValues, MinTTL, MaxTTL and DefaultTTL fields are defined.
            } else if (shouldAssignCachePolicy) {
              // Assigning default Managed-CachingOptimized Cache Policy. See details at https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/using-managed-cache-policies.html#managed-cache-policies-list
              Object.assign(behavior.Properties, {

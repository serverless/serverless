...
class AwsCompileCloudFrontEvents {
  ...
  compileCloudFrontEvents() {
    ...
    function createOrigin(functionName, origin, naming) {
      ...
      Object.assign(originObj, {
        Id: naming.getCloudFrontOriginId(
          originObj.S3OriginConfig,
          originObj.DomainName,
          originObj.OriginPath
        ),
      });
      return originObj;
    }
      ...
      if (functionObj.events) {
        functionObj.events.forEach(event => {
          if (event.cloudFront) {
            ...
            const existingOrigin = _.find(origins, o => o.Id === origin.Id);
            ...
            if (event.cloudFront.defaultOrigin === true) {
              if (defaultOrigin !== undefined && defaultOrigin !== origin) {
                throw new Error(
                  'Found more than one cloudfront event with "defaultOrigin" defined'
                );
              }
            ...
            if (_.isObject(event.cloudFront.behavior)) {
              behavior = _.assignIn(behavior, event.cloudFront.behavior);
            }

            behavior = _.assignIn(behavior, {
    ...
    behaviors = _.sortBy(behaviors, b => {
      if (b.PathPattern) {
        return 1;
      }
      return 0;
    ...
    if (lambdaAtEdgeFunctions.length) {
      ...
      if (behaviors.length !== _.uniqBy(behaviors, 'PathPattern').length) {
        throw new Error('Found more than one behavior with the same PathPattern');
      }
      ...
      if (behaviors[0].PathPattern) {
        ...
        if (origin === undefined) {
          if (origins.length > 1) {
            throw new Error(
              'Found more than one origin but none of the cloudfront event has "defaultOrigin" defined'
            );
          }
          origin = origins[0];
        }

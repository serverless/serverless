...
describe('AwsCompileCloudFrontEvents', () => {
  ...
  describe('#compileCloudFrontEvents()', () => {
    ...
    it('should throw an error if runtime is not supported', () => {
      awsCompileCloudFrontEvents.serverless.service.functions = {
        first: {
          name: 'first',
          runtime: 'nodejs12.x',
      ...
      expect(() => awsCompileCloudFrontEvents.compileCloudFrontEvents()).to.throw(
        /^The function has a runtime that is not supported by Lambda@Edge/
      );
    });
    ...
    it('should throw if more than one cloudfront event with different origins were defined as a default', () => {
      ...
      expect(() => {
        awsCompileCloudFrontEvents.compileCloudFrontEvents();
      }).to.throw(Error, 'Found more than one cloudfront event with "defaultOrigin" defined');
    });
    ...
    it('should create DefaultCacheBehavior if non behavior without PathPattern were defined but defaultOrigin flag was set', () => {
      awsCompileCloudFrontEvents.serverless.service.functions = {
        first: {
          name: 'first',
          events: [

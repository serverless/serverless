  ...
  describe('#compileStreamEvents()', () => {
    ...
    describe('when a DynamoDB stream ARN is given', () => {
      it('should create event source mappings when a DynamoDB stream ARN is given', () => {
        ...
        expect(
          awsCompileStreamEvents.serverless.service.provider.compiledCloudFormationTemplate
            .Resources.FirstEventSourceMappingDynamodbBuzz.Properties.DestinationConfig.OnFailure.Destination
        ).to.equal(awsCompileStreamEvents.serverless.service.functions.first.events[4].stream.destinationConfig.onFailure.destination.arn);
      });
      ...
      it('should allow specifying OnFailure destinations as CFN reference types', () => {
              ...
              },
              {
                stream: {
                  ...
                  destinationConfig: {
                    onFailure: {
                      destination: {
                        arn: { 'Fn::ImportValue': 'ForeignSQSArn' },
                        type: 'sqs',
                      },
                    }
                  },
        ...
        expect(
          awsCompileStreamEvents.serverless.service.provider.compiledCloudFormationTemplate
            .Resources.FirstEventSourceMappingFoo.Properties.EventSourceArn
        ...
        expect(
          awsCompileStreamEvents.serverless.service.provider.compiledCloudFormationTemplate
            .Resources.FirstEventSourceMappingBar.Properties.EventSourceArn
        ).to.deep.equal({ 'Fn::ImportValue': 'ForeignSQS' });

        // sqs with Fn::Join
        expect(
          awsCompileStreamEvents.serverless.service.provider.compiledCloudFormationTemplate
            .Resources.FirstEventSourceMappingBaz.Properties.EventSourceArn
        ).to.deep.equal({
          'Fn::Join': [
            ':',
            [
      ...
      it('should add the necessary IAM role statements', () => {
        ...
        const iamRoleStatements = [
          ...
          },
          {
            ...
            Resource: [
              'arn:aws:sns:region:account:snstopic',
            ],
          },
        ];
    ...
    describe('when a Kinesis stream ARN is given', () => {
      it('should create event source mappings when a Kinesis stream ARN is given', () => {
        ...
        expect(
          awsCompileStreamEvents.serverless.service.provider.compiledCloudFormationTemplate
            .Resources.FirstEventSourceMappingKinesisFizz.Properties.DestinationConfig.OnFailure.Destination
        ).to.equal(awsCompileStreamEvents.serverless.service.functions.first.events[4].stream.destinationConfig.onFailure.destination.arn);
      });

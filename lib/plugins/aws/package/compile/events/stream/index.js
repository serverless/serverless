...
class AwsCompileStreamEvents {
  ...
  invalidDestinationPropertyErrorMessage(functionName, property) {
    return [
        ...
        functionObj.events.forEach(event => {
          if (event.stream) {
            ...
            if (event.stream.destinationConfig) {
              //Check to see destinationConfig exists
              if (event.stream.destinationConfig.onFailure && event.stream.destinationConfig.onFailure.destination) {
                //Validate ARN pattern and type
                if (typeof event.stream.destinationConfig.onFailure.destination === 'object') {
                  if (!event.stream.destinationConfig.onFailure.destination.arn) {
                    throw new this.serverless.classes.Error(
                      this.invalidDestinationPropertyErrorMessage(functionName, 'arn')
                    );
                  }
                  if (typeof event.stream.destinationConfig.onFailure.destination.arn !== "string") {
                    ...
                    if (!this.isValidStackImport(event.stream.destinationConfig.onFailure.destination.arn)) {
                      throw new this.serverless.classes.Error(
                        this.invalidDestinationPropertyErrorMessage(functionName, 'arn')
                      );
                    }
                  }
                } else if (typeof event.stream.destinationConfig.onFailure.destination === 'string') {
                ...
                const OnFailureDestinationArn = event.stream.destinationConfig.onFailure.destination.arn;

                const destinationType = event.stream.destinationConfig.onFailure.destination.type || OnFailureDestinationArn.split(':')[2];
                // add on failure destination ARNs to PolicyDocument statements
                if (destinationType === 'sns') {
                  onFailureSnsStatement.Resource.push(EventSourceArn);
                } else if (destinationType === 'sqs') {
                  onFailureSqsStatement.Resource.push(EventSourceArn);
                } else {
                  const errorMessage = [

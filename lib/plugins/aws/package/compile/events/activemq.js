  ...
  compileActiveMQEvents() {
    this.serverless.service.getAllFunctions().forEach((functionName) => {
      ...
      functionObj.events.forEach((event) => {
        ...
        hasMQEvent = true;
        const { basicAuthArn, arn, batchSize, enabled, queue } = event.activemq;
      ...
      if (cfTemplate.Resources.IamRoleLambdaExecution && hasMQEvent) {
        functionObj.iamConfig.policyStatements.push(secretsManagerStatement);
        functionObj.iamConfig.policyStatements.push(brokerStatement);

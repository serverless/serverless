...
const { makeAndHashRuleName, makeEventBusTargetId, makeRuleName } = require('./helpers');
...
class AwsCompileEventBridgeEvents {
  constructor(serverless, options) {
    ...
    this.hooks = {
      'initialize': () => {
        // TODO: VALIDATE THIS DEPRECATION
        const hasFunctionsWithEventBridgeTrigger = Object.values(
          this.serverless.service.functions
        ).some(({ events }) => events.some(({ eventBridge }) => eventBridge));
        const shouldUseCloudFormation =
          this.serverless.service.provider.eventBridge &&
          this.serverless.service.provider.eventBridge.shouldUseCloudFormation;
        if (hasFunctionsWithEventBridgeTrigger && !shouldUseCloudFormation) {
          this.serverless._logDeprecation(
            'AWS_EVENT_BRIDGE_CUSTOM_RESOURCE',
            'AWS EventBridge resources are not being created using native CloudFormation, this is now possible and the use of custom resources is deprecated. Set `shouldUseCloudFormation: true` as a provider property to use this now.'
          );
        }
      },
  ...
  compileEventBridgeEvents() {
    ...
    const shouldUseCloudFormation = options ? options.shouldUseCloudFormation : false;
  ...
  _compileWithCustomResource({
    ...
    // TODO: CHECK IF eventBusName is object and throw error if it is - custom resources do not support that

      ...
      _.forEach(functionObject.events, event => {
        ...
        if (_.has(event, 'websocket')) {
          ...
          if (_.isObject(event.websocket)) {
            ...
            let resultTtlInSeconds = undefined;

            if (event.websocket.private) {
              websocketObj.private = true;
            }
            ...
            if (_.isString(event.websocket.authorizer)) {
              ...
              resultTtlInSeconds = Number.parseInt(event.websocket.authorizer.resultTtlInSeconds, 10);
            }
       
            resultTtlInSeconds = Number.isNaN(resultTtlInSeconds) ? undefined : resultTtlInSeconds;

            if (resultTtlInSeconds !== undefined) { 
              websocketObj.resultTtlInSeconds = resultTtlInSeconds;
            }

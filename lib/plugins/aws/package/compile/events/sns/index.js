        ...
        functionObj.events.forEach(event => {
          if (event.sns) {
            ...
            if (event.sns.redrivePolicy && event.sns.redrivePolicy.deadLetterTargetArn) {
              if (event.sns.redrivePolicy.deadLetterTargetUrl){
                // Remove queue url from resources
                delete template.Resources[subscriptionLogicalId].Properties.RedrivePolicy.deadLetterTargetUrl
              }
              ...
              let queueUrl = event.sns.redrivePolicy.deadLetterTargetUrl;
              const arnDefinition = event.sns.redrivePolicy.deadLetterTargetArn;
              if (typeof arnDefinition === 'object' && arnDefinition['Fn::GetAtt']) {
                queueUrl = { Ref: arnDefinition['Fn::GetAtt'][0] };
              } else if (typeof arnDefinition === 'string' && arnDefinition.startsWith('arn:')) {
                // obtain queue url from an arn like arn:aws:sqs:us-east-2:444455556666:queue1
                const queueParts = arnDefinition.split(':');
                const partsLength = queueParts.length;
                queueUrl = {
                  'Fn::Join': [
                    '',
                    `https://sqs.${queueParts[partsLength - 3]}.`,
                    { Ref: 'AWS::URLSuffix' },
                    `/${queueParts[partsLength - 2]}/${queueParts[partsLength - 1]}`,
                  ],
                };
              }
              if (!queueUrl) {
                throw new this.serverless.classes.Error(
                  `deadLetterTargetArn on ${functionName} must be specified either with a valid arn string 
                   starting with arn: or with the CloudFormation function Fn::GetAtt in order to calculate the queue url
                   automatically. 
                   Alternatively, you can define both deadLetterTargetArn and deadLetterTargetUrl with
                   any CloudFormation supported functions.`
                );
              }

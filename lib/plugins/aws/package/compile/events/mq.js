...
class AwsCompileMQEvents {
  constructor(serverless) {
    this.serverless = serverless;
    this.provider = this.serverless.getProvider('aws');

    this.hooks = {
      'package:compileEvents': this.compileMQEvents.bind(this),
    };

    this.serverless.configSchemaHandler.defineFunctionEvent('aws', 'mq', {
      type: 'object',
      properties: {
        arn: {
          anyOf: [
  ...
  compileMQEvents() {
    this.serverless.service.getAllFunctions().forEach((functionName) => {
      ...
      // It is required to add the following statement in order to be able to connect to MQ cluster
      const ec2Statement = {
        Effect: 'Allow',
        Action: [
      ...
      functionObj.events.forEach((event) => {
        ...
        hasMQEvent = true;
        const { basicAuthArn, arn, batchSize, enabled, queue } = event.mq;

        const mqEventLogicalId = this.provider.naming.getMQEventLogicalId(functionName, queue);
...
module.exports = AwsCompileMQEvents;

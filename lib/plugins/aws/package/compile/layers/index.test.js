...
describe('AwsCompileLayers', () => {
  ...
  describe('#compileLayers()', () => {
    ...
    it('should create a layer resource with a retention policy, has the same logical id when not modify', () => {
      ...
      return expect(Promise.all([awsCompileLayers.compileLayers(), secondAwsCompileLayers.compileLayers()])).to.be.fulfilled.then(() => {
        expect(
          _.findKey(awsCompileLayers.serverless.service.provider.compiledCloudFormationTemplate.Resources, r => r.Type === 'AWS::Lambda::LayerVersion')
        ).to.deep.equal(_.findKey(secondAwsCompileLayers.serverless.service.provider.compiledCloudFormationTemplate.Resources, r => r.Type === 'AWS::Lambda::LayerVersion'));
        expect(
          awsCompileLayers.serverless.service.provider.compiledCloudFormationTemplate.Outputs
            .TestLambdaLayerQualifiedArn
        ).to.deep.equal(secondAwsCompileLayers.serverless.service.provider.compiledCloudFormationTemplate.Outputs.TestLambdaLayerQualifiedArn);
      });
    });
    ...
    it('should create a layer resource with a retention policy, has different logical id when modify', () => {
      ...
      return expect(Promise.all([awsCompileLayers.compileLayers(), secondAwsCompileLayers.compileLayers()])).to.be.fulfilled.then(() => {
        expect(
          _.findKey(awsCompileLayers.serverless.service.provider.compiledCloudFormationTemplate.Resources, r => r.Type === 'AWS::Lambda::LayerVersion')
        ).to.not.deep.equal(_.findKey(secondAwsCompileLayers.serverless.service.provider.compiledCloudFormationTemplate.Resources, r => r.Type === 'AWS::Lambda::LayerVersion'));
        expect(
          awsCompileLayers.serverless.service.provider.compiledCloudFormationTemplate.Outputs
            .TestLambdaLayerQualifiedArn
        ).to.not.deep.equal(secondAwsCompileLayers.serverless.service.provider.compiledCloudFormationTemplate.Outputs.TestLambdaLayerQualifiedArn);
      });
    });

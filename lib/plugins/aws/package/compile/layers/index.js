...
class AwsCompileLayers {
  ...
  compileLayer(layerName) {
    ...
    const layerArtifactPath = this.provider.naming.getLambdaLayerArtifactPath(
      this.packagePath,
      layerName
    );
    const layerArtifactBinary = fs.readFileSync(layerArtifactPath);
  ...
  compareWithLastLayer(layerName) {
    ...
    return this.provider
      ...
      .then(data => {
        const lastHash = _.find(data.Stacks[0].Outputs, {
          OutputKey: layerHashOutputLogicalId,
        });
        const compiledTemplateOutputs = this.serverless.service.provider
          .compiledCloudFormationTemplate.Outputs;
        const newSha = compiledTemplateOutputs[layerHashOutputLogicalId].Value;

        if (lastHash != null && lastHash.OutputValue === newSha) {
          const layerS3keyOutputLogicalId = this.provider.naming.getLambdaLayerS3KeyOutputLogicalId(
            layerName
          );
          const lastS3Key = _.find(data.Stacks[0].Outputs, {
            OutputKey: layerS3keyOutputLogicalId,
          });
          compiledTemplateOutputs[layerS3keyOutputLogicalId].Value = lastS3Key.OutputValue;
  ...
  compileLayers() {
    ...
    return BbPromise.each(allLayers, layerName => {
      this.compileLayer(layerName);
      return this.compareWithLastLayer(layerName);
    });
  }

...
const cliArgs = ['package', '--region', 'us-east-1'];
const pluginPathsWhitelist = [packagePluginPath, vpcPluginPath];
const lifecycleHookNamesWhitelist = ['package:initialize', 'package:compileVpc'];

describe('AwsCompileVpc', () => {
  describe('when using the simple VPC config', () => {
  ...
  describe('when using the extended VPC config', () => {
    it('should compile the necessary CloudFormation resources', () => {
      return runServerless({
        cwd: join(fixturesPath, 'extended-config'),
        cliArgs,
        pluginPathsWhitelist,
        lifecycleHookNamesWhitelist,
      }).then(serverless => {

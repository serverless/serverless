        ...
        versionCompilationPromise = addFileContentsToHashes(artifactFilePath, [
          ...
          .then(() => {
            ...
            layerConfigurations.forEach(layer => {
              const serverlessLayerObject = this.serverless.service.getLayer(layer.serverless.name);
              const layerArtifactPath =
                serverlessLayerObject.package && serverlessLayerObject.package.artifact
                  ...
                  : path.join(
                      this.serverless.config.servicePath,
                      '.serverless',
                      this.provider.naming.getLayerArtifactName(layer.serverless.name)
                    );
  ...
  functionProperties.Layers.forEach(potentialLocalLayerObject => {
    if (potentialLocalLayerObject.Ref) {
      ...
      layerConfigurations.push({
        serverless: cfTemplate.Resources[potentialLocalLayerObject.Ref]._serverless,

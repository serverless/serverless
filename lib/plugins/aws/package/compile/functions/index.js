    ...
    if (functionObject.onError) {
      ...
      if (typeof arn === 'string') {
        ...
        if (splittedArn[0] === 'arn' && (splittedArn[2] === 'sns' || splittedArn[2] === 'sqs')) {
          ...
          const iamPolicyLambdaExecution = this.serverless.service.provider
            .compiledCloudFormationTemplate.Resources[this.provider.naming.getPolicyLogicalId()];
          let stmt;
          ...
          if (iamRoleLambdaExecution && iamPolicyLambdaExecution) {
            iamPolicyLambdaExecution.Properties.PolicyDocument.Statement.push(stmt);
          }
        } else {
    ...
    if (kmsKeyArn) {
      ...
      if (typeof arn === 'string') {
        ...
        if (splittedArn[0] === 'arn' && (splittedArn[2] === 'kms')) {
          ...
          // update the PolicyDocument statements (if default policy is used)
          if (iamRoleLambdaExecution && iamPolicyLambdaExecution) {
            iamPolicyLambdaExecution.Properties.PolicyDocument.Statement = _.unionWith(
              iamPolicyLambdaExecution.Properties.PolicyDocument.Statement,
              [stmt],
              _.isEqual
            );
          }

...
class AwsCompileFunctions {
      ...
      if (shouldVersionFunction || functionObject.provisionedConcurrency) {
        ...
        versionCompilationPromise = this.addFileContentsToHashes(artifactFilePath, [
          ...
          .then(() => {
            ...
            const mappedLayerProperties = layerProperties.map(layer => {
              const clonedLayer = _.clone(layer);
              clonedLayer.properties.Properties.Content.S3Key = '';
              return clonedLayer.properties;
            });

            const layerAndFunctionProperties = _.concat(mappedLayerProperties, functionProperties);

            _.forOwn(layerAndFunctionProperties, value => {
              if (_.isObject(value)) {
  ...
  addFileContentsToHashes(filePath, hashes) {
    ...
    return new BbPromise(cb => {
      const readStream = fs.createReadStream(filePath);
      readStream
        .on('data', chunk => {
          _.forOwn(hashes, hash => {
            hash.write(chunk);
          });
        })
        .on('close', () => {
  ...
  extractLayerPropertiesFromFunction(functionProperties, cfTemplate) {
    ...
    _.forOwn(functionProperties, value => {
      if (!_.isArray(value)) {
        return;
      }
      _.forOwn(value, potentialLayerObject => {
        if (!_.isObject(potentialLayerObject)) {
          return;
        }
        if (_.has(potentialLayerObject, 'Ref')) {

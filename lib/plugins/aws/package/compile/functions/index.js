      ...
      if (shouldVersionFunction || functionObject.provisionedConcurrency) {
        ...
        versionCompilationPromise = addFileContentsToHashes(artifactFilePath, [
          ...
          .then(() => {
            ...
            const artifactHashPromises = [];
            layerConfigurations.forEach(layer => {
              // get layer serverless key from function ref value
              // i.e. { Ref: TestLayerLambdaLayer } -> testLayer
              const layerName = layer.ref;
              let serverlessLayerObject;
              const layerKeyName = layerName.split('LambdaLayer')[0];
              try {
                serverlessLayerObject = this.serverless.service.getLayer(layerKeyName);
              } catch (error) {
                const layerKeyNameCapitalized =
                  layerKeyName[0].toLowerCase() + layerKeyName.slice(1);
                serverlessLayerObject = this.serverless.service.getLayer(layerKeyNameCapitalized);
              }
              const layerArtifactPath =
              ...
              artifactHashPromises.push(addFileContentsToHashes(layerArtifactPath, [versionHash]));
            });

            return BbPromise.all(artifactHashPromises);
          })
          .then(() => {

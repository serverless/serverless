...
class AwsCompileFunctions {
  ...
  compileFunction(functionName) {
    return BbPromise.try(() => {
      ...
      if (Array.isArray(fileSystemConfigs) && fileSystemConfigs.length === 1) {
        const fileSystemConfig = fileSystemConfigs[0];

        if (!functionResource.Properties.VpcConfig) {
          const errorMessage = [
            `Function "${functionName}": when using fileSystemConfigs, `,
        ...
        const shouldUpdateIamRole =
          typeof fileSystemConfig.arn === 'string' && fileSystemConfig.arn.startsWith('arn');
        if (shouldUpdateIamRole) {
          const iamRoleLambdaExecution = cfTemplate.Resources.IamRoleLambdaExecution;

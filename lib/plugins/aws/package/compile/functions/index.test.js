...
describe('AwsCompileFunctions #2', () => {
  describe('Asynchronous Invocations', () => {
    ...
    it('should add log statements to functions iamConfig object', async () => {
      ...
      const { serverless, awsNaming } = await runServerless({
        ...
        configExt: {
          functions: {
            ...
            canonical: { handler: 'index.handler' },
          },
        },
        cliArgs: ['package'],
      });
      ...
      serverless.service.getAllFunctions().forEach(functionName => {
        const { name: resolvedFunctionName } = serverless.service.getFunction(functionName);

        const FunctionNamelogGroupsPrefix = awsNaming.getLogGroupName(resolvedFunctionName);

        const logStatements = _.cloneDeep(logStatementsTemplate);

        logStatements[0].Resource.push({
          'Fn::Sub':
            'arn:${AWS::Partition}:logs:${AWS::Region}:${AWS::AccountId}' +
            `:log-group:${FunctionNamelogGroupsPrefix}:*`,
        });

        logStatements[1].Resource.push({
          'Fn::Sub':
            'arn:${AWS::Partition}:logs:${AWS::Region}:${AWS::AccountId}' +
            `:log-group:${FunctionNamelogGroupsPrefix}:*:*`,
        });

        const statements = serverless.service.getFunction(functionName).iamConfig.policyStatements;

        expect(statements).to.deep.include(logStatements[0]);
        expect(statements).to.deep.include(logStatements[1]);
      });
    });
  });

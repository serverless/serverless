...
describe('AwsCompileFunctions #2', () => {
  ...
  describe('when using fileSystemConfigs', () => {
    it('should apply changes when single fileSystemConfig provided', () => {
      const arn =
        'arn:aws:elasticfilesystem:region:111111111111:access-point/fsap-a1a1a1a1a1a1a1a1a';
      const localMountPath = '/mnt/path';
      runServerless({
        cwd: fixtures.map.functionFileSystemConfigs,
        cliArgs: ['package'],
      }).then(({ awsNaming, cfTemplate }) => {
        const cfResources = cfTemplate.Resources;
        const naming = awsNaming;
        const functionConfig = cfResources[naming.getLambdaLogicalId('foo')].Properties;
    ...
    it('should update default role if fileSystemConfig.arn is provided as ARN', () => {
      const arn =
        'arn:aws:elasticfilesystem:region:111111111111:access-point/fsap-a1a1a1a1a1a1a1a1a';
      runServerless({
        cwd: fixtures.map.functionFileSystemConfigs,
    ...
    it('should not update default role if fileSystemConfig.arn is not provided as ARN', () => {
      const arn = { 'Fn::ImportValue': 'efs-ap' };
      const roleArn = 'arn:aws:iam:region:111111111111:role/role';
      return fixtures
        .extend('functionFileSystemConfigs', {
          functions: {
            foo: {
              role: roleArn,
            },
    ...
    it('should not update the role when default IamRoleLambdaExecution is not used', () => {
      const roleArn = 'arn:aws:iam:region:111111111111:role/role';
      return fixtures
        .extend('functionFileSystemConfigs', {
          functions: {
        ...
        .then(fixturePath =>
          runServerless({
            ...
            expect(cfResources.IamRoleLambdaExecution).to.deep.undefined;
          })
        );
    });
    ...
    it('should support vpc defined on provider level', () => {
      const arn =
        'arn:aws:elasticfilesystem:region:111111111111:access-point/fsap-a1a1a1a1a1a1a1a1a';
      const localMountPath = '/mnt/path';
      const subnetIds = ['sg-0a0a0a0a'];
      const securityGroupIds = ['subnet-01010101'];
      return fixtures
        .extend('functionFileSystemConfigs', {
          provider: {
            vpc: {

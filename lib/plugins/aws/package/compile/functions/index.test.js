...
describe('AwsCompileFunctions #2', () => {
  ...
  describe('when function versions are used with layers defined in the same service', () => {
    let firstCfTemplate;
    let servicePath;
    let updateConfig;
    ...
    it('should create same version id for same layer zip content', async () => {
      const firstVersionId = firstCfTemplate.Outputs.FooLambdaFunctionQualifiedArn.Value.Ref;
      const originalLayer = path.join(servicePath, 'testLayer');
      const sourceChangeLayer = path.join(servicePath, 'extra_layers', 'testLayerDuplicate');
      const backupLayer = path.join(servicePath, 'extra_layers', 'testLayerBackup');

      await fs.rename(originalLayer, backupLayer);
      await fs.rename(sourceChangeLayer, originalLayer);
      ...
      await fs.rename(originalLayer, sourceChangeLayer);
      await fs.rename(backupLayer, originalLayer);
    ...
    it('should create different lambda version id when layer content changed', async () => {
      const firstVersionId = firstCfTemplate.Outputs.FooLambdaFunctionQualifiedArn.Value.Ref;
      const originalLayer = path.join(servicePath, 'testLayer');
      const sourceChangeLayer = path.join(servicePath, 'extra_layers', 'testLayerSourceChange');
      const backupLayer = path.join(servicePath, 'extra_layers', 'testLayerBackup');

...
describe('AwsCompileFunctions', () => {
  ...
  describe('when function versions are used with layers in the same stack', () => {
    ...
    it('should create different version ids for identical lambdas with different layer zip content', () => {
      ...
      return runServerless({
        cwd: fixtures.map.functionLayersSourceChange,
        cliArgs: ['package'],
        awsRequestStubMap: {
          CloudFormation: {
            describeStacks: { Stacks: [{ Outputs: [{ OutputKey: 'test' }] }] },
    ...
    it('should create different version ids when reservedConcurrency is used', () => {
      return fixtures
        .extend('functionDestinations', {
          functions: {
            func1: { handler: 'index.handler' },
            func2: { handler: 'index.handler', reservedConcurrency: 1 },
          },
        })
        .then(fixturePath => {
          return runServerless({
        ...
        .then(data => {
          expect(data.cfTemplate.Outputs.Func1LambdaFunctionQualifiedArn.Value.Ref).to.not.equal(
            data.cfTemplate.Outputs.Func2LambdaFunctionQualifiedArn.Value.Ref
          );
        });
    });

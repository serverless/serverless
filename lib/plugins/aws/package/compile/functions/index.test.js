...
describe('AwsCompileFunctions', () => {
  ...
  let exampleCompiledLayerResource;
  let layerArtifactAlternate;
  let layerArtifact;
  ...
  beforeEach(() => {
    ...
    layerArtifact = 'layer.zip';
    layerArtifactAlternate = 'layer2.zip';
    ...
    serverless.utils.writeFileSync(
      path.join(awsCompileFunctions.packagePath, layerArtifact),
      'foobar layer'
    );
    serverless.utils.writeFileSync(
      path.join(awsCompileFunctions.packagePath, layerArtifactAlternate),
      'barbaz layer'
    );

    exampleCompiledLayerResource = {
      Type: 'AWS::Lambda::LayerVersion',
      Properties: {
        CompatibleRuntimes: ['test1.0'],
        Content: {
          S3Bucket: 'my-bucket-us-west-2-123456789012',
          S3Key: `deployment1/${layerArtifact}`,
        },
  ...
  describe('#compileFunctions()', () => {
    ...
    describe('when function versions are used with layers in the same stack', () => {
      /*
       * Template function for testing lambda version generation with layers in isolation.
       *
       * The layer name affects the version id so to test and compare the affect of layer
       * property and content changes in insolation the template must be compiled twice with
       * the same resource name but different values.
       *
       * @param {Object} layerResource Should conform to "AWS::Lambda::LayerVersion" syntax
       * specified by AWS CloudFormation docs.
       * @param {string} versionDigest The expected digest that will be generated and appended to
       * the function name to create the function version id.
       */
      const isolatedLayerTestTemplate = (
        layerResource,
        versionDigest,
        additionalFunctionProperties = null
      ) => {
        // explicitly reset the template on each test
        awsCompileFunctions.serverless.service.provider.compiledCloudFormationTemplate = {
          Resources: {},
          Outputs: {},
        };
        ...
        if (additionalFunctionProperties !== null) {
          Object.keys(additionalFunctionProperties).forEach(key => {
            functionProperties[key] = additionalFunctionProperties[key];
          });
        }

        if (layerResource !== null) {
          // add a compiled lambda layer to cf template with consistent resource name
          const layerResourceName = 'TestLambdaLayer';
          const cfTemplate =
            awsCompileFunctions.serverless.service.provider.compiledCloudFormationTemplate;
          cfTemplate.Resources[layerResourceName] = layerResource;
          functionProperties.layers = [{ Ref: layerResourceName }];
        }
        ...
        const expectedOutputs = {
          FuncLambdaFunctionQualifiedArn: {
            Description: 'Current Lambda function version',
            Value: { Ref: `FuncLambdaVersion${versionDigest}` },
          },
        };
      ...
      it('should create different version ids when reservedConcurrency is used', () => {
        const additionalFunctionProperties = { reservedConcurrency: 1 };
        const nonReservedConcurrecyVersionDigest = 'FCo2JuiSBHVZxAmUUQHmoUBvayxgPRFR8y1U60HO5I';
        const reservedConcurrecyVersionDigest = 'gNU876hF2nIOvl4mTZ5yZivvOcb9eW9bKMtF5M';
        expect(nonReservedConcurrecyVersionDigest).not.to.equal(reservedConcurrecyVersionDigest);
        return isolatedLayerTestTemplate(
          exampleCompiledLayerResource,
          nonReservedConcurrecyVersionDigest
        ).then(() => {
          return isolatedLayerTestTemplate(
            exampleCompiledLayerResource,
            reservedConcurrecyVersionDigest,
            additionalFunctionProperties
          );
        });
      });
      ...
      it('should create different version ids for identical lambdas with and without layers', () => {
        return isolatedLayerTestTemplate(
          exampleCompiledLayerResource,
          'FCo2JuiSBHVZxAmUUQHmoUBvayxgPRFR8y1U60HO5I'
        ).then(() => {
          return isolatedLayerTestTemplate(null, 'Rk2uCHjn9BWyD0yH8GzU5kTmGVjCc6ZZx46sUUI1LQ');
        });
      });

      it('should create different version ids for identical lambdas with different layer zip content', () => {
        const exampleCompiledLayerResourceDifferentZip = _.cloneDeep(exampleCompiledLayerResource);
        exampleCompiledLayerResourceDifferentZip.Properties.Content.S3Key = `deployment1/${layerArtifactAlternate}`;
        return isolatedLayerTestTemplate(
          exampleCompiledLayerResource,
          'FCo2JuiSBHVZxAmUUQHmoUBvayxgPRFR8y1U60HO5I'
        ).then(() => {
          return isolatedLayerTestTemplate(
            exampleCompiledLayerResourceDifferentZip,
            'c8W1Ocd1AQRaAUC92QFBa3CCzwlzo0eVzzcwnk1uU4'
          );
        });
      });

      it('should create different version ids for identical lambdas with different layer properties', () => {
        const exampleCompiledLayerResourceDifferentProperties = _.cloneDeep(
          exampleCompiledLayerResource
        );
        exampleCompiledLayerResourceDifferentProperties.Properties.Description =
        ...
        return isolatedLayerTestTemplate(
          ...
          return isolatedLayerTestTemplate(
            exampleCompiledLayerResourceDifferentProperties,
            'oVPJuDKjciY5SFlFWLKMRTe2xV4e7Hbti2BpUr0'
          );
        });
      });

      it('should create the same version ids for identical lambdas with layers with only different S3Key paths', () => {
        ...
        const exampleCompiledLayerResourceS3KeyPathChange = _.cloneDeep(
          exampleCompiledLayerResource
        );
        exampleCompiledLayerResourceS3KeyPathChange.Properties.Content.S3Key = `deployment2/${layerArtifact}`;
        const expectedVersionDigest = 'FCo2JuiSBHVZxAmUUQHmoUBvayxgPRFR8y1U60HO5I';
        return isolatedLayerTestTemplate(exampleCompiledLayerResource, expectedVersionDigest).then(
          () => {
            return isolatedLayerTestTemplate(
              exampleCompiledLayerResourceS3KeyPathChange,

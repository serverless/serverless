...
describe('AwsCompileFunctions', () => {
  ...
  describe('when function versions are used with layers in the same stack', () => {
    ...
    before(() => {
      return runServerless({
        cwd: fixtures.map.functionLayers,
        cliArgs: ['package'],
        awsRequestStubMap: {
          CloudFormation: {
    ...
    it('should create different version ids for identical lambdas with and without layers', () => {
      expect(firstCfTemplate.Outputs.FooLambdaFunctionQualifiedArn.Value.Ref).to.not.equal(
        firstCfTemplate.Outputs.BarLambdaFunctionQualifiedArn.Value.Ref
      );
    });

    it('should create different version ids for identical lambdas with different layer zip content', () => {
      const firstVersionId = firstCfTemplate.Outputs.FooLambdaFunctionQualifiedArn.Value.Ref;

      const functionLayersBasePath = fixtures.map.functionLayers;
      const originalLayer = path.join(functionLayersBasePath, 'testLayer');
      const sourceChangeLayer = path.join(
        functionLayersBasePath,
        'extra_layers',
        'testLayerSourceChange'
      );
      const backupLayer = path.join(functionLayersBasePath, 'extra_layers', 'testLayerBackup');

      fs.renameSync(originalLayer, backupLayer);
      fs.renameSync(sourceChangeLayer, originalLayer);
      return fixtures
        .extend('functionLayers', {})
        .then(fixturePath => {
          return runServerless({
            cwd: fixturePath,
            cliArgs: ['package'],
        ...
        .then(data => {
          fs.renameSync(originalLayer, sourceChangeLayer);
          fs.renameSync(backupLayer, originalLayer);
          expect(firstVersionId).to.not.equal(
            data.cfTemplate.Outputs.FooLambdaFunctionQualifiedArn.Value.Ref
          );
        });
    });

    it('should create same version ids for identical lambdas with identical layer zip content', () => {
      const firstVersionId = firstCfTemplate.Outputs.FooLambdaFunctionQualifiedArn.Value.Ref;
    ...
    it('should create the same version ids for identical lambdas with differently ordered properties', () => {
      const firstVersionId = firstCfTemplate.Outputs.FooLambdaFunctionQualifiedArn.Value.Ref;

      return fixtures
        .extend('functionLayers', {
          functions: {
            foo: { layers: [{ Ref: 'TestLayerLambdaLayer' }], handler: 'index.handler' },
          },
        })
        .then(fixturePath => {
          return runServerless({
    ...
    it('should create different version ids if a property key changes but the value does not', () => {
      const firstVersionId = firstCfTemplate.Outputs.FooBarLambdaFunctionQualifiedArn.Value.Ref;

      return fixtures
        .extend('functionLayers', {
          functions: {
            fooBar: { handler: 'index.handler', provisionedConcurrency: 1 },
          },
        })
        .then(fixturePath => {
          return runServerless({
        ...
        .then(data => {
          expect(firstVersionId).to.equal(
            data.cfTemplate.Outputs.FooBarLambdaFunctionQualifiedArn.Value.Ref
          );
        });
    });
  });
  ...
  describe('#compileFunctions()', () => {
    ...
    it('should create different version ids when reservedConcurrency is used', () => {
      ...
      return fixtures
        .extend('function', {
          functions: { foo: { handler: 'index.handler', reservedConcurrency: 1 } },
        })
        .then(fixturePath => {
          return runServerless({
            cwd: fixturePath,

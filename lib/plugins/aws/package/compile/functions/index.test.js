...
describe('AwsCompileFunctions', () => {
  ...
  describe('#compileFunctions()', () => {
    ...
    it('should only add an SLS_LAMBDA_VERSION_DIGEST env var to versioned functions', () => {
      awsCompileFunctions.serverless.service.functions = {
        func: {
          handler: 'func.function.handler',
        },
      ...
      return expect(awsCompileFunctions.compileFunctions()).to.be.fulfilled.then(() => {
        expect(
          awsCompileFunctions.serverless.service.provider.compiledCloudFormationTemplate.Resources
            .FuncLambdaFunction.Properties.Environment.Variables.SLS_LAMBDA_VERSION_DIGEST
        ).to.exist;
        expect(
          awsCompileFunctions.serverless.service.provider.compiledCloudFormationTemplate.Resources
            .AnotherFuncLambdaFunction.Properties.Environment.Variables.SLS_LAMBDA_VERSION_DIGEST
        ).to.not.exist;
      });
    });
  });

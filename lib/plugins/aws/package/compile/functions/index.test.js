...
describe('AwsCompileFunctions', () => {
    ...
    exampleCompiledLayerResource = {
      ...
      Properties: {
        ...
        Content: {
          ...
          S3Key: path.join('deployment1', layerArtifact),
        },
  ...
  describe('#compileFunctions()', () => {
    ...
    describe('when function versions are used with layers in the same stack', () => {
       ...
       */
      const isolatedLayerTestTemplate = (layerResource, additionalFunctionProperties = null) => {
        ...
        return expect(awsCompileFunctions.compileFunctions()).to.be.fulfilled.then(() => {
          const outputs =
            awsCompileFunctions.serverless.service.provider.compiledCloudFormationTemplate.Outputs;
          const versionId = outputs.FuncLambdaFunctionQualifiedArn.Value.Ref.split(
            'LambdaVersion'
          ).pop();
          expect(versionId).to.exist;
          return versionId;
        });
      };
      ...
      it('should create different version ids when reservedConcurrency is used', () => {
        ...
        const ids = {};
        return isolatedLayerTestTemplate(exampleCompiledLayerResource)
          .then(id => {
            ids.first = id;
          ...
          .then(id => {
            ids.second = id;
            return expect(ids.first).to.not.equal(ids.second);
          });
      });
      ...
      it('should create different version ids for identical lambdas with and without layers', () => {
        ...
        return isolatedLayerTestTemplate(exampleCompiledLayerResource)
          .then(id => {
            ids.first = id;
            return isolatedLayerTestTemplate(null);
          })
          .then(id => {
            ids.second = id;
            return expect(ids.first).to.not.equal(ids.second);
      ...
      it('should create different version ids for identical lambdas with different layer zip content', () => {
        ...
        exampleCompiledLayerResourceDifferentZip.Properties.Content.S3Key = path.join(
          'deployment1',
          layerArtifactAlternate
        );
        const ids = {};
        return isolatedLayerTestTemplate(exampleCompiledLayerResource)
          .then(id => {
            ids.first = id;
            return isolatedLayerTestTemplate(exampleCompiledLayerResourceDifferentZip);
          })
          .then(id => {
            ids.second = id;
            return expect(ids.first).to.not.equal(ids.second);
      ...
      it('should create different version ids for identical lambdas with different layer properties', () => {
        ...
        return isolatedLayerTestTemplate(exampleCompiledLayerResource)
          .then(id => {
            ids.first = id;
            return isolatedLayerTestTemplate(exampleCompiledLayerResourceDifferentProperties);
          })
          .then(id => {
            ids.second = id;
            return expect(ids.first).to.not.equal(ids.second);
      ...
      it('should create the same version ids for identical lambdas with layers with only different S3Key paths', () => {
        ...
        exampleCompiledLayerResourceS3KeyPathChange.Properties.Content.S3Key = path.join(
          'deployment2',
          layerArtifact
        );
        const ids = {};
        return isolatedLayerTestTemplate(exampleCompiledLayerResource)
          .then(id => {
            ids.first = id;
            return isolatedLayerTestTemplate(exampleCompiledLayerResourceS3KeyPathChange);
          })
          .then(id => {
            ids.second = id;
            return expect(ids.first).to.equal(ids.second);
          });
      });

      it('should throw exception when layer zip artifact cannot be opened', () => {
        exampleCompiledLayerResource.Properties.Content.S3Key = 'file-does-not-exist.zip';
        return expect(isolatedLayerTestTemplate(exampleCompiledLayerResource)).to.be.rejected;
      });
    });

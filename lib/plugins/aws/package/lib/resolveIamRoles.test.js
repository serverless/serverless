...
describe('#resolveIamRoles()', () => {
  ...
  before(async () => {
    ...
    resource = cfTemp.Resources[naming.getRoleLogicalId()].Properties;
    serverless = sls;
  });

  it('should add all principals from provider.iamConfig to to CF Template', () => {
    const templatePrincipals = resource.AssumeRolePolicyDocument.Statement[0].Principal.Service;
  ...
  it('should add all provider managed policies from provider.iamConfig to CF Template', () => {
    const templateManagedPolicies = resource.ManagedPolicyArns;
  ...
  it('should add all policy statements from provider.iamConfig to CF Template', () => {
    const templatePolicyStatements = resource.Policies[0].PolicyDocument.Statement;

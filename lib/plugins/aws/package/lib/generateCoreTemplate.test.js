...
describe('#generateCoreTemplate()', () => {
  ...
  it('should throw error if S3 bucket name is invalid', () => {
    const bucketName = 'com.$erverle$$.deploy$';
    sinon.stub(awsPlugin, 'validateS3BucketName').returns('invalid-name');

    awsPlugin.serverless.service.provider.deploymentBucket = bucketName;
    return awsPlugin.generateCoreTemplate()
      .catch((err) => {
        expect(err.message).to.contain('Bucket name must end with a letter or number.');
      })
      .then(() => {});
  });

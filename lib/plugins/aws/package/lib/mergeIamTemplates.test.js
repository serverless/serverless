...
describe('#mergeIamTemplates() disableLogs flag', () => {
  const withDisableLogsTrueFunctionName = 'withDisableLogsTrue';
  const withDisableLogsFalseFunctionName = 'withDisableLogsFalse';
  const withoutDisableLogsFunctionName = 'withoutDisableLogs';
  let serverless = null;
  let withDisableLogsFalseFunction = null;
  let withoutDisableLogsFunction = null;
  let compiledCloudFormationTemplate = null;
  before(() => {
    return runServerless({
      cwd: 'tests/fixtures/functionDisableLogs',
      cliArgs: ['package'],
    }).then(localServerless => {
      serverless = localServerless;
      withDisableLogsFalseFunction = serverless.service.getFunction(
        withDisableLogsFalseFunctionName
      );
      withoutDisableLogsFunction = serverless.service.getFunction(withoutDisableLogsFunctionName);
      compiledCloudFormationTemplate = serverless.service.provider.compiledCloudFormationTemplate;
    });
  });

  it('should add a CloudWatch LogGroup resource if disableLogs flag not provided', () => {
    const normalizedName = serverless.providers.aws.naming.getLogGroupLogicalId(
      withoutDisableLogsFunctionName
    );
    expect(compiledCloudFormationTemplate.Resources[normalizedName]).to.deep.equal({
      Type: 'AWS::Logs::LogGroup',
      Properties: {
        LogGroupName: serverless.providers.aws.naming.getLogGroupName(
          withoutDisableLogsFunction.name
        ),
      },
    });
  });
  ...
  it('should add a CloudWatch LogGroup resource if disableLogs is false', () => {
    ...
    expect(compiledCloudFormationTemplate.Resources[normalizedName]).to.deep.equal({
      ...
      Properties: {
        LogGroupName: serverless.providers.aws.naming.getLogGroupName(
          withDisableLogsFalseFunction.name
        ),
      },
    });
  });
  ...
  it('should not add a CloudWatch LogGroup resource if disableLogs is true', () => {
    const normalizedName = serverless.providers.aws.naming.getLogGroupLogicalId(
      withDisableLogsTrueFunctionName
    );

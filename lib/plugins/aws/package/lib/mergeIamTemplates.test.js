...
describe('#mergeIamTemplates using runServerless util', async () => {
  ...
  it('should create Lambda execution IAM role with empty array in policy statements', () => {
    expect(
      cfResources[awsNaming.getRoleLogicalId()].Properties.Policies[0].PolicyDocument.Statement
    ).to.be.an('array').that.is.empty;
  });

  it('should create empty (for now) awsProvider.iamConfig and functions[].iamConfig objects', () => {
    // check inside serverless object returned by runServerless util
    expect(serverless).to.exist;
  });
});

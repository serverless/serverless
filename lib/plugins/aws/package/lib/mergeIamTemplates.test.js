...
describe('#mergeIamTemplates using runServerless util', async () => {
  ...
  it('should create awsProvider.iamConfig object', () => {
    expect(serverless.service.provider.iamConfig).to.exist;
  });

  it('should create iamConfig object for each function', () => {
    serverless.service.getAllFunctions().forEach(functionName => {
      expect(serverless.service.getFunction(functionName).iamConfig).to.exist;
    });
  });

  it('should add policy statements from provider.iamRoleStatements to provider.iamConfig.policyStatements', () => {
    expect(serverless.service.provider.iamConfig.policyStatements).to.deep.equal(
      testIamRoleStatements
    );
  });
  ...
  it('should add managed policies from provider.iamManagedPolicies to provider.iamConfig.managedPolicies', () => {
    testManagedPolicies.forEach(policy => {
      expect(serverless.service.provider.iamConfig.managedPolicies).to.deep.include(policy);
    });
  });

  it('should add VPC related policies to provider.iamConfig.managedPolicies if provider has vpc config', () => {
    if (serverless.service.provider.vpc) {
      expect(serverless.service.provider.iamConfig.managedPolicies).to.deep.include({
        'Fn::Join': [
          '',
          [
            'arn:',
...
describe('#mergeIamTemplates using runServerlessUtil. Empty config', async () => {
  let serverless;
  before(async () => {
    const { serverless: sls } = await runServerless({
      fixture: 'function',
      cliArgs: ['package'],
    });
    serverless = sls;

...
module.exports = {
  mergeIamTemplates() {
    ...
    const assumeRolePolicyDocumentStatements =
      this.serverless.service.provider.compiledCloudFormationTemplate.Resources[
    ...
    let hasSchedulerFunctions = false;
    ...
    this.serverless.service.getAllFunctions().forEach((functionName) => {
      if (!hasSchedulerFunctions) {
        ...
        events.forEach((event) => {
          if (event.schedule && event.schedule.method === 'scheduler') {
            hasSchedulerFunctions = true;
          }
        });
      }
    ...
    // allow invoking of lambda functions for scheduler
    if (hasSchedulerFunctions) {
      assumeRolePolicyDocumentStatements[0].Principal.Service.push('scheduler.amazonaws.com');

  ...
  constructor(serverless, options) {
    ...
    this.hooks = {
      'initialize': () => {
        ...
        this.serverless.service.package.artifactsS3KeyDirname = `${this.provider.getDeploymentPrefix()}/${this.serverless.service.getShortServiceName()}/${this.provider.getStage()}/code-artifacts`;
        if (this.serverless.processedInput.commands.join(' ') === 'package') {
          log.notice();
          log.notice(
            `Packaging ${this.serverless.service.getShortServiceName()} for stage ${this.serverless
              .getProvider('aws')

...
class AwsPackage {
  constructor(serverless, options) {
    ...
    this.hooks = {
       ...
       */
      'package:cleanup': () => this.serverless.pluginManager
        .spawn('aws:common:cleanupTempDir'),

      'package:createDeploymentArtifacts': () => this.serverless.pluginManager
        .spawn('aws:package:createDeploymentArtifacts'),

      'package:initialize': () => this.serverless.pluginManager
        .spawn('aws:package:initialize'),

      'package:compileFunctions': () => BbPromise.resolve(),

      'package:compileEvents': () => BbPromise.resolve(),

      'package:finalize': () => this.serverless.pluginManager
        .spawn('aws:package:finalize'),

      /**
       * Inner lifecycle hooks
       */

      // Package createDeploymentArtifacts inner lifecycle
      'aws:package:createDeploymentArtifacts:validate': () => BbPromise.bind(this)
        .then(() => this.serverless.pluginManager.spawn('aws:common:validate')),

      'aws:package:createDeploymentArtifacts:packageService': () => BbPromise.bind(this)
        .then(this.packageService),

      // Package initialize inner lifecycle
      'aws:package:initialize:generateCoreTemplate': () => BbPromise.bind(this)
        .then(this.generateCoreTemplate),

      'aws:package:initialize:mergeIamTemplates': () => BbPromise.bind(this)
        .then(this.mergeIamTemplates),

      'aws:package:initialize:generateArtifactDirectoryName': () => BbPromise.bind(this)

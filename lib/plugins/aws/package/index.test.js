  ...
  describe('hooks', () => {
    ...
    beforeEach(() => {
      ...
      setDeploymentBucketObjectStub = sinon
        .stub(awsPackage, 'setDeploymentBucketObject').resolves();
    ...
    afterEach(() => {
      ...
      awsPackage.setDeploymentBucketObject.restore();
    ...
    it('should run "package:initialize" hook', () => awsPackage
      .hooks['package:initialize']().then(() => {
        expect(setDeploymentBucketObjectStub.calledOnce).to.equal(true);
        expect(generateCoreTemplateStub.calledAfter(setDeploymentBucketObjectStub))
          .to.equal(true);
      })
    );

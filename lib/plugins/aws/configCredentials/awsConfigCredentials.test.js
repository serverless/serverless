...
describe('AwsConfigCredentials', () => {
  ...
  describe('#constructor()', () => {
    ...
    it('should have a "config:credentials:credentials" hook', () => {
      expect(awsConfigCredentials.hooks['config:credentials:credentials']).to.not.equal(undefined);
    });

    it('should run promise chain in order for "config:credentials:dredentials" hook', () => {
      const awsConfigCredentialsStub = sinon
        .stub(awsConfigCredentials, 'configureCredentials').returns(BbPromise.resolve());

      return awsConfigCredentials.hooks['config:credentials:credentials']().then(() => {

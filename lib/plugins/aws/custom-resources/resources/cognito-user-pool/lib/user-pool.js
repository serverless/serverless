...
const { CognitoIdentityProviderClient, ListUserPoolsCommand, DescribeUserPoolCommand, UpdateUserPoolCommand } = require('@aws-sdk/client-cognito-identity-provider')
...
async function findUserPoolByName(config) {
  ...
  async function recursiveFind(nextToken) {
    ...
    return cognito.send(new ListUserPoolsCommand(payload)
    ).then((result) => {

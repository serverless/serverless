...
const { EventBridgeClient, CreateEventBusCommand, DeleteEventBusCommand, PutRuleCommand, DeleteRuleCommand, PutTargetsCommand, RemoveTargetsCommand } = require('@aws-sdk/client-eventbridge')
...
async function createEventBus(config) {
  ...
  if (eventBus) {
    ...
    return eventBridge.send(new CreateEventBusCommand({
      Name: eventBus,
    }))
  }
  return Promise.resolve();
...
async function deleteEventBus(config) {
  ...
  if (eventBus) {
    ...
    return eventBridge.send(new DeleteEventBusCommand({
      Name: eventBus,
    }))
  }
  return Promise.resolve();
...
async function updateRuleConfiguration(config) {
  ...
  return eventBridge.send(new PutRuleCommand({
    Name: ruleName,
    EventBusName,
...
async function removeRuleConfiguration(config) {
  ...
  return eventBridge.send(new DeleteRuleCommand({
    Name: ruleName,
    EventBusName,
  }))
}
...
async function updateTargetConfiguration(config) {
  ...
  return removeTargetConfiguration(config).then(() =>
   eventBridge.send(new PutTargetsCommand({
      Rule: ruleName,
      EventBusName,
      Targets: [target],
   }))
...
async function removeTargetConfiguration(config) {
  ...
  return eventBridge.send(new RemoveTargetsCommand({

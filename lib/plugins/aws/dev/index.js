...
class AwsDev {
   ...
   */
  async update() {
    if (this.options.region && !supportedRegions.includes(this.options.region)) {
      throw new ServerlessError(
        `The "serverless dev" command does not support the "${
          this.options.region
        }" region. Supported regions are: ${supportedRegions.join(', ')}.`,
        'UNSUPPORTED_REGION'
      );
    }
    ...
    if (notNodeFunction) {
      ...
      throw new ServerlessError(
        `This command does not yet support the "${notNodeRuntime}" runtime. It currently only supports JavaScript and TypeScript on the Node.js runtime. Please refer to the documentation for more information: https://www.serverless.com/framework/docs/providers/aws/cli-reference/dev`,

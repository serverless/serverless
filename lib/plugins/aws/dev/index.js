...
class AwsDev {
  ...
  async dev() {
    ...
    try {
      if (this.serverless.serviceOutputs) {
        ...
        const endpoints = this.serverless.serviceOutputs.get('endpoints')
  ...
  async update() {
    if (
      this.options.region &&
      !supportedRegions.includes(this.options.region)
    ) {
      throw new ServerlessError(

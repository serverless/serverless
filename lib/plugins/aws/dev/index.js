...
const { log, style, progress, stringToSafeColor } = utils;
const logger = log.get('sls:dev');
const mainProgress = progress.get('sls:dev');
...
const getEventLog = (event) => {
  ...
  // S3
  if (event.Records && event.Records.length === 1 && event.Records[0].eventSource === 'aws:s3') {
...
const logFunctionEvent = (functionName, event, isVerbose, invocationColorFn) => {
  try {
    ...
    if (isVerbose) {
      logger.aside(`${util.inspect(event, { showHidden: true, depth: null, colors: true })}`);
    }
  } catch (e) {}
};

const logFunctionResponse = (functionName, response, isVerbose, invocationColorFn) => {
  try {
    if (response) {
      if (response.statusCode) {
        logger.aside(`${invocationColorFn('←')} λ ${functionName} (${response.statusCode})`);
      }
...
class AwsDev {
  ...
  async dev() {
    logger.logo();
    console.log(); // TODO replace with logger.blankLine() when it no longer adds an extra line

    mainProgress.notice('Connecting', { isMainEvent: true });
  ...
  async update() {
    ...
    this.serverless.service.getAllFunctions().forEach((functionName) => {
      ...
      const functionRuntime =
        functionConfig.runtime || this.serverless.service.provider.runtime || 'nodejs14.x';

      if (localRuntime !== functionRuntime) {
        isRuntimeMismatch = true;
      }
  ...
  async connect() {
    ...
    device.on('error', (e) => {
      logger.error('Disconnected. Retrying to connect...');
    });

    device.on('connect', () => {
      logger.success(`Connected ${style.aside('(Ctrl+C to cancel)')}`);
  ...
  async watch() {
    ...
    chokidar.watch(configFilePath).on('change', (event, path) => {
      log.warning(`If you made infrastructure changes, please restart the dev command.`);
    });
  }
}

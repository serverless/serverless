...
const devProgress = progress.get('dev');
...
class AwsDev {
  ...
  async dev() {
    ...
    devProgress.notice('Deploying Dev Mode Changes');
  ...
  async pack() {
    ...
    try {
      /**
       * We use esbuild to bundle and minify the shim file,
       * without actually writing it to disk. We simply append
       * it to the zip file below.
       */
      const shimFile = await esbuild.build({
        bundle: true,
        minify: true,
        write: false,
        platform: 'node',
      ...
      shimFileContents = shimFile.outputFiles[0].text;
    } catch (e) {
      console.error(e);
  ...
  async update() {
    ...
    const iamRoleStatements = _.get(this.serverless.service.provider, 'iam.role.statements', []);
  ...
  getDevModeFunctionId(functionName) {
    ...
    return `serverless/${region}/${serviceName}/${stage}/${functionName}`;
  }
   ...
   * "serverless/regionName/serviceName/stageName/functionName". This method splits the identifier by '/'
   ...
   */
  async connect() {
    devProgress.update('Connecting to Lambda Functions');
    ...
    device.on('connect', () => {
      ...
      devProgress.update('Listening for Events');
    });

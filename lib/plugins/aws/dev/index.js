...
const logFunctionEvent = (functionName, event, isVerbose) => {
  try {
    const eventLog = getEventLog(event);

    logger.aside(`→ λ ${functionName} ${eventLog}`);
...
class AwsDev {
  ...
  async connect() {
    ...
    device.on('connect', () => {
      logger.success('Connected. Listening for Events...');
      logger.blankLine();
    ...
    device.on('message', async (topic, buffer) => {
      ...
      logFunctionEvent(functionName, event, this.options.detailed);
      ...
      if (executionTimeInMs > timeoutInMs) {
        console.log();
        logNotice(
          `Local invocation of function "${functionName}" took ${executionTimeInMs}ms, which surpases the configured timeout of ${timeoutInMs}ms. Consider increasing the timeout, or optimizing your function.`
        );
        console.log();
      }

      logFunctionResponse(functionName, response.response, this.options.detailed);

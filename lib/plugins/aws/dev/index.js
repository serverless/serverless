...
class AwsDev {
  ...
  async connect() {
    ...
    device.subscribe(this.getTopicId(`_ping`), {
      qos: 1,
    });
    ...
    device.on('message', async (topic, buffer) => {
      ...
      /**
       * If the function name is _ping, this is just a ping message to check if the connection is still alive.
       * We respond with a pong message to confirm the connection is still alive, then return.
       */
      if (functionName === '_ping') {
        device.publish(this.getTopicId('_pong'), '{}', {
          qos: 1,
        });

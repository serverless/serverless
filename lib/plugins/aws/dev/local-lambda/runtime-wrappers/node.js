...
const importFunction = async (handlerFileAbsolutePath, handlerName) => {
  ...
  try {
    ...
    handlerFunction = (await import(fileUrl))[handlerName]
  } catch (error) {
    /**
     * Throw a helpful message if the user is mistakenly trying to use TypeScript by setting "type": "module" in package.json.
     */
    if (
      ...
      throw new Error(
        `If you are using TypeScript, please remove the "type": "module" property from package.json as Serverless uses esbuild to transpile your TypeScript code to CommonJS.`,
      )
    }
    throw e
  }

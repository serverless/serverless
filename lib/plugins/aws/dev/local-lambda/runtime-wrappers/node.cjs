...
const importFunction = async (handlerFileAbsolutePath, handlerName) => {
  ...
  try {
    ...
    // Handle ESM syntax errors
    if (
      // This happens when the user sets "type": "module" in their package.json
      error.code === 'ERR_REQUIRE_ESM' ||
      // This happens when the user does not configure the build settings in serverless.yml
      error.message === 'Cannot use import statement outside a module'
    ) {
      // We can't use ServerlessError here because it is ESM and this file is CommonJS
      throw new Error(
        'To use ESM syntax, please remove the "type" property from your "package.json" file and configure the build settings in your serverless.yml file. For more information please refer to the "build" documentation: https://www.serverless.com/framework/docs/providers/aws/guide/building',
      )
    }

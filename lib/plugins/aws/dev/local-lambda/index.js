...
class LocalLambda {
  constructor(config = {}) {
    ...
    this.environment = {
      ...process.env,
      ...(config.environment || {}),
      TS_NODE_PROJECT: path.resolve(config.serviceAbsolutePath, 'tsconfig.json'),
    };
  }
  ...
  async invoke(event = {}, context = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const child = spawn(runtimeWrapper.command, [runtimeWrapper.path, argsString], {
...
const runtimeWrappers = [
  {
    command: 'node',
    path: path.resolve(__dirname, 'runtime-wrappers/wrapper.js'),
    versions: ['nodejs14.x', 'nodejs16.x', 'nodejs18.x', 'nodejs20.x'],
    extensions: ['.js', '.mjs', '.cjs'],
  },
  {
    command: 'ts-node',
    path: path.resolve(__dirname, 'runtime-wrappers/wrapper.ts'),
    versions: ['nodejs14.x', 'nodejs16.x', 'nodejs18.x', 'nodejs20.x'],

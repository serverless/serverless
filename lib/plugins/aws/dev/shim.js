...
const topics = {
  invocation: getTopicId(`${process.env.SLS_FUNCTION}/invocation`),
  response: getTopicId(`${process.env.SLS_FUNCTION}/response`),
  ping: getTopicId(`_ping`),
  pong: getTopicId(`_pong`),
};
...
const waitForNoResponse = async () => {
  console.log('Waiting for a response from the local machine...');
  await publishMessage(topics.ping);

  const pong = await Promise.race([waitForMessage(topics.pong), waitForMs(1000)]);
  ...
  return 'A development session is activated on this Lambda function, but we have not received a response from your local machine.';
};
...
const waitForResponse = async () => {
  const { response, error } = await waitForMessage(topics.response);
...
const waitForMessage = async (topicToWaitFor) => {
  return new Promise((resolve) => {
    device.on('message', async (topic, messageBuffer) => {
      // console.log(topic);
      if (topic === topicToWaitFor) {
        // todo
...
export const handler = async (event, context) => {
  ...
  await publishMessage(topics.invocation, {
  ...
  const response = await Promise.race([waitForResponse(), waitForNoResponse()]);

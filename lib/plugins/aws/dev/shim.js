...
const topics = {
  ...
  invocation: constructTopicId(`${process.env.SLS_FUNCTION}/invocation`),
...
/**
 *
 * This function resolves only if we haven't received a heartbeat message from the local machine in the last 4 seconds.
...
const waitForNoResponse = async () => {
  const isConnected = await Promise.race([waitForMessage(topics.heartbeat), waitForMs(4000)]);
...
export const handler = async (event, context) => {
  console.log(process.env.AWS_REQUEST_ID);

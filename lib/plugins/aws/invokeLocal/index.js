...
class AwsInvokeLocal {
  ...
  checkDockerImage(imageName) {
    return spawnExt('docker', ['images', '-q', imageName])
      .then(({ stdoutBuffer }) => {
        ...
        return BbPromise.all([
          ...
          .then(() => {
            ...
            return spawnExt('docker', [
              ...
              .catch(({ code, stderrBuffer, stdoutBuffer, stdBuffer }) => {
                this.printBuffers(stderrBuffer, stdoutBuffer);
                throw Error(
                  `Failed to build docker image (exit code ${code} ${stdBuffer.toString()})`
                );
              });
          });
      });
  ...
  printBuffers(stderrBuffer, stdoutBuffer) {
    ...
    return this.ensurePackage()
      ...
      .then(() =>
        BbPromise.all([
          ...
          return spawnExt('docker', dockerArgs)
            .then(({ stdoutBuffer, stderrBuffer }) => {
              this.printBuffers(stderrBuffer, stdoutBuffer);
              return imageName;
            })
            .catch(({ code, stdoutBuffer, stderrBuffer }) => {
              this.printBuffers(stderrBuffer, stdoutBuffer);
              throw Error(
                `Failed to run docker for ${runtime} image (exit code ${code} ${stderrBuffer.toString()})`
              );
            });
        })
      );

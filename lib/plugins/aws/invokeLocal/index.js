...
class AwsInvokeLocal {
  ...
  invokeLocalJava(runtime, className, artifactPath, event, context) {
    const input = JSON.stringify({
      event: event || {},
      context,
    });
    ...
    return new BbPromise(resolve => {
      const javac = spawn('javac', [path.join(__dirname, 'Invoke.java')]);

      javac.stderr.on('data', (buf) => this.serverless.cli.consoleLog(`javac - ${buf.toString()}`));
      javac.stdin.end();
      javac.on('close', () => {

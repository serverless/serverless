...
class AwsInvokeLocal {
  ...
  async loadEnvVars() {
    ...
    for(let [name, value] of _.entries(configuredEnvVars)) {
      configuredEnvVars[name] = await this.resolveResource(value, this.serverless, cachedCredentials, lambdaDefaultEnvVars);
      console.log(`${name} = ${configuredEnvVars[name]}`);
    }
  ...
  async resolveResource(value, lambdaDefaultEnvVars) {
    const { cachedCredentials } = this.provider;
    if(!cache) {
    ...
    if(action == 'Fn::ImportValue') {
      ...
      retval = cache[action][resourceName];
    } else {
      ...
      if(!resource) {
        ...
        if(!func) {
          func = _.entries(this.serverless.service.functions).find(item => {
            ...
            const lowerName = functionName.substr(0, 1).toUpperCase() + functionName.substr(1, functionName.length - 1);
            name = `${lowerName}LambdaFunction`;
   
            const retval = name == resourceName;
            if(retval) {
              lambdaName = value.name;
            }
            return retval;
          });
        }
        ...
        if(!func) {
          ...
          console.log('No function found ' + resourceName);
          return value;
        }
      ...
      const handler = handlers[resource.Type];
        if(handler && handler[action]) {
          retval = handler[action](this, resource, resourceName, value[action], serviceConfig);
        } else {
          return value;
      }
    }
    ...
    switch(action) {
      case 'Ref':
      case 'Fn::ImportValue':
        cache[action][resourceName] = retval;
        break;
      case 'Fn::GetAtt':
        if(!cache[action][resourceName]) {
            cache[action][resourceName] = {};
        }
        cache[action][resourceName][value[action][1]] = retval;
        break;
    }

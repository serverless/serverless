...
class AwsInvokeLocal {
  constructor(serverless, options) {
    ...
    this.loadExports = memoize(this.loadExportsImpl);
    this.resolveResource = memoize(this.resolveResourceImpl);
    this.getCFResources = memoize(this.getCFResourcesImpl);
  ...
  loadExportsImpl(token, cfExports) {
    if (!cfExports) {
      cfExports = [];
    }
    return this.provider.request('CloudFormation', 'listExports', { NextToken: token })
      .then(result => {
        ...
        if (result.NextToken) {
          return this.loadExports(result.NextToken, cfExports);
        }
    ...
    return BbPromise.try(() => {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (action === 'Fn::ImportValue') {
          resolve(
            this.loadExports().then(cfExports => {

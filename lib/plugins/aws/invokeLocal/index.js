...
class AwsInvokeLocal {
  ...
  checkDockerImage() {
    ...
    return new BbPromise((resolve, reject) => {
      ...
      docker.on('close', error => (error ? reject(error) : resolve(Boolean(stdout.trim()))));
    });
  }
      ...
      (this.options.functionObj.layers || this.serverless.service.provider.layers || [])
        .map(layer => {
          ...
          const layerContentsPath = path.join(
            '.serverless', 'invokeLocal', 'layers', arnParts[6], arnParts[7]);
  ...
  invokeLocalDocker() {
    ...
    return this.checkDockerImage()
      ...
      .then(() => this.getLayerPaths())
      .then((layerPaths) => this.buildDockerImage(layerPaths))

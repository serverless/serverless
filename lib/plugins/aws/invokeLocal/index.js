...
class AwsInvokeLocal {
  ...
  constructor(serverless, options) {
    ...
    this.resolveExports = memoizee(this.resolveExportsImpl, { normalizer: ([value]) => JSON.stringify(value) });
    this.resolveResource = memoizee(this.resolveResourceImpl, { normalizer: ([value]) => JSON.stringify(value) });
    this.getCFResources = memoizee(this.getCFResourcesImpl, { normalizer: ([value]) => JSON.stringify(value) });
  ...
  resolveExportsImpl(token, cfExports) {
    if (!cfExports) {
      cfExports = [];
    }
    return this.provider.request('CloudFormation', 'listExports', { NextToken: token })
      .then(result => {
        cfExports.push(...result.Exports);
        if (result.NextToken) {
          return this.resolveExportsImpl(result.NextToken, cfExports);
        }
    ...
    return BbPromise.try(() => {
      ...
      if (action === 'Fn::ImportValue') {
        return BbPromise.resolve(
          this.resolveExportsImpl().then(cfExports => {
  ...
  getCFResourcesImpl() {
    return this.provider.request('CloudFormation', 'describeStackResources', {

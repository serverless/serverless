...
const envVarResolvers = {
  'AWS::S3::Bucket': require('./resolvers/s3'),
  'AWS::DynamoDB::Table': require('./resolvers/dynamodb'),
  'AWS::SNS::Topic': require('./resolvers/sns'),
  'AWS::SQS::Queue': require('./resolvers/sqs'),
  'AWS::Lambda::Function': require('./resolvers/lambda'),
  'AWS::SecretsManager::Secret': require('./resolvers/secrets'),
  'AWS::SSM::Parameter': require('./resolvers/ssm-parameter'),
  'AWS::StepFunctions::StateMachine': require('./resolvers/state-machine'),
};
...
class AwsInvokeLocal {
  ...
  loadExports(cf, token, cfExports) {
    if (!cfExports) {
      cfExports = [];
    }
    return cf
      .listExports({ NextToken: token })
      .promise()
      .then(result => {
        cfExports.push(...result.Exports);
        if (result.NextToken) {
          return this.loadExports(cf, result.NextToken, cfExports);
        }
      ...
      return new Promise((resolve, reject) => {
        ...
        if (value.Ref) {
          ...
          if (value[action].length !== 2) {
            reject(new ServerlessError(`Attribute definition unexpected ${JSON.stringify(value)}`));
          }
        ...
        if (cache[action] && cache[action][resourceName]) {
          switch (action) {
            case 'Ref':
            case 'Fn::ImportValue':
              return resolve(cache[action][resourceName]);
            case 'Fn::GetAtt':
              if (cache[action][resourceName][value[action][1]]) {
                return resolve(cache[action][resourceName][value[action][1]]);
              }
              break;
            default:
              throw new ServerlessError(`${resourceName} has been assigned a value that can't be resolved ${JSON.stringify(value)}`);
          }
        }
        ...
        if (action === 'Fn::ImportValue') {
          ...
          resolve(
            this.loadExports(cf).then(cfExports => {
              cfExports.forEach(exp => {
                cache[action][exp.Name] = exp.Value;
              });
              return cache[action][resourceName];
            })
          );
        } else {
          let resource = this.serverless.service.resources.Resources[resourceName];
          if (!resource) {
            // Find the function
            let lambdaName = resourceName;
            let func = _.entries(this.serverless.service.functions).find(item => {
              const [, val] = item;

              return val.name === resourceName;
            });

            if (!func) {
              func = _.entries(this.serverless.service.functions).find(item => {
                let name = '';
                const [functionName, itemValue] = item;

                const lowerName =
                  functionName.substr(0, 1).toUpperCase() +
                  functionName.substr(1, functionName.length - 1);
                name = `${lowerName}LambdaFunction`;

                const resultVal = name === resourceName;
                if (resultVal) {
                  lambdaName = itemValue.name;
                }
                return resultVal;
              });
            }
            ...
            if (!func) {
              // If no function return what we recieved
              return resolve(value);
            }

            // Construct fake resource definition for lambda
            resource = {
          ...
          if(action == 'Ref') {
            resolve(this.resolveResourceId(resourceName, serviceConfig)
            .then(result => {
              cache[action][resourceName] = result;
              return result;
            }));
          }
          const resolver = envVarResolvers[resource.Type];
          if (resolver && resolver[action]) {
            resolve(
              resolver[action](this, resource, resourceName, value[action], serviceConfig).then(
                result => {
                  switch (action) {
                    case 'Ref':
                    case 'Fn::ImportValue':
                    ...
                    case 'Fn::GetAtt':
                      ...
                      cache[action][resourceName][value[action][1]] = result;
                      break;
                    default:
                      break;
                  }
  ...
  resolveResourceId(resourceName, serviceConfig) {
    return this.getCFResources(serviceConfig).then(cf => {

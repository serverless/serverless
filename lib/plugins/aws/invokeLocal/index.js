...
class AwsInvokeLocal {
  ...
  getEnvVarsFromOptions() {
    ...
    return [].concat(this.options.env || []).reduce((envVarsFromOptions, itm) => {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        mvn.on('exit', async (code, signal) => {
          if (code === 0) {
            resolve(await this.callJavaBridge(artifactPath, className, handlerName, input));
          } else if (!isRejected) {
            if (process.env.SLS_DEBUG) {
              chunk

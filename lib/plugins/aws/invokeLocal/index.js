...
class AwsInvokeLocal {
  ...
  resolveRuntimeWrapperPath(filename) {
    return path.resolve(__dirname, 'runtimeWrappers', filename);
  }
  ...
  invokeLocalPython(runtime, handlerPath, handlerName, event, context) {
    ...
    return new BbPromise((resolve, reject) => {
      const python = spawn(
        runtime.split('.')[0],
        ['-u', this.resolveRuntimeWrapperPath('invoke.py'), handlerPath, handlerName],
        { env: process.env },
        { shell: true }
      );
        ...
        () => {
          const java = spawn(
            ...
            [
              ...
              '-jar',
              this.resolveRuntimeWrapperPath('java/target/invoke-bridge-1.0.1.jar'),
            ],
            { shell: true }
          );
  ...
  invokeLocalRuby(runtime, handlerPath, handlerName, event, context) {
    ...
    return new BbPromise((resolve, reject) => {
      const ruby = spawn(
        runtime,
        [this.resolveRuntimeWrapperPath('invoke.rb'), handlerPath, handlerName],
        {
          env: process.env,
          shell: true,
        }

...
class AwsInvokeLocal {
  ...
  async loadEnvVars() {
    ...
    for(let [name, value] of _.entries(configuredEnvVars)) {
      configuredEnvVars[name] = await resolveResource(value, this.serverless, cachedCredentials, lambdaDefaultEnvVars);
...
async function resolveResource(value, serverless, cachedCredentials, lambdaDefaultEnvVars) {
  if(!cache) {
  ...
  serviceConfig = {
    region: (lambdaDefaultEnvVars.AWS_REGION)? lambdaDefaultEnvVars.AWS_REGION : serverless.service.provider.region,
  ...
  if(action == 'Fn::ImportValue') {
    ...
    if(!resource) {
      // Find the function
      const func = _.entries(serverless.service.functions).find((functionName, value) => {
        let name = '';
        if(value.name) {
          name = value.name;
        } else {
          const lowerName = functionName.substr(0, 1).toLowerCase() + functionName.substr(1, functionName.length - 1);
      ...
      if(handler && handler[action]) {
        retval = handler[action](resource, resourceName, value.Ref, serviceConfig);
      } else {
        return value;
      }
    }

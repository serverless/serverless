...
class AwsInvokeLocal {
  ...
  checkDockerImage() {
    ...
    return new BbPromise((resolve, reject) => {
      ...
      docker.on('close', error => (error ? reject(error) : resolve(stdout)));
  ...
  pullDockerImage() {
    ...
    return new BbPromise((resolve, reject) => {
      const docker = spawn('docker', ['pull', `lambci/lambda:${runtime}`]);
      docker.on('close', error => (error ? reject(error) : resolve()));
    });
      ...
      (this.options.functionObj.layers || this.serverless.service.provider.layers || [])
        .map(layer => {
          ...
          return this.provider.request(
              ...
              Object.keys(zip.files)
                .map(filename => zip.files[filename].async('nodebuffer').then(fileData => {
                  ...
                  mkdirp.sync(path.join(
                    '.serverless', 'invokeLocal', 'layers', 'arnParts[6]', 'filename'));
            ...
            .then(() => path.join('.serverless', 'invokeLocal', 'layers', arnParts[6]));
        }));
  ...
  invokeLocal() {
    ...
    return this.checkDockerImage()
      .then(exists => (exists ? Promise.resolve(): this.pullDockerImage()))
      .then(() => this.buildDockerImage())
      .then(imageName => new BbPromise((resolve, reject) => {
        const dockerArgs = [

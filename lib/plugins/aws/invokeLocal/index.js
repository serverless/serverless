...
class AwsInvokeLocal {
  constructor(serverless, options) {
    ...
    this.hooks = {
      'before:invoke:local:loadEnvVars': () =>
        BbPromise.bind(this).then(this.extendedValidate).then(this.loadEnvVars),
          ...
          if (layer.Ref) {
            ...
            if (targetLayer.package && targetLayer.package.artifact) {
              ...
              return dirExists(layerArtifactContentPath).then((exists) => {
                ...
                return unzipper.Open.file(targetLayer.package.artifact).then((d) =>
                  d.extract({ path: layerArtifactContentPath, concurrency: 5 })
                );
              });
            }
          }
  ...
  extractArtifact() {
    ...
    return unzipper.Open.file(artifact).then((d) =>
      d.extract({ path: destination, concurrency: 5 })
    );
  }

...
class AwsInvokeLocal {
  ...
  async loadEnvVars() {
    ...
    for(const [name, value] of _.entries(configuredEnvVars)) {
      configuredEnvVars[name] = await this.resolveResource(value);
    }
      ...
      switch(action) {
        ...
        case 'Fn::GetAtt':
          if(cache[action][resourceName][value[action][1]]) {
            return cache[action][resourceName][value[action][1]];
          }
          break;
        default:
          return value;
    ...
    if(action === 'Fn::ImportValue') {
      ...
      const handler = handlers[resource.Type];
        if(handler && handler[action]) {
          retval = await handler[action](this, resource, resourceName, value[action], serviceConfig);
        } else {
          return value;
      }
    }

...
class AwsInvokeLocal {
  ...
  buildDockerImage() {
    ...
    const layerPathsPromise = Promise.all((this.options.functionObj.layers || []).map(layer => {
      if (layer.Ref) {
      ...
      this.serverless.cli.consoleLog(`Downloading layer ${layer}...`);
      return this.provider.request(
        'Lambda', 'getLayerVersion', { LayerName: layerArn, VersionNumber: layerVersion })
        .then(layerInfo => download(layerInfo.Content.Location))
        .then(jsZip.loadAsync)
        .then(zip => Promise.all(
          Object.keys(zip.files)
            .map(filename => zip.files[filename].async('string').then(fileData => {
              if (filename.endsWith('/')) {
                return;
              }
              mkdirp.sync(
                path.dirname(`.serverless/invokeLocal/layers/${arnParts[6]}/${filename}`));
              fs.writeFileAsync(
                `.serverless/invokeLocal/layers/${arnParts[6]}/${filename}`, fileData);
            }))))
        .then(() => `.serverless/invokeLocal/layers/${arnParts[6]}`);
    }));

    const imageName = 'sls-docker';

    return layerPathsPromise.then(layerPaths => new BbPromise((resolve, reject) => {
      let dockerfile = `FROM lambci/lambda:${runtime}`;
      for (const layerPath of layerPaths) {
        dockerfile += `\nADD --chown=sbx_user1051:495 ${layerPath} /opt`;
        // fs.copySync(layerPath, `.serverless/invokeLocal/${layerPath}`);
      }
      mkdirp.sync(path.dirname('.serverless/invokeLocal'));
      fs.writeFileSync('.serverless/invokeLocal/Dockerfile', dockerfile);
      this.serverless.cli.consoleLog('Building docker image...');
      const docker = spawn('docker', ['build', '-t', imageName,
        `${this.serverless.config.servicePath}`, '-f', '.serverless/invokeLocal/Dockerfile']);
      docker.on('close', error => (error ? reject(error) : resolve(imageName)));
    }));
  }
  ...
  invokeLocal() {
    ...
    return this.buildDockerImage().then(imageName => new BbPromise((resolve, reject) => {
      const dockerArgs = [
        'run', '--rm', '-v', `${this.serverless.config.servicePath}:/var/task`, imageName,

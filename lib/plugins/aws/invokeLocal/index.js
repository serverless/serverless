...
mkdirp.async = BbPromise.promisify(mkdirp);
...
class AwsInvokeLocal {
        ...
        layer => {
          if (layer.Ref) {
            ...
            if (targetLayer.package && targetLayer.package.artifact) {
              ...
              if (fs.existsSync(layerArtifactContentPath)) {
          ...
          if (!fs.existsSync(layerContentsCachePath)) {
            ...
            downloadPromise = mkdirp
              .async(path.join(layerContentsCachePath))
              .then(() =>
  ...
  buildDockerImage(layerPaths) {
    ...
    let cachedDockerfileEquals = BbPromise.resolve();

    if (this.serverless.utils.fileExistsSync(dockerfileCachePath)) {
      cachedDockerfileEquals = fs.readFileAsync(dockerfileCachePath).then(contents => {
        return contents.toString() === dockerfile && this.checkDockerImage(imageName);
      });
    }

    return cachedDockerfileEquals.then(cacheEquals => {
      if (cacheEquals) {
        return imageName;
      }
      return BbPromise.all([
        mkdirp.async(path.join('.serverless', 'invokeLocal', runtime)),
        mkdirp.async(path.join(cachePath, 'dockerfiles', runtime)),
      ])
        .then(() => {
          this.serverless.cli.log('Writing Dockerfile...');
          return BbPromise.all([
            fs.writeFileSync(dockerfilePath, dockerfile),
            fs.writeFileSync(dockerfileCachePath, dockerfile),
          ]);
        })
        .then(
          () =>
            new BbPromise((resolve, reject) => {
              ...
              docker.on('exit', error => {
                return error ? reject('Failed to build docker image') : resolve(imageName);
              });
            })
        );
    });

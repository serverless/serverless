...
const resolveCfFunctionValue = require('../utils/resolveCfFunctionValue');
...
class AwsInvokeLocal {
  constructor(serverless, options) {
    ...
    this.resolveCfFunctionValue = resolveCfFunctionValue;
    ...
    return BbPromise.try(() => {
      ...
      const promises = _.entries(configuredEnvVars).map(([name, value]) => {
        return this.resolveCfFunctionValue(this.provider, value)
          ...
          .catch(e => {
            throw new this.serverless.classes.Error(
              `Could not resolve ${name} environment variable. ${e.message}`
            );
          });
      });

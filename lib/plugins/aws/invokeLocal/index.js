...
class AwsInvokeLocal {
  ...
  loadEnvVars() {
    return BbPromise.try(() => {
      ...
      const fnImportPromises = _.entries(configuredEnvVars).map(([name, value]) => {
      ...
      const fnRefPromises = _.entries(configuredEnvVars).map(([name, value]) => {
        return (value.Ref
          ? resolveCfRefValue(this.provider, value.Ref)
          : BbPromise.resolve(value)
        ).then(
          resolvedValue => {
            configuredEnvVars[name] = resolvedValue;
      ...
      return BbPromise.all([...fnImportPromises, ...fnRefPromises]).then(() => {

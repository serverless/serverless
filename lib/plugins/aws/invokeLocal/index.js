...
const resolversShared = require('./resolvers');
...
class AwsInvokeLocal {
  ...
  loadEnvVars() {
    ...
    resolversShared.serverless = this.serverless;

    const promises = _.entries(configuredEnvVars).map(item => {
      const [name, value] = item;
      return this.resolveResource(value).then(output => {
        if (output.then) {
          return output.then(res => {
            configuredEnvVars[name] = res;
            return res;
          });
        }
    ...
    return BbPromise.all(promises).then(value => {

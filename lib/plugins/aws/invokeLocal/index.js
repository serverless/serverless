...
class AwsInvokeLocal {
  ...
  loadExportsImpl(token, cfExports) {
    ...
    return this.provider
      .request('CloudFormation', 'listExports', { NextToken: token })
      .then(result => {
    ...
    return BbPromise.try(() => {
      return new Promise((resolve, reject) => {
        ...
        if (action === 'Fn::ImportValue') {
          ...
          if (action === 'Ref') {
            resolve(
              this.resolveResourceId(resourceName).then(result => {
          ...
          if (resolver && resolver[action]) {
            resolve(
              resolver[action](this, resource, resourceName, value[action]).then(result => {
                if (!_.isString(result)) {

...
class AwsInvokeLocal {
  constructor(serverless, options) {
    ...
    this.resolveExports = memoizee(this.resolveExportsImpl, {
      normalizer: ([value]) => JSON.stringify(value),
    });
    this.resolveResource = memoizee(this.resolveResourceImpl, {
      normalizer: ([value]) => JSON.stringify(value),
    });
    this.getCFResources = memoizee(this.getCFResourcesImpl, {
      normalizer: ([value]) => JSON.stringify(value),
    });
  ...
  resolveResourceImpl(value) {
    return BbPromise.try(() => {
      ...
      if (action === 'Fn::ImportValue') {
        resourceResult = this.resolveExportsImpl().then(cfExports => {
        ...
        if (action === 'Ref') {
          resourceResult = this.resolveResourceId(resourceName).then(result => {
        ...
        if (resolver && resolver[action]) {
          resourceResult = resolver[action](this, resource, resourceName, value[action]).then(
            result => {
              if (!_.isString(result)) {

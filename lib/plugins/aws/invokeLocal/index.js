...
const envVarResolvers = {
  ...
  'AWS::SecretsManager::Secret': require('./resolvers/secretsManagerSecret'),
          ...
          if(action == 'Ref') {
            ...
            .then(result => {
              if(!_.isString(result)) {
                throw ServerlessError(`Could not resolve Ref: ${resourceName}`);
              }
          ...
          if (resolver && resolver[action]) {
            resolve(
              resolver[action](this, resource, resourceName, value[action], serviceConfig).then(
                result => {
                  if(!_.isString(result)) {
                    throw ServerlessError(`Could not resolve ${action}: ${resourceName} in aws`);
                  }
                  switch (action) {
                    case 'Fn::ImportValue':
                      cache[action][resourceName] = result;

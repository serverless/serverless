...
class AwsInvokeLocal {
  ...
  buildDockerImage(layerPaths) {
    ...
    mkdirp.sync(path.join('.serverless', 'invokeLocal', runtime));
    const dockerfilePath = path.join('.serverless', 'invokeLocal', runtime, 'Dockerfile');
    return fs
      .accessAsync(dockerfilePath)
      .catch(() => {
        ...
        return fs.writeFileAsync(dockerfilePath, dockerfile);
      })
      .then(
        () =>
          new Promise((resolve, reject) => {
            this.serverless.cli.log(`Building Docker image "${imageName}"...`);
            const docker = spawn('docker', [
              'build',
              '-t',
              imageName,
  ...
  ensurePackage() {
    if (this.options.package || this.options.p) {
      return fs
        .accessAsync(`${this.options.package || this.options.p}/serverless-state.json`)
  ...
  invokeLocalDocker() {
    ...
    return this.ensurePackage().then(() =>
      BbPromise.all([
        ...
        this.checkDockerImage(imageName).then(exists => {
          return exists
            ? imageName

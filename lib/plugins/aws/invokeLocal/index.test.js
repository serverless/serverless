...
describe('AwsInvokeLocal', () => {
  ...
  beforeEach(() => {
    ...
    const deferred = BbPromise.defer();
    Object.assign(deferred.promise, {
      child: {
        on: (key, callback) => {
    ...
    spawnStub = sinon.stub(deferred, 'resolve').returns(deferred.promise);
    deferred.resolve({
  ...
  describe('#callJavaBridge()', () => {
    beforeEach(() => {
      sinon.stub(awsInvokeLocal, 'callJavaBridge').resolves();
      awsInvokeLocal.options = {
        stage: 'dev',
        function: 'first',
        functionObj: {
  ...
  describe('#invokeLocalJava()', () => {
    ...
    describe('when attempting to build the Java bridge', () => {
      ...
      beforeEach(() => {
        callJavaBridgeMockedStub = sinon.stub(awsInvokeLocal, 'callJavaBridge').resolves();

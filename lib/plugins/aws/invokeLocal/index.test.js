...
describe('AwsInvokeLocal', () => {
  ...
  describe('#invokeLocal()', () => {
    ...
    it('throw error when using runtime other than Node.js or Python', () => {
      awsInvokeLocal.options.functionObj.runtime = 'go';
  ...
  describe('#invokeLocalJava', () => {
    ...
    describe('should build Java bridge', () => {
      let awsInvokeLocalMocked;
      ...
      afterEach(() => {
        callJavaBridgeMockedStub.restore();
        delete require.cache[require.resolve('./index')];
        delete require.cache[require.resolve('child_process')];
      });
      ...
      it('if it\'s not present yet', () =>
        awsInvokeLocalMocked.invokeLocalJava(
          ...
          expect(serverless.cli.consoleLog.lastCall.args[0]).to.contain('Building Java bridge, first invocation might take a bit longer.'); //eslint-disable-line

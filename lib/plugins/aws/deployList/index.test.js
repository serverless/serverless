...
describe('AwsDeployList', () => {
  ...
  describe('hooks', () => {
    it('should run "before:deploy:list:log" hook promise chain in order', () => {
      const validateStub = sinon.stub(awsDeploy, 'validate')
        .returns(BbPromise.resolve());

      return awsDeploy.hooks['before:deploy:list:log']().then(() => {
        expect(validateStub.calledOnce).to.be.equal(true);
      });
    });

    it('should run "deploy:list:log" hook promise chain in order', () => {
      const setBucketNameStub = sinon.stub(awsDeploy, 'setBucketName')
        .returns(BbPromise.resolve());
      const listDeploymentsStub = sinon.stub(awsDeploy, 'listDeployments')
        .returns(BbPromise.resolve());

      return awsDeploy.hooks['deploy:list:log']().then(() => {
        expect(setBucketNameStub.calledOnce).to.be.equal(true);
        expect(listDeploymentsStub.calledOnce).to.be.equal(true);
      });
    });
  });

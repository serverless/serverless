    ...
    .compileAuthorizers().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalAuthorizerName('authorizer')].Type
      ).to.equal('AWS::ApiGateway::Authorizer');

      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            naming.getLogicalAuthorizerName('authorizer')
          ].Properties.AuthorizerResultTtlInSeconds
      ).to.equal(300);

      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalAuthorizerName('authorizer')].Properties.IdentitySource
      ).to.equal('method.request.header.Authorization');

      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalAuthorizerName('authorizer')].Properties.Name
      ).to.equal('authorizer');

      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalAuthorizerName('authorizer')].Properties.RestApiId.Ref
      ).to.equal(naming.getLogicalApiGatewayName());

      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalAuthorizerName('authorizer')].Properties.Type
      ).to.equal('TOKEN');
    })
  );

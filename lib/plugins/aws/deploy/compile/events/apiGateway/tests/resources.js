  ...
  it('should construct the correct resourceLogicalIds object', () => awsCompileApigEvents
    .compileResources().then(() => {
      const expectedResourceLogicalIds = {
        'bar/-': naming.getLogicalApiGatewayResourceName('bar/-'),
        'foo/bar': naming.getLogicalApiGatewayResourceName('foo/bar'),
        foo: naming.getLogicalApiGatewayResourceName('foo'),
        'bar/{id}/foobar': naming.getLogicalApiGatewayResourceName('bar/{id}/foobar'),
        'bar/{id}': naming.getLogicalApiGatewayResourceName('bar/{id}'),
        'bar/{foo_id}/foobar': naming.getLogicalApiGatewayResourceName('bar/{foo_id}/foobar'),
        'bar/{foo_id}': naming.getLogicalApiGatewayResourceName('bar/{foo_id}'),
        'bar/foo': naming.getLogicalApiGatewayResourceName('bar/foo'),
        bar: naming.getLogicalApiGatewayResourceName('bar'),
      };
  ...
  it('should create resource resources when http events are given', () => awsCompileApigEvents
    .compileResources().then(() => {
      expect(awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[naming.getLogicalApiGatewayResourceName('foo/bar')]
        .Properties.PathPart).to.equal('bar');
      expect(awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[naming.getLogicalApiGatewayResourceName('foo/bar')]
        .Properties.ParentId.Ref).to.equal(naming.getLogicalApiGatewayResourceName('foo'));
      expect(awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[naming.getLogicalApiGatewayResourceName('foo')]
        .Properties.ParentId['Fn::GetAtt'][0])
        .to.equal(naming.getLogicalApiGatewayName());
      expect(awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[naming.getLogicalApiGatewayResourceName('bar')]
        .Properties.ParentId['Fn::GetAtt'][1])
        .to.equal('RootResourceId');
      expect(awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[naming.getLogicalApiGatewayResourceName('bar/{id}')]
        .Properties.ParentId.Ref)
        .to.equal(naming.getLogicalApiGatewayResourceName('bar'));
      expect(awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[naming.getLogicalApiGatewayResourceName('bar/{foo_id}')]
        .Properties.ParentId.Ref)
        .to.equal(naming.getLogicalApiGatewayResourceName('bar'));
      expect(awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[naming.getLogicalApiGatewayResourceName('bar/{foo_id}/foobar')]
        .Properties.ParentId.Ref)
        .to.equal(naming.getLogicalApiGatewayResourceName('bar/{foo_id}'));
    })
  );

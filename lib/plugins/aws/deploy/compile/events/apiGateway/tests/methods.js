...
describe('#compileMethods()', () => {
  ...
  beforeEach(() => {
    ...
    awsCompileApigEvents.resourceLogicalIds = {
      'users': awsCompileApigEvents.sdk.naming
        .getLogicalApiGatewayResourceName('users'),
      'users/{id}': awsCompileApigEvents.sdk.naming
        .getLogicalApiGatewayResourceName('users/{id}'),
      'users/create': awsCompileApigEvents.sdk.naming
        .getLogicalApiGatewayResourceName('users/create'),
      'users/list': awsCompileApigEvents.sdk.naming
        .getLogicalApiGatewayResourceName('users/list'),
      'users/update': awsCompileApigEvents.sdk.naming
        .getLogicalApiGatewayResourceName('users/update'),
      'users/delete': awsCompileApigEvents.sdk.naming
        .getLogicalApiGatewayResourceName('users/delete'),
    };
    awsCompileApigEvents.extractedResourceIds = {};
    Object.keys(awsCompileApigEvents.resourceLogicalIds).forEach((resourceName) => {
      awsCompileApigEvents.extractedResourceIds[resourceName] = awsCompileApigEvents.sdk.naming
        .extractResourceId(awsCompileApigEvents.resourceLogicalIds[resourceName]);
    });
  });
    ...
    .compileMethods().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
          awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
            awsCompileApigEvents.extractedResourceIds['users/create'], 'POST')
        ].Type
      ).to.equal('AWS::ApiGateway::Method');
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
          awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
            awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
        ].Type
      ).to.equal('AWS::ApiGateway::Method');
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
          awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
            awsCompileApigEvents.extractedResourceIds['users/update'], 'PUT')
        ].Type
      ).to.equal('AWS::ApiGateway::Method');
    })
  );
    ...
    return awsCompileApigEvents.compileMethods().then(() => {
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
          awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
            awsCompileApigEvents.extractedResourceIds['users/create'], 'POST')
        ].Properties.AuthorizerId.Ref
      ).to.equal(awsCompileApigEvents.sdk.naming.getLogicalAuthorizerName('authorizer'));
    });
  });
    ...
    const lambdaUriObject = {
      'Fn::Join': [
        '', [
          ...
          { 'Fn::GetAtt': [awsCompileApigEvents.sdk.naming.getLogicalLambdaName('first'), 'Arn'] },
          '/invocations',
        ],
      ],
    };
  ...
  it('should create preflight method for CORS enabled resource', () => {
    ...
    return awsCompileApigEvents.compileMethods().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
          awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
            awsCompileApigEvents.extractedResourceIds['users/create'], 'OPTIONS')
          ].Properties.Integration.IntegrationResponses[0]
          .ResponseParameters['method.response.header.Access-Control-Allow-Origin']
      ).to.equal(origin);
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
          awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
            awsCompileApigEvents.extractedResourceIds['users/update'], 'OPTIONS')
          ].Properties.Integration.IntegrationResponses[0]
          .ResponseParameters['method.response.header.Access-Control-Allow-Origin']
      ).to.equal(origin);
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
          awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
            awsCompileApigEvents.extractedResourceIds['users/delete'], 'OPTIONS')
          ].Properties.Integration.IntegrationResponses[0]
          .ResponseParameters['method.response.header.Access-Control-Allow-Origin']
      ).to.equal(origin);
  ...
  it('should merge all preflight origins, method, and headers for a path', () => {
    ...
    awsCompileApigEvents.resourceLogicalIds = {
      users: awsCompileApigEvents.sdk.naming.getLogicalApiGatewayResourceName('users'),
      'users/{id}': awsCompileApigEvents.sdk.naming.getLogicalApiGatewayResourceName('users/{id}'),
    };
    return awsCompileApigEvents.compileMethods().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
            awsCompileApigEvents.extractedResourceIds['users/{id}'], 'OPTIONS'
          )].Properties.Integration.IntegrationResponses[0]
          .ResponseParameters['method.response.header.Access-Control-Allow-Methods']
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
            awsCompileApigEvents.extractedResourceIds['users'], 'OPTIONS'
          )].Properties.Integration.IntegrationResponses[0]
          .ResponseParameters['method.response.header.Access-Control-Allow-Origin']
  ...
  describe('when dealing with request configuration', () => {
    it('should setup a default "application/json" template', () => {
      ...
      // TODO these next two consts are an example of a sub-optimally elegant pass off between
      // TODO .../apigateway/resources and .../apigateway/methods
      // TODO is there a better way?
      const logicalResourceName = awsCompileApigEvents.sdk.naming.getLogicalApiGatewayResourceName(
        awsCompileApigEvents.serverless.service.functions.first.events[0].http.path
      );
      const extractedId = awsCompileApigEvents.sdk.naming.extractResourceId(logicalResourceName);

      return awsCompileApigEvents.compileMethods().then(() => {
        expect(awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(extractedId, 'GET')
    ...
    it('should set custom request templates', () => {
      ...
      return awsCompileApigEvents.compileMethods().then(() => {
        expect(awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(extractedId, 'GET')
          ].Properties.Integration.RequestTemplates['template/1']
        ).to.equal('{ "stage" : "$context.stage" }');

        expect(awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(extractedId, 'GET')
          ].Properties.Integration.RequestTemplates['template/2']
    ...
    return awsCompileApigEvents.compileMethods().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.MethodResponses[1].StatusCode
      ).to.equal(400);
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.MethodResponses[2].StatusCode
      ).to.equal(401);
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.MethodResponses[3].StatusCode
      ).to.equal(403);
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.MethodResponses[4].StatusCode
      ).to.equal(404);
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.MethodResponses[5].StatusCode
      ).to.equal(422);
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.MethodResponses[6].StatusCode
      ).to.equal(500);
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.MethodResponses[7].StatusCode
      ).to.equal(502);
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.MethodResponses[8].StatusCode
      ).to.equal(504);
    });
  });
    ...
    return awsCompileApigEvents.compileMethods().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.Integration.IntegrationResponses[1]
      ).to.deep.equal({
        StatusCode: 400,
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.Integration.IntegrationResponses[2]
      ).to.deep.equal({
        StatusCode: 401,
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.Integration.IntegrationResponses[3]
      ).to.deep.equal({
        StatusCode: 403,
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.Integration.IntegrationResponses[4]
      ).to.deep.equal({
        StatusCode: 404,
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.Integration.IntegrationResponses[5]
      ).to.deep.equal({
        StatusCode: 422,
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.Integration.IntegrationResponses[6]
      ).to.deep.equal({
        StatusCode: 500,
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.Integration.IntegrationResponses[7]
      ).to.deep.equal({
        StatusCode: 502,
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            awsCompileApigEvents.sdk.naming.getLogicalApiGatewayMethodName(
              awsCompileApigEvents.extractedResourceIds['users/list'], 'GET')
          ].Properties.Integration.IntegrationResponses[8]
      ).to.deep.equal({
        StatusCode: 504,
  ...
  it('should add custom response codes', () => {
    ...
    return awsCompileApigEvents.compileMethods().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[awsCompileApigEvents.sdk.getLogicalApiGatewayMethodName(
            awsCompileApigEvents.extractedResourceIds['users/list'], 'GET'
          )].Properties.Integration.IntegrationResponses[0].ResponseTemplates['application/json']
      ).to.equal("$input.path('$.foo')");
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[awsCompileApigEvents.sdk.getLogicalApiGatewayMethodName(
            awsCompileApigEvents.extractedResourceIds['users/list'], 'GET'
          )].Properties.Integration.IntegrationResponses[0].SelectionPattern
      ).to.equal('');
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[awsCompileApigEvents.sdk.getLogicalApiGatewayMethodName(
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[awsCompileApigEvents.sdk.getLogicalApiGatewayMethodName(
            awsCompileApigEvents.extractedResourceIds['users/list'], 'GET'
          )].Properties.Integration.IntegrationResponses[1].ResponseTemplates['application/json']
      ).to.equal("$input.path('$.errorMessage')");
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[awsCompileApigEvents.sdk.getLogicalApiGatewayMethodName(
            awsCompileApigEvents.extractedResourceIds['users/list'], 'GET'
          )].Properties.Integration.IntegrationResponses[1].SelectionPattern
    ...
    return awsCompileApigEvents.compileMethods().then(() => {
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[awsCompileApigEvents.sdk.getLogicalApiGatewayMethodName(
            awsCompileApigEvents.extractedResourceIds['users/list'], 'GET'
          )].Properties.Integration.IntegrationResponses[1].ResponseTemplates['application/xml']

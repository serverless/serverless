...
describe('#compileMethods()', () => {
  ...
  it('should set the correct lambdaUri', () => {
    ...
    return awsCompileApigEvents.compileMethods().then(() => {
      expect(
        JSON.stringify(awsCompileApigEvents.serverless.service.resources.Resources.PostMethodApigEvent0.Properties
          .Integration.Uri
      )).to.equal(JSON.stringify(lambdaUriObject));
      expect(
        JSON.stringify(awsCompileApigEvents.serverless.service.resources.Resources.GetMethodApigEvent1.Properties
          .Integration.Uri
      )).to.equal(JSON.stringify(lambdaUriObject));
    });
  });

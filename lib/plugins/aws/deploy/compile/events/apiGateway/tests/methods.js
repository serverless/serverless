...
describe('#compileMethods()', () => {
  ...
  beforeEach(() => {
    ...
    awsCompileApigEvents.resourceLogicalIds = {
      users: awsCompileApigEvents.provider.naming
        .getLogicalApiGatewayResourceName('users'),
      'users/{id}': awsCompileApigEvents.provider.naming
        .getLogicalApiGatewayResourceName('users/{id}'),
      'users/create': awsCompileApigEvents.provider.naming
        .getLogicalApiGatewayResourceName('users/create'),
      'users/list': awsCompileApigEvents.provider.naming
        .getLogicalApiGatewayResourceName('users/list'),
      'users/update': awsCompileApigEvents.provider.naming
        .getLogicalApiGatewayResourceName('users/update'),
      'users/delete': awsCompileApigEvents.provider.naming
    ...
    Object.keys(awsCompileApigEvents.resourceLogicalIds).forEach((resourceName) => {
      awsCompileApigEvents.extractedResourceIds[resourceName] = awsCompileApigEvents.provider.naming
    ...
    .compileMethods().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
          awsCompileApigEvents.provider.naming.getLogicalApiGatewayMethodName(
  ...
  it('should set authorizer config if given as string', () => {
    ...
    return awsCompileApigEvents.compileMethods().then(() => {
      ...
      ).to.equal(awsCompileApigEvents.provider.naming.getLogicalAuthorizerName('authorizer'));
    });
  });
    ...
    const lambdaUriObject = {
      'Fn::Join': [
        '', [
          ...
          {
            'Fn::GetAtt': [
              awsCompileApigEvents.provider.naming.getLogicalLambdaName('first'), 'Arn',
            ],
          },
          '/invocations',
        ],
  ...
  it('should merge all preflight origins, method, and headers for a path', () => {
    ...
    awsCompileApigEvents.resourceLogicalIds = {
      users: awsCompileApigEvents.provider.naming.getLogicalApiGatewayResourceName('users'),
      'users/{id}': awsCompileApigEvents.provider.naming
        .getLogicalApiGatewayResourceName('users/{id}'),
    };
    return awsCompileApigEvents.compileMethods().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[awsCompileApigEvents.provider.naming.getLogicalApiGatewayMethodName(
  ...
  describe('when dealing with request configuration', () => {
    it('should setup a default "application/json" template', () => {
      ...
      const logicalResourceName = awsCompileApigEvents.provider.naming
        .getLogicalApiGatewayResourceName(
          awsCompileApigEvents.serverless.service.functions.first.events[0].http.path
        );
      const extractedId = awsCompileApigEvents.provider.naming
        .extractResourceId(logicalResourceName);

      return awsCompileApigEvents.compileMethods().then(() => {
        expect(awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[
            awsCompileApigEvents.provider.naming.getLogicalApiGatewayMethodName(extractedId, 'GET')
  ...
  describe('when dealing with response configuration', () => {
    it('should set the custom headers', () => {
      ...
      return awsCompileApigEvents.compileMethods().then(() => {
        expect(
          awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
              awsCompileApigEvents.provider.naming
                .getLogicalApiGatewayMethodName(extractedId, 'GET')
            ].Properties.Integration.IntegrationResponses[0]

...
describe('#compileMethods()', () => {
  ...
  it('should always set default status code of 200', () => {
    awsCompileApigEvents.compileMethods().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
  ...
  it('should be possible to overwrite default status code', () => {
    awsCompileApigEvents.serverless.service.functions = {
      first: {
        events: [
          {
    ...
    awsCompileApigEvents.compileMethods().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources.ApiGatewayMethodUsersCreatePut.Properties.Integration.IntegrationResponses[0]
          .StatusCode
      ).to.equal(201);
    });
  });

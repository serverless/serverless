...
describe('#compileMethods()', () => {
  ...
  beforeEach(() => {
    ...
    awsCompileApigEvents.resourceLogicalIds = {
      'users/create': 'ApiGatewayResourceUsersCreate', // NAME HERE
    ...
    .compileMethods().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalApiGatewayMethodName('UsersCreate', 'POST')].Type
      ).to.equal('AWS::ApiGateway::Method');
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalApiGatewayMethodName('UsersList', 'GET')].Type
      ).to.equal('AWS::ApiGateway::Method');
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalApiGatewayMethodName('UsersUpdate', 'PUT')].Type
      ).to.equal('AWS::ApiGateway::Method');
    })
  );
    ...
    return awsCompileApigEvents.compileMethods().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate.Resources[
            naming.getLogicalApiGatewayMethodName('UsersCreate', 'POST')
      ...
      ).to.equal(naming.getLogicalAuthorizerName('authorizer'));
    });
  });
  ...
  it('should set the correct lambdaUri', () => {
    const lambdaUriObject = {
      'Fn::Join': [
        '', [
          ...
          { 'Fn::GetAtt': [naming.getLogicalLambdaName('first'), 'Arn'] },
          '/invocations',
        ],
      ],
    };
    ...
    return awsCompileApigEvents.compileMethods().then(() => {
      ...
      expect(
        JSON.stringify(awsCompileApigEvents.serverless.service
          .provider.compiledCloudFormationTemplate.Resources[
            naming.getLogicalApiGatewayMethodName('UsersList', 'GET')
          ].Properties.Integration.Uri
      )).to.equal(JSON.stringify(lambdaUriObject));
    });
  });
  ...
  it('should add CORS origins to method only when CORS is enabled', () => {
    ...
    return awsCompileApigEvents.compileMethods().then(() => {
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalApiGatewayMethodName('UsersCreate', 'POST')].Properties
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalApiGatewayMethodName('UsersList', 'GET')].Properties
          .Integration.IntegrationResponses[0]
          .ResponseParameters['method.response.header.Access-Control-Allow-Origin']
      ).to.not.equal(origin);
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalApiGatewayMethodName('UsersUpdate', 'PUT')].Properties
          .Integration.IntegrationResponses[0]
          .ResponseParameters['method.response.header.Access-Control-Allow-Origin']
      ).to.equal(origin);
    });
  ...
  it('should create preflight method for CORS enabled resource', () => {
    ...
    return awsCompileApigEvents.compileMethods().then(() => {
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalApiGatewayMethodName('UsersCreate', 'OPTIONS')]
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalApiGatewayMethodName('UsersUpdate', 'OPTIONS')]
          .Properties.Integration.IntegrationResponses[0]
          .ResponseParameters['method.response.header.Access-Control-Allow-Origin']
      ).to.equal(origin);
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalApiGatewayMethodName('UsersDelete', 'OPTIONS')]
          .Properties.Integration.IntegrationResponses[0]
          .ResponseParameters['method.response.header.Access-Control-Allow-Origin']
      ).to.equal(origin);
    ...
    it('should setup a default "application/json" template', () =>
      awsCompileApigEvents.compileMethods().then(() => {
        expect(awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          // TODO 'UsersList' below is a fugly case of leakage
          .Resources[naming.getLogicalApiGatewayMethodName('UsersList', 'GET')].Properties

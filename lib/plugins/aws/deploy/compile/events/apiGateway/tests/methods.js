...
describe('#compileMethods()', () => {
  ...
  it('should add CORS Access-Control-Allow-Credentials header to method when CORS and LAMBDA integration are enabled', () => {
    awsCompileApigEvents.serverless.service.functions = {
      first: {
        events: [
          {
    ...
    return awsCompileApigEvents.compileMethods().then(() => {
      ...
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources.ApiGatewayMethodUsersCreateOptions.Properties
          .Integration.IntegrationResponses[0]
      ...
      // CORS enabled, "allowCredentials" omitted. Should default to "true"
      // Preflight request
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources.ApiGatewayMethodUsersUpdateOptions.Properties
          .Integration.IntegrationResponses[0]
          .ResponseParameters['method.response.header.Access-Control-Allow-Credentials']
      ).to.equal('\'true\'');
      ...
      // CORS enabled, "allowCredentials" set to "false"
      // Preflight request
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate
          .Resources.ApiGatewayMethodUsersListOptions.Properties
          .Integration.IntegrationResponses[0]
          .ResponseParameters['method.response.header.Access-Control-Allow-Credentials']
      ).to.equal('\'false\'');
      ...
      // CORS enabled, "allowCredentials" set to "true"
      // Preflight request
      expect(
        awsCompileApigEvents.serverless.service.provider.compiledCloudFormationTemplate

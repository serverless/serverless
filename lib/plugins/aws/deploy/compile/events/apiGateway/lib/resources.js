...
module.exports = {
  compileResources() {
    ...
    const restApiId = this.sdk.naming.getLogicalApiGatewayName();
    ...
    this.resourcePaths.forEach(path => {
      ...
      const logicalResourceName = this.sdk.naming.getLogicalApiGatewayResourceName(path);
      ...
      if (splitPath.length === 0) {
        resourceParentId = `{ "Fn::GetAtt": ["${
          this.sdk.naming.getLogicalApiGatewayName()
        }", "RootResourceId"] }`;
      } else {
        const logicalParentResourceName = this.sdk.naming

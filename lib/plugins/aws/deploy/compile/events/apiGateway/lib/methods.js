  ...
  compileMethods() {
    ...
    _.forEach(this.serverless.service.functions, (functionObject, functionName) => {
      functionObject.events.forEach(event => {
        if (event.http) {
          ...
          const extractedResourceId = this.sdk.naming.extractResourceId(resourceLogicalId);
          const logicalMethodName = this.sdk.naming.getLogicalApiGatewayMethodName(
            extractedResourceId, method);
          ...
          integrationResponses.push(
            ...
            { StatusCode: 500, SelectionPattern: '.*(Process\\s?exited\\s?before\\s?completing\\s?request|Task\\s?timed\\s?out\\s?|\\[500\\]).*' }, // eslint-disable-line max-len
            { StatusCode: 502, SelectionPattern: '.*\\[502\\].*' },
            { StatusCode: 504, SelectionPattern: '.*\\[504\\].*' }
          );

          const logicalFunctionName = this.sdk.naming.getLogicalLambdaName(functionName);
          ...
          const methodTemplate = `
            {
              ...
              "Properties" : {
                  ...
                  "Uri" : {
                    "Fn::Join": [ "",
                      [
                        ...
                        {"Fn::GetAtt" : ["${logicalFunctionName}", "Arn"]},
                        "/invocations"
                      ]
                    ]
                  },
                ...
                "RestApiId" : { "Ref": "${this.sdk.naming.getLogicalApiGatewayName()}" }
              }
            }
          `;
          ...
          if (event.http.authorizer) {
            ...
            if (typeof event.http.authorizer === 'string') {
              if (event.http.authorizer.indexOf(':') === -1) { // isARN?
                ...
                authorizerName = this.sdk.naming.extractAuthorizerIdFromArn(event.http.authorizer);
              }
            } else if (typeof event.http.authorizer === 'object') {
              if (event.http.authorizer.arn) {
                authorizerName = this.sdk.naming
            ...
            const logicalAuthorizerName = this.sdk.naming.getLogicalAuthorizerName(authorizerName);

            methodTemplateJson.Properties.AuthorizationType = 'CUSTOM';
            methodTemplateJson.Properties.AuthorizerId = {
              Ref: logicalAuthorizerName,
            };
            methodTemplateJson.DependsOn = logicalAuthorizerName;
          }

          if (event.http.private) methodTemplateJson.Properties.ApiKeyRequired = true;

          const methodObject = {
            [logicalMethodName]: methodTemplateJson,
          };
          ...
          if (this.methodDependencies) {
            this.methodDependencies.push(logicalMethodName);
          } else {
            this.methodDependencies = [logicalMethodName];
          }
        }
      });
    });
    ...
    if (!_.isEmpty(corsConfig)) {
      ...
      _.forOwn(corsConfig, (config, path) => {
        ...
        const extractedResourceId = this.sdk.naming.extractResourceId(resourceLogicalId);
        const logicalPreflightObjectName = this.sdk.naming.getLogicalApiGatewayMethodName(
          extractedResourceId, 'OPTIONS');

        const preflightObject = {
          [logicalPreflightObjectName]: JSON.parse(preflightTemplate),
        };

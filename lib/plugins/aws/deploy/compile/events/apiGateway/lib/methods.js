  ...
  compileMethods() {
    ...
    _.forEach(this.serverless.service.functions, (functionObject, functionName) => {
      functionObject.events.forEach(event => {
        if (event.http) {
          ...
          const extractedResourceId = naming.extractResourceId(resourceLogicalId);
          const logicalMethodName = naming.getLogicalApiGatewayMethodName(
          ...
          if (event.http.authorizer) {
            ...
            const logicalAuthorizerName = naming.getLogicalAuthorizerName(authorizerName);
    ...
    if (!_.isEmpty(corsConfig)) {
      ...
      _.forOwn(corsConfig, (config, path) => {
        // TODO track down resourceLogicalIds population
        const resourceLogicalId = this.resourceLogicalIds[path];
        ...
        const extractedResourceId = naming.extractResourceId(resourceLogicalId);
        const logicalPreflightObjectName = naming.getLogicalApiGatewayMethodName(

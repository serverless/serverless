    ...
    forEach(this.serverless.service.functions, (functionObject, functionName) => {
      functionObject.events.forEach(event => {
        if (event.http) {
          ...
          if (!path) {
            const errorMessage = [
              `Missing "path" property in function "${functionName}" for http event in serverless.yaml.`,
          ...
          if (!method) {
            const errorMessage = [
              `Missing "method" property in function "${functionName}" for http event in serverless.yaml.`,
              ` If you define an http event, make sure you pass a valid value for it,`,
              ` either as string syntax, or object syntax.`,
              ` Please check the docs for more options.`
            ].join('');

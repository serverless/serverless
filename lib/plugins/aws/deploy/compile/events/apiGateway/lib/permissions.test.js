...
describe('#awsCompilePermissions()', () => {
  ...
  it('should not add proxy authorizer function for Cognito User Pools', () => {
    awsCompileApigEvents.permissionMapping = [
      {
        lambdaLogicalId: 'FirstLambdaFunction',
        resourceName: 'FooBar',
    ...
    return awsCompileApigEvents.compilePermissions().then(() => {
      const resources = awsCompileApigEvents.serverless.service
        .provider.compiledCloudFormationTemplate.Resources;

      expect(resources).not.to.have.property('AuthorizerLambdaPermissionApiGateway');
    });
  });
});

      ...
      functionObject.events.forEach(event => {
        if (event.http && event.http.authorizer) {
          ...
          if (typeof authorizer === 'string') {
            if (authorizer.indexOf(':') === -1) {
              ...
              authorizerArn = `",{"Fn::GetAtt" : ["${
                this.sdk.naming.getLogicalLambdaName(authorizerName)}", "Arn"]},"`;
            } else {
              authorizerArn = authorizer;
              authorizerName = this.sdk.naming.extractLambdaNameFromArn(authorizerArn);
            }
            ...
            if (authorizer.arn) {
              ...
              authorizerArn = `",{"Fn::GetAtt" : ["${
                this.sdk.naming.getLogicalLambdaName(authorizer.name)}", "Arn"]},"`;
          ...
          const authorizerObject = {
            [this.sdk.naming.getLogicalAuthorizerName(authorizerName)]: authorizerTemplateJson,
          };

        ...
        if (event.http && event.http.authorizer) {
          ...
          if (typeof authorizer === 'string') {
            if (authorizer.indexOf(':') === -1) {
              ...
              authorizerName = this.provider.naming.extractLambdaNameFromArn(authorizerArn);
            }
            ...
            if (authorizer.arn) {
              ...
              authorizerArn = `",{"Fn::GetAtt" : ["${
                this.provider.naming.getLogicalLambdaName(authorizer.name)}", "Arn"]},"`;
          ...
          const authorizerObject = {
            [this.provider.naming.getLogicalAuthorizerName(authorizerName)]: authorizerTemplateJson,
          };

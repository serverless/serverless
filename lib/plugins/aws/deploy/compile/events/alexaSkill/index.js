    ...
    this.serverless.service.getAllFunctions().forEach((functionName) => {
      ...
      if (functionObj.events) {
        functionObj.events.forEach(event => {
          if (event.alexaSkill) {
            if (typeof event.alexaSkill === 'object') {
              if (event.alexaSkill.enabled) {
              ...
              const errorMessage = [
                `Alexa Skill event of function "${functionName}" is not an object.`,
                ' The correct syntax is: alexaSkill: <config>.',

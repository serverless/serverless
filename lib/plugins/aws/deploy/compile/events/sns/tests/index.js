  ...
  describe('#compileSNSEvents()', () => {
    ...
    it('should create corresponding resources when SNS events are given', () => {
      ...
      expect(awsCompileSNSEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[naming.getLogicalSnsTopicName('Topic 1')].Type
      ).to.equal('AWS::SNS::Topic');
      expect(awsCompileSNSEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[naming.getLogicalSnsTopicName('Topic 2')].Type
      ).to.equal('AWS::SNS::Topic');
      expect(awsCompileSNSEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[naming.getLambdaSnsTopicPermissionName('first', 'Topic 1')].Type
      ).to.equal('AWS::Lambda::Permission');
      expect(awsCompileSNSEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[naming.getLambdaSnsTopicPermissionName('first', 'Topic 2')].Type
      ).to.equal('AWS::Lambda::Permission');
    });
    ...
    it('should create single SNS topic when the same topic is referenced repeatedly', () => {
      ...
      expect(awsCompileSNSEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[naming.getLogicalSnsTopicName('Topic 1')].Properties.Subscription.length
      ).to.equal(2);
      expect(awsCompileSNSEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[naming.getLambdaSnsTopicPermissionName('first', 'Topic 1')].Type
      ).to.equal('AWS::Lambda::Permission');

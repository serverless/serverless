  ...
  describe('#compileScheduledEvents()', () => {
    ...
    it('should create corresponding resources when schedule events are given', () => {
      ...
      expect(awsCompileScheduledEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[awsCompileScheduledEvents.provider.naming.getCloudWatchEventName('first', 1)].Type
      ).to.equal('AWS::Events::Rule');
      expect(awsCompileScheduledEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[awsCompileScheduledEvents.provider.naming.getCloudWatchEventName('first', 2)].Type
      ).to.equal('AWS::Events::Rule');
      expect(awsCompileScheduledEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[awsCompileScheduledEvents.provider.naming.getCloudWatchEventName('first', 3)].Type
      ).to.equal('AWS::Events::Rule');
      expect(awsCompileScheduledEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[awsCompileScheduledEvents.provider.naming
        .getLambdaCloudWatchEventPermissionName('first', 1)].Type
      ).to.equal('AWS::Lambda::Permission');
      expect(awsCompileScheduledEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[awsCompileScheduledEvents.provider.naming
        .getLambdaCloudWatchEventPermissionName('first', 2)].Type
      ).to.equal('AWS::Lambda::Permission');
      expect(awsCompileScheduledEvents.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[awsCompileScheduledEvents.provider.naming
        .getLambdaCloudWatchEventPermissionName('first', 3)].Type
      ).to.equal('AWS::Lambda::Permission');
    });

  ...
  describe('#compileS3Events()', () => {
    ...
    it('should create corresponding resources when S3 events are given', () => {
      ...
      expect(awsCompileS3Events.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[awsCompileS3Events.provider.naming.getLogicalBucketName(
          awsCompileS3Events.serverless.service.functions.first.events[0].s3
        )].Type
      ).to.equal('AWS::S3::Bucket');
      expect(awsCompileS3Events.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[awsCompileS3Events.provider.naming.getLogicalBucketName(
          awsCompileS3Events.serverless.service.functions.first.events[1].s3.bucket
        )].Type
      ).to.equal('AWS::S3::Bucket');
      expect(awsCompileS3Events.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[awsCompileS3Events.provider.naming.getLambdaS3PermissionName('first')].Type
      ).to.equal('AWS::Lambda::Permission');
      expect(awsCompileS3Events.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[awsCompileS3Events.provider.naming.getLogicalBucketName(
          awsCompileS3Events.serverless.service.functions.first.events[1].s3.bucket
        )].Properties.NotificationConfiguration.LambdaConfigurations[0].Filter).to.deep.equal({
    ...
    it('should create single bucket resource when the same bucket referenced repeatedly', () => {
      ...
      expect(awsCompileS3Events.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[awsCompileS3Events.provider.naming.getLogicalBucketName(
          awsCompileS3Events.serverless.service.functions.first.events[0].s3
        )].Properties.NotificationConfiguration.LambdaConfigurations.length
      ).to.equal(2);
      expect(awsCompileS3Events.serverless.service.provider.compiledCloudFormationTemplate
        .Resources[awsCompileS3Events.provider.naming.getLambdaS3PermissionName('first')].Type
      ).to.equal('AWS::Lambda::Permission');

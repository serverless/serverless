  ...
  compileS3Events() {
        ...
        functionObj.events.forEach(event => {
          if (event.s3) {
            ...
            if (event.s3.lifecycle) {
              _.merge(bucketLifeCycleConfigurations[bucketName], event.s3.lifecycle);
            }
          }
        });
      }
    ...
    _.forEach(bucketsLambdaConfigurations, (bucketLambdaConfiguration, bucketName) => {
      ...
      if (bucketLifeCycleConfigurations[bucketName]) {
        bucketTemplate.Properties.LifecycleConfiguration = bucketLifeCycleConfigurations[bucketName];
      }

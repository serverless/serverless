...
class AwsCompileS3Events {
  ...
  compileS3Events() {
    ...
    const bucketLifeCycleConfigurations = {};
      ...
      if (functionObj.events) {
        functionObj.events.forEach(event => {
          if (event.s3) {
            ...
            if (event.s3.lifecycle) {
              bucketLifeCycleConfigurations[bucketName] =
                bucketLifeCycleConfigurations[bucketName] || [];

              bucketLifeCycleConfigurations[bucketName] =
                bucketLifeCycleConfigurations[bucketName].concat(
                  event.s3.lifecycle
                );
            }
          }
    ...
    _.forEach(bucketsLambdaConfigurations, (bucketLambdaConfiguration, bucketName) => {
      ...
      if (bucketLifeCycleConfigurations[bucketName]) {
        bucketTemplate.Properties.LifecycleConfiguration = {
          Rules: bucketLifeCycleConfigurations[bucketName],
        };
      }

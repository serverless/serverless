...
class AwsCompileS3Events {
  ...
  compileS3Events() {
    ...
    _.forEach(bucketsLambdaConfigurations, (bucketLambdaConfiguration, bucketName) => {
      ...
      const bucketLogicalID = this.provider.naming.getLogicalBucketName(bucketName);
    ...
    s3EnabledFunctions.forEach(functionName => {
      ...
      const permissionLogicalID = this.provider.naming.getLambdaS3PermissionName(functionName);

...
class AwsCompileFunctions {
  ...
  /**
   * Compile the Lambda role value from the given role attribute value.  Role can contain a valid
   * "Fn::GetAtt" object, a logical role name, or a role ARN.
   * @param role The role to compile into a valid Lambda Role attribute value.
   * @return {*} The valid Lambda Role attribute value compiled from the given role value.
   */
  compileRole(role) {
    if (typeof role === 'object') { // role is an "Fn::GetAtt" object
      return role;
    } else if (role.indexOf(':') === -1) { // role is a Logical Role Name

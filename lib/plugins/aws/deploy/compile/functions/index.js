...
class AwsCompileFunctions {
  constructor(serverless, options) {
    ...
    this.provider = 'aws';
    this.sdk = new SDK(serverless, options);
  ...
  compileFunction(functionName) {
    ...
    if (typeof this.serverless.service.provider.iamRoleARN === 'string') {
      ...
      newFunction.Properties.Role = { 'Fn::GetAtt': [this.sdk.naming.getLogicalRoleName(), 'Arn'] };
    }
    ...
    const newFunctionObject = {
      [this.sdk.naming.getLogicalLambdaName(functionName)]: newFunction,
    };
    ...
    newOutput.Value = { 'Fn::GetAtt': [this.sdk.naming.getLogicalLambdaName(functionName), 'Arn'] };

    const newOutputObject = {
      [this.sdk.naming.getLogicalLambdaArnName(functionName)]: newOutput,
    };
  ...
  cfLambdaFunctionTemplate() {
    return {
      ...
      Properties: {
        Code: {
          S3Bucket: {
            Ref: this.sdk.naming.getLogicalDeploymentBucketName(),
          },
          S3Key: 'S3Key',
        },

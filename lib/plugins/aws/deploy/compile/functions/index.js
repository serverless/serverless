...
class AwsCompileFunctions {
  ...
  compileFunction(functionName) {
    ...
    if (typeof this.serverless.service.provider.iamRoleARN === 'string') {
      ...
      newFunction.Properties.Role = { 'Fn::GetAtt': [this.provider.naming.getLogicalRoleName(), 'Arn'] };
    }
    ...
    const newFunctionObject = {
      [this.provider.naming.getLogicalLambdaName(functionName)]: newFunction,
    };
    ...
    newOutput.Value = { 'Fn::GetAtt': [this.provider.naming.getLogicalLambdaName(functionName), 'Arn'] };

    const newOutputObject = {
      [this.provider.naming.getLogicalLambdaArnName(functionName)]: newOutput,
    };
  ...
  cfLambdaFunctionTemplate() {
    return {
      ...
      Properties: {
        Code: {
          S3Bucket: {
            Ref: this.provider.naming.getLogicalDeploymentBucketName(),
          },
          S3Key: 'S3Key',
        },

...
describe('checkForChanges #2', () => {
  ...
  // TODO: Convert to runServerless test.
  // TODO: Try to delete extra params
  // TODO - IDEA: Stub out crypto and just return one identical hash for everything.
  //              The _number_ of hashes will still test the regression.
  it.only('should skip a deployment with identical hashes and package.artifact targeting .serverless directory', () => {
    return runServerless({
      ...
      awsRequestStubMap: {
        ...
        S3: {
          // **Note**: We'd ideally set `headObject` here to return HEAD values for
          // - compiled-cloudformation-template.json
          // - my-own.zip
          // but as constructed in https://github.com/serverless/test/blob/master/configure-aws-request-stub.js#L15-L24
          // it looks like we only get **one** value to stub. So we manually
          // stub out above.
          //
          // my-own.zip
          // {
          ...
          // }
          headObject: {
            Metadata: { filesha256: 'NEEDS_TO_BE_DIFFERENT' }
          },
          listObjectsV2: {
            Contents: [
              {
      ...
      console.log('TODO HERE RESP', { resp });
      // TODO: Assert on "Service files not changed. Skipping deployment..."
    });
  });

  // TODO: REMOVE OLD TEST
  // awsDeploy.serverless.service.package = {
  //   artifact: '.serverless/my-own.zip',
  // };

  // globbySyncStub.returns(['my-own.zip']);
  // cryptoStub
  //   .createHash()
  //   .update()
  //   .digest.onCall(0)
  ...
  //   });
  //   expect(readFileStub).to.have.been.calledWith(
  //     path.resolve(awsDeploy.serverless.config.servicePath, '.serverless/my-own.zip')
  //   );
  //   expect(awsDeploy.serverless.service.provider.shouldNotDeploy).to.equal(true);
  // });

...
module.exports = {
  ...
  async uploadCloudFormationFile() {
    ...
    let params = {
      ...
      Key: this.provider.getCompiledTemplateS3Path(),
  ...
  async uploadZipFile(artifactFilePath) {
    ...
    const s3Key = await this.provider.getServiceArtifactS3Path(artifactFilePath);
    ...
    const { base: fileName } = path.parse(artifactFilePath);
  ...
  async uploadCustomResources() {
    const artifactFilePath = path.join(
      ...
      '.serverless',
      `${this.provider.naming.getCustomResourcesArtifactBasename()}.zip`
    );
    ...
    if (this.serverless.utils.fileExistsSync(artifactFilePath)) {
      ...
      return await this.uploadZipFile(artifactFilePath);
    }

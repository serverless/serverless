...
module.exports = {
  ...
  uploadCloudFormationFile() {
    ...
    if (deploymentBucketObject) {
      ...
      params = createTags(params, deploymentBucketObject);
    }
...
function createTags(putParams, deploymentBucketObject) {
  const params = putParams;
  let taggingString = '';

  if (deploymentBucketObject.tags) {
    for (const tag of deploymentBucketObject.tags) {
      taggingString += `${encodeURIComponent(Object.keys(tag)[0])}=`;
      taggingString += `${encodeURIComponent(tag[Object.keys(tag)[0]])}&`;
    }
    taggingString = taggingString.substring(0, taggingString.length - 1);
    params.Tagging = taggingString;
  }

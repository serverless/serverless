...
const { log } = require('@serverlessinc/core-utils');
...
module.exports = {
  ...
  async checkIfDeploymentIsNecessary(objects, funcLastModifiedDate) {
    ...
    if (changedAfterDeploy) {
      logger.debug('function modification dates changed after last deploy - deploy');
      return;
    }
    logger.debug('artifacts on S3 (%o) and locally (%o)', remoteHashesMap, localHashesMap);
  ...
  async fixLogGroupSubscriptionFilters(params) {
    const log = log.get('check-for-changes');
    ...
    logger.debug('external subscription filters %o', externalOldSubscriptionFilters);
    if (

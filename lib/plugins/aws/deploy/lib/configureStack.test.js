...
describe('#configureStack', () => {
  ...
  it('should skip the check if there is no custom bucket defined', () => {
    sinon.stub(awsPlugin, 'validateS3BucketName');
    sinon.stub(awsPlugin.provider, 'request');

    return awsPlugin.configureStack()
      .then(() => {
        expect(awsPlugin.validateS3BucketName.callCount).to.be.equal(0);
        expect(awsPlugin.provider.request.callCount).to.be.equal(0);
      });
  });
});

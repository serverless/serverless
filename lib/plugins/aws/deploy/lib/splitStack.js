...
module.exports = {
  ...
  clusterDepGraph() {
    ...
    const orderedGraphNodes = depGraph.overallOrder().sort();

    const adjacencyMatrix = [];
    for (let i = 0; i < orderedGraphNodes.length; i += 1) {
      const arr = new Array(orderedGraphNodes.length).fill(0);
      adjacencyMatrix.push(arr);

      const itemA = orderedGraphNodes[i];
      for (let j = 0; j < orderedGraphNodes.length; j += 1) {
        const itemB = orderedGraphNodes[j];
        if (depGraph.dependenciesOf(itemB).includes(itemA)) {
          adjacencyMatrix[i][j]++;
        }
      }
    }

    const preparedMatrix = math.matrix(adjacencyMatrix);

    let clustered = mc.cluster(preparedMatrix, {
      expandFactor: 2,
      inflateFactor: 2,
      maxLoops: 10,
      multFactor: 2,
    ...
    // replace items in the clustered matrix with corresponding CloudFormation Resources and Outputs
    clustered = clustered.map((subArr) => subArr.map(item => orderedGraphNodes[item]));

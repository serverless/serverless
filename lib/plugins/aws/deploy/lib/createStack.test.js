...
describe('createStack', () => {
  ...
  describe('#createStack()', () => {
    ...
    // Family of test cases to verify that the stack name is checked
    [
      {
        name: 'should throw exception if the stack name contains spaces',
        serviceName: 'a service',
      },
      {
        name: 'should throw exception if the stack name starts with a number',
        serviceName: '1st',
      },
      {
        name: 'should throw exception if the stack name contains invalid characters',
        serviceName: 'func#1',
      },
      {
        name: 'should throw exception if the stack name is too big',
        serviceName: new Array(129).join('a'),
      },
    ].forEach((testCase) => {
      it(testCase.name, () => {
        awsDeploy.serverless.service.service = testCase.serviceName;
        expect(() => awsDeploy.createStack()).to.throw(ServerlessError, 'is not valid');
      });
    });
  });

...
module.exports = {
  ...
  async fixLogGroupSubscriptionFilters(params) {
    ...
    const matchedOldSubscriptionFilters = oldSubscriptionFilters.filter((oldSubscriptionFilter) => {
      const destinationMatchNewSubscriptionFilter = newSubscriptionFilters.find(
        (newSubscriptionFilter) => sameDestinationArn(newSubscriptionFilter, oldSubscriptionFilter)
      );
    ...
    const orderChangedOldSubscriptionFilters = matchedOldSubscriptionFilters.filter(
      (oldSubscriptionFilter) => {
        const matchNewSubscriptionFilter = newSubscriptionFilters.find((newSubscriptionFilter) =>
          subscriptionFilterComparator(newSubscriptionFilter, oldSubscriptionFilter)
        );
        return !matchNewSubscriptionFilter;
      }
    );
    return Promise.all(
      orderChangedOldSubscriptionFilters.map((oldSubscriptionFilter) =>
        this.provider.request('CloudWatchLogs', 'deleteSubscriptionFilter', {
          logGroupName,
          filterName: oldSubscriptionFilter.filterName,
        })

...
describe('updateStack', () => {
  ...
  describe('#monitorUpdate()', () => {
    it('should keep monitoring until UPDATE_COMPLETE stack status', () => {
      const describeStackStub = sinon.stub(awsDeploy.sdk, 'request');
      const cfDataMock = {
      ...
      describeStackStub.onCall(0).returns(BbPromise.resolve(DescribeReturn));
      describeStackStub.onCall(1).returns(BbPromise.resolve(DescribeReturn));
      describeStackStub.onCall(2).returns(BbPromise.resolve(finalDescribeReturn));

      return awsDeploy.monitorUpdate(cfDataMock, 10).then((stack) => {
        expect(describeStackStub.callCount).to.be.equal(3);
        expect(describeStackStub.calledWith(awsDeploy.options.stage, awsDeploy.options.region));

...
describe('#findAndGroupStacks()', () => {
  it('should return an empty result in case no S3 objects are provided', () => {
    const s3Response = {
      Contents: [],
    };

    expect(findAndGroupStacks(s3Response, 'test', 'dev')).to.deep.equal([]);
  });
  ...
  it('should group stacks', () => {
    ...
    expect(findAndGroupStacks(s3Response, 'test', 'dev')).to.deep.equal(expected);
  });
});

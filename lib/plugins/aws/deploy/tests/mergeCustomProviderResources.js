...
describe('mergeCustomProviderResources', () => {
  ...
  describe('#mergeCustomProviderResources()', () => {
    ...
    it('should be able to overwrite existing object properties', () => {
      const customResourcesMock = {
        Resources: {
          [awsDeploy.provider.naming.getLogicalDeploymentBucketName()]: {
            Type: 'Some::New::Type',
      ...
      return awsDeploy.mergeCustomProviderResources().then(() => {
        expect(awsDeploy.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[awsDeploy.provider.naming.getLogicalDeploymentBucketName()]
        ).to.deep.equal(customResourcesMock.Resources[
            awsDeploy.provider.naming.getLogicalDeploymentBucketName()
          ]);
      });
    });
    ...
    it('should keep the core template definitions when merging custom resources', () => {
      ...
      return awsDeploy.mergeCustomProviderResources().then(() => {
        ...
        expect(awsDeploy.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[awsDeploy.provider.naming.getLogicalDeploymentBucketName()]
        ).to.deep.equal(coreCloudFormationTemplate.Resources[
            awsDeploy.provider.naming.getLogicalDeploymentBucketName()
          ]
        );
        ...
        expect(
          awsDeploy.serverless.service.provider.compiledCloudFormationTemplate
            .Outputs[awsDeploy.provider.naming.getLogicalDeploymentBucketOutputVariableName()]
        ).to.deep.equal(coreCloudFormationTemplate.Outputs[
            awsDeploy.provider.naming.getLogicalDeploymentBucketOutputVariableName()
          ]
        );
      });
    });

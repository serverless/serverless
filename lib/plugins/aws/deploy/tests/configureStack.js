...
describe('#configureStack', () => {
  ...
  it('should merge the role template into the CloudFormation template', () => {
    ...
    roleTemplate[naming.getLogicalRoleName()].Properties.Path = naming.getRolePath();
    roleTemplate[naming.getLogicalRoleName()].Properties.RoleName = naming.getRoleName();


    return awsSdk.configureStack()
      .then(() => {
        expect(awsSdk.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalRoleName()]
        ).to.deep.equal(roleTemplate[naming.getLogicalRoleName()]);
      });
  });
  ...
  it('should merge policy template into the CloudFormation template', () =>
    awsSdk.configureStack()
      .then(() => {
        ...
        expect(awsSdk.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalPolicyName()].Type
  ...
  it('should update the necessary variables for the policy', () =>
    awsSdk.configureStack()
      .then(() => {
        expect(awsSdk.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalPolicyName()]
          .Properties
          .PolicyName
  ...
  it('should add custom IAM policy statements', () => {
    ...
    return awsSdk.configureStack()
      .then(() => {
        expect(awsSdk.serverless.service.provider.compiledCloudFormationTemplate
          .Resources[naming.getLogicalPolicyName()].Properties.PolicyDocument.Statement[1]
      ...
      .then(() => {
        expect(
          awsSdk.serverless.service.provider.compiledCloudFormationTemplate
            .Outputs[naming.getLogicalDeploymentBucketOutputVariableName()].Value
        ).to.equal(bucketName);
        // eslint-disable-next-line no-unused-expressions
        expect(
          awsSdk.serverless.service.provider.compiledCloudFormationTemplate
            .Resources[naming.getLogicalDeploymentBucketName()]
        ).to.not.exist;
      });
  });

  ...
  it('should use a custom bucket if specified', () => {
    ...
    return awsPlugin.configureStack()
      .then(() => {
        expect(
          awsPlugin.serverless.service.provider.compiledCloudFormationTemplate
            .Outputs[awsPlugin.provider.naming.getLogicalDeploymentBucketOutputVariableName()].Value
        ).to.equal(bucketName);
        // eslint-disable-next-line no-unused-expressions
        expect(
          awsPlugin.serverless.service.provider.compiledCloudFormationTemplate
            .Resources[awsPlugin.provider.naming.getLogicalDeploymentBucketName()]
        ).to.not.exist;
      });
  });
});

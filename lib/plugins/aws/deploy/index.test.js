  ...
  describe('hooks', () => {
    it('should run "before:deploy:initialize" hook promise chain in order', () => {
      const validateStub = sinon
        .stub(awsDeploy, 'validate').resolves();
    ...
    it('should run "deploy:setupProviderConfiguration" hook promise chain in order', () => {
      ...
      const mergeIamTemplatesStub = sinon
        .stub(awsDeploy, 'mergeIamTemplates').resolves();
    ...
    it('should run "before:deploy:compileFunctions" promise chain in order', () => {
      const generateArtifactDirectoryNameStub = sinon
        .stub(awsDeploy, 'generateArtifactDirectoryName').resolves();
    ...
    it('should run "deploy:initialize" promise chain in order', () => {
      const configureStackStub = sinon
        .stub(awsDeploy, 'configureStack').resolves();
    ...
    it('should run "deploy:deploy" promise chain in order', () => {
      const mergeCustomProviderResourcesStub = sinon
        .stub(awsDeploy, 'mergeCustomProviderResources').resolves();
    ...
    it('should notify about noDeploy', () => {
      sinon.stub(awsDeploy, 'mergeIamTemplates').resolves();
      sinon.stub(awsDeploy, 'mergeCustomProviderResources').resolves();
      sinon.stub(awsDeploy, 'setBucketName').resolves();
      sinon.stub(awsDeploy, 'cleanupS3Bucket').resolves();
      sinon.stub(awsDeploy, 'uploadArtifacts').resolves();
      sinon.stub(awsDeploy, 'updateStack').resolves();

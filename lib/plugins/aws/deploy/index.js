...
import observabilityIntegrationService from '@serverlessinc/sf-core/src/lib/platform/observability.js';
...
class AwsDeploy {
    ...
    this.hooks = {
      ...
      'before:deploy:deploy': async () => {
        ...
        if (this.isDashboardEnabled()) {
          this.observabilityIntegrationService = await observabilityIntegrationService(
            this.serverless
          );

          await this.observabilityIntegrationService.configureIntegrationContext();
          await this.observabilityIntegrationService.ensureIntegrationIsConfigured();
        }
      ...
      'before:aws:deploy:deploy:updateStack': () => {
        if (this.serverless.service.provider.shouldNotDeploy) return;
        const mainProgress = progress.get('main');
        mainProgress.notice('Updating AWS CloudFormation stack', { isMainEvent: true });
      },
      ...
      'after:deploy:deploy': async () => {
        ...
        if (this.isObservabilityConfigured()) {
          await this.observabilityIntegrationService.instrumentService();
        }
      },
  ...
  isDashboardEnabled() {
    const dashboardEnabled = Boolean(this.serverless.service.app);

    if (!dashboardEnabled && this.isObservabilityConfigured()) {
      throw new ServerlessError(
  ...
  isObservabilityConfigured() {
    const stageName = this.options?.stage || this.serverless.service.provider.stage || 'dev';

    let observabilityConfigured = false;

    if (
      this.serverless.service.stages &&
      (typeof this.serverless.service.stages?.default?.observability === 'boolean' ||
        typeof this.serverless.service.stages[stageName]?.observability === 'boolean')
    ) {

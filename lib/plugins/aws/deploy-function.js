...
class AwsDeployFunction {
  ...
  async ensureFunctionState() {
    ...
    const callWithRetry = async () => {
      ...
      this.logger.info(`Retrying ensure function state for function: ${this.options.function}.`);
      await wait(500);
      await callWithRetry();
    };
  ...
  async updateFunctionConfiguration() {
    ...
    this.progress.notice('Updating function configuration', { isMainEvent: true });

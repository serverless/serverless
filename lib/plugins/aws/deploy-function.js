...
class AwsDeployFunction {
  ...
  async updateFunctionConfiguration() {
    ...
    if (isConsoleEnabled) {
      if (!params.Environment) params.Environment = { Variables: {} };
      Object.assign(
    ...
    if (isConsoleEnabled) await this.console.activateOtelIngestionToken();

    try {
      await this.callUpdateFunctionConfiguration(params);
    } catch (error) {
      if (isConsoleEnabled && this.console.isFreshOtelIngestionToken) {

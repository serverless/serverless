...
class AwsDeployFunction {
  ...
  async updateFunctionConfiguration() {
    ...
    if (functionObj.snapStart) {
      const snapStartConfig = {};
      if (_.isObject(functionObj.snapStart)) {
        if (functionObj.snapStart.applyOn) {
          snapStartConfig.ApplyOn = functionObj.snapStart.applyOn;
        }
      }
      params.SnapStart = snapStartConfig;
    }

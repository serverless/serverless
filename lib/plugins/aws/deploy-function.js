...
class AwsDeployFunction {
  ...
  async updateFunctionConfiguration() {
    ...
    let hasServerlessConsoleSDK = false;
    let hasServerlessConsoleExternal = false;
    const sdkRegex = /(177335420605|321667558080):layer:sls-sdk/;
    const externalRegex = /(177335420605|321667558080):layer:sls-external-extension-/;
    if (
      params.Layers &&
      remoteFunctionConfiguration.Layers &&
      remoteFunctionConfiguration.Layers.some(
        ({ Arn }) => sdkRegex.test(Arn) || externalRegex.test(Arn)
      )
    ) {
      const serverlessConsoleLayerArns = remoteFunctionConfiguration.Layers.filter(
        ({ Arn }) => sdkRegex.test(Arn) || externalRegex.test(Arn)
      ).map(({ Arn }) => Arn);
      hasServerlessConsoleSDK = serverlessConsoleLayerArns.some((layerArn) =>
        layerArn.includes('sls-sdk')
      );
      hasServerlessConsoleExternal = serverlessConsoleLayerArns.some((layerArn) =>
        layerArn.includes('sls-external-extension')
      );
      params.Layers.push(...serverlessConsoleLayerArns);
    }
    if (
      params.Layers &&
      remoteFunctionConfiguration.Layers &&
    ...
    if (params.Environment) {
      ...
      if (hasServerlessConsoleSDK || hasServerlessConsoleExternal) {
        const { environmentKeys } = await apiRequest('/api/integrations/aws/layer/config', {
          urlName: 'integrationsBackend',
          noAuth: true,
        });
        environmentKeys.push('SERVERLESS_PLATFORM_STAGE');
        const whiteListedEnvVars = Object.keys(remoteFunctionConfiguration.Environment.Variables)
          .filter((key) => environmentKeys.includes(key))
          .reduce(
            (obj, key) => ({
              ...obj,
              [key]: remoteFunctionConfiguration.Environment.Variables[key],
            }),
            {}
          );

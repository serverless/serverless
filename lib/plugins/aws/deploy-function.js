...
class AwsDeployFunction {
  ...
  static serverlessConsoleLayerConfig(remoteFunctionConfiguration) {
    ...
    return {
      ...
      serverlessConsoleLayerArns,
    };
  }

  static mergeEnvironmentVariablesWithRemote(params, remoteFunctionConfiguration) {
    if (params.Environment) {
      const { hasServerlessConsoleLayers } = AwsDeployFunction.serverlessConsoleLayerConfig(
  ...
  static mergeLayerConfigurationWithRemote(params, remoteFunctionConfiguration) {
    ...
    const { serverlessConsoleLayerArns } = AwsDeployFunction.serverlessConsoleLayerConfig(
      remoteFunctionConfiguration
    );
    if (params.Layers && serverlessConsoleLayerArns.length > 0) {

...
class AwsRollback {
  ...
  async setStackToUpdate() {
    ...
    if (this.console.isEnabled) {
      if (!state.console) {
        log.warning(
          'Cannot enable console: Restored deployment had disabled console configuration'
        );
        this.console.isEnabled = false;
        return;
      }
      if (state.console.schemaVersion !== this.console.stateSchemaVersion) {
        throw new ServerlessError(
          'Cannot rollback deployment: ' +
            'Target deployment was packaged with different version of console integration',
          'CONSOLE_INTEGRATION_MISMATCH_ROLLBACK'
        );
      }
      if (state.console.orgId !== this.console.orgId) {
        throw new ServerlessError(
          'Cannot rollback deployment: Target deployment was packaged in scope of different "org"',
          'CONSOLE_ORG_MISMATCH_ROLLBACK'
        );
      }
      ...
      throw new ServerlessError(
        'Cannot rollback deployment: Target deployment was packaged with console integration, ' +

...
class AwsRollback {
  constructor(serverless, options) {
    ...
    this.hooks = {
      'before:rollback:initialize': () => this.validate(),
  ...
  async setStackToUpdate() {
    ...
    if (deployments.length === 0) {
      ...
      throw new ServerlessError(`${msg} ${hint}`, 'ROLLBACK_IMPOSSIBLE_NO_DEPLOYMENTS');
    }
    ...
    if (!exists) {
      ...
      throw new ServerlessError(`${msg} ${hint}`, 'ROLLBACK_IMPOSSIBLE_NO_DEPLOYMENT');
    }

...
const impl = {
   ...
   */
  addCredentials: (results, credentials, skipValidation = false) => {
    if (skipValidation || impl.validCredentials(credentials)) {
  ...
  /**
   * Add credentials from web identity token file
   * @param results The prefix of the profile's declaration in the environment
   */
  addTokenFileWebIdentityCredentials: results => {
    if (process.env.AWS_WEB_IDENTITY_TOKEN_FILE && process.env.AWS_ROLE_ARN) {
      const webIdentityCredentials = new AWS.TokenFileWebIdentityCredentials();
      impl.addCredentials(results, webIdentityCredentials, true);
    }
  },
};
...
class AwsProvider {
  ...
  getCredentials() {
    ...
    impl.addTokenFileWebIdentityCredentials(result);

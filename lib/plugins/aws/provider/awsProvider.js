        ...
        req.send((errParam, data) => {
          ...
          if (err) {
            if (err.message === 'Missing credentials in config') {
              const errorMessage = `AWS provider credentials not found.

  Learn how to set up AWS provider credentials in our docs here: ${chalk.green('https://git.io/vXsdd')}`;
              err.message = errorMessage;
              userStats.track('user_awsCredentialsFailed');
            }

...
const impl = {
  ...
  addProfileCredentials: (results, profile) => {
    if (profile) {
      ...
      return BbPromise.each(profileCredentials, credential => {
        if (credential.accessKeyId || credential.sessionToken || credential.roleArn) {
          impl.addCredentials(results, credential);
          profileFound = true;
        }
      }).then(() => {
        if (!profileFound) {
  ...
  addEnvironmentProfile: (results, prefix) => {
    if (prefix) {
      ...
      return impl.addProfileCredentials(results, profile);
    }
    return BbPromise.resolve();
  },
};
    ...
    const request = this.requestQueue.add(() =>
      persistentRequest(() =>
        BbPromise.resolve()
          ...
          .then(rawCredentials => {
            const credentials = Object.assign({}, rawCredentials);
    ...
    return impl
      ...
      .then(() => {
        ...
        if (this.serverless.service.provider.profile && !this.options['aws-profile']) {
          // config profile
          return impl.addProfileCredentials(result, this.serverless.service.provider.profile);
        }
        return null;
      })
      .then(() => {
        impl.addEnvironmentCredentials(result, 'AWS'); // creds for all stages
        return impl.addEnvironmentProfile(result, 'AWS');
      })
      .then(() => {
      ...
      .then(() => {
        if (this.options['aws-profile']) {
          return impl.addProfileCredentials(result, this.options['aws-profile']); // CLI option profile
        }
        return null;
      })
      .then(() => {

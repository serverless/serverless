...
class AwsProvider {
  ...
  getRegion() {
    return this.getProviderValue('region', 'us-east-1');
  }
  ...
  getStage() {
    return this.getProviderValue('stage', 'dev');
  }

  /**
   * Utility for returning a configured provider value
   *
   * The value is derived from the following:
   * - Provider option, else
   * - Serverless config, else
   * - Serverless service config, else
   * - `_default` parameter
   *
   * @param {string} key - Option/Config key name
   * @param {string} _default - Default value if key is not present
   * @returns {*}
   */
  getProviderValue(key, _default) {
    let returnValue = _default;
    if (this.options && this.options[key]) {
      returnValue = this.options[key];
    } else if (this.serverless.config[key]) {
      returnValue = this.serverless.config[key];
    } else if (this.serverless.service.provider[key]) {
      returnValue = this.serverless.service.provider[key];
    }
    return returnValue;
  }

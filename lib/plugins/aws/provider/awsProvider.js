...
const impl = {
  ...
  validCredentials: credentials => {
    if (
      ...
      ((credentials.accessKeyId &&
        ...
        credentials.roleArn ||
        // TokenFileWebIdentityCredentials can not be verified
        credentials instanceof AWS.TokenFileWebIdentityCredentials)
    ) {
      return true;
    }
  ...
  addTokenFileWebIdentityCredentials: results => {
    if (process.env.AWS_WEB_IDENTITY_TOKEN_FILE && process.env.AWS_ROLE_ARN) {
      ...
      impl.addCredentials(results, webIdentityCredentials);
    }
  },
};

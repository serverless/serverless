...
describe('AwsProvider', () => {
  ...
  describe('#request()', () => {
    ...
    it('should request to the specified region if region in options set', () => {
      ...
      return awsProvider
        .getCredentials()
        .then(creds => {
          expect(creds.region).to.eql(options.region);
        })
        .then(() =>
          awsProvider.request(
            'CloudFormation',
      ...
      describe('STS tokens', () => {
        ...
        it('should retain reference to STS tokens when updated via SDK', () => {
          ...
          return newAwsProvider
            .getCredentials()
            .then(creds => {
              const startToken = creds.credentials.sessionToken;
              expect(startToken).to.not.equal(expectedToken);
            })
            .then(() =>
              newAwsProvider.request(
                'CloudFormation',
                'describeStacks',
                { StackName: 'foo' },
                { region: 'ap-northeast-1' }
            ...
            .then(() => {
              ...
              return newAwsProvider.getCredentials();
            })
            .then(creds => {
              const actualToken = creds.credentials.sessionToken;
  ...
  describe('#getCredentials()', () => {
    ...
    it('should set region for credentials', () => {
      ...
      return newAwsProvider.getCredentials().then(credentials => {
    ...
    it('should not set credentials if credentials is an empty object', () => {
      ...
      return newAwsProvider.getCredentials().then(
        () => {
          throw new Error('Should reject');
        },
        err => {
          expect(err.message).to.match(/AWS provider credentials not found/);
        }
      );
    });

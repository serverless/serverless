...
describe('AwsProvider', () => {
  ...
  describe('#constructor()', () => {
    ...
    it('should set AWS cafile single', () => {
      ...
      const certContents = '-----BEGIN CERTIFICATE-----\n-----END CERTIFICATE-----';
      const filePath = path.join(tmpdir, 'ca.txt');
      fs.writeFileSync(filePath, certContents);
      process.env.cafile = filePath;
      const newAwsProvider = new AwsProvider(serverless, options);
    ...
    it('should set AWS cafile multiple', () => {
      const tmpdir = os.tmpdir();
      const certContents = '-----BEGIN CERTIFICATE-----\n-----END CERTIFICATE-----';
      const filePath1 = path.join(tmpdir, 'ca1.txt');
      const filePath2 = path.join(tmpdir, 'ca2.txt');
      fs.writeFileSync(filePath1, certContents);
      fs.writeFileSync(filePath2, certContents);
      process.env.cafile = `${filePath1},${filePath2}`;
      const newAwsProvider = new AwsProvider(serverless, options);

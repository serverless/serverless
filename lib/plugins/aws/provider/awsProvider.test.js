...
describe('AwsProvider', () => {
  ...
  describe('#getCredentials()', () => {
    ...
    it('should get credentials from web identity token file', () => {
      process.env.AWS_WEB_IDENTITY_TOKEN_FILE = 'notDefault';
      process.env.AWS_ROLE_ARN = 'notDefault';

      const credentials = newAwsProvider.getCredentials();
      expect(credentials.credentials).to.be.instanceOf(AWS.TokenFileWebIdentityCredentials);
    });
  ...
  describe('#getAlbTargetGroupPrefix()', () => {
    ...
    it('should throw error if prefix is longer than 16', () => {
      serverless.service.provider.alb = {};
      serverless.service.provider.alb.targetGroupPrefix = 'my-17-char-prefix';

      expect(() => awsProvider.getAlbTargetGroupPrefix()).to.throw(Error);
    });

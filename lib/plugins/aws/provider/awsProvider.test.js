...
describe('AwsProvider', () => {
  ...
  describe('#constructor()', () => {
    ...
    it('should set AWS cafile single', () => {
      ...
      fs.writeFileSync(`${tmpdir}/ca.txt`, '-----BEGIN CERTIFICATE-----\n-----END CERTIFICATE-----');
      process.env.cafile = `${tmpdir}/ca.txt`;
      const newAwsProvider = new AwsProvider(serverless, options);

      expect(typeof newAwsProvider.sdk.config.httpOptions.agent).to.not.equal('undefined');

      // clear env
      fs.unlink(`${tmpdir}/ca.txt`);
    ...
    it('should set AWS cafile multiple', () => {
      const tmpdir = os.tmpdir();
      const certContents = '-----BEGIN CERTIFICATE-----\n-----END CERTIFICATE-----';
      fs.writeFileSync(`${tmpdir}/ca1.txt`, certContents);
      fs.writeFileSync(`{$tmpdir}/ca2.txt`, certContents);
      process.env.cafile = `${tmpdir}/ca1.txt,${tmpdir}/ca2.txt`;
      const newAwsProvider = new AwsProvider(serverless, options);

      expect(typeof newAwsProvider.sdk.config.httpOptions.agent).to.not.equal('undefined');

      // clear env
      fs.unlink(`${tmpdir}/ca1.txt`);
      fs.unlink(`${tmpdir}/ca2.txt`);
      delete process.env.cafile;
    });
    ...
    it('should set AWS ca and cafile', () => {
      const tmpdir = os.tmpdir();
      const certContents = '-----BEGIN CERTIFICATE-----\n-----END CERTIFICATE-----';
      fs.writeFileSync(`${tmpdir}/ca.txt`, certContents);

...
describe('AwsProvider', () => {
  ...
  describe('#request()', () => {
    ...
    it('should retry if error code is 429', function (done) {
      ...
      clock = sinon.useFakeTimers(); // We don't want to actually wait in the tests
        ...
        error() {
          return {
            send(cb) {
              if (first) {
                cb(error);
                // Progress the clock. The provider has not taken the timeout yet, but it
                // will do that a moment. So progress the clock a little bit later.
                // Wait 10 ms, to avoid race conditions with bluebird promise resolution.
                realSetTimeout(() => clock.tick(10000), 10);
              } else {
                cb(undefined, {});
              }
              first = false;

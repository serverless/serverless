...
describe('AwsProvider', () => {
  ...
  describe('#getStageDescription()', () => {
    it('should return undefined when stageDescription is not provided', () => {
      const config = {};
      serverless = new Serverless(config);
      ...
      serverless.service = new serverless.classes.Service(serverless, serverlessYml);
      expect(new AwsProvider(serverless, config).getApiGatewayStageDescription()).to.be.undefined;
    });
    it('should throw an error when stageDescription is not provided but not object like', () => {
      const config = {};
      serverless = new Serverless(config);
      ...
      serverless.service = new serverless.classes.Service(serverless, serverlessYml);
      expect(() => {
        new AwsProvider(serverless, config).getApiGatewayStageDescription();
      }).to.throw(Error);
    });

    it('should throw an error when stageDescription is malformed', () => {
      const config = {};
      serverless = new Serverless(config);
    ...
    it('should return undefined wheng unsupported fields are provided', () => {
      const config = {};
      serverless = new Serverless(config);
      ...
      serverless.service = new serverless.classes.Service(serverless, serverlessYml);
      expect(new AwsProvider(serverless, config).getApiGatewayStageDescription())
        .to.be.undefined;
    });
  });

      ...
      _.forEach(metrics, (metric) => {
        _.forEach(metric, (funcMetric) => {
          if (funcMetric.Label === 'Invocations') {
            invocations += utils.reduceDatapoints(funcMetric.Datapoints, 'Sum');
          } else if (funcMetric.Label === 'Throttles') {
            throttles += utils.reduceDatapoints(funcMetric.Datapoints, 'Sum');
          } else if (funcMetric.Label === 'Errors') {
            errors += utils.reduceDatapoints(funcMetric.Datapoints, 'Sum');
          } else {
            duration += utils.reduceDatapoints(funcMetric.Datapoints, 'Average');
          }
        });
      });
      const formattedDuration = `${utils.getRoundedAvgDuration(duration, metrics.length)}ms`;
      // display the data

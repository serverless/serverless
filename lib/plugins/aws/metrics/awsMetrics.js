...
class AwsMetrics {
  ...
  showMetrics(metrics) {
    ...
    if (metrics && metrics.length > 0) {
      ...
      _.forEach(metrics, (metric) => {
        _.forEach(metric, (funcMetric) => {
          if (funcMetric.Label === 'Invocations') {
            const datapoints = funcMetric.Datapoints;
            invocations += datapoints
      ...
      // format the duration
      const functionsCount = metrics.length;
      const formattedRoundedAvgDuration =
        `${(Math.round(duration * 100) / 100) / functionsCount}ms`;
      // display the data

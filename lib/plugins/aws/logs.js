...
const { style, ServerlessError } = utils;
...
class AwsLogs {
    ...
    this.hooks = {
      'logs:logs': async () => {
        if (!this.options.tail) {
          ...
          this.logger.aside(`Streaming new logs for function: "${this.options.function}"`);
  ...
  async showLogs(logStreamNames) {
    ...
    const startTimeHumanReadable = dayjs(params.startTime).format('YYYY-MM-DD HH:mm:ss');
    ...
    if (!this.options.tail && (!results.events || !results.events.length)) {
      this.logger.aside(`No logs found from start time: ${startTimeHumanReadable}`);
      return;
    }

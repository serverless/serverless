...
module.exports = {
  normalizeCloudFormationTemplate(template) {
    ...
    Object.entries(normalizedTemplate.Resources).forEach(([key, value]) => {
      ...
      if (key === 'WebsocketsDeploymentStage' && _get(value.Properties, 'DeploymentId')) {
        const newVal = value;
        newVal.Properties.DeploymentId.Ref = 'WebsocketsDeployment';
      }
      if (value.Type && value.Type === 'AWS::Lambda::Function' && _get(value.Properties, 'Code')) {
        const newVal = value;

...
module.exports = {
  ...
  async update() {
    ...
    if (
      this.serverless.service.provider.stackPolicyDuringUpdate &&
      Object.keys(this.serverless.service.provider.stackPolicyDuringUpdate).length
    ) {
      params.StackPolicyDuringUpdateBody = JSON.stringify({
        Statement: this.serverless.service.provider.stackPolicyDuringUpdate,
      });
    }

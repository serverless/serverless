...
module.exports = {
  async checkStackProgress(
    ...
    await wait(options.frequency || process.env.SLS_AWS_MONITORING_FREQUENCY || 5000);

    try {
      let { StackEvents: stackEvents } = await this.provider.request(
        'CloudFormation',
        'describeStackEvents',
  ...
  async monitorStack(action, cfData, options = {}) {
    ...
    const stackStatus = await this.checkStackProgress(action, cfData, stackUrl, options, {});
    // empty console.log for a prettier output
    if (!this.options.verbose) this.serverless.cli.consoleLog('');
    this.serverless.cli.log(`Stack ${action} finished...`);
    return stackStatus;

...
module.exports = {
  monitorStack(action, cfData, options = {}) {
    ...
    return new Promise((resolve, reject) => {
      const recursivePromiseFn = () => {
        if (validStatuses.indexOf(stackStatus) === -1)
          return (() =>
            new Promise(callback => {
              const cfStackStatusCheckFrequency =
                options.frequency || process.env.SLS_AWS_MONITORING_FREQUENCY || 5000;
              ...
              }, cfStackStatusCheckFrequency);
            }))().then(() => recursivePromiseFn());
      };

      recursivePromiseFn().then(() => {
        // empty console.log for a prettier output
        if (!this.options.verbose) this.serverless.cli.consoleLog('');
        this.serverless.cli.log(`Stack ${action} finished...`);
        resolve(stackStatus);

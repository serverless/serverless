...
module.exports = {
  ...
  getScheduleLogicalId(functionName, scheduleIndex, scheduleName = undefined) {
    const normalizedFunctionName = this.getNormalizedFunctionName(functionName);
    const normalizedScheduleName = scheduleName
      ? this.normalizeNameToAlphaNumericOnly(`${scheduleName}`)
      : '';
    return `${normalizedFunctionName}EventsRuleSchedule${scheduleIndex}${normalizedScheduleName}`.slice(
      0,
      255
    );
  },
  ...
  getLambdaSchedulePermissionLogicalId(functionName, scheduleIndex, scheduleName = undefined) {
    const normalizedFunctionName = this.getNormalizedFunctionName(functionName);
    const normalizedScheduleName = scheduleName
      ? this.normalizeNameToAlphaNumericOnly(`${scheduleName}`)
      : '';
    return `${normalizedFunctionName}LambdaPermissionEventsRuleSchedule${scheduleIndex}${normalizedScheduleName}`.slice(
      0,
      255
    );
  },

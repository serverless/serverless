...
module.exports = {
  ...
  getLambdaLayerArtifactPath(packagePath, layerName) {
    const layerObject = this.provider.serverless.service.getLayer(layerName);
    if (layerObject.package && layerObject.package.artifact) {
      return layerObject.package.artifact;
    }
    return path.join(packagePath, this.getLayerArtifactName(layerName));
  },

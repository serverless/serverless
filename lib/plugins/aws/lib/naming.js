...
module.exports = {
  ...
  normalizeNameToPascalCase(name){
  ...
  // Schedule
  normalizeScheduleName(scheduleName){
    return this.normalizeNameToPascalCase(scheduleName)

  },
  getScheduleId(functionName) {
    return `${functionName}Schedule`;
  },
  getScheduleLogicalId(functionName, scheduleIndex, scheduleName) {
    let logicalId = `${this.getNormalizedFunctionName(functionName)}EventsRuleSchedule`;
    if (!_.isEmpty(scheduleName)){
      logicalId = `${logicalId}${this.normalizeNameToPascalCase(scheduleName)}`;
    }else{
      logicalId = `${logicalId}${scheduleIndex}`
    }
    return logicalId
  },

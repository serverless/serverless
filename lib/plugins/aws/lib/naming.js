...
module.exports = {
  ...
  getScheduleLogicalId(functionName, scheduleId) {
    const normalizedFunctionName = this.getNormalizedFunctionName(functionName);
    const normalizedScheduleName = this.normalizeNameToAlphaNumericOnly(`${scheduleId}`);
    return `${normalizedFunctionName}EventsRuleSchedule${normalizedScheduleName}`.slice(0, 255);
  },
  ...
  getLambdaSchedulePermissionLogicalId(functionName, scheduleId) {
    const normalizedFunctionName = this.getNormalizedFunctionName(functionName);
    const normalizedScheduleName = this.normalizeNameToAlphaNumericOnly(`${scheduleId}`);
    return `${normalizedFunctionName}LambdaPermissionEventsRuleSchedule${normalizedScheduleName}`.slice(
      0,
      255
    );
  },

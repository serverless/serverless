...
module.exports = {
  ...
  getScheduleLogicalId(functionName, scheduleId) {
    return `${this.getNormalizedFunctionName(functionName)}EventsRuleSchedule${`${scheduleId}`
      .replace(/-/g, 'Dash')
      .replace(/_/g, 'Underscore')}`.slice(0, 255);
  },
  ...
  getLambdaSchedulePermissionLogicalId(functionName, scheduleId) {
    return `${this.getNormalizedFunctionName(
      functionName
    )}LambdaPermissionEventsRuleSchedule${`${scheduleId}`
      .replace(/-/g, 'Dash')
      .replace(/_/g, 'Underscore')}`.slice(0, 255);
  },

...
module.exports = {
  ...
  camelCaseNormalizationIfNeeded(name) {
    if (_.isObject(this.provider) && this.provider.camelCaseNormalization) {
      // split up by underscores/dashes, convert each segment to make sure first letter is capitalized for camel casing
      // significantly reduces the length of autogenerated names
      let upperFirstCalled = false;
      return name
        .replace(/[_\\./]/, '-')
        .split('-')
        .map((str) => {
          if (!upperFirstCalled) {
            upperFirstCalled = true;
            return str;
          }
  ...
  normalizePathPart(rawPath) {
    if (_.isObject(this.provider) && this.provider.camelCaseNormalization) {
      return _.upperFirst(
        this.camelCaseNormalizationIfNeeded(rawPath)
          .replace(/-/g, 'Dash')
          .replace(/\{(.*)\}/g, '$1Var')
          .replace(/[^0-9A-Za-z]/g, '')
      );
  ...
  getNormalizedWebsocketsRouteKey(route) {
    return this.camelCaseNormalizationIfNeeded(route)
  ...
  getNormalizedResourceName(resourceName) {
    return this.normalizeName(
      this.camelCaseNormalizationIfNeeded(resourceName)
        .replace(/-/g, 'Dash')
        .replace(/_/g, 'Underscore')
    );
  },

...
module.exports = {
  ...
  getStackName() {
    ...
    return this.provider.serverless.service.getBaseName();
  },
  ...
  getServiceArtifactName() {
    // TODO 9604 do we need to use getBaseName() if that's used in the parent subdir / unique path already?
    ...
    return (
      customRoleName || {
        'Fn::Join': [
          '-',
          [
            this.provider.serverless.service.getShortServiceName(),
  ...
  getWebsocketsApiName() {
    ...
    return `${this.provider.getStage()}-${this.provider.serverless.service.getShortServiceName()}-websockets`;
  },
  ...
  getApiGatewayName() {
    ...
    return _.get(this.provider.serverless.service.provider.apiGateway, 'shouldStartNameWithService')
      ? `${this.provider.serverless.service.getShortServiceName()}-${this.provider.getStage()}`
      : `${this.provider.getStage()}-${this.provider.serverless.service.getShortServiceName()}`;
  },
  ...
  getStreamConsumerName(functionName, streamName) {
    ...
    if (consumerNamingMode) {
      const serviceName = this.provider.serverless.service.getShortServiceName();
  ...
  getAlbTargetGroupNameTagValue(functionName, albId, multiValueHeaders) {
    return `${this.provider.serverless.service.getShortServiceName()}-${functionName}-${albId}-${
  ...
  getEcrRepositoryName() {
    ...
    return `serverless-${this.provider.serverless.service.getBaseName(stage)}`
      .toLowerCase()
      .replace(/-+/g, '-')
      .replace(/-$/, '');
  },

...
describe('#naming()', () => {
  ...
  describe('#getStageLogicalId()', () => {
    it('should return the API Gateway stage logical id', () => {
      expect(sdk.naming.getStageLogicalId('dev')).to.equal('ApiGatewayStageDev');
    });

    it('should normalize - and _', () => {
      expect(sdk.naming.getStageLogicalId('dev_foo-bar'))
        .to.equal('ApiGatewayStageDevUnderscorefooDashbar');
    });
  });

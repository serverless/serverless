...
describe('#naming()', () => {
  ...
  describe('#getLambdaCloudWatchLogPermissionLogicalId()', () => {
    it('should return constant since it is shared by all subscription filter',
      () => {
        expect(sdk.naming.getLambdaCloudWatchLogPermissionLogicalId())
          .to.equal('LambdaPermissionLogsSubscriptionFilterCloudWatchLog');
      });
  });

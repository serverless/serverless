...
describe('#naming()', () => {
  ...
  describe('#getStackName()', () => {
    it('should use the service name and stage from the service and config as default', () => {
      serverless.service.service = 'myService';
      expect(sdk.naming.getStackName()).to.equal(`${serverless.service.service}-${options.stage}`);
    });

    it('should use the provider options to supply a custom stackName', () => {
      serverless.service.service = 'myService';
      options.stackName = 'custom-stack';
      expect(sdk.naming.getStackName()).to.equal(options.stackName);
    });

    it('should use serverless files provider property to supply a custom stackName', () => {
      serverless.service.service = 'myService';
      serverless.service.provider.stackName = 'custom-stack';
      expect(sdk.naming.getStackName()).to.equal(serverless.service.provider.stackName);
    });
  });
  ...
  describe('#getRoleName()', () => {
    it('uses the service name, stage, and region to generate a role name', () => {
      serverless.service.service = 'myService';
      const stackName = `${serverless.service.service}-${options.stage}`;
  ...
  describe('#getPolicyLogicalId()', () => {
    it('should return the expected policy name (IamPolicyLambdaExecution)', () => {
      expect(sdk.naming.getPolicyLogicalId()).to.equal('IamPolicyLambdaExecution');
    });
  });

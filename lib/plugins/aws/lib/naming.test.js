...
describe('#naming()', () => {
  ...
  describe('#getCloudFrontOriginId()', () => {
    it('should return CloudFront origin id for s3 origin', () => {
      expect(sdk.naming.getCloudFrontOriginId(true, 'domain.name', '/path')).to.equal(
        's3/domain.name/path'
      );
    });

    it('should return CloudFront origin id for custom origin', () => {
      expect(sdk.naming.getCloudFrontOriginId(false, 'domain.name', '/path')).to.equal(
        'custom/domain.name/path'
      );
    });

    it('should return CloudFront origin id from domain as a object', () => {
      expect(sdk.naming.getCloudFrontOriginId(false, {
        'Fn::Join': ['', [
          'Ref: ApiGatewayRestApi',
          '.execute-api.'
        ]]
       })).to.equal(
        'custom/FnJoinRefApiGatewayRestApiexecuteapi'
      );
    });
  });
});

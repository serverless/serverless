...
describe('updateStack', () => {
  ...
  describe('#update()', () => {
    ...
    beforeEach(() => {
      ...
      monitorStackStub = sinon.stub(awsDeploy, 'monitorStack').resolves();
    });
    ...
    it('should not monitor async deployments', () => {
      awsDeploy.serverless.service.provider.asyncStackOperation = true;

      return awsDeploy.update().then(() => {
        expect(updateStackStub.calledOnce).to.equal(true);
        expect(monitorStackStub.args[0]).to.deep.equal(['update', undefined, { async: true }]);
      });
    });
  });

...
describe('updateStack', () => {
  ...
  beforeEach(() => {
    ...
    // Silence logging to not produce noise in tests output
  ...
  describe('#update()', () => {
    ...
    it('should fail if updateStack failed', () => {
      awsDeploy.provider.request.restore();

      const stubError = new Error('stub-test');
      sinon.stub(awsDeploy.provider, 'request').returns(BbPromise.reject(stubError));

      return awsDeploy.update()
        .then(() => expect.fail(0, 1, 'Promise should be rejected'))
        .catch(e => {
          expect(e).to.be.equal(stubError);
        });
    });

    it('should fail if monitorStack failed', () => {
      awsDeploy.monitorStack.restore();

      const stubError = new Error('AWS is down');
      sinon.stub(awsDeploy, 'monitorStack').returns(BbPromise.reject(stubError));
    ...
    it('should succeed if monitorStack failed because no changes to stack happened', () => {
      awsDeploy.monitorStack.restore();

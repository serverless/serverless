...
module.exports = {
  async resolveConsoleUrl() {
    ...
    if (!org) return false;
    // FIXME 9604 do we need to update the globalNamespace to use service.getShortServiceName() instead of service.service?
  ...
  async displayServiceInfo() {
    if (this.serverless.processedInput.commands.join(' ') === 'info') {
      ...
      this.serverless.serviceOutputs.set('baseName', this.serverless.service.getBaseName());

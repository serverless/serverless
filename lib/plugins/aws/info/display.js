...
const { isVerboseMode, style } = utils;
...
export default {
  ...
  displayFunctions() {
    ...
    if (info.functions && info.functions.length > 0) {
      ...
      info.functions.forEach((f) => {
        outputSectionItems.push(
          `${f.name}: ${f.deployedName}${f.artifactSize ? style.aside(` (${filesize(f.artifactSize)})`) : ''
          }`
        );
      });
  ...
  displayStackOutputs() {
    this.serverless.stackOutputs = Object.fromEntries(
      this.gatheredData.outputs.map(({ OutputKey, OutputValue }) => [OutputKey, OutputValue])
    );

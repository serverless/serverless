...
class AwsInfo {
   ...
   */
  gather() {
    const stackName = this.sdk.naming.getStackName(this.options.stage);
    const info = {
    ...
    .then((result) => {
      ...
      if (result) {
        ...
        // Functions
        info.functions = [];
        outputs.filter(x => x.OutputKey.match(this.sdk.naming.getLogicalLambdaArnNameRegex()))
          .forEach(x => {
            const functionInfo = {};
            functionInfo.arn = x.OutputValue;
            functionInfo.name = this.sdk.naming.extractLambdaNameFromArn2(x.OutputValue);
            info.functions.push(functionInfo);
          });

        // Endpoints
        outputs.filter(x => x.OutputKey.match(this.sdk.naming.getServiceEndpointRegex()))
          .forEach(x => {

  ...
  gather() {
    ...
    .then((result) => {
      ...
      if (result) {
        ...
        // Functions
        info.functions = [];
        outputs.filter(x => x.OutputKey.match(this.provider.naming.getLogicalLambdaArnNameRegex()))
          .forEach(x => {
            const functionInfo = {};
            functionInfo.arn = x.OutputValue;
            functionInfo.name = this.provider.naming.extractLambdaNameFromArn2(x.OutputValue);
            info.functions.push(functionInfo);
          });

        // Endpoints
        outputs.filter(x => x.OutputKey.match(this.provider.naming.getServiceEndpointRegex()))
          .forEach(x => {

...
class AwsInfo {
   ...
   */
  gather() {
    const stackName = naming.getStackName(this.options.stage);
    const info = {
    ...
    .then((result) => {
      if (result) {
        ...
        info.functions = [];
        info.apiKeys = [];
        outputs.filter(x => x.OutputKey.match(naming.getLogicalLambdaArnNameRegex()))
          .forEach(x => {
            const functionInfo = {};
            functionInfo.arn = x.OutputValue;
            functionInfo.name = naming.extractLambdaNameFromArn2(x.OutputValue);
            info.functions.push(functionInfo);
          });

        // Endpoints
        outputs.filter(x => x.OutputKey.match(naming.getServiceEndpointRegex()))
          .forEach(x => {
            info.endpoint = x.OutputValue;
          });

        // API Keys
        outputs.filter(x => x.OutputKey.match(naming.getLogicalApiGatewayApiKeyRegex()))
          .forEach(x => {
            const apiKeyInfo = {};

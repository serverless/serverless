...
module.exports = {
  getStackInfo() {
    ...
    if (httpApiId) {
      if (httpApiId['Fn::ImportValue']) {
        sdkRequests.push(
          resolveCfImportValue(this.provider, httpApiId['Fn::ImportValue'])
            .then(result => {
              return this.provider.request('ApiGatewayV2', 'getApi', { ApiId: result });
            })
            .then(result => {
        ...
        sdkRequests.push(
          this.provider
            .request('ApiGatewayV2', 'getApi', {
              ApiId: this.serverless.service.provider.httpApi.id,
            })
            .then(result => {
              if (result) stackData.externalHttpApiEndpoint = result.ApiEndpoint;
            })

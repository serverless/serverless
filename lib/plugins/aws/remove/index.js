    ...
    this.hooks = {
      'initialize': async () => {
        if (this.serverless.processedInput.commands.join(' ') === 'remove') {
          log.notice(
            `Removing ${this.serverless.service.getShortServiceName()} from stage ${this.serverless
              .getProvider('aws')
      ...
      'finalize': async () => {
        ...
        log.notice();
        log.notice.success(
          `Service ${this.serverless.service.getShortServiceName()} has been successfully removed ${style.aside(
            `(${Math.floor(

...
describe('AwsRemove', () => {
  ...
  describe('#constructor()', () => {
    ...
    it('should run promise chain in order and perform an async removal if requested', () => {
      serverless.service.provider.asyncStackOperation = true;
      ...
      return awsRemove.hooks['remove:remove']().then(() => {
        ...
        expect(removeStackStub.calledAfter(emptyS3BucketStub)).to.be.equal(true);
        expect(monitorStackStub.calledAfter(emptyS3BucketStub)).to.be.equal(true);
        expect(monitorStackStub.args[0]).to.deep.equal(['removal', undefined, { async: true }]);
      });
    });
  });
});

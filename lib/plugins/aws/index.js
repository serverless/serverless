...
class SDK {
  ...
  request(service, method, params, stage, region, profile) {
    const that = this;
    const credentials = that.getCredentials(stage, region, profile);
  ...
  getCredentials(stage, region, profileArgument) {
    ...
    } else if (process.env[`${prefix}_PROFILE`]) {
      profile = process.env[`${prefix}_PROFILE`];
    } else {
      profile = this.serverless.service.getVariables(stage).profile;
    }

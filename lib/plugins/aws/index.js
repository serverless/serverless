...
const debug = require('debug')('AWS SDK Wrapper');
...
class SDK {
  constructor(serverless) {
    ...
    if (timeout) {
      debug(`Using AWS timeout: ${timeout}`);
      AWS.config.httpOptions.timeout = parseInt(timeout, 10);
    }
  }

  request(service, method, params, stage, region) {
    debug(`Making an AWS request: Service: ${service}, Method: ${method}, Params: ${
      util.inspect(params, false, null)}, Stage: ${stage}, Region: ${region}`);
    const that = this;
      ...
      const doCall = () => {
        f()
          ...
          .catch((e) => {
            if (e.statusCode === 429) {
              // TODO: DEBUG
              debug('Too many AWS requests. Sleeping for 5 seconds...');
  ...
  getCredentials(stage, region) {
    ...
    debug(`Using AWS Profile: ${credentials.profile}`);

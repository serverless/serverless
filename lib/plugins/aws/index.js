...
class SDK {
    ...
    return BbPromise.resolve(credentials)
      ...
      .then(() => {
        ...
        if (!credentials.accessKeyId || !credentials.secretAccessKey) {
          throw new this.serverless.Error('Cant find AWS credentials');
        }
        return credentials;
      });
  }
  ...
  getConfigDir() {
    ...
    if (!home) {
      throw new this.serverless.Error('Cant find homedir');
    }
  ...
  getProfile(awsProfile) {
    ...
    if (!profileConfig) {
      throw new this.serverless.Error(`Cant find profile ${awsProfile} in AWS credential file and/or AWS config file`);
    }
    return this.canonicalizeProfileCredentials();
  }

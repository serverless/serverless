...
describe('AwsDeployFunction', () => {
  ...
  describe('#checkIfFunctionExists()', () => {
    ...
    it('it should throw error if function doesn\'t exist in AWS', () => {
      const providerError = new ServerlessError('Function not found: arn:aws:something', 404);
      sinon.stub(awsDeployFunction.provider, 'request').returns(BbPromise.reject(providerError));

      return awsDeployFunction.checkIfFunctionExists()
        .then(() => expect.fail(0, 1, 'Expected rejection due to the non-deployed function'))
        .catch((e) => {
          expect(e).to.be.an.instanceof(ServerlessError);
          expect(e).not.to.be.equal(providerError);
          expect(e.message).contains('first'); // function name
          expect(e.message).contains('not yet deployed');
        });
    });
  });

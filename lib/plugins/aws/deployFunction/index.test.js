  ...
  describe('#deployFunction()', () => {
    ...
    describe('when artifact is provided', () => {
      ...
      beforeEach(() => {
        ...
        fsStateStub = sinon.stub(fs, 'statSync').returns({ size: 2048 });
      ...
      it('should read the provided artifact', () => awsDeployFunction.deployFunction().then(() => {
        const expectedFile = 'foo.zip';
        const expectedFunction = 'first';
        expect(fsReadFileStub).to.have.been.calledWithExactly(expectedFile);
        expect(fsStateStub).to.have.been.calledWithExactly(expectedFile);
        expect(getFunctionStub).to.have.been.calledWithExactly(expectedFunction);
      }));
    });
  });
});

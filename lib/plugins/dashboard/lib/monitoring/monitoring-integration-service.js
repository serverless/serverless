...
const MonitoringIntegrationService = async (serverless) => {
  ...
  const configureIntegrationContext = async ({
    ...
    const awsAccountId = awsCredentials.accountId;

    const { integrations } = await apiRequest(`/api/integrations/?orgId=${orgId}`, {
  ...
  const ensureIntegrationIsConfigured = async () => {
    ...
    await apiRequest(`/api/integrations/aws/${context.org.orgId}/pre/integration`, {
      urlName: 'integrationsBackend',
      accessKey: context.accessKey,
      authMethod: 'dashboard',
      method: 'POST',
  ...
  const instrumentService = async () => {
    ...
    if (Object.keys(context).length === 0 || !context.integration) {
      ...
      if (integration.status === 'failed') {
        log.error(
          `Your AWS account failed to integrate with Serverless Framework Observability.${integration.lastError ? `\n ${integration.lastError}` : ''
          }`
        );
        integrationSetupProgress.remove();
        return;

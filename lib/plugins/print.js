...
class Print {
  ...
  async print() {
    ...
    const svc = await getServerlessConfigFile(this.serverless);
    const service = svc;
    this.adorn(service);
    ...
    if (this.options.path) {
      ...
      for (const step of steps) {
        ...
        if (!conf) {
          throw new this.serverless.classes.Error(`Path "${this.options.path}" not found`);
        }
      }
    }
    ...
    if (this.options.transform) {
      if (this.options.transform === 'keys') {
        ...
        throw new this.serverless.classes.Error('Transform can only be "keys"');
      }
    }
    ...
    if (format === 'text') {
      if (Array.isArray(conf)) {
        ...
        if (_.isObject(conf)) {
          throw new this.serverless.classes.Error('Cannot print an object as "text"');
        }
      ...
      throw new this.serverless.classes.Error('Format must be "yaml", "json" or "text"');
    }

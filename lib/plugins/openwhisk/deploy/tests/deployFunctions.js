...
const Actions = require('openwhisk/lib/actions');
...
describe('deployFunctions', () => {
  ...
  beforeEach(() => {
    ...
    openwhiskDeploy.serverless.service.resources.openwhisk = {
      namespace: 'testing',
  ...
  describe('#deployFunctionHandler()', () => {
    it('should deploy function handler to openwhisk', () => {
      sandbox.stub(openwhiskDeploy, 'openWhiskClientFactory', () => {
        const create = params => {
        ...
        return { actions: {create: create} };
      });
      return expect(openwhiskDeploy.deployFunctionHandler(mockFunctionObject)).to.eventually.be.resolved;
    });

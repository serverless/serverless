  ...
  validate() {
    ...
    if (this.options.path) {
      ...
      if (this.options.data instanceof Buffer) {
        throw new this.serverless.classes.Error('The file path provided must point to a .json file.');
      }
    }
    ...
    Object.keys(CmdLineParamsOptions).forEach(key => {
      if (!this.options[key]) {
        ...
        throw new this.serverless.classes.Error(
          `Invalid ${key} parameter value, must be either ${CmdLineParamsOptions[key].join(' or ')}.`
        );
      }
    })
  }

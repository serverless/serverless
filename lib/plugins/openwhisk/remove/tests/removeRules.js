...
describe('OpenWhiskRemoveRules', () => {
  ...
  describe('#removeRules()', () => {
    it('should call removeRule for each rule', () => {
      ...
      const functions = { first: { events: { mockRuleObject } }, second: { events: { mockRuleObject } }};
      sandbox.stub(openwhiskRemove.serverless.service, 'getAllFunctions', () => Object.keys(functions));
      ...
      return openwhiskRemove.removeRules().then(() => {
        ...
        expect(stub.calledWith('mockRuleObject')).to.be.equal(true);
      });
    });
  });

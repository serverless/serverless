...
describe('#getWskProps()', () => {
  ...
  it('should instantiate openwhisk resources from properties file', () => {
    const mockObject = {
      apihost: 'https://openwhisk.ng.bluemix.net/api/v1/', auth: 'user:pass', namespace: 'blah@provider.com_dev'
    };

    const home = process.env[(process.platform == 'win32') ? 'USERPROFILE' : 'HOME'];
    const wskProps = 'APIHOST=openwhisk.ng.bluemix.net\nNAMESPACE=blah@provider.com_dev\nAUTH=user:pass\n'
  ...
  it('should instantiate openwhisk resources from environment variables', () => {
    const mockObject = {
      apihost: 'https://blah.blah.com/api/v1', auth: 'another_user:another_pass', namespace: 'user@user.com'
    };

...
class OpenWhiskCompileFunctions {
  ...
  compileRules(functionName, nameSpace, rules) {
    return Object.keys(rules).reduce((prev, rule) => {
      prev[rule] = { // eslint-disable-line no-param-reassign
        ruleName: rule,
        action: functionName,
        namespace: nameSpace,
        trigger: rules[rule],
  ...
  compileFunction(functionName, functionObject) {
    return this.readFunctionSource(functionObject.handler).then(code => {
      ...
      const MemorySize = functionObject.memory
        || this.serverless.service.provider.memory
        || 256;
      const Timeout = functionObject.timeout
        || this.serverless.service.provider.timeout
        || 60;

  ...
  compileFunction(functionName, functionObject) {
    return this.readFunctionSource(functionObject.handler).then(code => {
      ...
      const NameSpace = functionObject.namespace
        || `${this.serverless.service.resources.openwhisk.namespace}_${this.options.stage}`;
      ...
      const Parameters = Object.keys(functionObject.parameters || {})
        .map(key => ({key: key, value: functionObject.parameters[key]}))
 
      const newFunction = {
        name: FunctionName,
        nameSpace: NameSpace,
        action: {
          exec: { kind: Runtime, code: code}, 

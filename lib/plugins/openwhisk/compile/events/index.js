...
class OpenWhiskCompileTriggers {
  ...
  setup() {
    this.serverless.service.resources.openwhisk.triggers = {};
  }
  ...
  compileTriggers() {
    ...
    const manifestResources = this.serverless.resources;
    const owResources = this.serverless.service.resources.openwhisk;

    if (!owResources || !owResources.triggers) {
    ...
    if (manifestResources && manifestResources.triggers) {
      Object.keys(manifestResources.triggers).forEach(trigger => {
        owResources.triggers[trigger] = this.compileTrigger(trigger, manifestResources.triggers[trigger]);
      });
     }

...
class Axiom {
  constructor(serverless, options) {
    ...
    this.hooks = {
      'after:remove:remove': this.unsubscribeLogGroups.bind(this),
    }
    if (
  ...
  async subscribeLogGroups() {
    ...
    if (initializeResult.ingester.stackId) {
      await this.monitorStack(initializeResult.ingester.action, {
        StackId: initializeResult.ingester.stackId,
      })
    }
    ...
    await this.monitorStack(subscribeResult.ingester.action, {
      StackId: subscribeResult.ingester.stackId,
    })
  }
  ...
  async unsubscribeLogGroups() {
    ...
    if (stackId) {
      this.monitorStack(action, { StackId: stackId })
    }
  }

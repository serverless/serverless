...
class Invoke {
  ...
  loadEnvVarsForLocal() {
    ...
    const globalEnv = this.serverless.service.provider.environment;
...
=======
      IS_LOCAL: 'true',
      ...globalEnv
>>>>>>> 75f135ecf41402d25c478e7ea7ec043b2f2f39fc
    };

    if(this.serverless.service.functions && this.serverless.service.functions[this.options.function] &&
      this.serverless.service.functions[this.options.function].environment) {
        const functionEnv = this.serverless.service.functions[this.options.function].environment;
        for(let i in functionEnv) {
          defaultEnvVars[i] = functionEnv[i];
        }
    }

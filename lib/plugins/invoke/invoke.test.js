...
describe('Invoke', () => {
  ...
  beforeEach(() => {
    ...
    serverless.service.provider.environment = {
      globalVariable: 'test value',
      globalOverride: 'original',
    };
    serverless.service.functions = {
      test: {
        hander: 'test.handler',
        environment: {
          functionVariable: 'test value',
          globalOverride: 'function value',
        },
      },
    };
    invoke = new Invoke(serverless, { function: 'test' });
  });
  ...
  describe('hooks', () => {
    describe('invoke:local:loadEnvVars', () => {
      ...
      it('should set Function Environment', () =>
        expect(invoke.hooks['invoke:local:loadEnvVars']()).to.be.fulfilled.then(() => {
          expect(process.env.functionVariable).to.equal('test value');
          expect(process.env.globalOverride).to.equal('function value');
        }));

      it('should set Global Environment', () =>
        expect(invoke.hooks['invoke:local:loadEnvVars']()).to.be.fulfilled.then(() =>
          expect(process.env.globalVariable).to.equal('test value')
        ));

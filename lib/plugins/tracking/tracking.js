...
class Tracking {
  ...
  toggleTracking() {
    ...
    if (this.options.disable && !this.options.enable) {
      const that = this;
      this.serverless.utils.writeFile(path.join(serverlessPath, trackingFileName),
        'Keep this file to disable tracking').then(() => {
          that.serverless.cli.log('Tracking successfully disabled');
        }).catch((e) => {
          throw new that.serverless.classes.Error(`Tracking Failed to disable.The following message:
  ${e.message}`);
        });
    }
  }

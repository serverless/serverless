...
describe('AwsDeploy', () => {
  describe('#constructor()', () => {
    ...
    it('should run promise chain in order', () => {
      ...
      const compileFunctionsStub = sinon
        .stub(awsDeploy, 'compileFunctions').returns(BbPromise.resolve());
      ...
      return awsDeploy.hooks['deploy:deploy']().then(() => {
        ...
        expect(compileFunctionsStub.calledAfter(deployFunctionsStub)).to.be.equal(true);
        expect(updateStackStub.calledAfter(compileFunctionsStub)).to.be.equal(true);


        awsDeploy.validateInput.restore();
        awsDeploy.createStack.restore();
        awsDeploy.deployFunctions.restore();
        awsDeploy.compileFunctions.restore();

...
class Install {
  ...
  install() {
    return downloadTemplateFromRepo(this.options.name, this.options.url)
      .then(dirName => {
        const message = `Successfully installed "${this.options.name}" ${this.options.name !== dirName ? `as "${dirName}"` : ''}`;

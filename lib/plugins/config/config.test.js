...
describe('Config', () => {
  ...
  describe('#constructor()', () => {
    ...
    it('should have the lifecycle event "credentials" for the "credentials" sub-command', () => {
    ...
    it('should have a "before:config:credentials:credentials" hook', () => {
      expect(config.hooks['before:config:credentials:credentials']).to.not.equal(undefined);
    });

    it('should run promise chain in order for "before:config:credentials:credentials" hook', () => {
      const configStub = sinon
        .stub(config, 'validate').returns(BbPromise.resolve());

      return config.hooks['before:config:credentials:credentials']().then(() => {

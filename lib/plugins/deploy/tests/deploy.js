    ...
    it('should update lambda', () => {
      deploy.deploy().then(() => {
        ...
        expect(configStub.args[0][0].Description)
          .to.be.equal(S.instances.service.functions.create.description);
        expect(configStub.args[0][0].Handler)
          .to.be.equal(S.instances.service.functions.create.handler);
        expect(configStub.args[0][0].MemorySize)
          .to.be.equal(S.instances.service.functions.create.memory_size);
        expect(configStub.args[0][0].Timeout)
          .to.be.equal(S.instances.service.functions.create.timeout);
        ...
        expect(configStub.args[1][0].Description)
          .to.be.equal(S.instances.service.functions.list.description);
        expect(configStub.args[1][0].Handler)
          .to.be.equal(S.instances.service.functions.list.handler);
        expect(configStub.args[1][0].MemorySize)
          .to.be.equal(S.instances.service.functions.list.memory_size);
        expect(configStub.args[1][0].Timeout)
          .to.be.equal(S.instances.service.functions.list.timeout);

...
class Plugin {
  ...
  install() {
    return BbPromise.bind(this)
      ...
      .then((plugins) => {
        ...
        if (plugin) {
          return this.pluginInstall(pluginName, pluginVersion).then(pluginInstalled => {
            if (pluginInstalled) {
              return BbPromise.bind(this)
              .then(this.addPluginToServerlessFile(pluginName))
              .then(this.installPeerDependencies(pluginName))
              .then(() => {
    ...
    return BbPromise.bind(this)
      ...
      .then(plugins => {
        ...
        if (plugin) {
          ...
          return BbPromise.bind(this)
          .then(this.uninstallPeerDependencies(pluginName))
          .then(this.pluginUninstall(pluginName))
          .then(pluginStillAvailable => {
            if (!pluginStillAvailable) {
              return this.removePluginFromServerlessFile(pluginName)
              .then(() => {
  ...
  pluginInstall(pluginName, pluginVersion) {
    const servicePath = this.serverless.config.servicePath;
  ...
  installPeerDependencies(pluginName) {
  ...
  uninstallPeerDependencies(pluginName) {
    const pluginPackageJsonFilePath = path.join(this.serverless.config.servicePath,
      'node_modules', pluginName, 'package.json');

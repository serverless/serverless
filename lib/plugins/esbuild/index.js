...
class Esbuild {
  ...
  async _initEsbuild() {
    ...
    const localEsbuildPath = path.join(
      this.serverless.config.serviceDir,
      '.serverless',
      'esbuild'
    );

      // const localEsbuildPath = path.join(os.tmpdir(), 'esbuild');
    await copyFile(esbuildPath, localEsbuildPath);
    await chmod(localEsbuildPath, 0o755);
    if (process.env.ESBUILD_BINARY_PATH && existsSync(process.env.ESBUILD_BINARY_PATH)) {
      console.log('Using already set BINARY_PATH', process.env.ESBUILD_BINARY_PATH)
    } else {
      process.env.ESBUILD_BINARY_PATH = localEsbuildPath;
    }

    this.esbuildPath = localEsbuildPath;
    ...
    await Promise.all(
      Object.entries(updatedFunctionsToBuild).map(([alias, functionObject]) => {
        return limit(async () => {
          ...
          //   if (buildProperties.sourcemap === true) {
          //     esbuildArgs.push('--sourcemap');
          //   } else {
          //     esbuildArgs.push(`--sourcemap=${buildProperties.sourcemap}`);
          //   }
          // }

          // if (buildProperties.bundle) {
          //   esbuildArgs.push('--bundle');
          // }

          // if (buildProperties.minify) {
          //   esbuildArgs.push('--minify');
          // }

          // for (const external of functionObject.esbuild.external) {
          //   esbuildArgs.push(`--external:${external}`);
          // }

          // const esbuildSpawn = spawn(this.esbuildPath, esbuildArgs);

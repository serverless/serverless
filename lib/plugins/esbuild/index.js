...
class Esbuild {
  ...
  async _externals(runtime) {
    ...
    if (packageJson.dependencies) {
      ...
      const finalExternal = external.filter(
        (ex) => !ex.includes('!') && !externalDefault.includes(ex),
      )
      const finalExclude = exclude.filter(
        (ex) => !ex.includes('!') && !excludeDefault.includes(ex),
      )

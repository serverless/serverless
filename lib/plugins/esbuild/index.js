...
class Esbuild {
  ...
  async _initEsbuild() {
    ...
    const esbuildHelp = execSync(`${localEsbuildPath} --help`).toString('utf-8')
    console.log('esbuildHelp ', esbuildHelp)
    ...
    await Promise.all(
      Object.entries(updatedFunctionsToBuild).map(([alias, functionObject]) => {
        return limit(async () => {
          const functionName = path.extname(functionObject[handlerPropertyName]).slice(1);
          const handlerPath = functionObject[handlerPropertyName].replace(`.${functionName}`, '');
          const esbuildArgs = [functionObject.handlerPath, '--platform=node', `--outfile=${path.join(
            this.serverless.config.serviceDir,
            '.serverless',
            'build',
            handlerPath + '.js'

...
class Esbuild {
  ...
  async _package(handlerPropertyName = 'handler') {
    ...
    const zipPromise = new Promise(async (resolve, reject) => {
      ...
      output.on('open', async () => {
        ...
        zip.directory(path.join(this.serverless.config.serviceDir, '.serverless', 'build', 'node_modules'), 'node_modules')
  ...
  /**
   * Take the package.json and add an updated version with no dev dependencies to the .serverless/build directory
   */

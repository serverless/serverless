...
class Esbuild {
  ...
  async _buildProperties() {
    ...
    // If the user expcliitly sets the type to module then we should set the output format to esm
    if (packageJson.type === 'module') {
      defaultConfig.format = `esm`

      // We will also need to make sure we include the package.json in the build
      // for AWS Lambda to know that it is an ESM module

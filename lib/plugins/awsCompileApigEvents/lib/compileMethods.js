  ...
  compileMethods() {
    forEach(this.serverless.service.functions, (functionObject, functionName) => {
      forEach(functionObject.events.aws.http_endpoints, (path, method) => {
        ...
        const convertedRegionName = this.serverless.utils.convertRegionName(this.options.region);
        const awsAccountNumber = this.serverless.service
          .getVariables(this.options.stage, convertedRegionName).iamRoleArnLambda
          .match(/(.*):(.*):(.*):(.*):(.*):role\/.*/)[5];

        const lambdaUri = `arn:aws:apigateway:${
          this.options.region}:lambda:path/2015-03-31/functions/arn:aws:lambda:${
          this.options.region}:${awsAccountNumber}:function:${functionName}/invocations`;

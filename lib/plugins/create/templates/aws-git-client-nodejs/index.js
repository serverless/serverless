...
function prepareRepo (params, callback) {
	async.series([
		(callback) => prepareGit(callback),
		(callback) => prepareSshKey(params.path, params.sshKeyFilename, params.sshKey, callback),
		(callback) => cloneRepo(params.path, params.repo, params.branch, callback),
	], function(error, results) {
		if (error) callback(null, {status: 'deploy() async.series failed', error});
		else callback(error, {status: 'Completed deployment', results});
	});
}

function prepareGit (callback) {
	if (process.env.LAMBDA_TASK_ROOT && !fs.existsSync('/tmp/git')) {
		require('lambda-git')()
			.then(function () {
				// if running on lambda wait until lambda-git is ready before proceeding

				// required by https://docs.aws.amazon.com/lambda/latest/dg/nodejs-create-deployment-pkg.html
				process.env.PATH = [
					'/tmp/git/usr/bin/',
					process.env.PATH,
				...
				callback(null, {status: 'lambda-git: loaded'});
			})
			.catch(function (error) {
				callback(new Error(`lambda-git: loading failed with error: ${error}`));
			});
	} else {
		callback(null, {status: 'lambda-git: already loaded'});
	}
}

...
describe('Create', () => {
  ...
  describe('#create()', () => {
    ...
    it('should throw error if the directory for the service is not writable', () => {
      fse.mkdirsSync(tmpDir);
      process.chdir(tmpDir);

      sinon.stub(fse, 'copySync').throws(new Error('oops, read-only directory'));

      create.options.template = 'aws-nodejs';
      expect(() => create.create())
        .throws(ServerlessError, 'required permissions to write to the directory');

      fse.copySync.restore();
    });

    it('should notify to set the service name', () => {
      fse.mkdirsSync(tmpDir);
      process.chdir(tmpDir);

      const spyLog = sinon.spy(create.serverless.cli, 'log');

      create.options.name = '';
      create.options.path = '';
      create.create();

      expect(spyLog.called).to.be.true; // eslint-disable-line no-unused-expressions
      expect(spyLog.lastCall.args[0])
        .contains('service')
        .and.contains('serverless.yml')
        .and.contains('service name');

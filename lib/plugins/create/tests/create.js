...
describe('Create', () => {
  ...
  describe('#prompt()', () => {
    it('should set options in context', () => create.prompt().then(() => {
      expect(create.options.noGreeting).to.be.equal(false);
      expect(create.options.name).to.be.equal('new-service');
      expect(create.options.stage).to.be.equal('dev');
      expect(create.options.region).to.be.equal('aws_useast1');
    }));

    it('should set servicePath based on service name', () => create.prompt()
      .then(() => expect(create.serverless.instances.config.servicePath)
  ...
  describe('#validate()', () => {
    it('it should resolve if name is valid', () => {
  ...
  describe('#scaffold()', () => {
    ...
    it('should generate users.js', () => {
      create.scaffold(fakeYaml, fakeJson).then(() => {
        expect(create.serverless.instances.utils.fileExistsSync(path.join(tmpDir, 'users.js')))
          .to.be.equal(true);
      });
    });

...
describe('Create', () => {
  ...
  describe('#create()', () => {
    ...
    it('should generate scaffolding for "aws-golang" template', () => {
      const tmpDir = path.join(os.tmpdir(), (new Date).getTime().toString());
      const cwd = process.cwd();
      fse.mkdirsSync(tmpDir);
      process.chdir(tmpDir);
      create.options.template = 'aws-golang';
      ...
      return create.create().then(() => {
        ...
        expect(create.serverless.utils.fileExistsSync(path.join(tmpDir, 'handler.js')))
          .to.be.equal(true);
        expect(create.serverless.utils.fileExistsSync(path.join(tmpDir, 'hello.go')))
          .to.be.equal(true);

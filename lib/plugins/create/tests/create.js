...
describe('Create', () => {
  ...
  describe('#createFunction()', () => {
    ...
    it('should add a function to the service (aws-nodejs)', () => {
      const cwd = process.cwd();
      fse.mkdirsSync(tmpDir);
      process.chdir(tmpDir);
      create.options.template = 'aws-nodejs';
      ...
      return create.create().then(() => {
        const serviceDir = path.join(tmpDir, create.options.path);
        delete create.options.template;
        delete create.options.name;
        create.options.function = 'testFunction';
        create.options.handler = 'testFunction/index.handler';

        return create.createFunction().then(() => create.serverless.yamlParser.parse(
            path.join(tmpDir, 'my-new-service', 'serverless.yml')
          ).then((obj) => {
            expect(create.serverless.utils.fileExistsSync(
              path.join(serviceDir, 'testFunction', 'index.js'))).to.be.equal(true);
            const functionFileContent = fse
              .readFileSync(path.join(serviceDir, 'testFunction', 'index.js')).toString();

            expect((/module.exports.handler/).test(functionFileContent)).to.equal(true);

            expect(typeof obj.functions.testFunction).to.equal('object');
            if (obj.functions.testFunction) {
              expect(obj.functions.testFunction.handler).to.equal('testFunction/index.handler');
            }
            process.chdir(cwd);
          })
        );
    ...
    it('detect existing handler files', (done) => {
      fse.mkdirsSync(tmpDir);
      process.chdir(tmpDir);
      create.options.template = 'aws-nodejs';
      create.options.name = 'my_service';
      ...
      create.create().then(() => {
        ...
        delete create.options.name;
        create.options.function = 'testFunction';
        create.options.handler = 'handler.testFunction';
        create.createFunction().then(() => {
          done('Should detect existing file');
        })
        .catch((error) => {
    ...
    it('should detect existing functions', (done) => {
      fse.mkdirsSync(tmpDir);
      process.chdir(tmpDir);
      create.options.template = 'aws-nodejs';
      create.options.name = 'my_service';
      ...
      create.create().then(() => {
        delete create.options.template;
        delete create.options.name;
        create.options.function = 'duplicate';
        create.options.handler = 'duplhandler.duplicate';
        create.createFunction().then(() => {
          create.options.handler = 'duplhandler2.duplicate';
          create.createFunction().then(() => {
            done('Should detect existing function');
          })
          .catch((error) => {
            expect(error).to.match(/already exists/);
            done();
    ...
    it('should give an error for unsupported runtimes (aws-scala-sbt)', (done) => {
      fse.mkdirsSync(tmpDir);
      process.chdir(tmpDir);
      create.options.template = 'aws-scala-sbt';
      create.options.name = 'my_service';
      ...
      create.create().then(() => {
        ...
        delete create.options.name;
        create.options.function = 'testFunction';
        create.options.handler = 'testFunction.handler';

        create.createFunction().then(() => {
          done('Should give error for unsupported runtimes');
        })
        .catch(() => {
          done();
        });

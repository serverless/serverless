...
class Create {
  constructor(serverless, options) {
    ...
    this.hooks = {
      'create:create': () => BbPromise.bind(this)
        .then(this.create),
      'create:create:function': () => BbPromise.bind(this)
  ...
  createAWSNodeJSFuncFile(handlerPath) {
    let handlerInfo = path.parse(handlerPath);
    let handlerDir = path.join(this.serverless.config.servicePath, handlerInfo.dir);
    let handlerFile = handlerInfo.name + '.js';
    let handlerFunction = handlerInfo.ext.replace(/^\./, '');

    let templateFile = path.join(this.serverless.config.serverlessPath,
      'plugins', 'create', 'functionTemplates', 'aws-nodejs4.3', 'function.ejs');
    
    let templateText = fse.readFileSync(templateFile).toString();
    let jsFile = ejs.render(templateText, {
      handlerFunction
    });

    this.serverless.utils.writeFileDir(path.join(handlerDir, handlerFile));
  ...
  createFunction() {
    ...
    .then((config) => { 
      ...
      if (validFunctionRuntimes.indexOf(runtime) < 0) {
        ...
        this.serverless.cli.Error(errorMessage);
        throw new this.serverless.classes.Error(errorMessage);
      };

      let ymlEditor = require('yaml-edit')(serverlessYmlFileContent);
      let funcDoc = {};

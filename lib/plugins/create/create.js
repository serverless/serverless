...
class Create {
  ...
  scaffold(serverlessYamlParam) {
    ...
    // write handler.js
    fse.copySync(path.join(this.serverless.config.serverlessPath,
      'templates', this.options.provider, this.options.template, 'handler.js'), path.join(this.serverless
      .config.servicePath, 'handler.js'));

    let packageJson = this.serverless.utils.readFileSync(path.join(this.serverless.config.serverlessPath,

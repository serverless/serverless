...
class Create {
  ...
  create() {
    ...
    if ('template' in this.options) {
      ...
      return download.downloadTemplateFromRepo(this.options.name, this.options['template-url'])
        .then(dirName => {
          const message = [
            `Successfully installed "${this.options.name} `,
            `${this.options.name !== dirName ? `as "${dirName}"` : ''}`,
          ].join('');

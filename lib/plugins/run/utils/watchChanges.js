...
function watchChanges(service, servicePath, localEmulatorRootUrl) {
  ...
  _.each(service.functions, (functionConfig, functionName) => {
    ...
    const deps = getDependencies(path.join(servicePath, handlerPath, `${handlerFile}.js`));
    ...
    watch(
      ...
      (event, filename) => {
        ...
        logServerless([
          `Function '${service.service}-${functionName}' update triggered`,
          `by change on file ${fileLocation}`,
        ].join(' '));

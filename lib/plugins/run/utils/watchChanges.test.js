...
describe('watchChanges', () => {
  ...
  const handlerFunction = path.resolve(servicePath, 'function.js');
  ...
  it('creates watcher for function handler', () => {
    ...
    expect(watchStub.calledWith(expectedFiles)).to.be.equal(true);
  });

  it('creates watcher for function handler and its dependencies', () => {
    const deps = ['/path/to/deps/file1', '/path/to/deps/file2'];
  ...
  it('creates watcher for function handler and local dependencies only', () => {
    const localDep = '/path/to/deps/file1';
    const moduleDep = '/node_modules/module';

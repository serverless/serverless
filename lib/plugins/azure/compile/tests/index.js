  ...
  describe('#compileFunctions()', () => {
    ...
    function setupServerlessStubs (funcName, serverless, functionObject) {
      getAllFunctionsStub = sinon.stub(serverless.service, 'getAllFunctions', () => {
        return [funcName];
      })

      getFunctionStub = sinon.stub(serverless.service, 'getFunction', () => {
    ...
    it('should only compile functions if the functions map exists', () => {
      ...
      try {
        ...
        expect(error.message).to.equal('This plugin needs access to the Resources section of the Azure Resource Manager template');  
      }
    });

...
describe('#packageService()', () => {
  ...
  describe('#resolveFilePathsFromPatterns()', () => {
    const servicePath = 'lib/plugins/package/lib/__tests__/';

    it('should exclude all and include function/handler.js', () => {
      const params = {
        exclude: ['**'],
        include: ['src/function/handler.js'],
      };
      const expected = ['src/function/handler.js'];
      serverless.config.servicePath = servicePath;

      return expect(packagePlugin.resolveFilePathsFromPatterns(params))
        .to.be.fulfilled.then(actual => expect(actual).to.deep.equal(expected));
    });

    it('should include file specified with `!` in exclude params', () => {
      const params = {
        exclude: ['**','!src/utils/utils.js'],
        include: ['src/function/handler.js'],
      };
      const expected = ['src/function/handler.js','src/utils/utils.js'];
      serverless.config.servicePath = servicePath;

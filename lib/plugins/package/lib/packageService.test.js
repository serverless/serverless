...
describe('#packageService()', () => {
  ...
  describe('#packageService()', () => {
    ...
    it('should package single registry function individually', () => {
      serverless.service.functions = {
        'test-one': {
          name: 'test-one',
          package: {
            artifact: 'something/.serverless/functions/something',
          },
        },
        'test-two': {
          name: 'test-two',
      ...
      return packageService.packageService().then(() => {
        expect(packageFunctionStub.calledOnce).to.be.equal(true);
        expect(packageAllStub.calledOnce).to.be.equal(true);
        expect(serverless.service.functions['test-one'].package.individually).to.be.equal(true);
      });
    });
  });

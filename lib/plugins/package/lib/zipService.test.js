...
describe('#zipService()', () => {
  ...
  it('should use archiver when zip is not available', () => {
    const oldPath = process.env.PATH;
    const archiverCreateSpy = sinon.spy(archiver, 'create');
    const zipFileName = getTestArtifactFileName('archiver-fallback');

    process.env.PATH = '';

    return packageService.zipDirectory(servicePath, [], [], zipFileName).then(() => {
      expect(archiverCreateSpy.called).to.equal(true);
    }).finally(() => {
      process.env.PATH = oldPath;
      archiverCreateSpy.restore();
    });
  });
});

  ...
  packageService() {
    ...
    packagePromises = packagePromises.concat(_.map(allLayers, layerName => {
      ...
      if (layerObject.package.artifact) {
        const basename = path.basename(layerObject.package.artifact);
        fs.copySync(layerObject.package.artifact,path.join(this.serverless.config.servicePath, ".serverless", basename));
        layerObject.package.artifact=basename;
        return BbPromise.resolve();
      }

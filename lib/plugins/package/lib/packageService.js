...
module.exports = {
  ...
  async getExcludes(exclude, excludeLayers) {
    const configFilePath = await serverlessConfigFileUtils.getServerlessConfigFilePath(
      this.serverless
    );
  ...
  async resolveFilePathsAll() {
    return this.resolveFilePathsFromPatterns(
      await this.excludeDevDependencies({
        exclude: await this.getExcludes([], true),
  ...
  async resolveFilePathsFunction(functionName) {
    ...
    return this.resolveFilePathsFromPatterns(
      await this.excludeDevDependencies({
        exclude: await this.getExcludes(funcPackageConfig.exclude, true),
  ...
  async resolveFilePathsLayer(layerName) {
    ...
    return this.resolveFilePathsFromPatterns(
      await this.excludeDevDependencies({
        exclude: await this.getExcludes(layerPackageConfig.exclude, false),

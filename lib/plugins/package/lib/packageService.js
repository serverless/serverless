...
const getGoLangHandlers = service => Object.values(service.functions)
  .map(f => Object.assign({ runtime: service.provider.runtime }, f))
  .filter(f => f && f.runtime.startsWith('go'))
  .matp(f => f.handler);
...
module.exports = {
  ...
  packageAll() {
    ...
    const filesToChmodPlusX = getGoLangHandlers(this.serverless.service)

...
const MAX_CONCURRENT_PACKAGING = Number(process.env.SLS_MAX_CONCURRENT_PACKAGING) || 8;
...
module.exports = {
  ...
  // maps an array over an interator function that returns an array of promises
  // with the number of concurrent iterations limited to `limit`
  _asyncPool(array, fn, limit) {
    const returnValues = [];
    let nextIndex = 0;
    const inProgress = [];
    ...
    const enqueue = () => {
      ...
      // grab the next item and iterate the counter
      const item = array[nextIndex++];
      let concurrencyPromise = Promise.resolve();

      // enqueue the current item and put its promise in the return values array
      const p = Promise.resolve().then(() => fn(item, array));
      returnValues.push(p);

      // if we have more items than the limit, prepare the queue
      if (limit <= array.length) {
        // add the current promise to the in progress queue and watch for it to finish,
        // removing it from the in progress queue when it does
        const e = p.then(() => inProgress.splice(inProgress.indexOf(e), 1));
        inProgress.push(e);

        // if we have hit max concurrency, use Promise.race to wait for the first in
        // progress operation to finish
        if (inProgress.length >= limit) {
          concurrencyPromise = Promise.race(inProgress);
        }
      }

      // Enqueue the next item in the array
      return concurrencyPromise.then(() => enqueue());
    };

    return enqueue().then(() => Promise.all(returnValues));
  },
  ...
  packageService() {
    ...
    const packageFunction = functionName => {
    ...
    return Promise.resolve()
      .then(() => {
        if (allFunctions.length > 0) {
          this.serverless.cli.log(
            `Packaging functions... (Up to ${MAX_CONCURRENT_PACKAGING} in parallel)`
          );
          return this._asyncPool(allFunctions, packageFunction, MAX_CONCURRENT_PACKAGING);
        }
      ...
      .then(() => {
        if (allLayers.length > 0) {
          this.serverless.cli.log(
            `Packaging layers... (Up to ${MAX_CONCURRENT_PACKAGING} in parallel)`
          );
          return this._asyncPool(allLayers, packageLayer, MAX_CONCURRENT_PACKAGING);
        }

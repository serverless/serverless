...
module.exports = {
  ...
  filePrefixForFunction(functionName) {
    ...
    if (runtime.startsWith('dotnet')) {
      // todo find bin folder for this assembly
      const result = 'DashboardApi/bin/Release/netcoreapp2.1/publish';
      this.serverless.cli.debugLog(`filePrefixForFunction ${functionName}:${result}`);
      return result;
    }

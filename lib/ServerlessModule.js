...
class ServerlessModule {
  ...
  load() {
    ...
    // Get templates
    if (SUtils.fileExistsSync(path.join(_this._fullPath, 's-templates.json'))) {
      this.templates = SUtils.readAndParseJsonSync(path.join(_this._fullPath, 's-templates.json'));
    }
  }

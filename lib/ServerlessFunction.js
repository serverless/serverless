 ...
 * - cPath is optional.  It specifies component path.  You can nest functions in any amounts of subfolders (these subfolders are the cPath)
...
class ServerlessFunction {
  ...
  constructor(Serverless, config) {
    ...
    _this.handler    = path.posix.join(
      _this._config.cPath ? _this._config.cPath.split('/').join(path.sep) : null,
  ...
  updateConfig(config) {
    ...
    // Check cPath separators
    if (config.cPath.indexOf('\\') !== -1) throw new SError('Cannot use "\\" in config.cPath');

    // Set sPath
    if (config.component || config.cPath || config.function) {

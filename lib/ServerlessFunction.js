...
class ServerlessFunction {
  ...
  constructor(Serverless, config) {
    ...
    _this.endpoints.push(new _this._S.classes.Endpoint(_this._S, {
      sPath:  _this._config.sPath +  '@' + _this._config.sPath + '~GET'
    }));
    _this.templates  = {};
  }
          ...
          fs.readFileAsync(path.join(this._S.config.serverlessPath, 'templates', 'nodejs', 'handler.js'))
            .then((template) => {
              let subFolderLevel = this._config.sPath.split('/').length - 1,
                  libPath        = _.repeat('../', subFolderLevel) + 'lib',
                  handlerJs      = _.template(template)({libPath: libPath});

              return SUtils.writeFile(path.join(this._config.fullPath, 'handler.js'), handlerJs)
            })
        )
      } else if (this.getRuntime() === 'python2.7') {
        writeDeferred.push(
          SUtils.writeFile(path.join(this._config.fullPath, 'handler.py'), fs.readFileSync(path.join(this._S.config.serverlessPath, 'templates', 'python2.7', 'handler.py')))
        )
      }

  ...
  try {
    ...
    try {
      ...
      try {
        ...
        return import(tsNodePath).then(() => tsNodePath).catch((err) => {
  ...
  switch (path.extname(configurationPath)) {
    ...
    case '.js': {
      const configurationEventuallyDeferred = (async () => {
        try {
          await import(configurationPath);
        } catch {
          throw new ServerlessError(
        ...
        try {
          ...
          throw new ServerlessError(
            `Cannot load "${path.basename(configurationPath)}": Initialization error: ${error && error.stack ? error.stack : error}`,
    ...
    case '.mjs': {
      import('path').then((path) => {
        import.meta.resolve(configurationPath)
          .then(() => {
            import(`../utils/import-esm.js`).then(async (importEsm) => {
              try {
                const module = await importEsm.default(configurationPath);

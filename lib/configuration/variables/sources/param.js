...
module.exports = ({ service = {}, serviceInstanceParamsFromPlatform = {} }) => {
  return {
    resolve: async ({ address, resolveConfigurationProperty, options }) => {
      ...
      const configParams = new Map(Object.entries(_.get(service, 'params') || {}));
      ...
      const result = {
        value: allParams[address] ? allParams[address].value : null,
      };
      ...
      if (allParams[address] == null) {
        result.eventualErrorMessage = `The param "${address}" cannot be resolved from CLI options or stage params${
          service.org && service.app ? ' or Serverless Dashboard' : ''

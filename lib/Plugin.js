...
class ServerlessPlugin {
  ...
  cliPromptSelectStage(message, stage, addLocalStage) {
    ...
    let stages = _this.S.getProject().getStages();
  ...
  cliPromptSelectRegion(message, addAllRegions, existing, region, stage) {
    ...
    if (stage && _this.S.getProject().getRegions(stage).length === 1 && existing) {
      return BbPromise.resolve(_this.S.getProject().getRegions(stage)[0]);
    }
    ...
    if (stage){
      ...
      if (existing) {
        ...
        regionChoices = [];
        _this.S.getProject().getRegions(stage).forEach(function(region) {
          regionChoices.push(region)
        });
      } else {

        // Make sure there are regions left in stage
        if (_this.S.getProject().getRegions(stage).length === 4) {
        ...
        _this.S.getProject().getRegions(stage).forEach(function(regionInStage) {

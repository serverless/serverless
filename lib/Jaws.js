...
class Jaws {
  ...
  validateProject() {
    ...
    if (!this._projectJson || !_this._projectJson.stage) {
    ...
    let stages = Object.keys(_this._projectJson.stage);
    ...
    let hasOneRegion = stages.some(function(stageName) {
      return !!_this._projectJson.stage[stageName][0].region;
    });
  ...
  _loadPlugins(relDir, pluginMetadata) {
    ...
    for (let pluginMetadatum of pluginMetadata) {
      ...
      JawsUtils.jawsDebug(PluginClass.getName() + ' plugin loaded');

...
class Jaws {
  ...
  constructor(config) {
    ...
    if (this._projectRootPath) {
      ...
      if (!this._awsAdminKeyId)     this._awsAdminKeyId     = AWSUtils.profilesGet(this._awsProfile)[this._awsProfile].aws_access_key_id;
      if (!this._awsAdminSecretKey) this._awsAdminSecretKey = AWSUtils.profilesGet(this._awsProfile)[this._awsProfile].aws_secret_access_key;
    }

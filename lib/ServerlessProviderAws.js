...
class ServerlessProviderAws {
  ...
  constructor(serverless, config) {
    ...
    // TODO: Use Proof Of Concept?

    let _this = this;
    let request = function(service, method, params, stage, region) {
      let awsService = new AWS[service](_this.getCredentials(stage, region));
      let req        = awsService[method](params);

      // We can add in listeners that fire global events

...
class ServerlessProviderAws {
  ...
  addEnvironmentCredentials(credentials, prefix) { // separate credential environment variable prefix from obtaining the credentials from the environment.
    ...
    if(envCreds) {
      credentials.awsAdminKeyId =      envCreds.accessKeyId     || credentials.awsAdminKeyId;
      credentials.awsAdminSecretKey =  envCreds.secretAccessKey || credentials.awsAdminSecretKey;
      if (envCreds.sessionToken) {
        credentials.awsAdminSessionToken = envCreds.sessionToken;
      }
    }
  }
  /**
   ...
   */
  addProfileCredentials(credentials, prefix) { // separate profile environment variable prefix from obtaining credentials from the profile.
    let profCreds = this.getProfile(prefix + '_PROFILE', true);
    if(profCreds) {
      credentials.awsAdminKeyId =     profCreds.aws_access_key_id     || credentials.awsAdminKeyId;
      credentials.awsAdminSecretKey = profCreds.aws_secret_access_key || credentials.awsAdminSecretKey;
      if (profCreds.aws_session_token || profCreds.aws_security_token) {
        //                                 node.js aws-sdk standard       python boto standard
        credentials.awsAdminSessionToken = profCreds.aws_session_token || profCreds.aws_security_token;
      }
    }
  }
  /**
  ...
  getCredentials(stage, region) {
    ...
    this.addEnvironmentCredentials(credentials, 'AWS' + stage);                  // and also override these with the Amazon standard *stage specific* credential environment variable prefix.
    this.addEnvironmentCredentials(credentials, 'SERVERLESS_ADMIN_AWS' + stage); // finally override all prior with Serverless prefixed *stage specific* credentials if these are also provided.
    // next from profile

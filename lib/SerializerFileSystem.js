...
class SerializerFileSystem {
  ...
  deserializeFunction(func) {
    ...
    return BbPromise.try(function() {
        ...
        funcDir = func._config.filePath.split(path.sep).slice(0, -1).join(path.sep);
  ...
  serializeFunction(func) {
    ...
    return BbPromise.try(function() {
        ...
        // Validate: Check function exists
        if (!SUtils.fileExistsSync(func._config.filePath)) {
          throw new SError('Function could not be saved because it does not exist in your project');
        }
      })
      .then(function() {
        return SUtils.writeFile(func._config.filePath,
          JSON.stringify(func.toJson({ deep: true }), null, 2));
      });
  }
}

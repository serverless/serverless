...
class SerializerFileSystem {
  ...
  serializeProject(project, options) {
    ...
    return BbPromise
      ...
      // Save Stages
      .then(function() {
        return BbPromise.map(project.getAllStages(), (stageName) => {
    ...
    return BbPromise.try(function() {
      ...
      .then(() => {
        ...
        if (SUtils.fileExistsSync(templatesFilePath)) {
          ...
          template.setParents([component.getProject().getTemplates()])
        ...
        if (SUtils.fileExistsSync(templatesFilePath)) {
          ...
          return template.load()
            .then(function(template) {
              ...
              let parents = getParentTemplates(func.getFilePath());
  ...
  serializeFunction(func) {
    ...
    return BbPromise.try(function() {
        ...
        return SUtils.writeFile(path.join(func.getRootPath(), 's-function.json'),
          JSON.stringify(func.toObject({ deep: true }), null, 2));
      })
      .then(function() {

        // Save Templates
        if (fse.ensureFileSync(path.join(func.getRootPath(), 's-templates.json'))) {
  ...
  deserializeTemplates(templates) {
    ...
    return BbPromise.try(function() {
      ...
      //   console.log('deserializeTemplates 2')
      ...
      //   console.log('deserializeTemplates 3')

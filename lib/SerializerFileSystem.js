...
class SerializerFileSystem {
  ...
  serializeComponent(component, options) {
    ...
    return new BbPromise.try(function () {
      ...
      .then(function () {
        ...
        return SUtils.writeFile(component.getFilePath(), JSON.stringify(obj, null, 2));
      })
  ...
  serializeFunction(func) {
    ...
    return BbPromise.try(function() {
        ...
        // Save Templates
        if (fse.ensureFileSync(func.getTemplates().getFilePath())) {
  ...
  return new BbPromise(function(resolve, reject) {
    return glob('**/' + fileName, { root: root }, function (err, files) {
      ...
      files = files.map(function(f) {
        return path.join(path.dirname(root), f);
      });
      resolve(files);
    });
  });

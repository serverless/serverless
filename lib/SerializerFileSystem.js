...
class SerializerFileSystem {
          ...
          .each(function (variableFile) {
            ...
            if (file === 'common') {
              ...
              if (file.length == 1) {
                ...
                // Load Stage
                let stage = new _this._S.classes.Stage(_this._S, _this._project);
                stage.name = file[0];
                return stage.load()
                  .then(function (instance) {
  ...
  deserializeComponent(component) {
    ...
    return BbPromise.try(function() {
        ...
        if (!SUtils.fileExistsSync(component.getProject().getFilePath(component.name))) {
        ...
        // Load Component
        component = _.merge(component, SUtils.readFileSync(component.getProject().getFilePath(component.name)));
      })
      .then(function() {

        // Load Functions
        return globber(componentDir, 's-function.json')
          .each(function(rootPath) {
            let func = new _this._S.classes.Function(_this._S, component, rootPath);
            return func.load()
              .then(function(instance) {
  ...
  deserializeFunction(func) {
    ...
    return BbPromise.try(function() {
        ...
        if (!SUtils.fileExistsSync(func.getFilePath())) {
          throw new SError('Function could not be loaded because it does not exist in your project');
        }

        // Set Data
        func.fromObject(SUtils.readFileSync(func.getFilePath()));
  ...
  deserializeResources(resources) {
    ...
    return BbPromise.try(function() {
        ...
        if (!SUtils.fileExistsSync(resources._config.filePath)) return;
  ...
  deserializeStage(stage) {
    ...
    return BbPromise.try(function() {
        ...
        return fs.readdirSync(_this.getFilePath('_meta', 'variables'));
      })
      .each(function(variableFile) {
        ...
        let region = new _this._S.classes.Region(_this._S);
        region.name = region.varsFilenameToRegionName(variableFile.replace(`s-variables-${stage.name}-`, ''));
        return region.load(function(instance) {

...
class Function extends SerializerFileSystem {
  ...
  toObject() {
    ...
    return SUtils.exportObject(clone);
  }
  ...
  setRuntime( runtimeName ) {
    let runtime = this._S.getRuntime(runtimeName);
  ...
  /**
   * Get the role that should be used for this function during deployment
   * @param options { stage: "[stage]", region: "[region]" } both stage and region are required in order to support variables.
   * @returns {*}
     */
  getDeployedRole(options) {
    let role = null;
    if (!options.stage || !options.region) {
      throw new SError(`Stage and region options are required`);
    } else if (this.customRole) {
      role = this.toObjectPopulated(options).customRole;
    } else {
      role = this.getProject().getVariablesObject(options.stage, options.region).iamRoleArnLambda;
    }
    return role;
  }
  ...
  getRuntime() {
    return this._S.getRuntime(this.runtime);
  }
  ...
  run() {
    return this.getRuntime().run(this);
  }

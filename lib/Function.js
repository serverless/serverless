...
class Function extends SerializerFileSystem {
  ...
  constructor(S, component, data, rootPath) {
    ...
    if (!S || !component || !data || !rootPath) throw new SError('Missing required parameters');
  ...
  toObjectPopulated(options) {
    ...
    // Populate
    return SUtils.populate(this.getProject(), this.getTemplates(), this.toObject(), options.stage, options.region);
  }
  ...
  fromObject(data) {
    ...
    if (data.templates) {
      this.templates = new this._S.classes.Templates(this._S, data.templates);
    }
  ...
  getTemplates() {
    const componentTemplates = this.getComponent().getTemplates(),
          ownTemplates       = this.templates.toObject();

    return _.merge({}, componentTemplates, ownTemplates);
  }

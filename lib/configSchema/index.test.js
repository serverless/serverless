  ...
  for (const someCase of cases) {
    ...
    it(`should ${passOrFail} validation for ${someCase.description}`, () => {
      ...
      const modulesCacheStub = {
        [require.resolve('../classes/CLI')]: CustomCLI,
      };
      ...
      return fixtures.extend('validation', someCase.mutation).then(fixturePath =>
        runServerless({
          ...
          if (someCase.isValid) {
            expect(messages).not.to.contain('Error: ');
          } else {
            expect(messages).to.contain('Error: ');
            expect(messages).to.contain(someCase.errorMessage);
          }
          return;
        })
      );

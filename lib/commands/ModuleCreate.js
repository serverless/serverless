...
const CMD = class ModuleCreate extends ProjectCmd {
      ...
      case 'nodejs':
        if (_this._module.pkgMgr == 'npm') {
          ...
          if (!utils.dirExistsSync(actionPath)) {
            ...
            actionTemplateJson.cloudFormation.apiGateway.Endpoint.Path           = _this._module.name + '/' + _this._module.action;
            actionTemplateJson.cloudFormation.apiGateway.Endpoint.Method         = 'GET';
            actionTemplateJson.cloudFormation.apiGateway.Endpoint.Type           = 'AWS';
            actionTemplateJson.cloudFormation.lambda.Function.Properties.Runtime = 'nodejs';
            actionTemplateJson.cloudFormation.lambda.Function.Properties.Handler = path.join(
              'aws_modules',
              _this._module.name,
              _this._module.action,
              'handler.handler');
  ...
  _createSkeleton() {
    ...
    // Fetch skeleton resources
    let templatesPath      = path.join(__dirname, '..', 'templates');
    let actionTemplateJson = utils.readAndParseJsonSync(path.join(templatesPath, 'lambda.awsm.json'));
    let modulePath         = path.join(
      _this._JAWS._projectRootPath,  //TOOD: make this CWD if not in a JAWS project
      'aws_modules',
      _this._module.name);
    ...
    if (['lambda', 'both'].indexOf(_this._module.modType) != -1) {
      ...
      actionTemplateJson.name                                              = 'l' + _this._module.name.charAt(0).toUpperCase() + _this._module.name.slice(1) + _this._module.action.charAt(0).toUpperCase() + _this._module.action.slice(1);
      actionTemplateJson.cloudFormation.lambda.Function.Properties.Runtime = _this._module.runtime;

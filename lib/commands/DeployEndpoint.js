...
const CMD = class DeployEndpoint extends ProjectCmd {
  constructor(JAWS, stage, region, allTagged) {
    super(JAWS);
    this._stage       = stage;
    this._allTagged   = allTagged;
    this.Jaws        = JAWS;
    this._prjJson     = JAWS._projectJson;
    this._prjRootPath = JAWS._projectRootPath;
    this._prjCreds    = JAWS._meta.credentials;

    if (region && stage) {
      this._regions = this.Jaws._projectJson.stages[this._stage].filter(function(r) {
        return (r.region == region);
      });
    } else if (stage) {
      this._regions = this.Jaws._projectJson.stages[this._stage];
    }
  }
  ...
  run() {
    ...
    return this.Jaws.validateProject()
        ...
        .then(function() {
          ...
          let CmdTag = new Tag(_this.Jaws, 'endpoint');
  ...
  _promptRegions() {
    ...
    let regions = this.Jaws._projectJson.stages[this._stage];

...
const CMD = class DeployResources extends ProjectCmd {
  ...
  constructor(JAWS, stage, region) {
    ...
    if (region && stage) {
      this._regions = this._JAWS._projectJson.stages[this._stage].filter(r => {
        return (r.region == region);
      });
    } else if (stage) {
      this._regions = this._JAWS._projectJson.stages[this._stage];
    }
  }
  ...
  run() {
    ...
    return this._JAWS.validateProject()
      ...
      .then(function(answer) {
        if (answer) {
          _this._regions = [JawsUtils.getProjRegionConfigForStage(
            _this._JAWS._projectJson,
            _this._stage,
            answer[0].value)];
        }
      })
  ...
  _promptRegions() {
    ...
    let regions = _this._JAWS._projectJson.stages[_this._stage];
...
const ResourceDeployer = class ResourceDeployer extends ProjectCmd {

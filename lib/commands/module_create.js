...
CMD.prototype._getJavaPackageName = function () {
  ...
  var packageName = domain.split('.').reverse().join('.') + '.' + _this._module.name;
  ...
  if (['lambda', 'both'].indexOf(_this._module.modType) != -1) {
    ...
    switch (_this._module.runtime) {
      ...
      case 'java8':
        ...
        writeFilesDeferred.push(
          _this._writeFromMustache(path.join(templatesPath, _this._module.runtime), 'Lambda.java', path.join(actionPath, 'src', 'main', 'java', 'com', 'test_prj', 'users', 'Lambda.java'), replace)
        );
        break;
      default:

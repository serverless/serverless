...
CMD.prototype._createMVNPackageMgrSkeleton = function () {
  ...
  // Fetch skeleton resources
  var templatesPath = path.join(__dirname, '..', 'templates');
  var packageName = _this._getJavaPackageName();
...
CMD.prototype._getJavaPackageName = function () {
  var _this = this;
  if (!_this._module.runtime === 'java8') {
    throw new JawsError('This runtime is not java "' + _this._module.runtime + '"', JawsError.errorCodes.UNKNOWN)
  }
  var domain = _this._JAWS._meta.projectJson.domain;
  var packageName = domain.split('.').reverse().join('.');
  ...
  if (['lambda', 'both'].indexOf(_this._module.modType) != -1) {
    ...
    switch (_this._module.runtime) {
      ...
      case 'java8':
        ...
        // Edit awsm.json
        actionTemplateJson.lambda.cloudFormation.Handler = packageName + '.' + _this._module.name + '.Lambda::' + _this._module.action;
        ...
        writeFilesDeferred.push(
          _this._writeFromMustache(path.join(templatesPath, _this._module.runtime), 'Lambda.java', path.join(actionPath, 'src', 'main', 'java', 'Lambda.java'), replace)
        );
        break;
      default:

 ...
 * @param template default 'nodejs'
 * @returns {*}
 */
module.exports.run = function(JAWS, name, action, runtime, pkgMgr, moduleType, template) {
  var command = new CMD(JAWS, name, action, runtime, pkgMgr, moduleType, template);
  return command.run();
};
 ...
 * @constructor
 */
function CMD(JAWS, name, action, runtime, pkgMgr, modType, template) {
    ...
    case 'nodejs':
      if (_this._module.pkgMgr == 'npm') {
        ...
        if (!utils.fileExistsSync(path.join(modulePath, 'package.json'))) {
          var packageJsonTemplate = utils.readAndParseJsonSync(path.join(templatesPath,  _this._module.template, 'package.json'));
        ...
        if (!utils.dirExistsSync(actionPath)) {
          ...
          var handlerJs = fs.readFileSync(path.join(templatesPath, _this._module.template, 'handler.js'));
          var indexJs = fs.readFileSync(path.join(templatesPath, _this._module.template, 'index.js'));

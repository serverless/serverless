...
function CMD(name, stage, s3Bucket, notificationEmail, region, profile, noExeCf) {
  ...
  // Defaults
  this._name = name ? name : 'jaws-new-' + shortid.generate().replace(/\W+/g, '').substring(0, 19);
  this._stage = stage ? stage.toLowerCase().replace(/\W+/g, '').substring(0, 15) : 'dev';
...
CMD.prototype._prompt = Promise.method(function() {
  ...
  _this._prompts.properties.s3Bucket = {
    ...
    default: 'jaws.' + _this._name.toLowerCase() + '.com',
  ...
  _this._prompts.properties.notificationEmail = {
    ...
    default: 'me@' + _this._name.toLowerCase() + '.com',
...
CMD.prototype._createProjectJson = Promise.method(function(cfOutputs) {
  ...
  fs.writeFileSync(path.join(_this.projectRootPath, 'jaws.json'),
      JSON.stringify(jawsJson, null, 2));

  JawsCLI.log('Your project "' + _this.name
  ...
  return _this._prompt()
      ...
      .then(function() {
        if (_this._noExeCf) {
          ...
          return _this._createProjectJson();
        } else {

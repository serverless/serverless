  ...
  return Promise.try(function() {
    ...
    if (!_this._allTagged) {
      ...
      .then(function() {
        ...
        // If no region, deploy to all regions in stage
        if (_this._region) {

          // Status
          JawsCli.log('Deploying endpoint(s) to API Gateway within the stage "'
              + _this._stage
              + '" and region "'
              + _this._region
              + '"...');
          ...
          // Status
          JawsCli.log('Deploying endpoint(s) to all regions in "' + _this._stage + '"...');
          return _this.prjJson.project.stage[_this._stage];
      ...
      .each(function(region) {
        ...
        return deployer.deploy()
            .then(function(url) {
              JawsCli.log('API Gateway endpoints for stage "'
                  + _this._stage
                  + '" successfully deployed to region "'
                  + region
                  + '". Access them @ '
                  + url);
            });
      ...
      .then(function() {
        // Untag
        return _this._allTagged ? CMDTag.tagAll(_this._JAWS, 'api', true) : CMDTag.tag('api', null, true);
      });
});

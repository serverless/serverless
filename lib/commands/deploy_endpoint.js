...
function CMD(JAWS, stage, regions, allTagged) {
  var _this = this;
  _this._stage = stage;
  _this._regions = regions.length ? regions : Object.keys(this._JAWS._meta.projectJson.stages[this._stage]);
...
function ApiDeployer(stage, region, prjRootPath, prjJson, prjCreds) {
  ...
  for (var i = 0; i < _this._prjJson.stages[_this._stage].length; i++) {
    if (_this._region === _this._prjJson.stages[_this._stage][i].region) {
      _this._regionJson = _this._prjJson.stages[_this._stage][i];
    }
  }
...
ApiDeployer.prototype._saveApiId = Promise.method(function() {
  ...
  for (var i = 0; i < _this._prjJson.stages[_this._stage].length; i++) {
    if (_this._prjJson.stages[_this._stage][i].region === _this._region) {

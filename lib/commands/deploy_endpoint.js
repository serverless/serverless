 ...
 */
// TODO: Need to use async.whilst for projects with > 100 lambdas
ApiDeployer.prototype._fetchDeployedLambdas = Promise.method(function() {

  var _this = this;

  return AWSUtils.cfDescribeStackResources(
      _this._JAWS._meta.profile,
      _this._regionJson.region,
      _this._stage + '-' + _this._JAWS._meta.projectJson.name + '-l')
      .then(function(lambdaCfResources) {
        if (lambdaCfResources.StackResources) {
          _this._lambdas =  lambdaCfResources.StackResources;
        }
      })
      .catch(function(error) {
          JawsCli.log('Warning: JAWS could not find a deployed Cloudformation template containing lambda functions.');
          console.log(error);
      });
});

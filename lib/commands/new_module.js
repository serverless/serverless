...
CMD.prototype._createSkeleton = Promise.method(function() {
  ...
  var actionTemplateJson = JSON.parse(fs.readFileSync(path.join(templatesPath, 'action.awsm.json')));
  ...
  if (!utils.dirExistsSync(path.join(modulePath, 'awsm.json'))) {
    var moduleTemplateJson = JSON.parse(fs.readFileSync(path.join(templatesPath, 'module.awsm.json')));

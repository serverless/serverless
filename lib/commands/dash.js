...
var JawsError         = require('../jaws-error'),
    ...
    Tag               = require('./Tag'),
    CMDdeployLambda   = require('./DeployLambda'),
    CMDdeployEndpoint = require('./DeployEndpoint');
...
CMD.prototype._prepareResources = Promise.method(function() {
  ...
  return JawsUtils.findAllAwsmJsons(path.join(_this._JAWS._projectRootPath, 'aws_modules'))
    .then(function(jsonPaths) {
      ...
      // Add Lambdas back in
      let modules = lambdas.concat(hybrids);
...
CMD.prototype._prepareSummary = Promise.method(function() {
  ...
  _this._summary = 'Dashboard for project "' + _this._JAWS._projectJson.name + '"' + os.EOL
  ...
  // Add Stage Data
  for (var stage in _this._JAWS._projectJson.stages) {

    _this._summary = _this._summary
      + chalk.white('       ' + stage + ' ');

    for (var i = 0; i < _this._JAWS._projectJson.stages[stage].length; i++) {
      _this._summary = _this._summary
        + chalk.grey(_this._JAWS._projectJson.stages[stage][i].region + ' ')
    }
...
CMD.prototype._promptStage = Promise.method(function() {
  ...
  let stages = Object.keys(_this._JAWS._projectJson.stages);
...
CMD.prototype._promptRegion = Promise.method(function() {
  ...
  let regions = _this._JAWS._projectJson.stages[_this._stage].map(function(s) {
    return s.region;
  });

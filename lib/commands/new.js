...
module.exports = function(JAWS) {
  ...
  JAWS.new = function() {
    ...
    var getOrSetAdminCreds = new Promise(function(resolve, reject) {
      ...
      //Get creds from ~/.aws/config if it exists
      if (fs.existsSync(JAWS._utils.getUserHome() + '/.aws/config')) {
        adminPrompts.push({
              type: 'input',
              name: 'awsCliProfile',
              message: 'What AWS CLI Profile should be used for your admin user?:',
              default: 'default',
            }
        );
        inquirer.prompt(prompts, function(answers) {
          ...
          var creds = JAWS._utils.getAwsAdminCreds(answers.awsCliProfile);
          project.awsAdminKeyId = creds.key;
          project.awsAdminSecretKey = creds.secret;
        });
      } else {
        resolve();
      }

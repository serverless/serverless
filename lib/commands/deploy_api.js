...
module.exports.deployApi = function(JAWS, stage, region, allTagged) {
  ...
  // Status
  JawsCli.log('Deploying endpoint(s)...');
  ...
  return (allTagged ? Promise.resolve() : tagCmd.tag('api', null, false))
      .then(function() {
        ...
        var stages = Object.keys(JAWS._meta.projectJson.project.stages);
        if (!stages.length) {
          throw new JawsError(
              'You have no stages in this project',
              JawsError.errorCodes.UNKNOWN);
        }
      ...
      .each(function(region) {
        ...
        return deployer.deploy()
            .then(function(url) {
              JawsCli.log('API Gateway endpoints successfully deployed @ ' + url);

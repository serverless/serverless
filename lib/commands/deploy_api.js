...
module.exports = function(JAWS) {
      ...
      return new Promise(function(resolve, reject) {
        ...
        async.eachSeries(state.lambdas, function(lambda, lambdaCb) {
          ...
          async.eachSeries(lambdaResources, function(lambdaResource, resourceCb) {
            ...
            // Add lambda resource to state.lambdaResources.  Tells APIG which resources must be deleted.
            if (state.lambdaResources.indexOf(lambdaResource) === -1) state.lambdaResources.push(lambdaResource);
          ...
          // TODO: May want to simply delete all API resources to begin with, then rebuild
   ...
   * @private
   */
  JAWS._dapiProcessMethods = function(state) {

 ...
 * Validate
 */
function _validate(stage, region) {

  // Check State Exists
  if (!JAWS._meta.projectJson.project.stages[stage]) {
    reject(new JawsError(
        'This stage doesn\'t exist.  Please generate the stage if you would like to deploy your api to it.',
        JawsError.errorCodes.UNKNOWN));
  } else {
    var stageData = JAWS._meta.projectJson.project.stages[state.stage];
  ...
  if (state.region) {
    for (var i = 0;i < JAWS._meta.projectJson.project.stages[state.stage];i++) {
      if (JAWS._meta.projectJson.project.stages[state.stage].region === state.region) {
        regionsData = [JAWS._meta.projectJson.project.stages[state.stage].region];
        regionsData[0].region = state.region;
      }
    }
    if (!regionData) {
      reject(new JawsError(
          'This region doesn\'t exist in this stage.',
          JawsError.errorCodes.UNKNOWN));
    }
  } else {
    regionsData = JAWS._meta.projectJson.project.stages[state.stage];
  }
 ...
 * @private
 */
function _processRegions(state) {

  // If No Region, Set All Regions.  Otherwise, Get Region
  if (!stage.region) {
    state.region = JAWS._meta.projectJson.project.stages[state.stage];
  }
  if (stage.region)
  ...
  module.exports = function(JAWS) {
      ...
      return new Promise(function(resolve, reject) {
        ...
        if (state.restApiId) {
          ...
          client.createRestApi({
            ...
            JAWS._meta.projectJson.project.regions[state.region].restApiId = response.id;
     ...
     * @returns {bluebird|exports|module.exports}
     */
    JAWS.deployApi = function(stage, region) {
      return new Promise(function(resolve, reject) {

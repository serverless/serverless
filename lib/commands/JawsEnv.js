...
let CMD = class JawsEnv extends ProjectCmd {
  ...
  _getEnvFiles() {
    ...
    if (_this._region != 'all' || _this._stage == 'local') {
      ...
      regionGets.push(_this.getEnvFileAsMap(_this._JAWS, _this._stage, _this._region).then(envVars => {
        return {regionName: _this._region, vars: envVars.map, raw: envVars.raw};
      }));
    } else {
      if (!this._JAWS._projectJson.stages[_this._stage]) {
        return Promise.reject(new JawsError('Invalid stage ' + _this._stage, JawsError.errorCodes.UNKNOWN));
      }

      this._JAWS._projectJson.stages[_this._stage].forEach(regionCfg => {
        regionGets.push(
  ...
  getEnvKey(key) {
    ...
    return this._getEnvFiles(_this._JAWS, stage, _this._region)
      .then(function(envMapsByRegion) {
        ...
        envMapsByRegion.forEach(mapForRegion => {
          let value = mapForRegion.vars[key] || chalk.red('NOT SET');

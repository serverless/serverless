...
const Deployer = class Deployer extends ProjectCmd {
    ...
    return AWSUtils.cfGetLambdasStackTemplate(_this._JAWS._meta.profile, _this._region, _this._stage, projName)
      ...
      .then(cfTemplateBody => {
        ...
        if (cfTemplateBody) {
          ...
          return JawsUtils.getAllLambdaNames(_this._JAWS._meta.projectRootPath)
            .then(allLambdaNames => {
...
var CMD = class DeployLambda extends ProjectCmd {
  ...
  _setRegions() {
    ...
    if (region) { //user specified specific region to deploy to
      this._regions = [JawsUtils.getProjRegionConfigForStage(projJson, stage, region)];
    } else {
      this._regions = projJson.stages[stage];
    }

    JawsUtils.jawsDebug('Deploying to regions:');
    JawsUtils.jawsDebug(this._regions);
    return Promise.resolve();
  }

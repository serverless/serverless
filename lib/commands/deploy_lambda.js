...
module.exports = function(JAWS) {
  ...
  JAWS.optimizeNodeJs = function(tmpDistDir, jawsFilePath) {
    var baseDir = path.join(tmpDistDir, 'back'),  //path.dirname(jawsFilePath),
    ...
    if (builder == 'systemjs') {
      return systemJsBundle(baseDir, entries, tmpDistDir, minify, mangle, exclude, ignore);
    } else if (builder == 'browserify') {
      return browserifyBundle(baseDir, entries, tmpDistDir, minify, mangle, exclude, ignore);
    } else {
      return Promise.reject(
  ...
  JAWS.packageNodeJs = function(tmpDistDir, lambdaJawsFilePath, stage) {
    ...
    if (jawsJson.lambda.package && jawsJson.lambda.package.optimize && jawsJson.lambda.package.optimize.builder) {
      return _this.optimizeNodeJs(tmpDistDir, lambdaJawsFilePath)
          .then(function(optimizedCodeBuffer) {
            ...
            //TODO: do we always prepend the dirname(s) of where the lambda jaws.json is onto the file name?
            //so we can have external modules (like native modules) or will the handlerFileName always include
            //dir names relative to back??
    ...
    } else {  //user chose  not to optimize
      return Promise.reject(new JawsError('Bypass optimize feat not yet implemented', JawsError.errorCodes.UNKNOWN));
    }
  };
  ...
  JAWS.bundleLambda = function(jawsFilePath, stage) {
    var _this = this,
        ...
        excludePaths = jawsJson.lambda.package.excludePaths || [];

    console.log('Packaging', fullLambdaName, 'in dist dir', tmpDistDir);

    fs.mkdirSync(tmpDistDir);

    //Copy back dir omitting excludePaths
    wrench.copyDirSyncRecursive(srcBackDir, targetBackDir, {
      exclude: function(name, prefix) {
        ...
        return excludePaths.some(function(sRegex) {
          var re = new RegExp(sRegex),
              relPath = (relPath.charAt(0) == path.delimiter) ? relPath.substr(1) : relPath,
    ...
    return AWSUtils.getEnvFile(
        _this._meta.profile,
        _this._meta.projectJson.envVarBucket.region,
        _this._meta.projectJson.envVarBucket.name,
        projName,
        stage
    )
        .then(function(s3ObjData) {
          fs.writeFileSync(path.join(targetBackDir, '.env'), s3ObjData.Body); //always add env file at root of back

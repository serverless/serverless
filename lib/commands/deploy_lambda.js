 ...
 * @param srcBackDir
 * @param targetBackDir
 * @param excludePaths list of regular expressions
 */
function copyBackDirToTmp(srcBackDir, targetBackDir, excludePaths) {
...
module.exports = function(JAWS) {
  ...
  JAWS.optimizeNodeJs = function(tmpDistDir, jawsFilePath) {
    ...
    var exclude = optimizeData.exclude || [],
        ...
        entries = [path.join(baseDir, path.join(path.dirname(jawsFilePath), handlerFileName + '.js'))],
        mangle = true;
  ...
  JAWS.bundleLambda = function(jawsFilePath, stage) {
    ...
    copyBackDirToTmp(srcBackDir, targetBackDir, excludePaths);

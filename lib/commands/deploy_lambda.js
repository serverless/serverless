...
function generateIncludePaths(tmpDistDir, includePaths) {
  ...
  includePaths.forEach(function(p) {
    try {
      var fullPath = path.resolve(path.join(tmpDistDir, 'back', p)),
      ...
      wrench
          ...
          .forEach(function(file) {
            ...
            if (fs.lstatSync(filePath).isFile()) {
              compressPaths.push({fileName: p, data: filePath});
            }
          });
    }
  });
...
function checkForDuplicateLambdaNames(lambdaJawsPaths) {
  var lambdaNameMap = {};
...
module.exports = function(JAWS) {
  ...
  JAWS.optimizeNodeJs = function(tmpDistDir, jawsFilePath) {
    ...
    //handlerEntryPt = path.dirname(jawsFilePath).split(path.sep + 'back' + path.sep)[1],
  ...
  JAWS.deployLambdas = function(stage, deployAllTagged, allAtOnce) {
    ...
    return utils.findAllLambdas(JAWS._meta.projectRootPath)
        .then(function(lambdaJawsPaths) {
          checkForDuplicateLambdaNames(lambdaJawsPaths);

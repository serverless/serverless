...
function _validateJawsProjAttrsForLambdaDeploy(JAWS, region, stage) {
  ...
  if (!regionObj.iamRoleArnLambda) {
    throw new JawsError(
        'iamRoleArnLambda stage ' + stage + ' in region ' + region + ' not setup in project jaws.json',
        JawsError.errorCodes.INVALID_PROJECT_JAWS
    );
  }
}
 ...
 * @private
 */
module.exports._deployLambasInAllRegions = function(JAWS, packagedLambdas, stage, allAtOnce, region) {
  var _this = this,
    ...
    async.each(regions, function(region, regionCB) {  //Loop over each region
          ...
          var q = async.queue(function(task, cb) {
            _this.createOrUpdateLambda(
                ...
                task.zipBuffer,
                task.fullLambdaName,
                utils.getProjRegionConfig(JAWS._meta.projectJson.project.stages[stage], region).iamRoleArnLambda
            )
                .then(function() {
        ...
        function(err) { //end of all regions, success or fail
          if (err) {
            ...
            // Status
            JawsCLI.log('Lambda(s) successfully deployed');
 ...
 * @private
 */
module.exports._makeLambdaPackages = function(JAWS, lambdaJawsPaths, stage) {
  var _this = this,
      deployableJawsFiles = _this.getDeployableLambdas(lambdaJawsPaths),
      builderQueue = [];

  deployableJawsFiles.forEach(function(jawsFile) {
    builderQueue.push(_this.bundleLambda(JAWS, jawsFile, stage));
  });
...
module.exports.deployLambdas = function(JAWS, stage, deployAllTagged, allAtOnce, region) {
  ...
  // Status
  JawsCLI.log('Deploying lambda(s)...');
  ...
  return utils.checkForDuplicateLambdaNames(JAWS._meta.projectRootPath)
      ...
      .then(function(lambdaJawsPaths) { // Step 1: make zips for each lambda tagged as deployable
        return _this._makeLambdaPackages(JAWS, lambdaJawsPaths, stage);
      })
      .then(function(packagedLambdas) { // Step 2: For each region, deploy all lambda packages
        return _this._deployLambasInAllRegions(JAWS, packagedLambdas, stage, allAtOnce, region);
      });
};

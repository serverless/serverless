 ...
 * @param JAWS
 * @param stage
 * @param region - Defaults to all regions
 * @param allTagged
 * @param allAtOnce
 * @returns {*}
 */

module.exports.run = function(JAWS, stage, region, allTagged, allAtOnce) {
  var command = new CMD(JAWS, stage, region, allTagged, allAtOnce);
  return command.run();
};
...
function CMD(JAWS, stage, regions, allTagged, allAtOnce) {
  this._JAWS = JAWS;
  this._stage = stage;
  this._regions = regions.length ? regions : Object.keys(_this._JAWS._meta.projectJson.project.stages[_this._stage]);
...
CMD.prototype.run = Promise.method(function() {
  ...
  return _this._promptStage()
      ...
      .each(function(region) {
        ...
        JawsCLI.log('Successfully deployed lambdas to regions');
...
Deployer.prototype.deploy = Promise.method(function() {
  ...
  return Promise.try(function() {})
      ...
      .then(function() {
        JawsCLI.log('Done deploying lambdas in ' + _this._region);
        console.log(arns);
      });
});
...
Deployer.prototype._createOrUpdateLambda = Promise.method(function(packagedLambda) {
  ...
  console.log(this._JAWS._meta.projectJson.project.stages[this._stage]);
  ...
  return lGetFunctionAsync({ FunctionName: packagedLambda.fullLambdaName })
      .then(function() {
        ...
        JawsCLI.log('AWS Lambda: "'
            + _this._stage + ' - '
            + _this._region
            + ' - '
            + packagedLambda.fullLambdaName
            + '" exists already, updating it...');
...
Packager.prototype._createDistFolder = Promise.method(function() {
  ...
  // Status
  JawsCLI.log('Packaging "' + _this._lambdaName + '"...');
  utils.logIfVerbose('Packaging ' +  _this._lambdaName + ' in dist dir ' + _this._distDir);

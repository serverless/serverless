...
module.exports.run = function(JAWS, stage, region, allTagged, noExeCf) {
  var command = new CMD(JAWS, stage, region, allTagged, noExeCf);
  return command.run();
};
 ...
 * Deploy lambdas
 *
 * @returns {Promise} list of awsm.json paths to lambda arn
 */
      ...
      .each(function(lambdaPath) {
        ...
        return packager.package()
            ...
            .then(function(result) {
              var pathToArn = {};
              pathToArn[lambdaPath] = result.arn;
              awsmToArns.push(pathToArn);
            });
      })
      .then(function() {

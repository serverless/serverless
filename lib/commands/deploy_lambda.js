    ...
    b
        .bundle(function(err, bundledBuf) {
          if (err) {
            ...
            console.log('bundled file wrote to', bundledFilePath);
            if (minify) {
              ...
              console.log('minified file wrote to', minifiedFilePath);
              resolve(result.code);
            } else {
              resolve(bundledBuf);
            }
...
module.exports = function(JAWS) {
  ...
  JAWS.optimizeNodeJs = function(tmpDistDir, jawsFilePath) {
    var baseDir = path.dirname(jawsFilePath),
        ...
        builder = lambdaJson.lambda.build,
        minify = lambdaJson.lambda.minify || false,

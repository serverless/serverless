 ...
 * @private
 */
function _deployLambasInAllRegions(JAWS, packagedLambdas, allAtOnce) {
...
module.exports = function(JAWS) {
   ...
   * @private
   */
  JAWS.packageNodeJs = function(tmpDistDir, jawsFilePath, stage, excludeFiles, ignoreFiles, skipOptimize) {
    var _this = this;
    if (!skipOptimize) {
      return _this.optimizeNodeJs(tmpDistDir, jawsFilePath, excludeFiles, ignoreFiles)
          ...
          .then(function(compressedCodeBuffer) {
            return {jawsFilePath: jawsFilePath, zipBuffer: compressedCodeBuffer};
          });
    } else {
  ...
  JAWS.deployLambdas = function(stage, deployAllTagged, allAtOnce) {
    ...
    return lambdasPromise
        ...
        .then(function(packagedLambdas) { //Step 2: For each region, deploy all lambda packages
          return _deployLambasInAllRegions(_this, packagedLambdas, allAtOnce);
        });
  };
};

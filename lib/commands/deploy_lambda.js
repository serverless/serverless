    ...
    .each(function (lambdaAwsmPath) {
      ...
      return Promise.try(function () {
        ...
        .then(function (packagedLambda) {
          ...
          var zipBuffer = fs.createReadStream(packagedLambda.zippedFilePath);
        ...
        case 'nodejs':
          return _this._packageNodeJs()
            .then(function (zippedFilePath) {
              packageData.zippedFilePath = zippedFilePath;
              return packageData;
            });
          break;
        case 'java8':
          return _this._packageJava()
            .then(function (zippedFilePath) {
              packageData.zippedFilePath = zippedFilePath;
              return packageData;
            });
...
Packager.prototype._createDistFolder = Promise.method(function () {
  ...
  if (_this._awsmJson.lambda.cloudFormation.Runtime === 'nodejs') {
...
/**
 * Packager: Package Java
 *
 * @returns {Promise} zippedFilePath
 * */

Packager.prototype._packageJava = function () {
  var _this = this;
  return new Promise(function (resolve, reject) {
    var modulePath = path.dirname(_this._lambdaPath);
    return utils.getJavaMvnPom(modulePath)
      .then(function (pom) {
        utils.runMvnSync(modulePath);
        var jarFilePath = path.join(modulePath, 'target', pom.project.name + '-' + pom.project.version + '.jar');
        if (!utils.fileExistsSync(jarFilePath)) {
          return Promise.reject(new JawsError('jar file "' + jarFilePath + '" not found, please run mvn package first in the module directory', JawsError.errorCodes.UNKNOWN));
        }
        ...
        fs.createReadStream(jarFilePath).pipe(fs.createWriteStream(zippedFilePath))
          .on('finish', function () {
            resolve(zippedFilePath);
          })
          .on('error', reject);
      });
  });
...
Packager.prototype._packageNodeJs = Promise.method(function () {
  ...
  return deferred
    .then(function (compressedCodeBuffer) {
      ...
      return Promise.resolve(zippedFilePath);
    });
});

        ...
        if (_this._noExeCf) {
          ...
          return cfDeferred
              .then(function(cfData) {
                return AWSUtils.monitorCf(cfData, _this._profile, _this._region, createOrUpdate);
              })
              .then(function() {
                spinner.stop(true);
              })
...
Deployer.prototype._generateLambdaCf = function(taggedLambdaPkgs) {
  ...
  return AWSUtils.cfGetLambdasStackTemplate(_this._JAWS._meta.profile, _this._region, _this._stage, projName)
      ...
      .then(function(lambdaCfTemplate) {
        ...
        utils.logIfVerbose('writing lambdas cf to: ' + lambdasCfPath);

        utils.writeFile(lambdasCfPath, JSON.stringify(lambdaCfTemplate, null, 2));
...
Packager.prototype.createLambdaName = function() {
  ...
  utils.logIfVerbose('lambdaName: ' + name);
  return name;
};

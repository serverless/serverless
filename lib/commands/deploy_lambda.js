  ...
  return Promise.try(function() {
      ...
      .then(function() {
        ...
        var stage  = _this._stage,
            ...
            meta   = _this._JAWS._meta;

        var options = { taggedLambdaPkgs: awsmLambdas,
                        lambdaRoleArn: utils.getProjRegionConfigForStage(meta.projectJson, stage, region).iamRoleArnLambda,
                        projName: meta.projectJson.name,
                        stage: stage, region: region,
                        meta: meta }

        return [options.lambdaRoleArn, _this._generateLambdaCf(options)];
      })
      ...
      .then(function() {
        JawsCLI.log('Lambda Deployer:  Applying Event Sources' + _this._region);

        var applyLambdaEventSource = require('../lambda_event_source').applyLambdaEventSource;
        applyLambdaEventSource(awsmLambdas, _this._stage, _this._region, _this._JAWS._meta.profile, _this._JAWS._meta.projectJson)
      })
      .then(function() {
...
Deployer.prototype._generateLambdaCf = require('../cloud_formation/generate_lambda_cf').createLambdaCf;

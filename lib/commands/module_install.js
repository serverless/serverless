...
module.exports.install = function(JAWS, url, saveCf, dontInstallDep) {
  var command = new CMD(JAWS, url, saveCf, dontInstallDep, false);
  return command.run();
};

module.exports.update = function(JAWS, url, saveCf, dontInstallDep) {
  var command = new CMD(JAWS, url, saveCf, dontInstallDep, true);
  return command.run();
};
...
CMD.prototype.run = Promise.method(function() {
  ...
  return _this._JAWS.validateProject()
      ...
      .then(function(module) {
        ...
        if (deferredDepInstalls.length) {
          JawsCLI.log('Installing dependencies');
        }

...
const CMD = class StageRegion extends ProjectCmd {
    ...
    return _this._JAWS.validateProject()
      ...
      .then(function() {
        return utils.generateResourcesCf(
          ...
          _this._JAWS._projectJson.name,
          _this._JAWS._projectJson.domain,
          _this._stage,
          _this._region,
          ''
        );
      ...
      .then(() => {
        return AWSUtils.createBucket(_this.Jaws._awsProfile, _this._region, _this.Jaws.getProjectBucket(_this._region, _this._stage));
      })
      .then(function() {
  ...
  _promptStage() {
    ...
    if (_this._type === 'stage') {
      ...
      let stages = Object.keys(_this._JAWS._projectJson.stage);

      // Check if project has stage
      if (!stages.length) {
        return Promise.reject(new JawsError('This project has no stage'));
      }
  ...
  _validate() {
    ...
    if (!_this._JAWS._projectJson.stage) {
      throw new JawsError('Project\'s jaws.json is malformed or has no existing stage object defined');
    }
    ...
    // Set JAWS Bucket
    _this._jawsBucket = utils.generateJawsBucketName(_this._stage, _this._region, _this._JAWS._projectJson.domain);

    // Stage Validations
    if (_this._type === 'stage') {

      // Make sure stage is not already defined
      if (_this._JAWS._projectJson.stage[_this._stage]) {
    ...
    if (_this._type === 'region') {
      ...
      if (_this._JAWS._projectJson.stage[_this._stage].some(function(r) {
...
JAWS_DATA_MODEL_STAGE=${stage}`;
  ...
  _putCfFile() {
    return AWSUtils.putCfFile(
      this._JAWS._awsProfile,
      this._JAWS._projectRootPath,
      this._region,
      this._jawsBucket,
      this._JAWS._projectJson.name,
      this._stage,
      'resources');
  }
  ...
  _updateProjectJson() {
    ...
    if (_this._JAWS._projectJson.stage[_this._stage]) {
      _this._JAWS._projectJson.stage[_this._stage].push(regionObj);
    } else {
      _this._JAWS._projectJson.stage[_this._stage] = [regionObj];
    }

    return utils.writeFile(
      path.join(_this._JAWS._projectRootPath, 'jaws.json'),
      JSON.stringify(_this._JAWS._projectJson, null, 2));
  }
};

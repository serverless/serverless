 ...
 *   ES6-MODULAR TODO:
 *
 * - remember that the lambda.awsm.json can now pull in any cloudformation resource which will be put into lambda-cf.json
 * - There is only 1 reserved index and that is cloudFormation.lambda - is the the lambda for the APIG endpoint (if any)
 * - see lib/templates/lambda.awsm.json for examples
 *
 * - Non lambda CloudFormation resources should be defined in module.awsm.json and pulled into resources-cf.json
 *
 *
 *
 *
...
const CMD = class Postinstall extends ProjectCmd {
  ...
  _installFiles() {
    ...
    let srcAwsmPath     = path.join(_this._JAWS._projectRootPath, pkgMgrDir, _this._moduleName, 'awsm'),
        ...
        awsModsPath     = path.join(_this._JAWS._projectRootPath, 'slss_modules');

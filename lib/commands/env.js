    ...
    return Promise.all([
      ...
      _this.getEnvFileAsMap(
        ...
        .spread(function(jawsJsonPaths, envMap) {
          ...
          jawsJsonPaths.forEach(function(ljp) {
            if (ljp.envVars) {
              ljp.envVars.forEach(function(key) {
                if (envInBackMap[key]) {
                  envInBackMap[key].push(ljp.name);
                } else {
                  envInBackMap[key] = [ljp.name];
                }
              });
            }
          });
          ...
          if (localEnvKeys.length) {
            console.log('where vars are used (yellow indicates not set in', stage, '):');

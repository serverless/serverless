  ...
  return utils.findAllAwsmJsons(backDir)
      ...
      .spread(function(awsmJsonPaths, envMapsByRegion) {
        ...
        if (showWhereUsed) {
          ...
          if (awsmKeys.length) {
            JawsCLI.log('awsm.json:lambda.envVars and regions where they are used:');
            awsmKeys.forEach(function(key) {
              var regionList = envMapsByRegion.filter(function(rMap) {
                    return (rMap.vars[key]) ? true : false;
                  }),
                  regionNames = regionList.map(function(rMap) {
                    return rMap.regionName;
                  });
              ...
              //TODO: always list all the regions, make ones where var is defined in GREEN and ones not RED
              JawsCLI.log(chalk.bold('regions defined in') + ': ' + regionNames.join(',') + '\n');
            });
          }
        }

...
const DashboardService = (serverless, options) => {
  ...
  const ensureIntegrationIsConfigured = async () => {
    ...
    context.integration.checkForErrors = true;
    return true;
  };
  ...
  const instrumentService = async () => {
    ...
    if (Object.keys(context).length === 0 || !context.integration) {
      integrationSetupProgress.remove();
      return;
    } else if (context.integration.checkForErrors) {
      const integration = await waitUntilIntegrationIsReady();
      ...
      try {
        ...
        for (const chunk of chunkedResources) {
          ...
          if (flowId) {
            ...
            emptyFlowResponse = true;
          }
        }

        if (emptyFlowResponse) {
          log.notice.success(
            'The region your are deploying to requires set up. The function instrumentation will continue in the background.\n  Instrumentation will be configured in the background check the progress in Serverless Dashboard.'
          );
        } else {

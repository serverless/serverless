...
const DashboardService = (serverless, options) => {
  ...
  const waitForInstrumentation = async (flowIds, startMessage, successMessage) => {
    ...
    while (waiting) {
      ...
      integrationSetupProgress.update(
        `${startMessage}\nUpdating ${completeFunctions.length}/${context.targetInstrumentations.length} functions`
      );
  ...
  const ensureIntegrationIsConfigured = async () => {
    ...
    log.notice.success(
      `Your AWS Account is being integrated with Serverless Dashboard.\n  You will receive an email when this process is complete or you can monitor progress in your dashboard account.\n  ${dashboardFrontend}/${context.org.orgName}/settings`
    );
  ...
  const instrumentService = async () => {
    ...
    const { isInstrumented } = await checkInstrumentationStatus('prod');

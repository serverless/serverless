...
const getServerlessConfigFilePath = (configFileName, srvcPath) => {
  const servicePath = srvcPath || process.cwd();

  if (configFileName) {
    const customPath = path.join(servicePath, configFileName);
...
const getServerlessConfigFile = _.memoize((configFileName, srvcPath) => getServerlessConfigFilePath(configFileName, srvcPath)
  .then(configFilePath => {

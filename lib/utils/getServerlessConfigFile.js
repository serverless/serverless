...
const getServerlessConfigFile = _.memoize((serverless) =>
  getServerlessConfigFilePath(serverless)
  .then(configFilePath => {
  ...
  (serverless) => `${
    serverless.processedInput.options.config} - ${serverless.config.servicePath}`);

...
exports.generateProjectBucketName = function(projectDomain) {

  // Sanitize
  projectDomain = projectDomain.trim().toLowerCase();

  return `serverless.${projectDomain}`;
};
...
exports.populate = function(S, populatedData, stage, region) {
  ...
  for (let i = 0; i < Object.keys(project.data.modules).length; i++) {
    let module = Object.keys(project.data.modules);
  ...
  traverse(populatedData).forEach(function(val) {
    ...
    if (typeof val === 'string' && val.match(/\$\${([^{}]*)}/g) != null) {
      ...
      if (!templates[templateModule][templateName])throw new SError(`Missing template in module: ${templateName} in ${templateModule}`);
  ...
  traverse(populatedData).forEach(function(val) {
    ...
    if (typeof(val) === 'string' && val.match(/\${([^{}]*)}/g) != null) {
      ...
      val.match(/\${([^{}]*)}/g).forEach(function(variableSyntax) {
        ...
        if (!value) throw new SError(`This variable is not defined: ${variableName}`);

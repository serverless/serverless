...
exports.populate = function(S, populatedData, stage, region) {
  ...
  if (!S || !populatedData || !stage || !region) throw new SError(`Missing required params: populatedData, stage, region`);

  // Get Meta instance
  let metaData = new S.classes.Meta(S, {
    projectPath: S._projectRootPath
  });
  ...
  traverse(populatedData).forEach(function(val) {
    ...
    if (typeof(val) === 'string' && val.match(/\${([^{}]*)}/g) != null) {
      ...
      val.match(/\${([^{}]*)}/g).forEach(function(variableSyntax) {
        ...
        let value;
        if (stage && region) {
          value = metaData.data.private.stages[stage].regions[region].variables[variableName];

        } else if (stage) {
          value = metaData.data.private.stages[stage].variables[variableName];

        } else if (!stage && !region) {
          value = metaData.data.private.variables[variableName];

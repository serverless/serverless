...
exports.populate = function(S, unPopulatedData, stage, region) {
  ...
  if (typeof region != 'undefined' && !meta.data.stages[stage].regions[region]) throw new SError(`Region doesn't exist in provided stage`);
  ...
  if (project.data.components && project.data.components.modules) {
    ...
    for (let i = 0; i < Object.keys(project.data.components.modules).length; i++) {
      let module = Object.keys(project.data.components.modules)[i],
          moduleInstance = new S.classes.Module(S, {module: module});
      templates[module] = moduleInstance.templates;
    }
  ...
  traverse(unPopulatedData).forEach(function(val) {
    ...
    if (typeof(val) === 'string' && val.match(/\${([^{}]*)}/g) != null) {
      ...
      val.match(/\${([^{}]*)}/g).forEach(function(variableSyntax) {
        ...
        if (meta.data.stages[stage].regions[region].variables[variableName]) {
          value = meta.data.stages[stage].regions[region].variables[variableName]
        } else if (meta.data.stages[stage].variables[variableName]) {
          value = meta.data.stages[stage].variables[variableName];
        } else if (meta.data.variables[variableName]) {
          value = meta.data.variables[variableName];
        }

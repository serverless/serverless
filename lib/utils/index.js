        ...
        return _this.readRecursively(baseDir, '*s-function.json')
            .then(function(functionFilePaths) {
              ...
              for (let i = 0; i < functionFilePaths.length; i++) {
                ...
                // If baseDir is inside the function folder, amend path and baseDir
                if (filePath.indexOf('/') === -1 || filePath.indexOf('\\')) {
                  filePath = path.join(baseDir.split('modules')[1], filePath);

                  // Also amend the baseDir
                  baseDir = baseDir.replace(baseDir.split('modules')[1], '');
                }

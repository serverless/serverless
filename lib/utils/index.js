...
exports.getJavaPackageName = function (domain, moduleName) {
  var packageName = domain.split('.').reverse().join('.') + '.' + moduleName;
  return packageName.replace(/-/g, '_');
};

exports.getJavaPackagePath = function (parentDir, packageName) {
  var dest = path.join(parentDir, 'src', 'main', 'java');
  packageName.split('.')
...
exports.runMvnSync = function (modulePath) {
  var cwd = process.cwd();
  process.chdir(modulePath);
  if (exec('mvn -Dmaven.test.skip=true clean package', {silent: false}).code !== 0) {
    throw new JawsError('Error executing mvn clean package on ' + modulePath, JawsError.errorCodes.UNKNOWN);
  }
  process.chdir(cwd);
};

exports.getJavaMvnPom = function (modulePath) {
  var pom = path.join(modulePath, 'pom.xml');
  var pomContents = fs.readFileSync(pom);
  var parser = new xml2js.Parser();
  return parser.parseStringAsync(pomContents)
    .then(function (result) {
      return result;
    });
};

exports.getModulePath = function (projectRootPath, moduleName) {
  return path.join(
    projectRootPath,
    'aws_modules',
    moduleName);

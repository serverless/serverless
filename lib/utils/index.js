...
exports.getFunctions = function(baseDir, type, functionPaths) {

  let _this = this,
      resolvedPaths = [];

  // Get functions by function paths
  if (functionPaths) {

    for (let functionPath of functionPaths) {
      let tempPath = '';

      functionPath = expandHomeDir(functionPath);

      if (functionPath.indexOf('/') == 0) {
        tempPath = path.join(baseDir, functionPath, 'lambda.awsm.json');
      } else {
        tempPath = path.resolve(baseDir, functionPath, './lambda.awsm.json');
      }
    ...
    resolvedPaths.forEach(p => {
      if (!_this.fileExistsSync(p)) {
        throw new JawsError(`Invalid function path ${p}`, JawsError.errorCodes.INVALID_RESOURCE_NAME);
      }
    });
  ...
  // Get functions by base directory
  if (!functionPaths) {

    return _this.readRecursively(baseDir, '*lambda.awsm.json')
        .then(function (functionPaths) {

          _this.jawsDebug('lambda.awsm.json paths found: ', functionPaths);
          ...
          return new Promise(function (resolve, reject) {
            ...
            async.eachLimit(functionPaths, 10, function (functionPath, cb) {
              ...
              functionPath = path.join(baseDir, functionPath);
              let functionJson = _this.readAndParseJsonSync(functionPath);

              if (type) {
                if (typeof functionJson.cloudFormation[type] !== 'undefined') resolvedPaths.push(functionPath);
              } else {
                resolvedPaths.push(functionPath);
              }
...
exports.findAllEnvletsForAwsm = function(projectRootPath, modName) {
  ...
  return this.getFunctions(path.join(projectRootPath, 'aws_modules', modName), 'lambda')

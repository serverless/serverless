...
module.exports = {
  ...
  getCwdType: function() {
    ...
    if (this.fileExistsSync(path.join(cwd, 's-project.json'))) {
      return { project: this.readFileSync(path.join(cwd, 's-project.json')).name }
    } else if (this.fileExistsSync(path.join(cwd, 's-component.json'))) {
      return { component: this.readFileSync(path.join(cwd, 's-component.json')).name }
    } else if (this.fileExistsSync(path.join(cwd, 's-function.json'))) {
      return { function: this.readFileSync(path.join(cwd, 's-function.json')).name }
    } else {
      return false;
    }
  },

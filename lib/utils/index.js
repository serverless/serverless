...
exports.populate = function(projectPath, populatedData, stage, region) {

  let _this = this;

  // Validate required params
  if (!projectPath || !populatedData || !stage || !region) throw new SError(`Missing required params: metaData, prjPopulatedData, stage, region`);

  // Validate stage exists
  if (typeof stage != 'undefined' && !metaData.private.stages[stage]) throw new SError(`Stage doesn't exist`);

  // Validate region
  if (typeof region != 'undefined' && !metaData.private.stages[stage].regions[region]) throw new SError(`Region doesn't exist in provided stage`);

  // TODO: Get meta data

  // TODO: Get all templates and combine to master object
  ...
  traverse(populatedData).forEach(function(val) {
    ...
    if (typeof val === 'string' && val.match(/\$\${([^{}]*)}/g) != null) {
      ...
      // TODO: Remove.  Get all templates first, doing this FS IO on each loop is slow.
  ...
  traverse(populatedData).forEach(function(val) {
    ...
    if (typeof(val) === 'string' && val.match(/\${([^{}]*)}/g) != null) {
      ...
      val.match(/\${([^{}]*)}/g).forEach(function(variableSyntax) {
        ...
        let value;
        if (stage && region) {
          value = metaData['private'].stages[stage].regions[region].variables[variableName];

        } else if (stage) {
          value = metaData['private'].stages[stage].variables[variableName];

        } else if (!stage && !region) {
          value = metaData['private'].variables[variableName];

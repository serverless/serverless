...
exports.populate = function(S, data, stage, region) {
  ...
  if (!S || !project || !stage || !region) throw new SError(`Missing required params: Serverless, project, stage, region`);

  let meta    = new S.classes.Meta(S).load().get();
  let project = new S.classes.Project(S).load().get();
  ...
  if (project.components && project.components.modules) {
    ...
    // Reformat object to exclude components.  Later, we can include component level templates
    let tempTemplates = project.getTemplates();
    let templates     = {};
    for (let prop in templates) {
      templates = _.extend(templates,  templates[prop]);
    }

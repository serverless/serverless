...
/**
 * Populate
 * - Populates a Serverless Project
 */

exports.populate = function(meta, project, stage, region) {

  let _this = this;

  // Validate required params
  if (!meta || !project || !stage || !region) throw new SError(`Missing required params: meta, project, stage, region`);
  ...
  if (project.components && project.components.modules) {

    let templates = {};
    for (let i = 0; i < Object.keys(project.components.modules).length; i++) {
      let module = Object.keys(project.components.modules)[i];
      templates[module] = project.components.modules[module].templates;
    }

    // Populate templates
    traverse(project).forEach(function(val) {

...
describe('#getServerlessConfigFile()', () => {
  ...
  it('should read a config file if servicePath is an absolute path to a file', () => {
    const serverlessFilePath = path.join(tmpDirPath, 'serverless.yml');

    writeFileSync(serverlessFilePath, 'service: my-yml-service');
    return expect(getServerlessConfigFile(serverlessFilePath)).to.be.fulfilled
      .then(result => result)
      .catch((ex) => {
        throw ex;
      })
  ...
  it('should return the file content if an absolute path to a file is found', () => {
    const serverlessFilePath = path.join(tmpDirPath, 'serverless.yml');

    writeFileSync(serverlessFilePath, 'service: my-yml-service');
    return expect(getServerlessConfigFile(serverlessFilePath)).to.be.fulfilled.then((result) => {
      expect(result).to.deep.equal({ service: 'my-yml-service' });
    });
  });

  it('should return file content if an absolute path to a file is found', () => {
    const serverlessFilePath = path.join(tmpDirPath, 'serverless.yaml');
  ...
  it('should return file content if an absolute path to a file found', () => {
    const serverlessFilePath = path.join(tmpDirPath, 'serverless.js');
    writeFileSync(
      serverlessFilePath,
      'module.exports = {"service": "my-json-service"};'
    ...
    return expect(getServerlessConfigFile(serverlessFilePath)).to.be.fulfilled.then(
      (result) => {
        expect(result).to.deep.equal({ service: 'my-json-service' });
      }
    );
  ...
  it('should return resolved value if an absolute path to a promise-using  file found', () => {
    const serverlessFilePath = path.join(tmpDirPath, 'serverless.js');
    writeFileSync(
      serverlessFilePath,
      'module.exports = new Promise(resolve => { resolve({"service": "my-json-service"}); });'
  ...
  it('throws an error, if an absolute path to an export not a plain object', () => {
    const serverlessFilePath = path.join(tmpDirPath, 'serverless.js');
    writeFileSync(
      serverlessFilePath,
      'module.exports = function config() {};'
    ...
    return expect(getServerlessConfigFile(serverlessFilePath))
      .to.be.rejectedWith('serverless.js must export plain object');
  });

  it('returns an empty string if file at absolute path has unsupported type', () => {
    const filePath = path.join(tmpDirPath, 'serverless.nosupport');
    writeFileSync(filePath, '');

    return expect(getServerlessConfigFile(filePath)).to.be.fulfilled.then((result) => {
      expect(result).to.equal('');
    });
  });
});

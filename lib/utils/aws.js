...
exports.cfCreateResourcesStack = function(awsProfile,
  ...
  return CF.createStackAsync(params)
    .catch(function(err) {
      if (err.cause.code === 'AlreadyExistsException') {
        throw new JawsError('CloudFormation stack "' + stackName + '" already exists. '
                  + 'You will need to delete the stack manually, or choose a different name for your project or stage',
                  JawsError.errorCodes.STACK_IN_BAD_STATE);
      } else {
        throw err;
      }
    });
...
exports.cfUpdateResourcesStack = function(JAWS, stage, region) {
  ...
  return _this.putCfFile(awsProfile, projRootPath, region, bucketName, projName, stage, 'resources')
      ...
      .catch(function(err) {
        if (err.cause.code === 'ValidationError' && err.cause.message.indexOf('ROLLBACK_COMPLETE') !== -1) {
          throw new JawsError('CloudFormation stack "' + stackName + '" is in the ROLLBACK_COMPLETE state. '
                    + 'This is a terminal state. You will need to manually delete this stack.',
                    JawsError.errorCodes.STACK_IN_BAD_STATE);
        } else {
          throw err;
        }

...
module.exports.configAWS = function(updateConfig) {
 ...
 * @returns {BbPromise}
 */
/* MOVED TO CF FILE (actually we're using the built in method directly ;)
...
exports.cfListStackResources = function(awsProfile, awsRegion, stackName, nextToken) {
  ...
  let CF = BbPromise.promisifyAll(new AWS.CloudFormation({
    apiVersion: '2010-05-15',
  }));
...
exports.lambdaPublishVersions = function(awsProfile, awsRegion, functionNames) {
  ...
  let L                   = new AWS.Lambda({
      ...
      publishVersionAsync = BbPromise.promisify(L.publishVersion, L);
...
exports.lambdaCreateAlias = function(awsProfile, awsRegion, functionName, functionVersion, aliasName) {
  ...
  let L                = new AWS.Lambda({
      ...
      createAliasAsync = BbPromise.promisify(L.createAlias, L);
...
exports.lambdaGetGreatestVersion = function(awsProfile, awsRegion, functionName) {
  ...
  let _this                       = this,
      ...
      listVersionsByFunctionAsync = BbPromise.promisify(L.listVersionsByFunction, L);
 ...
 * @param stage
 * @param projName
 * @returns {BbPromise} string of JSON template body
 */
/* MOVED TO CF FILE. ACTUALLY WE'RE USING THE BUILT IN METHOD DIRECTLY ;)
...
exports.cfCreateLambdasStack = function(JAWS, stage, region, lambdaRoleArn) {
  let _this        = this,
      ...
      bucketName   = JAWS.getProjectBucket(region, stage),
 ...
 * @returns {BbPromise}
 */
/* MOVED TO S3 FILE (actually we're using the built in method directly ;)
 ...
 * @param awsRegion
 * @param params
 * @returns {BbPromise} s3 data object response
 */
/* MOVED TO S3 FILE (actually we're using the built in method directly ;)
 ...
 * @returns {BbPromise.string} key of zip file in s3
 */
 /* MOVED TO S3 FILE

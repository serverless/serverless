...
function parseBitbucketServerURL(url) {
  ...
  const downloadUrl =
    `${url.protocol}//${url.hostname}/rest/api/latest/projects/${owner}/repos/${repo}/archive${url.search}&format=zip`;
...
function parseRepoURL(inputUrl) {
  return new BbPromise((resolve, reject) => {
    ...
    if (!url.host) {
      return reject(new ServerlessError('The URL you passed is not a valid URL'));
    }

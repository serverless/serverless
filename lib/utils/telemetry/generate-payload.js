...
const getServiceConfig = ({ configuration, options, variableSources }) => {
  ...
  const defaultRuntime = isAwsProvider
    ...
    : _get(configuration, 'provider.runtime');
  ...
  const resources = (() => {
    ...
    return {
      general: resolveResourceTypes(_get(configuration.resources, 'Resources')),
    };
  })();
  ...
  const result = {
    ...
    provider: {
      ...
      stage: options.stage || _get(configuration, 'provider.stage') || 'dev',
      region: isAwsProvider
        ? options.region || configuration.provider.region || 'us-east-1'
        : _get(configuration, 'provider.region'),
    },
...
module.exports = ({
  ...
  const payload = {
    ...
    commandOptionNames,
    console: {
      isAuthenticated: isConsoleAuthenticated,
    },
    dashboard: {
      userId,
    },

...
const shouldIncludeServiceConfig = (serverless) => {
...
const getServiceSpecificConfig = (serverless) => {
...
module.exports = async (serverless) => {
  ...
  const command = (() => {
    const resolvedCommand = serverless.processedInput.commands.join(' ');
    if (resolvedCommand === '') {
      // Situation where someone runs `serverless` for interactive mode
      return 'interactive';
    }
  ...
  if (shouldIncludeServiceConfig(serverless)) {
    _.merge(payload, getServiceSpecificConfig(serverless));
  }

...
describe('downloadTemplateFromRepo', () => {
  ...
  describe('downloadTemplateFromRepo', () => {
    ...
    it('should reject an error if the passed URL is not a valid GitHub URL', () => {
      return expect(downloadTemplateFromRepo('http://no-git-hub-url.com/foo/bar'))
    ...
    it('should reject an error if a directory with the same service name is already present', () => {
      ...
      return expect(downloadTemplateFromRepo('https://github.com/johndoe/existing-service'))
        .to.be.rejectedWith(Error);
    });
  ...
  describe('parseRepoURL', () => {
    ...
    it('should reject an error if URL is not valid', () => {
      return expect(parseRepoURL('non_valid_url'))
        .to.be.rejectedWith(Error);
    });

    it('should throw an error if URL is not of valid provider', () => {
      return expect(parseRepoURL('https://kostasbariotis.com/repo/owner'))
        .to.be.rejectedWith(Error);
    });

    it('should parse a valid GitHub URL', () => {
      return expect(parseRepoURL('https://github.com/serverless/serverless'))
        .to.be.fulfilled
        .then((output) => {
    ...
    it('should parse a valid GitHub URL with subdirectory', () => {
      return expect(parseRepoURL('https://github.com/serverless/serverless/tree/master/assets'))
        .to.be.fulfilled
        .then((output) => {
          expect(output).to.deep.eq({
            owner: 'serverless',
    ...
    it('should parse a valid GitHub Entreprise URL', () => {
      return expect(parseRepoURL('https://github.mydomain.com/serverless/serverless'))
        .to.be.fulfilled
        .then((output) => {
          expect(output).to.deep.eq({
            owner: 'serverless',
    ...
    it('should parse a valid GitHub Entreprise URL with authentication', () => {
      return expect(parseRepoURL('https://username:password@github.com/serverless/serverless/'))
        .to.be.fulfilled
        .then((output) => {
          expect(output).to.deep.eq({
            owner: 'serverless',
    ...
    it('should parse a valid GitLab URL ', () => {
      return parseRepoURL('https://gitlab.com/serverless/serverless')
        .then((output) => {
          expect(output).to.deep.eq({
            owner: 'serverless',
            repo: 'serverless',

  ...
  try {
    ...
    try {
      ...
      if (error.code === 'EACCES'){
        log.error('Cannot store information on approached deprecation. Please ensure process has write access to the ~/.servelress directory');
      } else {
        log.error('An error occurred when copying the health status to the ~.serverless directory');
      }
    }
  } finally {

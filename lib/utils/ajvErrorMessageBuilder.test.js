...
describe('#ajvErrorMessageBuilder', () => {
  it('should build user friendly message for unsupported function event', () => {
    ...
    const expectedErrors = [
      {
        ...
        params: { additionalProperty: 'websocketssss' },
        message: 'should NOT have additional properties',
        friendlyMessage: "Error: Unsupported function event 'websocketssss'",
      },
      {
        keyword: 'anyOf',
        dataPath: ".functions['someFunc'].events[2]",
    ...
    expect(ajvErrorMessageBuilder.addUserFriendlyMessage(errors)).to.deep.equal(expectedErrors);
  });

  it('should throw an error if empty array is provided', () => {
    expect(() => ajvErrorMessageBuilder.buildErrorMessages([])).to.throw(
    ...
    const expectedErrors = [
      {
        ...
        params: { additionalProperty: 'httpApi' },
        message: 'should NOT have additional properties',
        friendlyMessage: "Error: Unsupported function event 'httpApi'",
      },
      {
        keyword: 'additionalProperties',
        dataPath: ".functions['someFunc'].events[3].httpApi",
        ...
        params: { additionalProperty: 'pathhh' },
        message: 'should NOT have additional properties',
        friendlyMessage: "Error: Unsupported parameter 'pathhh' for 'httpApi' event",
      },
    ];
  ...
  it('should build error message for deeply property that is not friendly', () => {
    ...
    expect(ajvErrorMessageBuilder.buildErrorMessages(errors)).to.deep.equal([
      "Error: .functions['someFunc'].events[0].yourPluginEvent.anotherProp should be number",
    ]);
  });
});

...
module.exports = function(config) {
  ...
  CloudFormation.sCreateOrUpdateResourcesStack = function(project, stage, region, stackName, templateUrl) {
    ...
    // If no stack name, create
    if (!stackName) return createStack();

    // Check to see if Stack Exists
    return CloudFormation.getStackPolicyPromised({
        StackName: stackName
      })
      .then(function(data) {
      ...
      .catch(function(e) {
        ...
        if (e.cause.message == 'No updates are to be performed.') {
          throw new SError('No CloudFormation updates are to be performed');
        }
  ...
  CloudFormation.sMonitorCf = function(cfData, checkFreq) {
    ...
    let validStatuses = ['CREATE_COMPLETE', 'CREATE_IN_PROGRESS', 'UPDATE_COMPLETE', 'UPDATE_COMPLETE_CLEANUP_IN_PROGRESS'];

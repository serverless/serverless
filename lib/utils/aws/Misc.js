...
module.exports.profilesSet = function(awsProfile, awsRegion, accessKeyId, secretKey) {
  SUtils.sDebug('Setting new AWS profile:', awsProfile);
...
module.exports.getEnvFiles = function(serverless, region, stage) {
  ...
  if (region != 'all' || stage == 'local') {  //single region
    ...
    regionGets.push(_this.getEnvFileAsMap(serverless, region, stage).then(envVars => {
...
module.exports.getEnvFileAsMap = function(Serverless, region, stage) {
  ...
  if (stage == 'local') {
    deferred = Promise.resolve(fs.readFileSync(path.join(Serverless._projectRootPath, '.env')));
  } else {
    let bucket = SUtils.getProjRegionConfigForStage(Serverless._projectJson, stage, region).regionBucket;
    let config = {
      profile: Serverless._awsProfile,
      region: region
    };

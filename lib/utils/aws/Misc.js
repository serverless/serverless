    ...
    serverless._projectJson.stages[stage].forEach(regionCfg => {
      regionGets.push(
          _this.getEnvFileAsMap(serverless, regionCfg.region, stage)
              .then(envVars => {
                return {region: regionCfg.region, vars: envVars.map, raw: envVars.raw};
              }));
    });
  }
...
module.exports.getEnvFileAsMap = function(Serverless, region, stage) {
  ...
  if (stage == 'local') {
    deferred = Promise.resolve(fs.readFileSync(path.join(Serverless.config.projectPath, 'back', '.env')));
  } else {
    ...
    SCli.log(`Getting ENV file from S3 bucket: ${bucketName} in ${bucketRegion}`);

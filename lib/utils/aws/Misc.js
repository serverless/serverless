...
module.exports.addEnvironmentCredentials = function(config) {
  // Set Admin API Keys
  config.awsAdminKeyId     = process.env.SERVERLESS_ADMIN_AWS_ACCESS_KEY_ID || config.awsAdminKeyId;
  config.awsAdminSecretKey = process.env.SERVERLESS_ADMIN_AWS_SECRET_ACCESS_KEY || config.awsAdminSecretKey;
  if(process.env.SERVERLESS_ADMIN_AWS_SESSION_TOKEN) {
    config.awsAdminSessionToken = process.env.SERVERLESS_ADMIN_AWS_SESSION_TOKEN;
  }
}

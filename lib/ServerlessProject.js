...
class ServerlessProject {
  ...
  load() {
    ...
    return BbPromise.try(function () {
        ...
        if (!SUtils.fileExistsSync(path.join(_this._S.config.projectPath, 's-project.json'))) {
          throw new SError('Project could not be loaded because it does not exist in your project: ' + _this._config.sPath);
        }
  ...
  getPopulated(options) {
    ...
    for (let prop in this.components) {
      clone.components[prop] = this.components[prop].getPopulated(options);
    }

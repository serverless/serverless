...
class ServerlessProject {
  ...
  getTemplates() {
    ...
    // Load project-level templates
    let templates = _.cloneDeep(this.templates);

    // Load module-level templates
    for (let i = 0; i < Object.keys(this.components).length; i++) {
      let component = this.components[Object.keys(this.components)[i]];
      for (let j = 0; j < Object.keys(component.modules).length; j++) {
        let module =  component.modules[Object.keys(component.modules)[j]];
        for (let k = 0; k < Object.keys(module.templates).length; k++) {

          if (templates[Object.keys(module.templates)[k]]) {

            // If exists in project, extend it with the module template
            templates[Object.keys(module.templates)[k]] = _.assign(
              templates[Object.keys(module.templates)[k]],
              module.templates[Object.keys(module.templates)[k]])
          } else {

            // Otherwise, set template
            templates[Object.keys(module.templates)[k]] = module.templates[Object.keys(module.templates)[k]];
          }
        }
      }
    }

...
/*
    ...
    if (path.join(_this._S.config.projectPath, c, sf))

      if (SUtils.fileExistsSync(path.join(_this._S.config.projectPath, c, sf, 's-module.json'))) {
        let moduleJson = SUtils.readAndParseJsonSync(path.join(_this._S.config.projectPath, c, sf, 's-module.json'));

        // If no cloudFormation in module, skip...
        if (!moduleJson.cloudFormation) return;
        ...
        if (moduleJson.cloudFormation.resources) {
          ...
          if (cfResourceKeys.length > 0) {
            _this.sDebug('Merging in CF Resources from module: ' + moduleJson.name);
          }
          ...
          cfResourceKeys.forEach(function (resourceKey) {
            ...
            _this.cloudFormation.Resources[resourceKey] = moduleJson.cloudFormation.resources[resourceKey];
          });
        }
      }
*/
    if (options.populate) {
      ...
      return this.get().cloudFormation;
    }
  }

...
class ServerlessProject {
  ...
  getPopulated(stage, region) {
    let data = this.get();
    return SUtils.populate(this.options.projectPath, data, stage, region);
  }
  ...
  getResources(stage, region) {
    ...
    if (!this.options.projectPath) throw new SError('Instance does not have a project path set');

    // Required: Stage & Region
    if (!stage || !region) throw new SError('Both "stage" and "region" params are required');

    let data = this.getPopulated(stage, region);

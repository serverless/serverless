...
class ServerlessProject {
  ...
  getResources(options) {
    ...
    return BbPromise.try(function() {
      ...
      for (c of _this.components) {
        cPaths.push(c._config.fullPath);
      }
          ...
          for (let i = 0; i < cfSnippets.length; i++) {
            ...
            if (cf.resources) {
              ...
              cfResourceKeys.forEach(function (resourceKey) {
                ...
                cfTemplate.Resources[resourceKey] = cf.resources[resourceKey];
              });
            }
          }
        });
    ...
    .then(function() {
      if (options.populate) {
        return SUtils.populate(_this._S.state.getMeta(), _this.getTemplates(), cfTemplate, options.stage, options.region);
      } else {
        return cfTemplate;
      }
    });

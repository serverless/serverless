...
class EndpointPackageApiGateway extends JawsPlugin {
  ...
  _validateAndSanitize() {
    ...
    for (let i = 0; i < _this.Jaws.ctx.functions.length; i++) {

      // Require function config
      let functionJson = require(_this.Jaws.ctx.functions[i]);
      ...
      try {
        ...
        _this.Jaws.ctx.functions.splice(i,1);
        continue;
      }
      ...
      functionJson.path = _this.Jaws.ctx.functions[i];

      // Change function path to object
      _this.Jaws.ctx.functions[i] = functionJson;
    }

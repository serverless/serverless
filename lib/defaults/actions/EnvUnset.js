...
usage: jaws env unset`,
  ...
  envUnset(evt) {
    ...
    return _this.Jaws.validateProject()
      ...
      .then(function() {
        JawsCLI.log('Successfully unset env var: ' + _this.evt.key);
      });  
  }
  ...
  _promptKey(){
    ...
    if (!_this.Jaws._interactive || _this.evt.key) return BbPromise.resolve();
  ...
  _promptStage(){
    ...
    return JawsCLI.select('Which stage are you unsetting the env var from: ', choices, false)
  ...
  _unsetEnvVar(){
    ...
    return awsMisc.getEnvFiles(_this.Jaws, _this.evt.region, _this.evt.stage)
      .then(envMapsByRegion => {
        ...
        envMapsByRegion.forEach(mapForRegion => {
          ...
          if (_this.evt.stage == 'local') {
            putEnvQ.push(utils.writeFile(path.join(_this.Jaws._projectRootPath, '.env'), contents));
          } else {
            let config = {
            ...
            let bucket = JawsUtils.getProjRegionConfigForStage(_this.Jaws._projectJson, _this.evt.stage, mapForRegion.regionName).projectBucket;

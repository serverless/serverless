...
usage: jaws module create <module resource> <action>`,
  ...
  _generateActionAwsmJson() {
    ...
    if (this._createLambda) {
      actionTemplateJson.cloudFormation.lambda.Function.Runtime = this._runtime;

      // Create files for lambda actions
      switch (this._runtime) {
        case 'nodejs':
          actionTemplateJson.cloudFormation.lambda.Function.Handler = path.join('aws_modules', this._resource, this._action, 'handler.handler');
          break;
        default:
      ...
      case 'nodejs':
        if (_this.pkgMgr == 'npm') {
          ...
          if (!JawsUtils.dirExistsSync(actionPath)) {
            ...
            deferredWrites.push(
              JawsUtils.writeFile(path.join(actionPath, 'awsm.json'), JSON.stringify(actionTemplateJson, null, 2)),

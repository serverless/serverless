...
usage: jaws env list <stage> <region>`,
  ...
  /**
   *
   * @param stageRegion <array> stage region
   * @returns {Promise}
   */
  envList() {
    let _this       = this,
        stageRegion = Array.prototype.slice.call(arguments, 0);

    JawsUtils.jawsDebug('stage and region param', stageRegion);

    if (!stageRegion || stageRegion.length !== 2) {
      return Promise.reject(new JawsError('Must specify a stage and region'), JawsError.errorCodes.UNKNOWN);
    }

    let stage      = stageRegion[0],
        region     = stageRegion[1],
        awsModsDir = path.join(this.Jaws._projectRootPath, 'aws_modules');

    return JawsUtils.findAllAwsmJsons(awsModsDir)
      .then(awsmJsonPaths => {
        return [awsmJsonPaths, _this.Jaws.getEnvFiles(region, stage)];
      })

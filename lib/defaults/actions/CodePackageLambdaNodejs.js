...
class CodePackageLambdaNodejs extends JawsPlugin {
  ...
  codePackageLambdaNodejs(evt) {
    ...
    let awsConfig = {
      region:          _this.evt.currentRegion.region,
  ...
  _createDistFolder() {
    ...
    // Status
    JawsUtils.jawsDebug('Packaging "' + _this.evt.currentFunction.name + '"...');
    JawsUtils.jawsDebug('Saving in dist dir ' + _this.evt.distDir);
    ...
    JawsUtils.jawsDebug('Packaging stage & region:', _this.evt.stage, _this.evt.currentRegion.region);

    // Get ENV file from S3
    return _this.S3.sGetEnvFile(
        _this.evt.currentRegion.jawsBucket,
  ...
  _generateIncludePaths() {
    ...
    _this.evt.currentFunction.package.optimize.includePaths.forEach(p => {

      try {
        fullPath = path.resolve(path.join(_this.evt.distDir, p));
  ...
  _browserifyBundle() {
    ...
    let b           = browserify({
      basedir:          _this.evt.distDir,
      entries:          [_this.evt.currentFunction.cloudFormation.lambda.Function.Properties.Handler.split('.')[0] + '.js'],
      standalone:       'lambda',
    ...
    if (_this.evt.currentFunction.package.optimize.babel) {
      b.transform(babelify);
    }

    if (_this.evt.currentFunction.package.optimize.transform) {
      JawsUtils.jawsDebug('Adding transform', _this.evt.currentFunction.package.optimize.transform);
      b.transform(_this.evt.currentFunction.package.optimize.transform);
    }

    // optimize.exclude
    _this.evt.currentFunction.package.optimize.exclude.forEach(file => {
      JawsUtils.jawsDebug('Excluding', file);
      b.exclude(file);
    });

    // optimize.ignore
    _this.evt.currentFunction.package.optimize.ignore.forEach(file => {
      JawsUtils.jawsDebug('Ignoring', file);
      b.ignore(file);
    });

    // Perform Bundle
    let bundledFilePath = path.join(_this.evt.distDir, 'bundled.js');   // Save for auditing
    let minifiedFilePath = path.join(_this.evt.distDir, 'minified.js'); // Save for auditing
    ...
    return new Promise(function(resolve, reject) {
      b.bundle(function(err, bundledBuf) {
        if (err) {
          ...
          if (_this.evt.currentFunction.package.optimize.minify) {

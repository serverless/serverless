...
class Packager extends ProjectCmd {
  ...
  _createDistFolder() {
    ...
    JawsUtils.jawsDebug('copying', _this._JAWS._meta.projectRootPath, 'to', _this._distDir);
...
class DeployLambda extends JawsPlugin {
   ...
   * @returns {Promise.<T>}
   */
  deployLambda(stage, region, noExeCf, ...lambdaPaths) {
    let _this = this;
  ...
  _getRegions() {
    ...
    JawsUtils.jawsDebug('Deploying to regions:');
    JawsUtils.jawsDebug(regions);
  ...
  _setLambdaAwsmPaths(lambdaPaths) {
    if (!lambdaPaths || lambdaPaths.length == 0) {  //deploying labmda @ cwd
      let awsmPath = path.join(process.cwd(), 'awsm.json');
      JawsUtils.jawsDebug(`deploying ${awsmPath}`);
      this._lambdaAwsmPathsToDeploy = [awsmPath];
    }

    for (let lambdaPath of lambdaPaths) {
      let awsmPath = '';
      if (lambdaPath.indexOf('/') == 0) {
        awsmPath = path.join(lambdaPath, 'awsm.json');
      } else {
        awsmPath = path.resolve(process.cwd(), lambdaPath, './awsm.json');
      }
      this._lambdaAwsmPathsToDeploy.push(awsmPath);
    }

    JawsUtils.jawsDebug('deploying:');
    JawsUtils.jawsDebug(this._lambdaAwsmPathsToDeploy);

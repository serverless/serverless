  ...
  run() {
    ...
    return BbPromise.try(function() {
        ...
        return BbPromise.try(function() {
          ...
          .then(function(packagedLambda) {
            let jawsBucket = _this._JAWS.getJawsBucket(_this._region, _this._stage),
                lambdaName = JawsUtils.getLambdaName(lambdaAwsmPath);
      ...
      .then(function() {
        ...
        let lambdaRoleArn = JawsUtils.getProjRegionConfigForStage(_this._JAWS._projectJson, _this._stage, _this._region)
          .iamRoleArnLambda;
...
class Packager {
  ...
  /**
   *
   * @returns {Promise} {wsmFilePath: "", zipBuffer: compressedCodeBuffer}
   */
  run() {
    let _this = this;
    ...
    return this._createDistFolder()
      .then(function() {
        ...
        // Package by runtime
        switch (_this._awsmJson.cloudFormation.lambda.Function.Runtime) {
          case 'nodejs':
            return _this._packageNodeJs();
            break;
          default:
            return Promise.reject(new JawsError('Unsupported lambda runtime ' + _this._awsmJson.cloudFormation.lambda.Function.Runtime));
            break;
        }
      });
  }
  ...
  _packageNodeJs() {
    ...
    if (_this._awsmJson.package
      ...
      deferred = _this._optimizeNodeJs()
        .then(optimizedCodeBuffer => {
          ...
          let handlerFileName = _this._awsmJson.cloudFormation.lambda.Function.Handler.split('.')[0],
              compressPaths   = [
    ...
    return deferred
      .then(function(compressedCodeBuffer) {
        ...
        return Promise.resolve({awsmFilePath: _this._lambdaAwsmPath, zipBuffer: compressedCodeBuffer});
      });
  }
  ...
  _browserifyBundle() {
    ...
    let b           = browserify({
      ...
      entries:          [_this._awsmJson.cloudFormation.lambda.Function.Handler.split('.')[0] + '.js'],
      standalone:       'lambda',
...
usage: jaws lambda deploy [rel or abs path to lambda dirs. default is cwd]`,
   ...
   */
  deployLambdas(stage, region, noExeCf) {
    let _this       = this,

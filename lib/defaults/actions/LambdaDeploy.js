      ...
      .each(function(lambdaAwsmPath) {
        return BbPromise.try(function() {
          ...
          .then(function(packagedLambda) {
            ...
            //TODO: use stream and .upload() to get intelligent concurrent handling of parts
            //http://docs.aws.amazon.com/AWSJavaScriptSDK/guide/node-examples.html#Amazon_S3__Uploading_an_arbitrarily_sized_stream__upload_
            //Buffer to stream: https://github.com/gagle/Node-Streamifier
    ...
    if (_this._awsmJson.package
      ...
      deferred = _this._optimizeNodeJs()
        .then(optimizedCodeBuffer => {
          let ocbWithNewline  = optimizedCodeBuffer.concat(new Buffer('\n')), //lambda UI freaks if not end in newline
              envData         = fs.readFileSync(path.join(_this._distDir, '.env')),

...
class EndpointProvisionApiGateway extends JawsPlugin {
  ...
  endpointProvisionApiGateway(evt) {
    ...
    let awsConfig = {
      region:          _this.evt.current.region.region,
    ...
    .then(function() {
      ...
      _this.evt.url = 'https://'
          + _this.evt.current.region
          + '.execute-api.'
          +  _this.evt.current.region.region
          + '.amazonaws.com/'
          + _this.evt._stage
          + '/';
  ...
  _findOrCreateApi() {
    ...
    if (_this.evt.current.region.restApiId) {

      let params = {
        restApiId: _this.evt.current.region.restApiId /* required */
      };
      ...
      return _this.ApiGateway.getRestApiPromised(params)
          .then(function(response) {
            ...
            _this.evt.current.region.restApiId = response.id;
            JawsCli.log(
                '"'
                + _this.evt.current.stage + ' - '
                + _this.evt.current.region.region
  ...
  _buildEndpoints() {
    ...
    return BbPromise.try(function() {
      ...
      _this.evt.current.function = currentFunction;
  ...
  _createEndpointResources() {
    ...
    let _this      = this,
        endpoint   = _this.evt.current.function.cloudFormation.apiGateway.Endpoint,
  ...
  _createEndpointIntegration() {
    ...
    if (_this.evt.currentResource.cloudFormation.lambda) {
      ...
      let cfLogicalResourceId = _this.Lambda.sGetLambdaName(_this.evt.current.function),
          lambda              = null;
  ...
  _updateLambdaAccessPolicy() {

    let _this       = this,
        endpoint    = _this.evt.current.function.cloudFormation.apiGateway.Endpoint;

    // Sanitize Path - Remove first and last slashes, if any
    endpoint.cloudFormation.apiGateway.Path = endpoint.cloudFormation.apiGateway.Path.split('/');
    endpoint.cloudFormation.apiGateway.Path = endpoint.cloudFormation.apiGateway.Path.join('/');

    // Create new access policy statement
    let statement          = {};
    statement.Action       = 'lambda:InvokeFunction';
    statement.FunctionName =  _this.evt.currentLambda.PhysicalResourceId;
  ...
  _createDeployment() {
    ...
    let params = {
      restApiId:    _this.evt.current.region.restApiId, /* required */
      stageName:    _this.evt.current.stage, /* required */
      ...
      stageDescription: _this.evt.current.stage,
    ...
    return _this.ApiGateway.createDeployment(params)
        .then(function(response) {
          _this.evt.currentDeployment = response;
        })
        .catch(function(error) {
          throw new JawsError(error.message);
        });

...
class EndpointProvisionApiGateway extends JawsPlugin {
    ...
    if (evt.region.restApiId) {
      ...
      return _this.ApiGateway.getRestApiPromised(params)
          .then(function(response) {
            ...
            evt.region.restApiId = response.id;
            JawsUtils.jawsDebug(
                '"'
                + evt.stage + ' - '
                + evt.region.region
          ...
          .then(function(response) {
            ...
            for (let i = 0; i < response.items.length;i++) {
              ...
              if (response.items[i].name === _this.Jaws._projectJson.name) {
                evt.region.restApiId = response.items[i].id;
  ...
  _createEndpointResources(evt) {
    ...
    let _this      = this,
        endpoint   = evt.function.cloudFormation.apiGateway.Endpoint,

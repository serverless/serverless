...
class ProjectCreate extends JawsPlugin {
  ...
  _prompt() {
    ...
    if (!_this.Jaws._interactive) return Promise.resolve();
  ...
  _prepareProjectData() {
    ...
    if (_this._stage.toLowerCase() == 'local') {
      return Promise.reject(new JawsError('Stage ' + _this._stage + ' is reserved'));
    }

    // Validate: AWS only allows Alphanumeric and - in name
    let nameOk = /^([a-zA-Z0-9-]+)$/.exec(_this._name);
    if (!nameOk) {
      return Promise.reject(new JawsError('Project names can only be alphanumeric and -'));
    }
    ...
    if (_this._domain === 'myapp.com') {
      _this._domain = 'myapp-' + JawsUtils.generateShortId(8).toLowerCase() + '.com';
    }

    if (!/[a-z0-9-.]/.test(_this._domain)) {
      return Promise.reject(new JawsError('Domain must contain only lowercase letters, numbers, periods (.), and dashes (-)'));
    }
...
JAWS_DATA_MODEL_STAGE=${stage}`;
  ...
  _createCfStack(cfTemplateURL) {
    if (this._noCf) {
      ...
      let stackName = AWSUtils.cfGetResourcesStackName(this._stage, this._name);

...
class ProjectCreate extends JawsPlugin {
  ...
  _validateAndPrepare() {
     ...
     */
     /*
    _this.evt.name              = evt.name || null;
    _this.evt.domain            = evt.domain || null;
    _this.evt.stage             = evt.stage ? evt.stage.toLowerCase().replace(/\W+/g, '').substring(0, 15) : null;
    _this.evt.notificationEmail = evt.notificationEmail;
    _this.evt.region            = evt.region;
    _this.evt.runtime           = evt.runtime || 'nodejs';
    _this.evt.noCf              = evt.noCf;
    */
    if (_this.Jaws.cli) {
  ...
  _prompt() {
    ...
    return this.promptInput(prompts, overrides)
      .then(function(answers) {
        ...
        let choices = AWSUtils.validLambdaRegions.map(r => {
          return {
            key:   '',
            value: r,
            label: r,
  ...
  _prepareProjectData() {
    ...
    // Set JAWS Bucket
    _this._jawsBucket = JawsUtils.generateJawsBucketName(_this.evt.stage, _this.evt.region, _this.evt.domain);

    // If no profile, ensure access keys, create profile
    if (!_this._awsProfile) {
      AWSUtils.profilesSet('default', _this.evt.region, _this.Jaws._awsAdminKeyId, _this.Jaws._awsAdminSecretKey);
  ...
  _createJawsBucket() {
    ...
    let S3 = require('../../utils/aws/S3')({profile: this._awsProfile, region: this.evt.region});
    return S3.sCreateBucket(this._jawsBucket);
  }
...
JAWS_DATA_MODEL_STAGE=${stage}`;
  ...
  _putCfFile() {
    let CloudFormation = require('../../utils/aws/CloudFormation')({profile: this._awsProfile, region: this.evt.region});
  ...
  _createCfStack(cfTemplateURL) {
    ...
    if (this.evt.noCf) {
      ...
      let stackName = CloudFormation.sGetResourcesStackName(this.evt.stage, this.evt.name);
...
After creating CF stack, remember to put the IAM role outputs and jawsBucket in your project jaws.json in the correct stage/region.`);
    ...
    // Create CF stack
    return CloudFormation.sCreateResourcesStack(
      _this._projectRootPath,
      _this.evt.name,
      _this.evt.stage,
      _this.evt.domain,
      ...
      .then(cfData => {
        return CloudFormation.sMonitorCf(cfData, 'create')
          .then(cfStackData => {
  ...
  _createProjectJson(cfStackData) {
    ...
    jawsJson.stages[_this.evt.stage] = [{
    ...
    jawsJson.name   = _this.evt.name;
    jawsJson.domain = _this.evt.domain;

...
class ProjectCreate extends JawsPlugin {
  ...
  _createJawsBucket() {
    ...
    return AWSUtils.createBucket(this._awsProfile, this.evt.region, this._jawsBucket);
  }
...
JAWS_DATA_MODEL_STAGE=${stage}`;
  ...
  _createCfStack(cfTemplateURL) {
    if (this.evt.noCf) {
      ...
      let stackName = AWSUtils.cfGetResourcesStackName(this.evt.stage, this.evt.name);
...
After creating CF stack, remember to put the IAM role outputs and jawsBucket in your project jaws.json in the correct stage/region.`);
    ...
    return AWSUtils.cfCreateResourcesStack(
      ...
      .then(cfData => {
        return AWSUtils.monitorCf(cfData, _this._awsProfile, _this.evt.region, 'create')
          .then(cfStackData => {

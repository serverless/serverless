...
class Serverless {
  constructor({
    ...
    this.orgId = configuration && configuration.orgId || null;
    this.orgName = configuration && configuration.orgName || null;
    this.appId = configuration && configuration.appId || null;
    this.appName = configuration && configuration.app || null;
    this.credentialProviders = credentialProviders;

    this.isDashboardEnabled = !!(configuration.org && configuration.app);

    this.serviceDir = ensureString(serviceDir, {
    ...
    if (this.serviceDir != null) {
      ...
      this.configurationFilename = ensureString(configurationFilename, {
      ...
      if (path.isAbsolute(this.configurationFilename)) {
        throw new ServerlessError(
          `"configurationFilename" cannot be absolute path. Received: ${configurationFilename}`,
          'INVALID_ABSOLUTE_PATH_CONFIGURATION_FILENAME'
        );
      }

      // Store the original Service Configuration
      this.configurationInput = ensurePlainObject(configuration, {
        name: 'configuration',
        Error: ServerlessError,
    ...
    this.config = new Config(this, { serviceDir });

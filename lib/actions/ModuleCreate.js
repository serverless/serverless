...
usage: serverless module create`,
  ...
  _createModuleSkeleton() {
    ...
    let pathFunction   = path.join(pathModule, _this.evt.function);
    ...
    writeDeferred.push(
        ...
        fs.mkdirSync(path.join(pathModule, 'lib')),
        fs.mkdirSync(path.join(pathModule, 'package')),
        SUtils.writeFile(path.join(pathModule, 'lib', 'index.js'), libJs),
        SUtils.writeFile(path.join(pathModule, 'package.json'), JSON.stringify(packageJsonTemplate, null, 2)),
        SUtils.writeFile(path.join(pathModule, 's-module.json'), JSON.stringify(moduleJsonTemplate, null, 2))
    );

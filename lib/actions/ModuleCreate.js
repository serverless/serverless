...
usage: serverless module create`,
  ...
  _promptModuleFunction(){
    ...
    let prompts = {
      properties: {
        module:              {
          description: 'Enter a name of your new module: '.yellow,
  ...
  _createModuleSkeleton() {
    let _this                = this,
        ...
        moduleJsonTemplate   = _this._generateModuleAwsmJson(),
    ...
    writeDeferred.push(
      ...
      SUtils.writeFile(path.join(modulePath, 'package.json'), JSON.stringify(packageJsonTemplate, null, 2)),
      SUtils.writeFile(path.join(modulePath, 's-module.json'), JSON.stringify(moduleJsonTemplate, null, 2))
    );
    
    // write module/lib  structure
    writeDeferred.push(
      fs.mkdirSync(path.join(modulePath, 'lib')),
      fs.mkdirSync(path.join(modulePath, 'lib', 'package')),
      fs.mkdirSync(path.join(modulePath, 'lib', 'package', 'lib')),
      fs.mkdirSync(path.join(modulePath, 'lib', 'package', 'functions')),
      SUtils.writeFile(path.join(modulePath, 'lib', 'package', 'index.js'), '')
    ); 

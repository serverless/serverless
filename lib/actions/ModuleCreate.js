...
usage: serverless module create`,
  ...
  moduleCreate(evt) {
    ...
    return _this.S.validateProject()
        ...
        .then(function() {
          ...
          console.log("------- ModuleCreate Event: ", _this.evt);
          // Return Event
          return _this.evt;
        });
  };
  ...
  _createModuleSkeleton() {
    ...
    writeDeferred.push(
        fs.mkdirSync(_this.evt.pathFunction),
        SUtils.writeFile(path.join(_this.evt.pathModule, 'lib', 'index.js'), libJs),
        SUtils.writeFile(path.join(_this.evt.pathFunction, 'handler.js'), handlerJs),
        SUtils.writeFile(path.join(_this.evt.pathFunction, 'event.json'), JSON.stringify(eventJson, null, 2)),
        SUtils.writeFile(path.join(_this.evt.pathFunction, 's-function.json'), JSON.stringify(functionJsonTemplate, null, 2))
    ); 

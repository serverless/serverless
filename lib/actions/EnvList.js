...
usage: serverless env list`,
  ...
  envList(evt) {
    ...
    _listEnvVars() {
      let _this = this;
      let awsModsDir = path.join(_this.S._projectRootPath, 'back', 'modules');

      return SUtils.findAllAwsmJsons(awsModsDir)
        .then(awsmJsonPaths => {
          return [awsmJsonPaths, awsMisc.getEnvFiles(_this.S, _this.evt.region, _this.evt.stage)];
        })

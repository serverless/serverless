  ...
  class EndpointRemover extends SPlugin {
    ...
    _validateAndPrepare() {
      ...
      if (this.S.cli && !this.endpoints.length && !this.evt.options.all) {
        ...
        if(cwdType.function) {
          this.endpoints = this.project.getFunction(cwdType.function).getAllEndpoints();
        } else if (cwdType.component) {
          this.endpoints = this.project.getComponent(cwdType.component).getAllEndpoints();
        } else {
    ...
    _removeByRegion(region) {
      ...
      return this.aws.getApiByName(restApiName, stage, region)
        ...
        .then((resourses) => {
          return BbPromise.map(this.endpoints, ((endpoint) => this._endpointRemove(endpoint, region, resourses, restApiId)), {concurrency: 5});
        });
    }

    _endpointRemove(endpoint, region, resourses, restApiId) {

      let stage = this.evt.options.stage,
          resourse;

      let getResourseToRemove = function (res) {
        if (!res.parentId) return res.id;

        // check if parent resourse has no other children
        if (_.filter(resourses, {parentId: res.parentId}).length > 1) {
          return res.id;
        } else {
          let parentResourse = _.find(resourses, {id: res.parentId});
          // Skip if it's root - We can't remove the root resource.
          if(parentResourse.path === '/') return res.id;
          return getResourseToRemove(parentResourse);
        }
      }
      ...
      return BbPromise
        .try(() => {
          ...
          resourse = _.find(resourses, {path: '/' + endpoint.path} );

          if (!resourse || !resourse.resourceMethods[endpoint.method]) {
        ...
        .then(() => {
          ...
          if (_.keys(resourse.resourceMethods).length > 1) {
            SUtils.sDebug(`Removing method "${endpoint.method}" of "${resourse.path}" resourse`);

            params.httpMethod = endpoint.method;
            delete resourse.resourceMethods[endpoint.method];
            return this.aws.request('APIGateway', 'deleteMethod', params, stage, region);
          } else {
            // removing resourse
            SUtils.sDebug(`Removing resourse "${resourse.path}"`);
            params.resourceId = getResourseToRemove(resourse);
            delete resourse.parentId

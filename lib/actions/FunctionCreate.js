...
usage: serverless function create <function>`,
  ...
  functionCreate(evt) {
    ...
    return this.S.validateProject()
        ...
        .then(function() {
          SCli.log('Successfully created function: "'  + _this.evt.function.name + '"');
          return _this.evt;
        });
  }
  ...
  _createFunctionSkeleton() {
    ...
    _this.evt.function.pathFunction    = path.join('back', 'modules', _this.evt.module.name, functionShortName);
    ...
    writeDeferred.push(
        fs.mkdirSync(path.join(_this.S._projectRootPath, _this.evt.function.pathFunction)),
        SUtils.writeFile(path.join(path.join(_this.S._projectRootPath, _this.evt.function.pathFunction), 'handler.js'), handlerJs),
        SUtils.writeFile(path.join(_this.S._projectRootPath, _this.evt.function.pathFunction, 'event.json'), JSON.stringify(eventJson, null, 2)),
        SUtils.writeFile(path.join(_this.S._projectRootPath, _this.evt.function.pathFunction, 's-function.json'), JSON.stringify(functionJson, null, 2))
    );
  ...
  _generateFunctionJson() {
    ...
    let fullFunctionName  = _this.evt.module.name.charAt(0).toUpperCase() + _this.evt.module.name.slice(1) + _this.evt.function.charAt(0).toUpperCase() + _this.evt.function.slice(1);
    functionJsonTemplate.functions[fullFunctionName] = functionJsonTemplate.functions.functionTemplate;
    functionJsonTemplate.functions[fullFunctionName].name = fullFunctionName;

    // Remove Template
    delete functionJsonTemplate.functions.functionTemplate;

    // Add Function Handler
    functionJsonTemplate.functions[fullFunctionName].handler = path.join('modules', _this.evt.module.name, _this.evt.function, 'handler.handler');

    // Add endpoint path
    functionJsonTemplate.functions[fullFunctionName].endpoints[0].path = _this.evt.module.name + '/' + _this.evt.function;

...
usage: serverless function create <function>`,
  ...
  functionCreate(evt) {
    ...
    if (_this.S.cli) {
      ...
      _this.evt.function = _this.S.cli.params[0];
  ...
  _validateAndPrepare() {
    ...
    if (!this.S._interactive) {
      ...
      if (!this.evt.function) {
        return BbPromise.reject(new SError('Missing function name'));
      }
    }
    ...
    // get module name from CWD s-module.json
    this.evt.module = SUtils.readAndParseJsonSync(path.join(process.cwd(), 's-module.json')).name;
  ...
  _createSkeleton() {
    ...
    if (!SUtils.fileExistsSync(path.join(modulePath, 's-module.json'))) {
      ...
      let moduleTemplateJson = _this._generateModuleJson();
    ...
    switch (_this.evt.runtime) {
      case 'nodejs':
        if (_this.evt.pkgMgr == 'npm') {
          ...
          let functionPath = path.join(modulePath, 's-module', _this.evt.function);
          if (!SUtils.dirExistsSync(functionPath)) {

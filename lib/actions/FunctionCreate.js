...
usage: serverless function create <function>`,
    ...
    _createFunction() {
      ...
      }, path.join(this.S.getProject().getFilePath(), this.evt.options.path));
    ...
    _scaffold() {
      ...
      return BbPromise.try(function() {
        ...
        if (_this.evt.options.component.getRuntime().name === 'nodejs') {
          ...
          writeDeferred.push(
            SUtils.writeFile(path.join(_this.function.getFilePath(), 'handler.js'), handlerJs)
          );
          ...
          writeDeferred.push(
            SUtils.writeFile(path.join(_this.function.getFilePath(), 'handler.py'), handlerPy)
          );
        }

        writeDeferred.push(
          SUtils.writeFile(path.join(_this.function.getFilePath(), 'event.json'), {})
        );

...
usage: serverless function create <function>`,
  ...
  functionCreate(evt) {
    ...
    return this.S.validateProject()
        ...
        .then(function() {
          ...
          console.log("------- FunctionCreate Event: ", _this.evt);
          return _this.evt;
        });
  }
  ...
  _validateAndPrepare() {
    ...
    if (!SUtils.dirExistsSync(modulePath)) {
      ...
      this.evt.module = SUtils.readAndParseJsonSync(path.join(modulePath, 's-module.json'));
      this.evt.module.pathModule = path.join(modulePath, 's-module.json');
    }

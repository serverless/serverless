          ...
          .then(envMapsByRegion => {
            ...
            envMapsByRegion.forEach(mapForRegion => {
              ...
              if (_this.evt.options.stage == 'local') {
                putEnvQ.push(utils.writeFile(path.join(_this.S.config.projectPath, '.env'), contents));
              } else {
                let projectName  = _this.meta.data.variables.project,
                    bucketName   = _this.meta.data.variables.projectBucket,

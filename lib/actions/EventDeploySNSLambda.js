  ...
  class EventDeploySNSLambda extends SPlugin {
    ...
    eventDeploySNSLambda(evt) {
      ...
      let event          = _this.S.state.getEvents({ paths: [_this.evt.options.path] })[0],
          ...
          lambdaArn      = "arn:aws:lambda:" + _this.evt.options.region + ":" + awsAccountId + ":function:" + _this.S.state.getProject().name + "-" + _this.evt.options.path.split('/')[0] + "-" + _this.evt.options.path.split('/')[1] + "-" + _this.evt.options.path.split('/')[2].split('#')[0] + ":" + _this.evt.options.stage;

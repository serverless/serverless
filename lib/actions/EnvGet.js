...
usage: serverless env get`,
    ...
    _validateAndPrepare(){
      ...
      if (!_this.S.config.interactive) {
        // Check Params
        if (!_this.options.stage || !_this.options.region || !_this.key) {
    ...
    _getEnvVar() {
      ...
      return awsMisc.getEnvFiles(_this.S, _this.options.region, _this.options.stage)
          .then(envMapsByRegion => {
            ...
            SCli.log(`Values for ${_this.key} in stage ${_this.options.stage} by region:`);

            envMapsByRegion.forEach(mapForRegion => {

              let value;
              if (mapForRegion.vars && mapForRegion.vars[_this.key]) {
                value = mapForRegion.vars[_this.key];
                valByRegion[mapForRegion.regionName] = {};
                valByRegion[mapForRegion.regionName][_this.key] = value;
              } else {

...
module.exports = function(SPlugin, serverlessPath) {
  ...
  class EndpointDeployApiGateway extends SPlugin {
    ...
    _deployAuthorizers() {
      ...
      return BbPromise.try(function() {
          ...
          .each(function(f) {
            ...
            let fPopulated = f.toObjectPopulated({ stage: _this.evt.options.stage, region: _this.evt.options.region })

            // If no authorizer data, skip
            if (!fPopulated.authorizer || !Object.keys(fPopulated.authorizer).length) return;

            let authorizerName = fPopulated.authorizer && fPopulated.authorizer.name ? fPopulated.authorizer.name : fPopulated.name,
                updateId       = false;

            _this.deployedAuthorizers.forEach(function(da) {
              if (da.name === authorizerName) updateId = da.id;
            });

            if (!updateId) {
              return _this._createAuthorizer(f, fPopulated);
            } else {
              return _this._updateAuthorizer(f);
            }
          });
    }
    ...
    _createAuthorizer(f, fPopulated) {

      let _this = this;

      // Fetch Lambda
      let params = {
        FunctionName: f.getDeployedName({ stage: _this.evt.options.stage, region: _this.evt.options.region }),

 ...
 * - paths:              (Array)   Array of endpoint paths to deploy.  Format: ['users/show#eventName']
  ...
  class EventDeploy extends SPlugin {
    ...
    _validateAndPrepare() {
      ...
      if (_this.S.cli &&
        ...
        if (SUtils.fileExistsSync(path.join(process.cwd(), 's-function.json'))) {
          ...
          throw new SError(`You must be in a function folder to deploy its events.  Otherwise, use the command "serverless dash deploy"`);
        }
      }
    ...
    _deployEventsByRegion(region) {
      ...
      return new BbPromise(function(resolve, reject) {
        ...
        if (['stream', 'DynamoDB', 'dynamodb', 'kinesis', 'Kinesis'].indexOf(event.type) != -1) event.type = 'Stream';

        let subAction = 'eventDeploy' + event.type,
            newEvt = {
              options: {

...
module.exports = function(SPlugin, serverlessPath) {
  ...
  class Packager {
    ...
    _createDistFolder() {
      ...
      // Status
      SUtils.sDebug(`"${_this.evt.options.stage} - ${_this.evt.options.region} - ${_this.function.data.name}": Copying in dist dir ${_this.pathDist}`);
      ...
      wrench.copyDirSyncRecursive(
        path.join(_this.S.config.projectPath, _this.evt.options.component),
        _this.pathDist,
        {
          exclude: function(name, prefix) {
            ...
            return excludePatterns.some(sRegex => {
              ...
              if (willExclude) {
                SUtils.sDebug(`"${_this.evt.options.stage} - ${_this.evt.options.region} - ${_this.function.data.name}": Excluding - ${relPath}`);
              }

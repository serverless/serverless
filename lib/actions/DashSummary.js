    ...
    dashSummary() {
      ...
      components.forEach(function(component) {
        ...
        Object.keys(component.functions).forEach(function(func) {
          functionsNum++;
          SCli.log(`        |_ ${component.functions[func].name} (${Object.keys(component.functions[func].endpoints).length} endpoints - ${component.functions[func].events.length} events)`);

          // list endpoints for function
          Object.keys(component.functions[func].endpoints).forEach(function(endpoint) {
            endpointsNum++;
            SCli.log(`            |_ ${component.functions[func].endpoints[endpoint].path} - (${component.functions[func].endpoints[endpoint].method} endpoint)`);
          });

          // list events for function
          for (let i = 0; i < component.functions[func].events.length; i++) {
            eventsNum++;

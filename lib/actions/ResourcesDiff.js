  ...
  class ResourcesDiff extends SPlugin {
    ...
    _getLocalTemplate() {
      ...
      const fileName = `s-resources-cf-${this.evt.options.stage}-${this.evt.options.region.replace(/-/g,'')}.json`,
            filePath = this.S.getProject().getRootPath( '_meta', 'resources', fileName );
    ...
    _getDeployedTemplate() {
      const StackName = this.S.getProject().getRegion(this.evt.options.stage, this.evt.options.region).getVariables().resourcesStackName;

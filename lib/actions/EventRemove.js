...
class EventRemover extends SPlugin {
    ...
    _validateAndPrepare() {
      ...
      if (this.S.cli && !this.evt.options.names.length && !this.evt.options.all) {
        ...
        if(cwdType.function) {
          this.endpoints = this.project.getFunction(cwdType.function).getAllEvents();
        } else if (cwdType.component) {
          this.endpoints = this.project.getComponent(cwdType.component).getAllEvents();
        } else {
          this.endpoints = this.project.getAllEvents();
        }
      }

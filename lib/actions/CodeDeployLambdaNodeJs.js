...
class Deployer {
  ...
  _compress(evt) {
    ...
    SUtils.sDebug(`${evt.function.name} - ${evt.stage} - ${evt.region.region}: Compressed file created - ${evt.function.pathCompressed}`);
  ...
  _upload(evt) {
    ...
    SUtils.sDebug(`${evt.function.name} - ${evt.stage} - ${evt.region.region}: Uploading to - ${evt.region.regionBucket}`);
    ...
    return _this.Lambda.getFunctionPromised(params)
        ...
        .then(function() {
          ...
          if (!lambda) {
            ...
            SUtils.sDebug(`${evt.function.name} - ${evt.stage} - ${evt.region.region}: Creating Lambda function...`);

...
module.exports   = function(S) {
  ...
  class Deployer {
    ...
    _validateAndPrepare() {
      ...
      if (_this.functionPopulated.vpc) {
        // If both id arrays are empty, we assume no VPC
        if (_.isEmpty(_this.functionPopulated.vpc.securityGroupIds) && _.isEmpty(_this.functionPopulated.vpc.subnetIds)) {
          delete _this.functionPopulated.vpc;
        } else if (_.isEmpty(_this.functionPopulated.vpc.securityGroupIds) || _.isEmpty(_this.functionPopulated.vpc.subnetIds)) {
          return BbPromise.reject(new SError(`VPC configuration incomplete/invalid: ${_this.functionPopulated.vpc}`));
        }
      }
    ...
    /**
     * Prepare environment.
     * Converts the environment declared in the function definition to
     * an AWS Lambda Environment object (see http://docs.aws.amazon.com/lambda/latest/dg/API_Environment.html)
     */
    _prepareEnvironment() {
      let result;

      SUtils.sDebug(`"${this.evt.options.stage} - ${this.evt.options.region} - ${this.functionName}": Prepare environment ...`);

      if (_.isPlainObject(this.functionPopulated.environment) && !_.isEmpty(this.functionPopulated.environment)) {

        const defaultVars = {
            SERVERLESS_PROJECT: this.project.getName(),
            SERVERLESS_STAGE: this.evt.options.stage,
            SERVERLESS_REGION: this.evt.options.region,
            SERVERLESS_DATA_MODEL_STAGE: this.evt.options.stage ? this.project.getStage(this.evt.options.stage).getName() : this.evt.options.stage
          };

        const environment = _.defaults(defaultVars, _.clone(this.functionPopulated.environment));

        result = {
          Variables: {}
        };

        // Loop through variables instead of just assigning them because AWS Lambda environment is restricted to
        // strings only (additionally matching [^,]*). Print a warning for unsupported variables.
        _.forOwn(environment, (value, key) => {
          if (!_.isString(value) || !/[^,]*/.test(value)) {
            SCli.log(`WARNING: Variable '${key}' ignored. Value '${value}' does not match requirements.`);
            return;
          }
      ...
      return _this.aws.request('Lambda', 'getFunction', params, _this.evt.options.stage, _this.evt.options.region)
        ...
        .then(function () {
          ...
          if (!_this.lambda) {
            ...
            SUtils.sDebug(`"${_this.evt.options.stage} - ${_this.evt.options.region} - ${_this.functionName}": Creating Lambda function...`);
            ...
            let params = {
              ...
              Environment:  _this._prepareEnvironment(),
              Publish:      true, // Required by Serverless Framework & recommended best practice by AWS
              Timeout:      _this.functionPopulated.timeout
            };

            // Set VPC configuration if set in function definition
            if (_this.functionPopulated.vpc) {
              SUtils.sDebug(`"${_this.evt.options.stage} - ${_this.evt.options.region} - ${_this.functionName}": Configuring VPC ...`);

...
module.exports   = function(SPlugin, serverlessPath) {
  ...
  class Deployer {
    ...
    _validateAndPrepare() {
      ...
      _this.functionName    = _this.Lambda.sGetLambdaName(_this.project.name, _this.function._config.component, null, _this.function.name);
      ...
      if (_this.function._config.sPath.split('/').length == 3) {
        ...
        if (_this.fileExistsSync(path.join(
                _this.S.config.projectPath,
                _this.function._config.sPath.split('/').splice(0, 2).join(path.sep),
                's-module.json'))) {
          _this.functionName = _this.Lambda.sGetLambdaName(_this.project.name, _this.function._config.component, _this.function._config.sPath.split('/')[1], _this.function.name);
        }
      }
      // If nametemplate, use that
      if (_this.function.nameTemplate) {
        _this.functionName = _this.function.getPopulated(_this.evt.options.stage, _this.evt.options.region).nameTemplate;
      }

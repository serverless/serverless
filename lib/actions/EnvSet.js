...
usage: serverless env set`,
  ...
  envSet(evt) {
    ...
    return _this.S.validateProject()
      ...
      .then(function() {
        ...
        console.log("------- EnvSet Event: ", _this.evt);
  ...
  _prompt() {
    ...
    return _this.cliPromptInputEnvKey('Enter env var key to set a value to: ', _this.evt.key)
      .then(key => {
        _this.evt.key = key;
        BbPromise.resolve();
      })
      .then(function(){
        return _this.cliPromptInputEnvValue('Enter env var value to set to the provided key: ', _this.evt.value)
          .then(value => {

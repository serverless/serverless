...
module.exports = function(SPlugin, serverlessPath) {
  ...
  return class FunctionLogs extends SPlugin {
        ...
        parameters: [
          {
            ...
            description: 'Name of the function you want get logs from (componentName/functionName)',
            position: '0'
          }
        ]
      });
    ...
    _validateAndPrepare() {
      ...
      if (this.S.cli && !this.evt.options.functionName) {

        if(cwdType.function) {
          this.function = this.project.getFunction(cwdType.function);
        } else {
          throw new SError(`You must be in a function folder to run this command`);
        }
      }

      if (!this.S.cli && !this.evt.options.functionName) throw new SError(`Please provide a function name as a parameter`);

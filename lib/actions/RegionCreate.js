...
usage: serverless region create`,
    ...
    _validateAndPrepare() {
      ...
      if (this.S.getProvider('aws').validRegions.indexOf(_this.evt.options.region) == -1) {
      ...
      region.getVariables().updateConfig({
        filePath: project.getFilePath('_meta', 'variables', `s-variables-${this.evt.options.stage}-${_this.evt.options.region}.json`)
      });
    ...
    _createProjectBucket() {
      ...
      const stage      = this.evt.options.stage,
            bucketName = _this.S.getProject().getVariables().projectBucket,
            s3Region   = _this.S.getProject().getVariables().projectBucketRegion;
...
SERVERLESS_PROJECT_NAME=${projectName}`;
      ...
      return this.S.getProvider('aws')
        .request('S3', 'putObject', params, stage, projectBucketRegion);
    }

...
usage: serverless region create`,
    ...
    _validateAndPrepare() {
      ...
      if (!_this.meta.data.stages[_this.evt.options.stage]) {
      ...
      if (_this.meta.data.stages[_this.evt.options.stage].regions[_this.evt.options.region]) {
        return BbPromise.reject(new SError('Region "' + _this.evt.options.region + '" is already defined in the stage "' + _this.evt.options.stage + '"'));
      }

      // Update Meta
      _this.meta.data.stages[_this.evt.options.stage].regions[_this.evt.options.region] = {
        variables: {
...
SERVERLESS_PROJECT_NAME=${project.data.name}`;
      ...
      return this.S3.sPutEnvFile(
          this.meta.data.variables.projectBucket,
          project.data.name,

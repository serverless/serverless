...
usage: serverless region create`,
    ...
    _validateAndPrepare() {
      ...
      _this.S.state.set({ meta: _this.meta });
      _this.S.state.save();
    }
...
SERVERLESS_PROJECT_NAME=${this.S.state.project.get().name}`;

      return this.S3.sPutEnvFile(
          this.S.state.meta.get().variables.projectBucket,
          this.S.state.project.get().name,

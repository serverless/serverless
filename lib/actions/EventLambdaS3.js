    ...
    eventLambdaS3(evt) {
      ...
      return _this.aws.request('Lambda', 'removePermission', params, stage, region)
        ...
        .then(function(data) {
          ...
          let match = _.find(params.NotificationConfiguration.LambdaFunctionConfigurations, function (o) {
            return JSON.stringify(lambdaConfig.Events) === JSON.stringify(o.Events) &&
                lambdaConfig.LambdaFunctionArn === o.LambdaFunctionArn &&
                JSON.stringify(lambdaConfig.Filter.Key.FilterRules) === JSON.stringify(o.Filter.Key.FilterRules);
          });
          if (!match) {
            params.NotificationConfiguration.LambdaFunctionConfigurations.push(lambdaConfig);
          } else {
            let configPos = params.NotificationConfiguration.LambdaFunctionConfigurations.indexOf(match);

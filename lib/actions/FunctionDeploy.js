...
module.exports = function(SPlugin, serverlessPath) {
  ...
  class FunctionDeploy extends SPlugin {
    ...
    _validateAndPrepare() {
      ...
      if (!SUtils.dirExistsSync(path.join(_this.S.getProject().getRootPath(), '_meta', '_tmp'))) {
        fse.mkdirSync(path.join(_this.S.getProject().getRootPath(), '_meta', '_tmp'));
      }
    ...
    _removeTmpFolder() {
      if (!this.evt.options.dontRemoveTmp) {
        return fse.removeAsync(path.join(this.S.getProject().getRootPath(), '_meta', '_tmp'));
      } else {

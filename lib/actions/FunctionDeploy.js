  ...
  class FunctionDeploy extends SPlugin {
    ...
    registerActions() {
      ...
      this.S.addAction(this.functionDeploy.bind(this), {
        ...
        parameters: [
          {
            parameter: 'functionNames', // Only accepting paths makes it easier for plugin developers.  Otherwise, people should use dash deploy
    ...
    functionDeploy(evt) {
      ...
      return new BbPromise(function(resolve, reject) {
        ...
        if (!_this.S.getProject().getStages().length) return reject(new SError('No existing stages in the project'));
    ...
    _validateAndPrepare() {
      ...
      // Set Defaults
      _this.functions = [];
      _this.evt.options.functionNames  = _this.evt.options.functionNames ? _this.evt.options.functionNames : [];
      ...
      // Set Deploy Regions
      _this.regions  = _this.evt.options.region ? [_this.evt.options.region] : _this.S.getProject().getAllRegions(_this.evt.options.stage);


      if (_this.evt.options.functionNames.length) {
        _this.evt.options.functionNames.forEach(function(functionName) {
          _this.functions.push(_this.project.getFunction(functionName));
        });
      }
      ...
      if (_this.S.cli &&
        !_this.evt.options.functionNames.length &&
      ...
      if (!SUtils.dirExistsSync(path.join(_this.S._projectPath, '_meta', '_tmp'))) {
        fse.mkdirSync(path.join(_this.S._projectPath, '_meta', '_tmp'));
      }
    ...
    _deployCodeByRegion(region) {
      ...
      return new BbPromise(function(resolve, reject) {
        ...
        async.eachLimit(_this.evt.options.functions, 5, function(func, cb) {
    ...
    _removeTmpFolder() {
      if (!this.evt.options.dontRemoveTmp) {
        return fse.removeAsync(path.join(_this.S._projectPath, '_meta', '_tmp'));
      } else {

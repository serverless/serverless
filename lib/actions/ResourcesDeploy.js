...
usage: serverless resources deploy`,
    ...
    resourcesDeploy(evt) {
      ...
      // Get Meta & Project
      this.meta    = this.S.state.get().meta;
      this.project = this.S.state.get().project;
    ...
    _validateAndPrepare() {
      ...
      if (!_this.meta.stages[_this.evt.options.stage] && _this.evt.options.stage != 'local') {
        return BbPromise.reject(new SError('Stage ' + _this.evt.options.stage + ' does not exist in your project'));
      }

      // Validate region: make sure region exists in stage
      if (!_this.meta.stages[_this.evt.options.stage].regions[_this.evt.options.region]) {
    ...
    _updateResources() {
      ...
      let _this = this;
      let regionVars = _this.meta.stages[_this.evt.options.stage].regions[_this.evt.options.region].variables;

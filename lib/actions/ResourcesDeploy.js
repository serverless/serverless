...
usage: serverless resources deploy`,
    ...
    _putCfFile(cfTemplate) {
      const time                 = new Date().getTime(),
            ...
            projectBucket        = this.S.getProject().getVariables().projectBucket,
            projectBucketRegion  = this.S.getProject().getVariables().projectBucketRegion,
      ...
      return this.S.getProvider('aws')
        .uploadToProjectBucket(params, stage, projectBucketRegion)
        .then(function() {
          const hostname = new (require('aws-sdk').S3)().endpoint.hostname;
          return `https://${hostname}/${projectBucket}/${key}`;
        });
    };
  }

...
usage: serverless resources deploy`,
    ...
    _validateAndPrepare() {
      ...
      if (!_this.meta.data.stages[_this.evt.options.stage] && _this.evt.options.stage != 'local') {
        return BbPromise.reject(new SError('Stage ' + _this.evt.options.stage + ' does not exist in your project'));
      }

      // Validate region: make sure region exists in stage
      if (!_this.meta.data.stages[_this.evt.options.stage].regions[_this.evt.options.region]) {
    ...
    _updateResources() {
      ...
      let _this = this;
      let regionVars = _this.meta.data.stages[_this.evt.options.stage].regions[_this.evt.options.region].variables;
      ...
      // Build CF Template
      let getResourcesOptions = {
        stage: _this.evt.options.stage,
        region: _this.evt.options.region
      }
      _this.cfTemplate = _this.project.getResources(getResourcesOptions);
      ...
      return SUtils.writeFile(
        ...
        .then(function() {
          ...
          return _this.S3.sPutCfFile(
            _this.meta.data.variables.projectBucket,

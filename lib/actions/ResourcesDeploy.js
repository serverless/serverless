...
usage: serverless resources deploy`,
    ...
    _updateResources() {
      ...
      let _this = this;
      let regionVars = _this.S.state.meta.get().stages[_this.evt.options.stage].regions[_this.evt.options.region].variables;
      ...
      .then(function(resources) {
        ...
        return SUtils.writeFile(
          ...
          return _this.S3.sPutCfFile(
            _this.S.state.meta.get().variables.projectBucket,
            _this.S.config.projectPath,
            _this.S.state.project.get().name,
            _this.evt.options.stage,
            _this.evt.options.region,
            _this.cfTemplate
            )
            ...
            .then(() => {
              ...
              // Save State
              _this.S.state.set({ meta: _this.S.state.meta.get(), project: _this.S.state.project.get() });
              _this.S.state.save();

...
class Packager {
  ...
  _createDistFolder(evt) {
    ...
    // Status
    SUtils.sDebug(`${evt.function.name} - ${evt.stage} - ${evt.region.region}: Copying in dist dir ${evt.function.pathDist}`);
        ...
        evt.function.pathDist,
        {
          exclude: function(name, prefix) {
            ...
            return excludePatterns.some(sRegex => {
              ...
              if (willExclude) {
                SUtils.sDebug(`${evt.function.name} - ${evt.stage} - ${evt.region.region}: Excluding - ${relPath}`);
              }
  ...
  _optimize(evt) {
    ...
    if (evt.function.package.optimize.builder.toLowerCase() == 'browserify') {
      SUtils.sDebug(`${evt.function.name} - ${evt.stage} - ${evt.region.region}: Optimizing via Browserify...`);
  ...
  _browserifyBundle(evt) {
    ...
    if (evt.function.package.optimize.transform) {
      SUtils.sDebug(`${evt.function.name} - ${evt.stage} - ${evt.region.region}: Adding transform - ${evt.function.package.optimize.transform}`);
    ...
    return new BbPromise(function(resolve, reject) {
      ...
      b.bundle(function(err, bundledBuf) {
        if (err) {
          ...
          SUtils.sDebug(`${evt.function.name} - ${evt.stage} - ${evt.region.region}: Bundled file created - ${bundledFilePath}`);

          if (evt.function.package.optimize.minify) {

            SUtils.sDebug(`${evt.function.name} - ${evt.stage} - ${evt.region.region}: Minifying...`);
            ...
            SUtils.sDebug(`${evt.function.name} - ${evt.stage} - ${evt.region.region}: Minified file created - ${minifiedFilePath}`);

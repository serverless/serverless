...
usage: serverless module install <github-url>`,
    ...
    moduleInstall(options) {
      ...
      return _this._downloadModule()
          ...
          .then(function() {
            SCli.log('Successfully installed ' + _this.options.module.name + ' module.');
            ...
            return new BbPromise(function(resolve, reject) {
              ...
              new Download({
                ...
                .run(function(error) {
                  ...
                  _this.options.pathTempModule = tempModulePath;
                  resolve();
                });
            });
          });
    ...
    _validateAndPrepare() {
      let _this = this,
          srcModuleJsonPath = path.join(_this.options.pathTempModule, 's-module.json');
      ...
      _this.options.module = srcModuleJson;

      _this.options.module.pathModule = path.join(this.S.config.projectPath, 'back', 'modules', srcModuleJson.name);

      // if same module name exists, throw error
      if (SUtils.dirExistsSync(_this.options.module.pathModule)) {
    ...
    _installModule() {
      ...
      wrench.copyDirSyncRecursive(_this.options.pathTempModule, _this.options.module.pathModule, {
        forceDelete: true,
        excludeHiddenUnix: false,
      });

      // install deps if package.json exists
      if (SUtils.fileExistsSync(path.join(_this.options.module.pathModule, 'package.json'))) {
        SUtils.npmInstall(_this.options.module.pathModule);
      }

...
usage: serverless module install <github-url>`,
  ...
  moduleInstall(evt) {
    ...
    return this.S.validateProject()
        ...
        .then(function() {
          SCli.log('Successfully installed ' + _this.evt.module + ' module.');
          return _this.evt;
        });
  }
          ...
          return new BbPromise(function(resolve, reject) {
            ...
            new Download({
              ...
              .run(function(error) {
                ...
                _this.evt.tempModulePath = tempModulePath;
                resolve();
              });
          });
        });
  ...
  _validateAndPrepare() {
    let _this = this,
        srcModuleJsonPath = path.join(_this.evt.tempModulePath, 's-module.json');
    ...
    _this.evt.module = srcModuleJson.name;
    
    _this.evt.modulePath = path.join(this.S._projectRootPath, 'back', 'modules', srcModuleJson.name);
    
    // if same module name exists, throw error
    if (SUtils.dirExistsSync(_this.evt.modulePath)) {
    ...
    if (
        ...
        (!srcModuleJson.cloudFormation.apiGatewayIamPolicyDocumentStatements)
       ) {
  ...
  _installModule() {
    ...
    wrench.copyDirSyncRecursive(_this.evt.tempModulePath, _this.evt.modulePath, {
      forceDelete: true,
      excludeHiddenUnix: false,
    });
    
    // install deps if package.json exists
    if (SUtils.fileExistsSync(path.join(_this.evt.modulePath, 'package.json'))) {
      SUtils.npmInstall(_this.evt.modulePath); 
    }
  ...
  _updateCfTemplate() {
    let _this             = this,
        ...
        cfExtensionPoints = SUtils.readAndParseJsonSync(path.join(_this.evt.tempModulePath, 's-module.json')).cloudFormation;

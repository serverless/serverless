  ...
  class DashDeploy extends SPlugin {
    ...
    _prompt() {
      ...
      _this.functions.forEach(function(func){
        if (!data[func.module]) data[func.module] = {};
        if (!data[func.module][func.data.name]) data[func.module][func.data.name] = {
          function:  func,
          endpoints: []
        };
      });
      ...
      _this.endpoints.forEach(function(endpoint){
        data[endpoint.module][endpoint.function].endpoints.push(endpoint);
      });
      ...
      for (let i = 0; i < Object.keys(data).length; i++) {
        ...
        for (let j = 0; j < Object.keys(data[module]).length; j++) {
          ...
          choices.push({
            spacer: func.function.component + ' - ' + func.function.module + ' - ' + func.function.data.name
          });


          choices.push({
            key:        '  ',
            value:      func.function.sPath,
            label:      'function - ' + func.function.sPath,
            type:       'function'
          });
          ...
          for (let k = 0; k < func.endpoints.length; k++) {
            choices.push({
              key:        '  ',
              value:      func.endpoints[k].sPath,
              label:      'endpoint - ' + func.endpoints[k].sPath,
              type:       'endpoint'
            });
          }
        }

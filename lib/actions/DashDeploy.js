  ...
  class DashDeploy extends SPlugin {
      ...
      if (isComponent){
        ...
        Object.keys(component.data.modules).forEach(function(moduleName) {
          ...
          Object.keys(component.data.modules[moduleName].functions).forEach(function(functionName) {
            ...
            _this.evt.options.functionPathsDash.push(componentName + path.sep + moduleName + path.sep + functionName);

            component.data.modules[moduleName].functions[functionName].endpoints.forEach(function(endpoint) {
              _this.evt.options.endpointPathsDash.push(componentName + path.sep + moduleName + path.sep + functionName + '@' + endpoint.path + '~' + endpoint.method);
            });
          });
        });
        ...
        let functionName  = SUtils.readAndParseJsonSync(path.join(CWD, 's-function.json')).name,
            ...
            componentName = SUtils.readAndParseJsonSync(path.join(CWD, '..', '..', 's-component.json')).name,
            FunctionInstance = new _this.S.classes.Function(_this.S, {
    ...
    _prompt() {
      ...
      _this.functions.forEach(function(func){
        ...
        if (!data[func.config.module]) data[func.config.module] = {};
        if (!data[func.config.module][func.data.name]) data[func.config.module][func.data.name] = {
          function:  func,
          endpoints: []
        };
      });
      ...
      _this.endpoints.forEach(function(endpoint){
        data[endpoint.config.module][endpoint.config.function].endpoints.push(endpoint);
      });
      ...
      for (let i = 0; i < Object.keys(data).length; i++) {
        ...
        for (let j = 0; j < Object.keys(data[module]).length; j++) {
          ...
          choices.push({
            spacer: func.function.config.component + ' - ' + func.function.config.module + ' - ' + func.function.data.name
          });


          choices.push({
            key:        '  ',
            value:      func.function.config.sPath,
            label:      'function - ' + func.function.config.sPath,
            type:       'function'
          });
          ...
          for (let k = 0; k < func.endpoints.length; k++) {
            choices.push({
              key:        '  ',
              value:      func.endpoints[k].config.sPath,
              label:      'endpoint - ' + func.endpoints[k].config.sPath,
              type:       'endpoint'
            });
          }
        }

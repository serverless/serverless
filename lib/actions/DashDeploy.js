    ...
    _prompt() {
      ...
      for (let i = 0; i < _this.components.length; i++) {
        ...
        for (let j = 0; j < Object.keys(component.functions).length; j++) {
          ...
          let func = component.functions[Object.keys(component.functions)[j]]; // XXX

          // Push function sPath as spacer
          choices.push({
            spacer: func._config.sPath                  // XXX
          });

          choices.push({
            key:        '  ',
            value:      func._config.sPath, // XXX
            label:      'function - ' + func._config.sPath, // XXX
            type:       'function'
          });

          // If no endpoints, skip iteration
          if (!func.endpoints || !func.endpoints.length) continue; //XXX

          // If endpoints, find them
          for (let k = 0; k < func.endpoints.length; k++) { // XXX
            choices.push({
              key:        '  ',
              value:      func.endpoints[k]._config.sPath,  // XXX
              label:      'endpoint - ' + func.endpoints[k]._config.sPath,  // XXX
              type:       'endpoint'
            });
          }
        }

  ...
  class DashDeploy extends SPlugin {
    ...
    _prompt() {
      ...
      if(cwdType.function) {
        ...
        // either we're in a subfolder or get all functions of the project
        let allFunctions = _this.project.getAllFunctions();
        ...
        // match functions in current subfolder
      ...
      _.each( functions, function(func){
        ...
        _.each( func.getAllEndpoints(), function(endpoint){
          choices.push({
            ...
            label:      `${endpoint.path} (${endpoint.method} endpoint)`,
            type:       'endpoint'
          });
        });
        ...
        _.each( func.getAllEvents(), function(event){
          choices.push({
            key:        '  ',
            value:      event.name,
            label:      `${event.name} (${event.type} event)`,
            type:       'event'
          });
        });
      });
      ...
      return new BbPromise(function(resolve, reject) {
        ...
        return _this.S.actions.functionDeploy({
            options: {
              ...
              functions:   _this.evt.options.selectedFunctions
            }
          })
          .then(function(evt) {
        ...
        .then(function() {
          ...
          if (!_this.evt.options.selectedEndpoints || !_this.evt.options.selectedEndpoints.length) return resolve();

          return _this.S.actions.endpointDeploy({
              options: {
                stage:       _this.evt.options.stage,
                region:      _this.evt.options.region,
                endpoints:       _this.evt.options.selectedEndpoints
              }
            })
            .then(function(evt) {
        ...
        .then(function() {
          ...
          // If user selected endpoints, deploy them
          if (!_this.evt.options.selectedEvents || !_this.evt.options.selectedEvents.length) return resolve();

          return _this.S.actions.eventDeploy({
              options: {
                stage:       _this.evt.options.stage,
                region:      _this.evt.options.region,
                events:      _this.evt.options.selectedEvents
              }
            })
            .then(function(evt) {

...
module.exports = function(SPlugin, serverlessPath) {
  ...
  class DashDeploy extends SPlugin {
    ...
    _validateAndPrepare() {
      ...
      _this.functions = _this.S.state.getFunctions({ paths: _this.evt.options.paths });
      _this.endpoints = _this.S.state.getEndpoints({ paths: _this.evt.options.paths });
    ...
    _prompt() {
      ...
      _this.endpoints.forEach(function(endpoint){
        console.log(endpoint.getComponent().name)
        data[endpoint.getComponent().name][endpoint.getFunction()._config.sPath].endpoints.push(endpoint);
      });
      ...
      for (let i = 0; i < Object.keys(data).length; i++) {
        ...
        let component = Object.keys(data)[i];

        for (let j = 0; j < Object.keys(data[component]).length; j++) {

          let func = data[component][Object.keys(data[component])[j]];

          // Push module name as spacer
          choices.push({
            spacer: component + ' - ' + func.name
          });

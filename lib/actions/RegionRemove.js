...
usage: serverless region remove`,
    ...
    _validateAndPrepare() {
      ...
      if (this.S.getProvider('aws').validRegions.indexOf(this.evt.options.region) == -1) {
    ...
    _listS3Objects() {
      ...
      let prefix   = ['serverless', this.S.getProject().getName(), this.evt.options.stage, this.evt.options.region].join('/'),
          s3Region = this.S.getProject().getVariablesObject().projectBucket.split('.')[1],
          params   = {
            Bucket: _this.S.getProject().getVariablesObject().projectBucket,
            Prefix: prefix
          };
    ...
    _removeS3Objects(objects) {
      ...
      let s3Region = this.S.getProject().getVariablesObject().projectBucket.split('.')[1];

      if (objects.length) {
        let params = {
            Bucket: this.S.getProject().getVariablesObject().projectBucket,
            Delete: {
              Objects: objects
            }
          };
    ...
    _removeVariables() {
      ...
      return fs.unlinkAsync(path.join(this.S._projectPath, '_meta', 'variables', fileName));
    }

...
usage: serverless region remove`,
      ...
      return this.cliPromptSelectStage('Select an existing stage for your new region: ', this.evt.options.stage, false)
        ...
        .then(() => {
          if (this.S.state.meta.getRegions(this.evt.options.stage).length)
            return this.cliPromptSelectRegion('Select a new region for your existing stage: ', false, true, this.evt.options.region, this.evt.options.stage);
          else
            throw new SError(`No existing regions in "${this.evt.options.stage}" stage`);
        })
    ...
    _removeVariables() {
      ...
      console.log(path.join(this.S.config.projectPath, '_meta', 'variables', fileName));
      fs.unlinkAsync(path.join(this.S.config.projectPath, '_meta', 'variables', fileName));
    }

...
usage: serverless region remove`,
    ...
    _listS3Objects() {
      ...
      let prefix   = ['serverless', this.S.getProject().getName(), this.evt.options.stage, this.evt.options.region].join('/'),
          s3Region = this.S.getProject().getVariables().projectBucket.split('.')[1],
          params   = {
            Bucket: _this.S.getProject().getVariables().projectBucket,
            Prefix: prefix
          };
    ...
    _removeS3Objects(objects) {
      ...
      let s3Region = this.S.getProject().getVariables().projectBucket.split('.')[1];

...
module.exports = function(SPlugin, serverlessPath) {
  ...
  class EndpointDeploy extends SPlugin {
    ...
    _validateAndPrepare() {
      ...
      // Set defaults
      _this.evt.options.endpointNames  = _this.evt.options.endpointNames ? _this.evt.options.endpointNames : [];
      _this.regions            = _this.evt.options.region ? [_this.evt.options.region] : _this.project.getAllRegions(_this.evt.options.stage);

      if (_this.evt.options.endpointNames.length) {
        _this.evt.options.endpointNames.forEach(function(endpointName) {
      ...
      if (_this.S.cli &&
          !_this.evt.options.endpointNames.length &&
          !_this.evt.options.all) {

        if(cwdType.function) {
          _this.endpoints = _this.project.getFunction(cwdType.function).getAllEndpoints();
        } else if (cwdType.component) {
          _this.endpoints = _this.project.getComponent(cwdType.component).getAllEndpoints();
        } else {
          _this.endpoints = _this.project.getAllEndpoints();
        }

        SCli.log('Deploying all endpoints in ' + Object.values(cwdType)[0] + ' ' + Object.keys(cwdType)[0] + '...');
      }
    ...
    _deployEndpointsByRegion(region) {
      ...
      let restApi;
      if (_this.project.getVariablesObject(_this.evt.options.stage, region)['apiGatewayApi']) {
        restApi = _this.project.getVariablesObject(_this.evt.options.stage, region)['apiGatewayApi'];
      } else {
      ...
      return _this._findOrCreateRestApi(
        ...
        .then(function(restApiData) {
          ...
          // Persist API to meta
          _this.project.getVariablesObject(_this.evt.options.stage, region)['apiGatewayApi'] = restApiData.name;

          // TODO: do not save all meta upon each endpoint deploy. `sls endpoint deploy -a` is awfully slow this way
          return _this.project.save().then(function(){

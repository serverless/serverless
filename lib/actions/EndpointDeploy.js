...
module.exports = function(SPlugin, serverlessPath) {
  ...
  class EndpointDeploy extends SPlugin {
          ...
          Object.keys(component.data.modules).forEach(function(moduleName) {
            ...
            Object.keys(component.data.modules[moduleName].functions).forEach(function(functionName) {
              ...
              component.data.modules[moduleName].functions[functionName].endpoints.forEach(function(endpoint) {
                _this.evt.options.paths.push(componentName + path.sep + moduleName + path.sep + functionName + '@' + endpoint.path + '~' + endpoint.method);
              });
            });
          });
    ...
    _deployEndpointsByRegion(region) {
      ...
      let restApi;
      if (_this.meta.data.stages[_this.evt.options.stage].regions[region].variables['apiGatewayApi']) {
        restApi = _this.meta.data.stages[_this.evt.options.stage].regions[region].variables['apiGatewayApi'];
      } else {
      ...
      return ApiGateway.sFindOrCreateRestApi(
        ...
        .then(function(restApiData) {
          ...
          // Persist API to meta
          _this.meta.data.stages[_this.evt.options.stage].regions[region].variables['apiGatewayApi'] = restApiData.name;
          _this.meta.save();

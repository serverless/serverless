...
class EndpointDeploy extends SPlugin {
  ...
  _validateAndPrepare(evt) {
    ...
    // Get Endpoints From Paths

    if (_this.S.cli) {

      // If CLI & paths, get endpoints
  ...
  _promptStage(evt) {
    ...
    // Show prompt
    return _this.cliPromptSelect('Endpoint Deployer - Choose a stage: ', choices, false)
        .then(results => {
  ...
  _processDeployment(evt) {
    ...
    return BbPromise.try(function() {
        ...
        .then(function() {
          ...
          // Status
          SCli.log('Successfully deployed endpoints in "' + evt.stage + '" to the following regions: ' + evt.regions);

...
usage: serverless stage create`,
    ...
    _updateCfTemplate() {
      ...
      if (cfTemplate.Parameters.Stage.AllowedValues.indexOf(this.evt.stage) == -1) {
        cfTemplate.Parameters.Stage.AllowedValues.push(this.evt.stage);
      }
      if (cfTemplate.Parameters.DataModelStage.AllowedValues.indexOf(this.evt.stage) == -1) {
...
SERVERLESS_PROJECT_NAME=${this.S._project.name}`;
    ...
    //_updateProjectMeta(cfStackData) {
    //
    //  let _this     = this,
    //      regionObj = {
    ...
    //  }
    //
    //  if (_this.S._project.stages[_this.evt.stage]) {
    //    _this.S._project.stages[_this.evt.stage].push(regionObj);
    //  } else {
    //    _this.S._project.stages[_this.evt.stage] = [regionObj];
    //  }
    //
    //  return SUtils.writeFile(

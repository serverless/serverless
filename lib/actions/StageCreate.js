...
usage: serverless stage create`,
    ...
    _prompt() {
      ...
      .then(function() {
        if (!_this.profiles.length) {
          return _this._promptMissingCredentials();
        }
      })
      .then(function() {
    ...
    _promptMissingCredentials() {

      let _this = this;

      // Skip if non-interactive or profile is provided
      if (!_this.S.config.interactive || _this.evt.options.profile) return BbPromise.resolve();
      ...
      return BbPromise.try(function() {
          ...
          if (!_this.profiles.length) {
            prompts.properties.awsAdminKeyId = {
              description: `You don\'t have any profiles for ${_this.provider.getProviderName()} on this machine.  Let's create one.  Please enter the ACCESS KEY ID for your Admin AWS IAM User: `.yellow,
              required: true,
        ...
        .then(function(answers) {
          ...
          return _this.cliPromptSelect('Do you want to use this profile only for this stage?: ', choices, false)
            .then(results => {
    ...
    _createStage() {
      ...
      stage.getVariables().updateConfig({
        filePath: project.getFilePath('_meta', 'variables', `s-variables-${this.evt.options.stage}.json`)
      });

      ...
      return BbPromise.try(function() {
            ...
            prompts.properties.stage = {
              ...
              conform:     function(stage) {
                return _this.S.classes.Stage.validateStage(stage);
              }
            };
            ...
            return _this.cliPromptSelect(`For the "${_this.evt.options.stage}" stage, do you want to use an existing ${_this.provider.getProviderName()} profile or create a new one?`, choices, false)
                .then(function(values) {
                  ...
                  prompts.properties.profile = {
                    description: 'Enter the name of the new profile: '.yellow,
                    default: (_this.project.getName()).toLowerCase(),
                    required: true,
...
console.log
    ...
    _validateAndPrepare() {
      ...
      // Validate Stage
      if (!this.S.classes.Stage.validateStage(this.evt.options.stage)) {
      ...
      // set API keys to current runtime
      ...
      // Write to admin.env
      let adminEnv = this.S.getProject().getRootPath('admin.env'),
          profileEnvVar = 'AWS_PROFILE_' + this.evt.options.stage.toUpperCase();
    ...
    _createStage() {
      this.stage = new this.S.classes.Stage(this.S, this.project, {name: this.evt.options.stage});

...
module.exports = function(SPlugin, serverlessPath) {
  ...
  class ProjectCreate extends SPlugin {
    ...
    createProject(evt) {
      ...
      this.project.data.name              = this.evt.options.name || this.project.data.name;
      this.project.data.notificationEmail = this.evt.options.notificationEmail || this.project.data.notificationEmail;
    ...
    _createProjectScaffolding() {
      ...
      _this.S.updateConfig({
        projectPath: path.resolve(path.join(path.dirname('.'), _this.project.data.name))
      });
      ...
      return SUtils.writeFile(
        ...
        .then(function() {
          ...
          fs.mkdirSync(path.join(_this.S.config.projectPath, 'meta', 'private', 'variables'));
          fs.mkdirSync(path.join(_this.S.config.projectPath, 'meta', 'public', 'variables'));
          fs.mkdirSync(path.join(_this.S.config.projectPath, 'meta', 'private', 'resources'));
          fs.mkdirSync(path.join(_this.S.config.projectPath, 'plugins'));
          fs.mkdirSync(path.join(_this.S.config.projectPath, 'plugins', 'custom'));

          return BbPromise.all([
            SUtils.writeFile(path.join(_this.S.config.projectPath, 'admin.env'), adminEnv),
            SUtils.writeFile(path.join(_this.S.config.projectPath, 'README.md'), readme),
            fs.writeFileAsync(path.join(_this.S.config.projectPath, '.gitignore'), fs.readFileSync(path.join(_this._templatesDir, 'gitignore')))
          ]);
        })
        .then(function() {
          ...
          _this.meta.data.private.variables.domain            = _this.evt.options.domain;
          _this.meta.data.private.variables.notificationEmail = _this.evt.options.notificationEmail;
          _this.meta.save();
    ...
    _createStageAndRegion() {
      ...
      return this.S.actions.stageCreate(newEvt);
    }
    ...
    _deployResources() {
      ...
      return this.S.actions.resourcesDeploy(newEvt);
    }
  }

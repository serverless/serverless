...
module.exports = function(SPlugin, serverlessPath) {
  ...
  class ProjectCreate extends SPlugin {
    ...
    _createProjectScaffolding() {
      ...
      let _this = this;
      let projectPath = path.resolve(path.join(path.dirname('.'), _this.project.data.name));
      ...
      return SUtils.writeFile(
        ...
        .then(function() {
          ...
          // Create Folders
          fs.mkdirSync(path.join(projectPath, '_meta'));
          fs.mkdirSync(path.join(projectPath, '_meta', 'variables'));
          fs.mkdirSync(path.join(projectPath, '_meta', 'resources'));

          return BbPromise.all([
            SUtils.writeFile(path.join(projectPath, 'admin.env'), adminEnv),
            SUtils.writeFile(path.join(projectPath, 'README.md'), readme),
            fs.writeFileAsync(path.join(projectPath, '.gitignore'), fs.readFileSync(path.join(_this._templatesDir, 'gitignore')))
          ]);
        })
        .then(function() {
          ...
          _this.meta.data.variables.project           = _this.project.data.name;
          _this.meta.data.variables.projectBucket     = _this.projectBucket;
          _this.meta.data.variables.domain            = _this.evt.options.domain;
          _this.meta.data.variables.notificationEmail = _this.evt.options.notificationEmail;
          _this.meta.save();

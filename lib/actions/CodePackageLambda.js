...
module.exports = function(SPlugin, serverlessPath) {
  ...
  class Packager {
    ...
    _validateAndPrepare() {
      ...
      _this.function = _this.S.getProject().getFunction( _this.evt.options.functionName );

      if (!_this.function) BbPromise.reject(new SError(`Function could not be found: ${_this.evt.options.functionName}`));
    ...
    _createDistFolder() {
      ...
      // Set Dist Dir
      let d          = new Date();
      _this.pathDist = path.join(_this.S._projectPath, '_meta', '_tmp', _this.function.name + '@' + d.getTime());
      ...
      let key  = ['serverless', _this.project.name, _this.evt.options.stage, _this.evt.options.region, 'envVars', '.env'].join('/'),
        s3Region = _this.project.getVariablesObject().projectBucket.split('.')[1],
        params = {

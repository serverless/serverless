...
usage: serverless component create`,
    ...
    _createComponentSkeleton() {
      ...
      let _this         = this,
          writeDeferred = [],
          component     = new _this.S.classes.Component(_this.S, {
      ...
      // Prep package.json
      packageJsonTemplate.name        = _this.evt.options.component;
      packageJsonTemplate.description = 'Dependencies for a Serverless Component written in Node.js';

      // Write base module structure
      writeDeferred.push(
        fs.mkdirSync(path.join(_this.S.config.projectPath, _this.evt.options.component)),
        fs.mkdirSync(path.join(_this.S.config.projectPath, _this.evt.options.component, 'lib')),
        SUtils.writeFile(path.join(_this.S.config.projectPath, _this.evt.options.component, 'lib', 'index.js'), libJs),
        SUtils.writeFile(path.join(_this.S.config.projectPath, _this.evt.options.component, 'package.json'), JSON.stringify(packageJsonTemplate, null, 2)),
        component.save()
      );

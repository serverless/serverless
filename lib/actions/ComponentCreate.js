    ...
    _scaffold() {
      ...
      return BbPromise.try(function() {
        ...
        if (_this.evt.options.runtime === 'nodejs') {
          let packageJsonTemplate = SUtils.readAndParseJsonSync(path.join(_this.S.getServerlessPath(), 'templates', 'nodejs', 'package.json')),
            libJs = fs.readFileSync(path.join(_this.S.getServerlessPath(), 'templates', 'nodejs', 'index.js'));

          writeDeferred.push(
            SUtils.writeFile(_this.S.getProject().getFilePath(_this.evt.options.name, 'lib', 'index.js'), libJs),
            SUtils.writeFile(_this.S.getProject().getFilePath(_this.evt.options.name, 'package.json'), JSON.stringify(packageJsonTemplate, null, 2))
          );
          ...
          writeDeferred.push(
            SUtils.writeFile(_this.S.getProject().getFilePath(_this.evt.options.name, 'lib', '__init__.py'), initPy),
            SUtils.writeFile(_this.S.getProject().getFilePath(_this.evt.options.name, 'vendored', '__init__.py'), blankInitPy),
            SUtils.writeFile(_this.S.getProject().getFilePath(_this.evt.options.name, 'requirements.txt'), requirements)
          );
        }

...
usage: serverless component create`,
    ...
    _validateAndPrepare() {
      ...
      if (SUtils.doesComponentExist(this.evt.options.name, this.S.config.projectPath)) {
    ...
    _installComponentDependencies() {
      ...
      if (_this.evt.options.runtime === 'nodejs') {
        ...
        SCli.log(`-----------------`);
        SUtils.npmInstall(path.join(this.S.config.projectPath, this.evt.options.name));
        SCli.log(`-----------------`);
        ...
        SUtils.pipPrefixInstall(
          path.join(this.S.config.projectPath, this.evt.options.name, 'requirements.txt'),
          path.join(this.S.config.projectPath, this.evt.options.name, 'vendored')
        );
        SCli.log(`-----------------`);
      }
      return BbPromise.resolve();

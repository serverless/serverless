 ...
 * - takes component and function names
...
usage: serverless component create`,
    ...
    _prompt() {
      ...
      ['component', 'function'].forEach(memberVarKey => {
      ...
      let prompts = {
        properties: {
          ...
          function: {
            default:     'show',
            description: 'Enter a name for your component\'s first function: '.yellow,
      ...
      return _this.cliPromptInput(prompts, overrides)
        .then(function(answers) {
          ...
          return _this.cliPromptSelectRuntime('Select a runtime for your new component: ', _this.evt.options.runtime)
            .then(runtime => {
              _this.evt.options.runtime = runtime;
              BbPromise.resolve();
            });
        });
    };
    ...
    _createFunctionSkeleton(component) {

      let _this = this,
        newEvt = {
          options: {
            //sPath: component._config.sPath + "/" + _this.evt.options.function
    ...
    _installComponentDependencies() {
      ...
      if (_this.evt.options.runtime === 'nodejs') {
        ...
        SUtils.pipPrefixInstall(
          path.join(this.S.config.projectPath, this.evt.options.component, 'requirements.txt'),
          path.join(this.S.config.projectPath, this.evt.options.component, 'vendored')
        );
        SCli.log(`-----------------`);
      }
      return BbPromise.resolve();

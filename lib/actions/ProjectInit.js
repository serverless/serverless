  ...
  class ProjectInit extends SPlugin {
    ...
    _scaffold() {
      ...
      return BbPromise.try(function() {
          ...
          _this.project.cloudFormation = SUtils.readAndParseJsonSync(path.join(_this.S.config.serverlessPath, 'templates', 's-resources-cf.json'));

          // Create s-resources-cf.json
          if (!_this.project.cloudFormation &&
            !SUtils.fileExistsSync(path.join(

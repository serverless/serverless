...
module.exports   = function(S) {
  ...
  const path     = require('path'),
      SError     = require( path.join(S.getServerlessPath(), 'Error')),
      SCli       = require( path.join(S.getServerlessPath(), 'utils/cli')),
  ...
  class ProjectInit extends S.classes.Plugin {
    ...
    _prompt() {
      ...
      // Set temp name
      if (S.hasProject()) {
        name = _this.evt.options.name ? _this.evt.options.name : (S.getProject().getName() + '-' + S.utils.generateShortId(6)).toLowerCase();
      } else {
    ...
    _scaffold() {
      ...
      return BbPromise.try(function() {
            ...
            if( !S.hasProject() ) {
              ...
              S.setProject(new S.classes.Project(S));
            }

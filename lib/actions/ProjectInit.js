...
module.exports = function(SPlugin, serverlessPath) {
  ...
  class ProjectInit extends SPlugin {
    ...
    _prompt() {
      ...
      if(_this.evt.options.awsAdminSessionToken) { // TODO: is this appropriate?  Would they ever supply these on the command line?  Should we provide the opportunity to provide a profile instead of values?
        overrides.awsAdminSessionToken = _this.evt.options.awsAdminSessionToken;
      }
      ...
      return this.cliPromptInput(prompts, overrides)
        .then(function(answers) {
          ...
          if(answers.awsAdminSessionToken) {
            _this.S.config.awsAdminSessionToken = answers.awsAdminSessionToken; // TODO: is this appropriate?  Would this ever be provided, should we allow the provision of a profile?
          }
    ...
    _validateAndPrepare() {
      ...
      this.AwsMisc = require('../utils/aws/Misc'); // TODO why add this, why not use the module global version?

      // If Profile, extract API Keys
      if (this.evt.options.profile) {
        // TODO should this mirror the behavior of ~/lib/Serverless.js, line 96 if profile loading fails?
        AwsMisc.addProfileCredentials(this.S.config, this.evt.options.profile);
      }
      // TODO should this mirror the behavior of ~/lib/Serverless.js, line 99 with an else?
      ...
      awsConfig.region = this.evt.options.region;
      ...
      return BbPromise.try(function() {
        ...
        .then(function() {
          ...
          if (!SUtils.fileExistsSync(_this.project.getFilePath( 'admin.env' ))) {
            ...
            if(_this.S.config.awsAdminSessionToken) {
              adminEnv  += 'SERVERLESS_ADMIN_AWS_SESSION_TOKEN=' + _this.S.config.awsAdminSessionToken + os.EOL;
            }

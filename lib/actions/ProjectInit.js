...
module.exports = function(SPlugin, serverlessPath) {
  ...
  class ProjectInit extends SPlugin {
    ...
    projectInit(evt) {
      ...
      let profilesList = this.S.getProvider().getAllProfiles();
    ...
    _prompt() {
      ...
      if (_this.S.hasProject()) {
        name = _this.evt.options.name ? _this.evt.options.name : (_this.S.getProject().getName() + '-' + this.utils.generateShortId(6)).toLowerCase();
      } else {
        name = _this.evt.options.name ? _this.evt.options.name : ('serverless-' + this.utils.generateShortId(6)).toLowerCase();
      }
      ...
      let prompts = {
        properties: {
          ...
          bucket:            {
            ...
            default:     isName ? (name + '-' + this.utils.generateShortId(6)).toLowerCase() + '.com' : name + '.com',
      ...
      return this.cliPromptInput(prompts, overrides)
        .then(function(answers) {
          ...
          // If API keys entered, create profile on local machine
          ...
          let choices = _this.S.getProvider('aws').validRegions.map(r => {
            return {
              key:   '',
              value: r,
              label: r
    ...
    _scaffold() {
      ...
      return BbPromise.try(function() {
          ...
          _this.templates        = _this.project.getTemplates();
          _this.templates.updateConfig({
            filePath: _this.project.getFilePath('s-templates.json')
          });

          _this.projectVariables = _this.project.getVariables();
          _this.projectVariables.updateConfig({
            filePath: _this.project.getFilePath('_meta', 'variables', 's-variables-common.json')
          });
          _this.projectVariables.fromObject({
          ...
          if (!_this.project.cloudFormation && // TODO: Remove Back-compat support - In case initializing older project w/ cloudFormation property
            !_this.utils.fileExistsSync( _this.project.getFilePath( 's-resources-cf.json' ))) {
            files.push(
              _this.utils.writeFile(
                _this.project.getFilePath(  's-resources-cf.json' ),
                JSON.stringify(
                  _this.utils.readFileSync(path.join(_this.S.getServerlessPath(), 'templates', 's-resources-cf.json')),
                  null, 2)
              ));
          }

          _this.projectResources = _this.project.getResources();
          _this.projectResources.updateConfig({
            filePath: _this.project.getFilePath('s-resources-cf.json')
          });
        ...
        .then(function() {
          ...
          if (!_this.utils.fileExistsSync(_this.project.getFilePath( 'admin.env' ))) {
          ...
          let packageJson;
          if (!_this.utils.fileExistsSync(_this.project.getFilePath( 'package.json' ))) {

            // Prepare new package.json
            packageJson               = _this.utils.readFileSync(path.join(_this.S.config.serverlessPath, 'templates', 'nodejs', 'package.json'));
            ...
            packageJson       = _this.utils.readFileSync(_this.project.getFilePath( 'package.json' ));
            packageJson.name  = _this.project.getName();
          ...
          if (!_this.utils.fileExistsSync(_this.project.getFilePath( 'README.md' ))) {
          ...
          if (!_this.utils.fileExistsSync(_this.project.getFilePath( '.gitignore' ))) {
          ...
          if (!_this.utils.fileExistsSync(_this.project.getFilePath( '.env' ))) {
            files.push(_this.utils.writeFile(

...
class Builder {
  ...
  _getApiResources(evt) {
    ...
    return _this.ApiGateway.getResourcesPromised(params)
        .then(function(response) {
          ...
          //    evt.resourceParent = evt.apiResources[i];
          //  }
          //}
  ...
  _createEndpointResources(evt) {
    ...
    let findParent = function(resource) {
      ...
      for (let i = 0; i < evt.apiResources.length; i++) {
        if (evt.apiResources[i].path === parentPath) {
          evt.resourceParent = evt.apiResources[i];
          continue;
        }
      }

      return evt.resourceParent;
    };


    // Check paths to see if resources need building
    for (let i = 0; i < evt.apiResources.length; i++) {
      if (evt.apiResources[i].path === evt.endpoint.path) {
        evt.resource = evt.apiResources[i];
        break;
      }
    }
    ...
    return new BbPromise(function(resolve, reject) {
      ...
      async.eachSeries(eResources, function(eResource, cb) {
        ...
        for (let i = 0; i < evt.apiResources.length; i++) {
          ...
          if (incrementedPath.indexOf(evt.apiResources[i].path) !== -1) {
            if (evt.apiResources[i].path.length > parentPath.length) {
              evt.resourceParent = evt.apiResources[i];
            }
          }

          // Resource exists, save it to Event object, break loop
          if (evt.apiResources[i].path === incrementedPath) {
            resourceExists = true;
            continue;
          }
        }
        ...
        let params = {
          parentId:  evt.resourceParent.id, /* required */

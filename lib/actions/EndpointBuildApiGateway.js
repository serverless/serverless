...
module.exports = function(S) {
  ...
  class Builder {
    ...
    _validateAndPrepare() {
      ...
      if (!_this.endpoint.methodRequestParameters) {
        throw new SError('Endpoint does not have a "methodRequestParameters" property');
      }
      if (!_this.endpoint.integrationRequestParameters) {
        throw new SError('Endpoint does not have a "integrationRequestParameters" property');
      }
    ...
    _fetchDeployedLambda() {
      ...
      return _this.aws.request('Lambda', 'getFunction', params, _this.evt.options.stage, _this.evt.options.region)
        .then(function(data) {
          ...
          // Prepare StatementId
          _this.lambdaPolicyStatementId = ('s_apig-' + _this.endpoint.path + '-' + _this.endpoint.method + "-" + _this.evt.options.stage)
    ...
    _createEndpointMethod() {
      ...
      if (!_.isEmpty(_this.endpoint.methodRequestParameters)) {
        requestParameters = _this.endpoint.methodRequestParameters;
      }
      else if(!_.isEmpty(_this.endpoint.requestParameters)) {
        // Handle the deprecated definition as `requestParameters`
        SCli.log('WARNING: Use of deprecated `requestParameters` definition in APIG endpoints. Use `methodRequestParameters` and `integrationRequestParameters` instead!');
    ...
    _createEndpointIntegration() {
      ...
      let requestParameters = _this.endpoint.integrationRequestParameters;
      if (_.isEmpty(requestParameters) && !_.isEmpty(_this.endpoint.requestParameters)) {
        SCli.log('WARNING: Use of deprecated `requestParameters` definition in APIG endpoints. Use `methodRequestParameters` and `integrationRequestParameters` instead!');
    ...
    _addLambdaPermissionForEndpoint() {
      ...
      let params = {
        FunctionName: _this.deployedLambda.FunctionArn,
        StatementId: _this.lambdaPolicyStatementId,

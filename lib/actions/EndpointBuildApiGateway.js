...
module.exports = function(SPlugin, serverlessPath) {
  ...
  class Builder {
    ...
    _validateAndPrepare() {
      ...
      if (!_this.project.getVariablesObject(_this.evt.options.stage, _this.evt.options.region).iamRoleArnLambda) {
        throw new SError('No Lambda IAM Role found');
      }

      // Define useful variables
      _this.awsAccountNumber     = _this.project.getVariablesObject(_this.evt.options.stage, _this.evt.options.region).iamRoleArnLambda.replace('arn:aws:iam::', '').split(':')[0];
      _this.restApiName          = _this.project.getVariablesObject(_this.evt.options.stage, _this.evt.options.region).apiGatewayApi;
      _this.resource             = null;
      ...
      _this.endpointPath = _this.evt.options.endpointName.split('#')[0];
      _this.endpointMethod = _this.evt.options.endpointName.split('#')[1];
      _this.endpoint = _this.project.getEndpoint( _this.endpointPath, _this.endpointMethod );

      if (!_this.endpoint) BbPromise.reject(new SError(`Endpoint could not be found: ${_this.evt.options.endpointName}`));

...
class EndpointBuildApiGateway extends SPlugin {
  ...
  _createEndpointIntegration(evt) {
    ...
    if (!evt.function.cloudFormation.lambda) {
      return BbPromise.reject(new SError('API Gateway integration currently works with Lambdas only.'));
    }
    ...
    if (!lambda) {
      return BbPromise.reject(new SError('Could not find a lambda deployed in this stage/region with this function name: '
          + cfLogicalResourceId));
    }

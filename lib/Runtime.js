...
module.exports = function(S) {
  ...
  class Runtime {
     ...
     * - If deploy is set to true, only variables not supported by AWS Lambda environment are set
     */

    getEnvVars(func, stage, region, deploy) {
      ...
      let result = _.defaults(defaultVars, envVars);

      if (deploy === true) {
        S.utils.sDebug(`"${stage} - ${region} - ${func.getName()}": Stripping supported env vars ...`);

        const strippedResult = {};

        // Remove variables set with AWS Lambda environment

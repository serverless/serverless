...
class ServerlessState {
  ...
  setAsset(data) {
    if (data instanceof this._S.classes.Project) {
      ...
      this.project.components[data._config.component].functions[data.name] = data;
    } else if (data instanceof this._S.classes.Endpoint) {
      let func = this.project.components[data._config.component].functions[data._config.function];
      let added = false;

  ...
  getComponents(options) {
    ...
    for (let i = 0; i < allComponents.length; i++) {
      ...
      if (options.component) {
        if (component._config.sPath.split('/')[0] == options.component) { // XXX
          foundComponents.push(options.returnPaths ? component._config.sPath : component);   // XXX
        }
        continue;
      }

      for (let j = 0; j < options.paths.length; j++) {
        if (options.paths[j].indexOf(component._config.sPath) !== -1) {        // XXX
          foundComponents.push(options.returnPaths ? component._config.sPath : component);   // XXX
          break;
        }
      }

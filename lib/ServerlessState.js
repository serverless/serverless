...
class ServerlessState {
  ...
  getEvents(options) {
    ...
    // Return all events if no options specified
    if (!options) return allEvents;

    // Return all event paths if only returnPaths is specified
    if (options && Object.keys(options).length === 1 && options.returnPaths === true) return allEvents.map(function(e) { return e._config.sPath });

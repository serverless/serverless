...
class ServerlessState {
  ...
  load() {
    this.data = {
      meta:     new this.S.classes.Meta(this.S),
  ...
  set(data) {
    this.data.meta.data    = _.merge(this.data.meta.data, data.meta);
    this.data.project.data = _.merge(this.data.project.data, data.project);
  }
  ...
  get(options) {
    ...
    if (options.populate && options.state && options.region) {
      return {
        meta:    _.cloneDeep(this.data.meta.data),
        project: _.cloneDeep(SUtils.populate(this.data.meta.data, this.data.project.data, options.state, options.region))
      }
    } else {
      return {
        meta:    _.cloneDeep(this.data.meta.data),
        project: _.cloneDeep(this.data.project.data)
      }
    }
  }
  ...
  getResources(options) {
    ...
    if (options.populate) {
      return SUtils.getResources(this.get(options.populate, options.state, options.region).project);
    } else {
  ...
  getStages() {
    return Object.keys(this.data.meta.data.stages);
  }
  ...
  getRegions(stage) {
    return Object.keys(this.data.meta.data.stages[stage].regions);
  }
  ...
  getComponents(options) {
    ...
    for (let i = 0; i < Object.keys(_this.data.project.data.components).length; i++) {
      ...
      let componentName = Object.keys(_this.data.project.data.components)[i];
      ...
      components.push(_.cloneDeep(_this.data.project.data.components[componentName]));
    }

...
class ServerlessMeta
{
  ...
  load() {
    ...
    if (!SUtils.fileExistsSync(path.join(_this._S.config.projectPath, '_meta', 'variables'))) {
  ...
  getRegions(stage) {
    ...
    if (!stage) throw new SError('Stage is required');
  ...
  save() {
    ...
    for (let i = 0; i < Object.keys(clone.stages).length; i++) {
      ...
      for (let j = 0; j < Object.keys(stage.regions).length; j++) {
        fs.writeFileSync(path.join(_this._S.config.projectPath, '_meta', 'variables', 's-variables-' + Object.keys(clone.stages)[i] + '-' + Object.keys(clone.regions)[j].replace(/-/g, '') + '.json'),

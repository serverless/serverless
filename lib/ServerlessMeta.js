...
class ServerlessMeta {
    ...
    let _getVariables = function(type) {
      ...
      for (let i = 0; i < variableFiles.length; i++) {
        ...
        if (file === 'common') {
          ...
          _this.data[type].variables = SUtils.readAndParseJsonSync(path.join(_this.options.projectPath, 'meta', type, 'variables', variableFiles[i]));
          ...
          if (file.length === 1) {
            ...
            // Set Stage Variables
            _this.data[type].stages[file[0]].variables = SUtils.readAndParseJsonSync(path.join(_this.options.projectPath, 'meta', type, 'variables', variableFiles[i]));
            ...
            if (!_this.data[type].stages[file[0]].regions[region]) _this.data[type].stages[file[0]].regions[region] = {
              variables: SUtils.readAndParseJsonSync(path.join(_this.options.projectPath, 'meta', type, 'variables', variableFiles[i]))
            };
          }
        }
      }
  ...
  save() {
    ...
    let _saveVariables = function(type) {
      ...
      fs.writeFileSync(path.join(_this.options.projectPath, 'meta', type, 'variables', 's-variables-common.json'),
      ...
      if (type === 'private') {
        ...
        for (let i = 0; i < Object.keys(_this.data[type].stages).length; i++) {
          ...
          fs.writeFileSync(path.join(_this.options.projectPath, 'meta', type, 'variables', 's-variables-' + Object.keys(_this.data[type].stages)[i] + '.json'),
            JSON.stringify(stage.variables, null, 2));

          // Save Stage Region Variables
          for (let j = 0; j < Object.keys(stage.regions).length; j++) {
            fs.writeFileSync(path.join(_this.options.projectPath, 'meta', type, 'variables', 's-variables-' + Object.keys(_this.data[type].stages)[i] + '-' + Object.keys(stage.regions)[j].replace(/-/g, '') + '.json'),
    ...
    if (SUtils.dirExistsSync(path.join(_this.options.projectPath, 'meta', 'public'))) _saveVariables('public');
    if (SUtils.dirExistsSync(path.join(_this.options.projectPath, 'meta', 'private'))) _saveVariables('private');
  }

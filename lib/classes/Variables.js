...
const { extractServiceFromStateVar } = require('../app/utils');
...
class Variables {
  constructor(serverless) {
    ...
    this.selfRefSyntax = RegExp(/^state:/g);
  ...
  getValueFromSource(variableString, propertyString) {
    ...
    if (this.tracker.contains(variableString)) {
      ...
      } else if (variableString.match(/state/)) {
        ret = this.getValueFromState(variableString);
      } else {
        const errorMessage = [
  ...
  getValueFromState(variableString) {
    const service = extractServiceFromStateVar(variableString);
    const variable = variableString.split('.').pop();
    ...
    try {
      cont = this.serverless.utils.readFileSync(stateFilePath);
    } catch (error) {
      // fail silently
    }

    return BbPromise.resolve(cont[variable]);
  }

...
class Variables {
  constructor(serverless) {
    ...
    this.variableResolvers = [
      ...
      { regex: this.fileRefSyntax, method: this.getValueFromFile.bind(this) },
      {
        ...
        method: this.getValueFromCf.bind(this),
        isDependentService: true,
        name: 'CloudFormation',
      },
      {
        regex: this.s3RefSyntax,
        method: this.getValueFromS3.bind(this),
        isDependentService: true,
        name: 'S3',
      },
      { regex: this.stringRefSyntax, method: this.getValueFromString.bind(this) },
      {
        ...
        method: this.getValueFromSsm.bind(this),
        isDependentService: true,
        name: 'SSM',
      },
  ...
  disableDepedentServices(func) {
    ...
    // these services.
    for (const i in this.variableResolvers) {
      if (this.variableResolvers[i].isDependentService) {
        // save original
        this.variableResolvers[i].original = this.variableResolvers[i].method;
        // knock out
        this.variableResolvers[i].method = variableString =>
          BbPromise.reject(dependencyMessage(variableString, this.variableResolvers[i].name));
      }
    }
    return func().finally(() => {
      // restore
      for (const i in this.variableResolvers) {
        if (this.variableResolvers[i].isDependentService) {
          this.variableResolvers[i].method = this.variableResolvers[i].original;
        }
      }
    });
  }

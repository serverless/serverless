...
class Variables {
  ...
  renderMatches(value, matches, results) {
    ...
    for (let i = 0; i < matches.length; i += 1) {
      this.assertFound(matches[i].variable, results[i]);
  ...
  _describeVariableType(variableString) {
  ...
  assertFound(variableString, valueToPopulate) {
    if (!this.serverless.config.throwOnUnresolvedVariableReferences) {
    ...
    if (
      ...
      const varType = this._describeVariableType(variableString);

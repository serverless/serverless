  ...
  populateService(processedOptions) {
    ...
    traverse(this.service).forEach(function (propertyParam) {
      ...
      if (typeof property === 'string') {
        const escapePattern = RegExp(RegExp(variableSyntaxProperty).toString().replace('\\$', '\\$\\$').slice(1,-1));
        const escapeRegex = RegExp(escapePattern);
        // const escapes = property.match(/\$(\$\{.*\})/);
        const escapes = property.match(escapeRegex);
        if (escapes && escapes.length > -1) {

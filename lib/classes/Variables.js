...
class Variables {
  constructor(serverless) {
    ...
    this.variableResolvers = [
      { regex: this.slsRefSyntax, method: this.getValueFromSls.bind(this) },
      { regex: this.envRefSyntax, method: this.getValueFromEnv.bind(this) },
      { regex: this.optRefSyntax, method: this.getValueFromOptions.bind(this) },
      { regex: this.selfRefSyntax, method: this.getValueFromSelf.bind(this) },
      { regex: this.fileRefSyntax, method: this.getValueFromFile.bind(this) },
      { regex: this.cfRefSyntax, method: this.getValueFromCf.bind(this) },
      { regex: this.s3RefSyntax, method: this.getValueFromS3.bind(this) },
      { regex: this.stringRefSyntax, method: this.getValueFromString.bind(this) },
      { regex: this.ssmRefSyntax, method: this.getValueFromSsm.bind(this) },
      { regex: this.deepRefSyntax, method: this.getValueFromDeep.bind(this) },
    ];
    this.dependentServices = [
      { name: 'CloudFormation', resolver: this.variableResolvers[4] },
      { name: 'S3', resolver: this.variableResolvers[5] },
      { name: 'SSM', resolver: this.variableResolvers[6] },
    ];
  }
  ...
  disableDepedentServices(func) {
    ...
    this.dependentServices.forEach(dependentService => {
      // save original
      dependentService.originalResolver = dependentService.resolver.method
      // knock out
      dependentService.resolver.method = variableString => BbPromise.reject(dependencyMessage(variableString, dependentService.name));
    });
    return func().finally(() => {
      this.dependentServices.forEach(dependentService => {
        // restore
        dependentService.resolver.method = dependentService.originalResolver
      });
    });
  }
  prepopulateService() {
  ...
  getValueFromSource(variableString, propertyString) {
    ...
    if (this.tracker.contains(variableString)) {
      ...
      for (const { regex, method } of this.variableResolvers) {
        if (variableString.match(regex)) {
          ret = method(variableString);
          break;
        }
      }
      if (!ret) {

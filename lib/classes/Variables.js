...
class Variables {
   ...
   */
  populateObject(objectToPopulate) { console.log(`populateObject: enter ${objectToPopulate}`)
    ...
    return this.assignProperties(objectToPopulate, populations)
      .then((changes) => { console.log(`populateObject: changes ${changes}`)
        if (changes) {
   ...
   */
  populateMatches(matches) { console.log(`populateMatches: enter ${matches}`)
    return _.map(matches, (match) => {
   ...
   * is true
   */
  populateValue(valueToPopulate, root) { console.log(`populateValue: enter ${valueToPopulate}, ${root}`)
    ...
    return BbPromise.all(populations)
      ...
      .then((result) => {
        console.log(`populateValue: result ${result}`)
   ...
   */
  getValueFromSource(variableString, propertyString) { console.log(`getValueFromSource: enter ${variableString}, ${propertyString}`)
    let ret;
    if (this.tracker.contains(variableString)) {
      ...
      if (variableString.match(this.envRefSyntax)) {
        ...
        console.log('before getValueFromFile')
        ret = this.getValueFromFile(variableString);
        console.log('after getValueFromFile')
    ...
    } console.log(`getValueFromSource: exit ${variableString}, ${propertyString}`)
    return ret;
  }
  ...
  getValueFromFile(variableString) { console.log(`getValueFromFile: enter ${variableString}`)
...
console.log('getValueFromFile: 1')
    let valueToPopulate;
...
console.log(`getValueFromFile: ${jsFile}`)
      const variableArray = variableString.split(':');
console.log(`getValueFromFile: ${variableArray}`)
...
console.log(`getValueFromFile: ${returnValueFunction}`)
...
console.log(`getValueFromFile: valueToPopulate ${valueToPopulate}`)
      return BbPromise.resolve(valueToPopulate).then((valueToPopulateResolved) => {
console.log(`getValueFromFile: valueToPopulateResolved ${valueToPopulateResolved}`)
        let deepProperties = variableString.replace(matchedFileRefString, '');
        deepProperties = deepProperties.slice(1).split('.');
        deepProperties.splice(0, 1);
console.log(`getValueFromFile: deepProperties ${deepProperties}`)
        return this.getDeepValue(deepProperties, valueToPopulateResolved)
          .then((deepValueToPopulateResolved) => {
console.log(`getValueFromFile: deepValueToPopulateResolved ${deepValueToPopulateResolved}`)
...
console.log(`getDeepValue: enter ${deepProperties}, ${valueToPopulate}`)
    return BbPromise.reduce(deepProperties, (computedValueToPopulateParam, subProperty) => {
console.log(`reduce: enter ${computedValueToPopulateParam}, ${subProperty}`)
...
console.log(`reduce: computedValueToPopulate ${computedValueToPopulate}`)

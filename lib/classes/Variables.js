...
class Variables {
  constructor(serverless) {
    ...
    this.variableResolvers = [
      {regex: this.slsRefSyntax, funcName: 'getValueFromSls'},
      {regex: this.envRefSyntax, funcName: 'getValueFromEnv'},
      {regex: this.optRefSyntax, funcName: 'getValueFromOptions'},
      {regex: this.selfRefSyntax, funcName: 'getValueFromSelf'},
      {regex: this.fileRefSyntax, funcName: 'getValueFromFile'},
      {regex: this.cfRefSyntax, funcName: 'getValueFromCf'},
      {regex: this.s3RefSyntax, funcName: 'getValueFromS3'},
      {regex: this.stringRefSyntax, funcName: 'getValueFromString'},
      {regex: this.ssmRefSyntax, funcName: 'getValueFromSsm'},
      {regex: this.deepRefSyntax, funcName: 'getValueFromDeep'},
    ];
  ...
  disableDepedentServices(func) {
    ...
    this.dependentServices.forEach(dependentService => {
      // save original
      dependentService.original = !dependentService.custom
        ? this[dependentService.method]
        : this.customVariableResolverFuncs[dependentService.method];
      // knock out
      if (!dependentService.custom) {
  ...
  getValueFromSource(variableString, propertyString) {
    ...
    if (this.tracker.contains(variableString)) {
      ...
      for (const {regex, funcName, custom} of this.variableResolvers) {
        if (variableString.match(regex)) {
          if (!custom) {
            ret = this[funcName].bind(this)(variableString);
          } else {
            ret = this.customVariableResolverFuncs[funcName](variableString);
          }
          break;
        }
      }

...
class Variables {
  ...
  disableDepedentServices(func) {
    ...
    // these services.
      ...
      if (resolver.isDependentService) {
        // save original
        resolver.original = resolver.method;
        // knock out
        resolver.method = variableString =>
          BbPromise.reject(dependencyMessage(variableString, resolver.name));
      }
    }
    return func().finally(() => {
      // restore
      for (const resolver of this.variableResolvers) {
        if (resolver.isDependentService) {
          resolver.method = resolver.original;
        }
      }
    });
  }

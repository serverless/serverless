...
const lifecycleLog = require('log').get('serverless:lifecycle')
...
class PluginManager {
  ...
  invoke(commandsArray, allowEntryPoints) {
    ...
    this.registerCommandInvokation(commandsArray.join(':'), events);
  ...
  registerCommandInvokation(commandName, events) {
    this.debug.previousPlugin = this.debug.currentPlugin;
    this.debug.previousEvent = this.debug.currentEvent;

    this.debug.push({
      commandName,
      triggeredBy: this.debug.currentPlugin || 'console',
      on: this.debug.currentEvent || 'console',
      events,
    });
  }

  registerCommandCompletion() {
    this.debug.currentPlugin = this.debug.previousPlugin;
    this.debug.currentEvent = this.debug.previousEvent;
  }

  registerHook(hook) {
    this.debug.currentPlugin = hook.pluginName;
    this.debug.currentEvent = hook.hook.name;
  }

  displayRecordedCommandLifecycleInformation(commandName, event, pluginName, tab = 1) {
    const recordedCommand = _.find(this.debug, command => {
      return (
        commandName === command.commandName &&
        event === command.on &&
        pluginName === command.triggeredBy
      );
    });
    lifecycleLog.debug('Invoking command %s', recordedCommand.commandName);
    this.serverless.cli.consoleLog(
      chalk.yellow(`${Array(tab).join('  |')}_ Invoking command ${recordedCommand.commandName}`)
    );
    const hooksByEvent = {};
    this.getHooks(recordedCommand.events).forEach(hook => {
      if (!_.isEmpty(hooksByEvent[hook.hook.name])) {
        hooksByEvent[hook.hook.name].push(hook);
        return;
      }
      hooksByEvent[hook.hook.name] = [hook];
    });
    recordedCommand.events.forEach(commandEvent => {
      const lifecycleInformation = `${Array(tab + 1).join('  |')}_ Event ${commandEvent}`;
      if (!hooksByEvent[commandEvent]) {
        this.serverless.cli.consoleLog(`${chalk.yellowBright(lifecycleInformation)} -- No hook`);
        return;
      }
      this.serverless.cli.consoleLog(chalk.yellowBright(`${lifecycleInformation}`));
      hooksByEvent[commandEvent].forEach(hook => {
        this.serverless.cli.consoleLog(
          chalk.green(`${Array(tab + 2).join('  |')}_ Hook from ${hook.pluginName}`)
        );
        const commandsFromThisHook = _.filter(this.debug, command => {
          return hook.pluginName === command.triggeredBy && commandEvent === command.on;
        });
        commandsFromThisHook.forEach(command => {
          this.displayRecordedCommandLifecycleInformation(
            command.commandName,
            commandEvent,
            hook.pluginName,
            tab + 3

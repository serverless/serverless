...
class PluginManager {
  ...
  normalize() {
    // Fires a synthetic event ('_system_:normalize') which is intended to provide
    // plugins with the ability to transform the data structure into some
    // standard format. As no I/O is expected to take place during this phase,
    // this event, unlike other events, is expected to be synchronous.
    // Specifically, any promises returned from hooks that receive this events
    // are ignored.
    const hooks = this.hooks['_system_:normalize'] || [];
    hooks.forEach(curr => curr());
  }

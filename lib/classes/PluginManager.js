...
class PluginManager {
  ...
  createCommandAlias(alias, command) {
    ...
    if (command.startsWith(alias)) {
      throw new this.serverless.classes.Error(`Command "${alias}" cannot be overridden by an alias`);
    }
  ...
  getCommand(commandsArray, allowEntryPoints) {
    ...
    return _.reduce(
      ...
      (current, name, index) => {
        ...
        if (name.match(proTemplateMatch) && 'interactiveCli' in current.commands) {
          return current.commands.interactiveCli;
        }

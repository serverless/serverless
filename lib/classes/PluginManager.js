...
class PluginManager {
  ...
  invoke(commandsArray, allowEntryPoints) {
    ...
    this.validateConfigDependency(command);
  ...
  validateConfigDependency(command) {
    if (_.has(command, 'configDependent') && command.configDependent) {
      getServerlessConfigFile(process.cwd()).then((result) => {
        const isRunInService = !!result;

        if (!isRunInService) {
          throw new this.serverless.classes.Error('This command can only be run in a Serverless service directory');
        }
      });
    }
  }

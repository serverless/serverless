...
class PluginManager {
  ...
  loadCorePlugins() {
    ...
    const corePlugins = this.serverless.utils
      ...
      .plugins.map(corePluginPath => require(path.join(pluginsDirectoryPath, corePluginPath)));
  ...
  loadServicePlugins(servicePlugs) {
    ...
    this.loadPlugins(
      pluginsObject.modules
        ...
        .map(name => {
          const pluginPath = name.startsWith('./') ? path.join(servicePath, name) : name;
          let Plugin;
          try {
  ...
  loadEnterprisePlugin() {
    ...
    module.paths.unshift(path.join(__dirname, '../../node_modules'));
    this.loadPlugins([require('@serverless/enterprise-plugin')]);

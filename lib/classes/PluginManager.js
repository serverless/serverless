...
class PluginManager {
  ...
  loadVariableResolvers(pluginInstance) {
    ...
    for (const [variablePrefix, resolverOrOptions] of _.entries(
      ...
      if (!options) {
        this.serverless.cli.log(`Warning! Ignoring falsey variableResolver for ${variablePrefix} in ${pluginName}.`)
        continue
      }
      ...
      if (!_.isObject(options)) {
        ...
        throw new Error(
          `Custom variable resolver prefix ${variablePrefix} defined by ${pluginName} doesn't match against /[a-Z0-9_-]/`
        );
      }

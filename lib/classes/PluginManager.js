  ...
  loadPlugins(plugins) {
    plugins.forEach((plugin) => {
      try {
        ...
        const errorMessage = `Serverless plugin "${plugin}" not found.

  Make sure it's installed and listed correctly in plugins section in 'serverless.yml' file.`;

        if (!this.cliOptions.help) {
          throw new this.serverless.classes.Error(`ERROR: ${errorMessage}`);
        }

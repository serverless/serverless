...
class PluginManager {
  ...
  getModules(servicePlugs) {
    if (Array.isArray(servicePlugs)) {
      return servicePlugs;
    }
    if (servicePlugs && Array.isArray(servicePlugs.modules)) {
      return servicePlugs.modules;
    }

    return [];
  }

  getLocalPath(servicePlugs) {
    return this.getCustomLocalPath(servicePlugs) || this.getDefaultLocalPath();
  }
  ...
  getCustomLocalPath(servicePlugs) {
    return (
      servicePlugs &&
      servicePlugs.localPath &&
      typeof servicePlugs.localPath === 'string' &&
      servicePlugs.localPath
    );
  }

  parsePluginsObject(servicePlugs) {
    return { modules: this.getModules(servicePlugs), localPath: this.getLocalPath(servicePlugs) };
  }

  getLocalPluginsPaths(servicePlugs) {
    const localPluginsPaths = [];

    // Adding custom local path if provided
    //
    // plugins:
    //   localPath: './myPluginPath'
    //   modules: ['Plugin1']
    const customLocalPath = this.getCustomLocalPath(servicePlugs);
    if (customLocalPath) {
      localPluginsPaths.push(customLocalPath);
    }

    // Adding plugins with relative path references
    //
    // plugins: ['./myPluginDir/Plugin1']
    //
    // OR
    //
    // plugins:
    //   localPath: './myPluginPath'
    //   modules: ['Plugin1', './myPluginDir/Plugin2'] -> Plugin2 does not use localPath like Plugin1
    localPluginsPaths.push(...this.getModules(servicePlugs).filter(name => name.startsWith('./')));

    // Plugins loaded from .serverless_plugins folder are not compiled within this method
    return localPluginsPaths;
  }

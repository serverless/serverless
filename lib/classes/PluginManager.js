...
class PluginManager {
  ...
  updateAutocompleteCacheFile() {
    ...
    try {
      ...
      const generateAutoCompleteFile = path.join(__dirname, '..', '/utils/process/write-autocomplete.js'); // eslint-disable-line
      // Spawn a detached process, passing the options as an environment property
      spawn(process.execPath, [generateAutoCompleteFile, JSON.stringify(options)], {
        detached: true,
        stdio: 'ignore',
      }).unref();

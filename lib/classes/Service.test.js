...
describe('Service', () => {
  ...
  describe('#loadServiceFileParam', () => {
    let serverless;
    let serviceInstance;
    let consoleLogSpy;
    const serviceFilename = 'someFilename';

    beforeEach(() => {
      serverless = new Serverless();
      serviceInstance = new Service(serverless);
      serviceInstance.serverless.cli = new serverless.classes.CLI();
      consoleLogSpy = sinon.spy(serviceInstance.serverless.cli, 'consoleLog');
    });

    it('should show console log error when "plugins" param defined as string uses variables', () => {
      const serverlessFileParam = {
        service: 'some-service',
        plugins: '${file(./plugins.yml):plugins}',
        provider: { name: 'aws' },
      };

      serviceInstance.loadServiceFileParam(serviceFilename, serverlessFileParam);

      expect(consoleLogSpy.called).to.equal(true);
      const message = consoleLogSpy.args.join('\n');
      expect(message).to.have.string('Using variables in plugins section is not allowed');
    });

    it('should show console log error when "plugins" param defined as array uses variables', () => {
      const serverlessFileParam = {
        service: 'some-service',
        plugins: ['welcome', 'something-${file(./plugins.yml)}-added'],
        provider: { name: 'aws' },
      };

...
describe('Service', () => {
  ...
  describe('#load()', () => {
    ...
    xit('should translate HTTP events from string notation to object notation', () => {
      const SUtils = new Utils();
      const serverlessYml = {
        service: 'new-service',
        provider: 'aws',
      ...
      return serviceInstance.load().then(() => {
        expect(serviceInstance.functions.functionA.events[0].http)
          .to.deep.equal({ method: 'GET', path: 'bar' });
        expect(serviceInstance.functions.functionA.events[1].http)
          .to.deep.equal({ method: 'POST', path: 'foo' });
      });
    });

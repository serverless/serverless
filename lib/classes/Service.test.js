...
describe('Service', () => {
  ...
  describe('#load()', () => {
    ...
    it('should translate events from shorthand notation to full notation', () => {
      const SUtils = new Utils();
      const serverlessYml = {
        service: 'new-service',
        provider: 'aws',
      ...
      return serviceInstance.load().then(() => {
        expect(serviceInstance.functions.functionA.events[0].http).to.deep.equal({path: 'greet', method: 'GET'});
      });
    });

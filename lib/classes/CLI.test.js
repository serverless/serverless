...
describe('CLI', () => {
  ...
  describe('#isConfigIndependentRequest()', () => {
    it('should not load config file if running "config credentials" command', () =>
      runServerless({
        ...
        expect(serverless.service.service).to.equal(null);
      }));
    it('should load config file if running config dependent command', () =>
      runServerless({
        cwd: fixtures.map.configTypeYml,
        cliArgs: ['info'],
        // eslint-disable-next-line no-shadow
      }).then(({ serverless }) => {
        expect(serverless.service.service).to.equal('yml-service');
      }));
  });
...
describe('CLI [new tests]', () => {
  it('Should show help when requested and in context of errorneous service configuration', () =>
    runServerless({
      cwd: fixtures.map.configSyntaxError,
      cliArgs: ['--help'],
    }).then(({ stdoutData }) => {

...
describe('Variables', () => {
  ...
  describe('#prepopulateService', () => {
    ...
    describe('basic population tests', () => {
      ...
      it('should not populate variables containing *', () => {
        const property = { name: 'region', getter: (provider) => provider.getRegion() };

        awsProvider.options[property.name] = '${self:foobar, *.a.thing}';
        serverless.variables.populateService().should.be.fulfilled
          .then(() => expect(property.getter(awsProvider)).to.be.eql('${self:foobar, *.a.thing}'));
      });

      it('should populate variables containing * if it is sourrounded by "', () => {
        const property = { name: 'region', getter: (provider) => provider.getRegion() };

        awsProvider.options[property.name] = '${self:foobar, "*.a.thing"}';
        serverless.variables.populateService().should.be.fulfilled
          .then(() => expect(property.getter(awsProvider)).to.be.eql('*.a.thing'));
      });
  ...
  describe('#populateObject()', () => {
    ...
    describe('significant variable usage corner cases', () => {
      ...
      beforeEach(() => {
        service = makeDefault();
        // eslint-disable-next-line no-template-curly-in-string
        service.provider.variableSyntax = variableSyntax; // default

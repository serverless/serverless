...
describe('Variables', () => {
  ...
  describe('#getValueFromFile()', () => {
    ...
    it('should notify if unable to populate from a javascript file', () => {
      const serverless = new Serverless();
      const SUtils = new Utils();
      const tmpDirPath = testUtils.getTmpDirPath();
      const jsData = `module.exports.hello=function(){
      ...
      expect(() => serverless.variables.getValueFromFile('file(./hello.js):hello.one.two.invalid'))
        .throws(ServerlessError, 'variable syntax');
    });

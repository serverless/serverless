...
describe('Utils', () => {
  ...
  describe('#writeFile()', () => {
    ...
    it('should report failure if unable to write a file',
      () => serverless.utils.writeFile(null, null)
        .then(() => {
          expect.fail(0, 1, 'Did not reject the promise');
        })
        .catch((e) => {
  ...
  describe('#appendFileSync()', () => {
    ...
    it('should report failure if unable to append a line to a file',
      () => serverless.utils.appendFileSync(null, 'line')
        .then(() => {
          expect.fail(0, 1, 'Did not reject the promise');
        })
        .catch((e) => {
  ...
  describe('#readFile()', () => {
    ...
    it('should report failure if unable to read from file',
      () => serverless.utils.readFile(null)
        .then(() => {
          expect.fail(0, 1, 'Did not reject the promise');
        })
        .catch((e) => {
  ...
  describe('#logStat()', () => {
    ...
    beforeEach(() => {
      ...
      // Stub the fetch() function to return a valid response by default
      const promiseWithJson = BbPromise.resolve(
        { json: () => 'a json response' }
      );
      fetchStub.onFirstCall().returns(promiseWithJson);
    ...
    it('should send the gathered information', () => {
      serverless.service = serviceStub;
    ...
    // Test variations of `resources` section
    [
      {
        name: 'should not detect custom resources when no custom resources defined',
        resources: {},
        expectedHasCustomResources: false,
      },
      {
        name: 'should not detect custom resources when empty Resources defined',
        resources: {
          Resources: {},
        },
        expectedHasCustomResources: false,
      ...
      {
        name: 'should detect custom resources when non-empty Resources defined',
        resources: {
          Resources: {
            foo: 'bar',
          },
        },
        expectedHasCustomResources: true,
      },
      {
        name: 'should not detect custom resources when empty Outputs defined',
        resources: {
          Outputs: {},
        },
        expectedHasCustomResources: false,
      ...
      {
        name: 'should detect custom resources when non-empty Outputs defined',
        resources: {
          Outputs: {
            foo: 'bar',
          },
      ...
      it(testCase.name, () => {
        serverless.service = serviceStub;
        serverless.service.resources = testCase.resources;

        return utils.logStat(serverless).then(() => {
          expect(fetchStub.calledOnce).to.equal(true);

          const parsedBody = JSON.parse(fetchStub.args[0][1].body);
          expect(parsedBody.properties.service.hasCustomResourcesDefined)
            .to.equal(testCase.expectedHasCustomResources);
        });
      });
    });

    it('should tolerate a bad response', () => {
      const promiseWithBadResponse = BbPromise.resolve('a non-json response');
      fetchStub.onFirstCall().returns(promiseWithBadResponse);

      return utils.logStat(serverless)
        .catch(() => {
          expect.fail(0, 1, 'Did not catch the request failure');
        });
    });

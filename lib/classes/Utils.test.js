  ...
  describe('#logStat()', () => {
    ...
    it('should include serviceName when serverless.yml includes the service shorthand', () => {
      ...
      return expect(utils.logStat(serverless)).to.be.fulfilled.then(() => {
        ...
        const data = trackStub.args[0][0];
        expect(data.properties.service.serviceName).to.equal('my-service-shorthand');
      });
    });
    ...
    it('should include serviceName when serverless.yml includes the service name property', () => {
      ...
      return expect(utils.logStat(serverless)).to.be.fulfilled.then(() => {
        ...
        const data = trackStub.args[0][0];
        expect(data.properties.service.serviceName).to.equal('my-service-name');
      });
    });
    ...
    it('should send the gathered information', () => {
      ...
      return expect(utils.logStat(serverless)).to.be.fulfilled.then(() => {
        ...
        expect(data.properties.service.serviceName).to.equal('new-service');
        // functions property

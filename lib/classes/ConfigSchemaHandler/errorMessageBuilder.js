...
const addUserFriendlyMessage = errors => {
  return errors.map(error => {
    ...
    if (error.params && error.params.additionalProperty && isEventParam) {
      ...
      friendlyMessage = `Error: Unsupported parameter '${error.params.additionalProperty}' for '${eventName}' event`;
    }
    ...
    if (error.params && error.params && error.params.additionalProperty && isFunctionEvent) {
      friendlyMessage = `Error: Unsupported function event '${error.params.additionalProperty}'`;
    }
    if (friendlyMessage) {

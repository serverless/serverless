...
module.exports.buildErrorMessages = (ajvErrors, userConfig = {}) =>
  structureAjvErrors(ajvErrors).map(error => {
    switch (error.type) {
      case 'ajvError':
        return buildAjvErrorMessage(error, userConfig);
      case 'groupedError':
...
const buildGroupedErrorMessage = (groupedError, userConfig) => {
  ...
  const ajvErrorWithKeywordForObject = findErrorWithKeywordForObject(groupedError.errors);
  if (
    ajvErrorWithKeywordForObject &&
    groupedError.errors.find(err => err.keyword === 'anyOf') &&
    typeof _.get(userConfig, groupedError.dataPath.slice(1)) === 'object'
  ) {
    result = buildAjvErrorMessage(ajvErrorWithKeywordForObject);
  }

  const ajvErrorWithKeywordForString = findErrorWithKeywordForString(groupedError.errors);
  if (
    ajvErrorWithKeywordForString &&
    groupedError.errors.find(err => err.keyword === 'anyOf') &&
    typeof _.get(userConfig, groupedError.dataPath.slice(1)) === 'string'
  ) {
    result = buildAjvErrorMessage(ajvErrorWithKeywordForString, userConfig);
  }
...
const findErrorWithKeywordForString = ajvErrors =>
  ajvErrors.find(err => KEYWORDS_FOR_STRINGS_REGEX.test(err.keyword));
...
const findErrorWithKeywordForObject = ajvErrors =>
  ajvErrors.find(err => KEYWORDS_FOR_OBJECTS_REGEX.test(err.keyword));

 ...
 */
const getListOfErrorMessages = (ajvErrors, userConfig = {}) =>
  normalizeAjvErrors(ajvErrors).map(error => {
    switch (error.type) {
      case 'ajvError':
 ...
 *
 * Normalization is performed purely on dataPath property of AJV error. First, initial
 * errors are grouped by dataPath, which leads to creating groupedError, a
 * collection of AJV errors with same dataPath.
 *
 ...
 *
 */
const normalizeAjvErrors = ajvErrors => {

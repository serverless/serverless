 ...
 */
module.exports = (ajvErrors, userConfig = {}, schema = {}) =>
  groupAjvErrors(ajvErrors).map(error => {
    switch (error.type) {
      case 'ajvError':
        error.message = buildAjvErrorMessage(error, userConfig);
        break;
      case 'groupedError':
        error.message = buildGroupedErrorMessage(error, userConfig, schema);
        error.keyword = setGroupedErrorKeyword(error, userConfig);
        break;
      default:
...
const setGroupedErrorKeyword = (groupedError, userConfig) => {
  let result = 'unknown';
  ...
  if (
    ...
    result = ajvErrorsWithKeywordsForObject[0].keyword;
  }
  ...
  if (
    ...
    result = ajvErrorsWithKeywordsForString[0].keyword;
  }

  return result;
};

const buildGroupedErrorMessage = (groupedError, userConfig, schema = {}) => {
  ...
  if (incorrectEventIndentationErrorMessage(groupedError, userConfig, schema)) {
    result = incorrectEventIndentationErrorMessage(groupedError, userConfig, schema);
  }
 ...
 *       method: get
 */
const incorrectEventIndentationErrorMessage = (groupedError, userConfig, schema) => {
  if (
    ...
    const supportedEvents = getListSupportedEvents(schema);
    const unsupportedProperties = Object.keys(configUnderValidation)
      .filter(el => !supportedEvents.includes(el))
      .map(el => `'${el}'`);
    const propertyOrProrerties = unsupportedProperties.length > 1 ? 'properties' : 'property';
    return [
      `${getFormattedDataPath(groupedError.dataPath)} has improperly indented`,
      `${propertyOrProrerties}: ${unsupportedProperties.join(', ')}`,
    ].join(' ');
  }
...
const getListSupportedEvents = schema => {
  const eventSchemas = _.get(
    schema,
    "properties.functions.patternProperties['^[a-zA-Z0-9-_]+$'].properties.events.items.anyOf"
  );
  if (!Array.isArray(eventSchemas)) return [];
  return eventSchemas.map(obj => obj.required[0]).filter(el => el !== '__schemaWorkaround__');
};

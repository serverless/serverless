  ...
  describe('function event related errors', () => {
    ...
    it('should have proper message', () => {
      const expectedErrorMessages = [
        ...
        "functions.someFunc.events[5].http has improperly indented properties: 'path', 'method'",
      ];
      const validate = ajv.compile(schema);
      validate(userConfig);
      ajvErrors = validate.errors;
      expect(
        normalizeAjvErrors(ajvErrors, userConfig, schema).map(err => err.message)
      ).to.deep.equal(expectedErrorMessages);
    });
  });

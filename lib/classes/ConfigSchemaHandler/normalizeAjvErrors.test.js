...
describe('#normalizeAjvErrors', () => {
  ...
  describe('Reporting', () => {
    ...
    it('should report error for improper indentation when defining event', () =>
      expect(
        errors.some(error => {
          if (error.dataPath !== ".functions['foo'].events[1].http") return false;
          if (error.keyword !== 'anyOf') return false;
          error.isExpected = true;
          return true;
        })
      ).to.be.true);
    it('should report error in anyOf case, where two types are possible (string and object), and object was used', () =>
      expect(
        errors.some(error => {
    ...
    it('should report error for string pattern in anyOf case, where two types are possible (string and object), there is only one string type error, and invalid string was used', () =>
      expect(
        errors.some(error => {
    ...
    it('should report error for missing required property in anyOf case, where two types are possible (string and object), there is only one object type error, and object was used', () =>
      expect(
        errors.some(error => {

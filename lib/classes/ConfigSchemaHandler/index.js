...
class ConfigSchemaHandler {
  ...
  validateConfig(userConfig) {
    if (this.serverless.service.provider.name === 'aws') {
      // TODO: remove this after provider schema is fine-tuned
    ...
    if (messages.length) {
      if (this.serverless.service.configValidationMode === 'error') {
        ...
        this.serverless.cli.log(
          'Note: Configuration validation is a new experimental functionality',
          'Serverless',
          { color: 'grey' }
        );
        this.serverless.cli.log(
          '- If errors are influenced by an external plugin, enquiry at plugin repository so schema extensions are added (https://www.serverless.com/framework/docs/providers/aws/guide/plugins#extending-validation-schema)',
          'Serverless',
          { color: 'grey' }
        );
        this.serverless.cli.log(
          '- If errors seem invalid, please report at https://github.com/serverless/serverless/issues/new?template=bug_report.md',
          'Serverless',
          {
            color: 'grey',
          }
        ...
        this.serverless.cli.log(
          "- If you find this functionality not helpful, you may turn it off with `configValidationMode: 'off'` setting",
          'Serverless',
          { color: 'grey' }
        );
      }

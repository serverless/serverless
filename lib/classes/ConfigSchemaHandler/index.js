  ...
  handleErrorMessages(messages) {
    if (messages.length) {
      if (this.serverless.service.configValidationMode === 'error') {
        ...
        if (this.serverless.configurationInput.configValidationMode === undefined) {
          this.serverless.cli.log(' ');
          this.serverless._logDeprecation(
            'CONFIG_VALIDATION_MODE_DEFAULT',
            'Starting from next major Serverless will throw by default. Adapt to this' +
              'behavior now by adding configValidationMode: error to service configuration'
          );
          this.serverless.cli.log(' ');
        }
        this.serverless.cli.log(' ');

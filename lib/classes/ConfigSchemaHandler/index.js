...
class ConfigSchemaHandler {
  ...
  extendFunctionEventDefinition(providerName, name, configSchema) {
    if (this.serverless.service.provider.name !== providerName) {
      return;
    }

    const existingEventDefinitionIndex = this.schema.properties.functions.patternProperties[
      FUNCTION_NAME_PATTERN
    ].properties.events.items.anyOf.findIndex(
      eventDefinition => name === eventDefinition.required[0]
    );

    if (existingEventDefinitionIndex === -1) {
      return;
    }

    addPropertiesToSchema(
      this.schema.properties.functions.patternProperties[FUNCTION_NAME_PATTERN].properties.events
        .items.anyOf[existingEventDefinitionIndex].properties[name],
      configSchema
    );
  }

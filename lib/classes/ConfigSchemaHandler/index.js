...
class ConfigSchemaHandler {
  ...
  addPropertiesToSchema(subSchema, extension = { properties: {}, required: [] }) {
  ...
  defineProvider(name, options = {}) {
    ...
    if (options.provider) {
      try {
        this.addPropertiesToSchema(this.schema.properties.provider, options.provider);
      } catch (error) {
    ...
    if (!this.schema.properties.provider.properties.stage) {
      this.addPropertiesToSchema(this.schema.properties.provider, {
  ...
  defineCustomProperties(configSchemaParts) {
    try {
      this.addPropertiesToSchema(this.schema.properties.custom, configSchemaParts);
    } catch (error) {
      if (error instanceof PropertyCollisionError) {
        throw new ServerlessError(
  ...
  defineFunctionEventProperties(providerName, name, configSchema) {
    ...
    try {
      this.addPropertiesToSchema(definitionToUpdate, configSchema);
    } catch (error) {
      if (error instanceof PropertyCollisionError) {
        throw new ServerlessError(

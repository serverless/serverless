...
class ConfigSchemaHandler {
  ...
  defineFunctionEvent(providerName, name, configSchema) {
    ...
    const existingFunctionEvents = this.schema.properties.functions.patternProperties[
      FUNCTION_NAME_PATTERN
    ].properties.events.items.anyOf.map(functionEventSchema =>
      Object.keys(functionEventSchema.properties).pop()
    );

    if (existingFunctionEvents.includes(name)) {
      throw new this.serverless.classes.Error(
...
function addPropertiesToSchema(subSchema, extension = { properties: {}, required: [] }) {
  ...
  Object.keys(extension.properties).some(extensionPropertiesKey => {
    const isColliding = existingSubSchemaPropertiesKeys.includes(extensionPropertiesKey);
  ...
  if (collidingExtensionPropertyKey) {
    throw new this.serverless.classes.Error(
      `Property '${collidingExtensionPropertyKey}' already have a definition - this property might have already been defined by the Serverless framework or one other plugin`,
      'SCHEMA_COLLISION'
    );
  }

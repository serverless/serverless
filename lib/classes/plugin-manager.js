...
class PluginManager {
  ...
  async resolveServicePlugins(servicePlugs) {
    const reorderedServicePlugins = this.reorderServicePlugins(servicePlugs);
    const pluginsObject = this.parsePluginsObject(reorderedServicePlugins);
  ...
  reorderServicePlugins(servicePlugins) {
    // List of plugins to prioritize if they exist
    const priorityPlugins = ['serverless-esbuild', 'serverless-python-requirements'];

    // List of prioritized plugins that are in the service
    const prioritized = servicePlugins.filter((p) => priorityPlugins.includes(p));

    // List of the rest of the plugins that are in the service
    const rest = servicePlugins.filter((p) => !priorityPlugins.includes(p));

    // Reorder by return the prioritized plugins first, then the rest of the plugins

...
class PluginManager {
  ...
  addPlugin(Plugin) {
    const pluginUtils = {};
    Object.assign(pluginUtils, getPluginWriters(Plugin._serverlessExternalPluginName || Plugin.name));
  ...
  async loadAllPlugins(servicePlugins) {
    ...
    const loadModules = (await import('../plugins/index.js')).default;
    const plugins = await loadModules();

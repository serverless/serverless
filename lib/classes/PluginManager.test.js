...
describe('PluginManager', () => {
  ...
  describe('#loadVariableResolvers()', () => {
    it('should load the plugin variable getters', () => {
      ...
      expect(pluginManager.serverless.variables.variableResolvers[0].isDependentService).to.equal(
        undefined
      );
      expect(pluginManager.serverless.variables.variableResolvers[0].name).to.equal(undefined);
      expect(typeof pluginManager.serverless.variables.variableResolvers[0].method).to.equal(
        'function'
      );
    });
  });

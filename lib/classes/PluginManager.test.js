...
describe('PluginManager', () => {
  ...
  class Provider1PluginMock {
    constructor() {
      ...
      this.hooks = {
        ...
        '_system_:normalize': () => ++this.normalizeFired,
      };

      // used to test if the function was executed correctly
      this.deployedFunctions = 0;

      // used to test if the normalize event was relayed correctly
      this.normalizeFired = 0;
    }
  ...
  describe('#normalize()', () => {
    beforeEach(function () { // eslint-disable-line prefer-arrow-callback
      ...
      // this plugin is proiver neutral so it should should recieve all events
      pluginManager.addPlugin(SynchronousPluginMock);
    });
    it('should fire a _system_:normalize event', () => {
      pluginManager.normalize();
      expect(pluginManager.plugins[1].normalizeFired).to.equal(1);

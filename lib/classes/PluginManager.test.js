...
describe('PluginManager', () => {
  ...
  describe('#parsePluginsObject()', () => {
    it('should parse array object', () => {
      ...
      parsePluginsObject(servicePlugins, {
    ...
    const parsePluginsObject = (servicePlugins, expectedResult) => {
  ...
  describe('Plugin / Load local plugins', function () {
    ...
    it('should load plugins from custom folder', () => {
      ...
      //Had to use contructor.name because the class will be loaded via require and the reference will not match with SynchronousPluginMock
      expect(pluginManager.plugins).to.satisfy(plugins => plugins.some(plugin => plugin.constructor.name == 'SynchronousPluginMock'));
    });

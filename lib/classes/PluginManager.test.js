...
describe('PluginManager', () => {
  ...
  describe('#loadVariableResolvers()', () => {
    it('should load the plugin variable getters with short sytnax', () => {
      ...
      expect(pluginManager.serverless.variables.variableResolvers[0].name).to.equal(null);
      expect(typeof pluginManager.serverless.variables.variableResolvers[0].resolver).to.equal(
        'function'
      );
    });

    it('should load the plugin variable getters with long syntax', () => {
      class VariableGetterPlugin {
        getTestVariable() {
          return Promise.resolve('testVariable');
        }
    ...
    it('should validate the plugin variable getters function', () => {
      class VariableGetterPlugin {
        getTestVariable() {
          return Promise.resolve('testVariable');
        }
      ...
      expect(() => pluginManager.loadVariableResolvers(pluginInstance)).to.throw(
        "Custom variable resolver for test defined by VariableGetterPlugin specifies a resolver that isn't a function: not a function"
      );
    });

    it('should load the plugin variable getters name if necessary', () => {
      class VariableGetterPlugin {
        getTestVariable() {
          return Promise.resolve('testVariable');
        }
      ...
      expect(pluginManager.serverless.variables.variableResolvers[0].name).to.equal('test');
      expect(typeof pluginManager.serverless.variables.variableResolvers[0].resolver).to.equal(
        'function'
      );
    });

    it('should validate the plugin variable getters name if necessary', () => {
      class VariableGetterPlugin {
        getTestVariable() {
          return Promise.resolve('testVariable');
        }
      ...
      const pluginInstance = new VariableGetterPlugin();
      expect(() => pluginManager.loadVariableResolvers(pluginInstance)).to.throw(
        /Custom variable resolver for test defined by VariableGetterPlugin specifies isDisabledAtPrepopulation but doesn't provide a string for a name/
      );
    });
  });

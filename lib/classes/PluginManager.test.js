...
describe('PluginManager', () => {
  ...
  describe('#validateCommand()', () => {
    ...
    it('should throw error with invalid command', () => {
      expect(() => { pluginManager.validateCommand(['invalid-command']); })
        .to.throw(Error);
    });

    it('should not throw error with a valid command', () => {
      expect(() => { pluginManager.validateCommand(['bar']); })
        .not.to.throw(Error);
    });
  });
  ...
  describe('#validateOptions()', () => {
    it('should throw an error if a required option is not set', () => {
      ...
      }).to.throw(Error, /command requires.*Usage: baz usage text/);

      expect(() => {
        pluginManager.validateOptions(bar);
      }).to.throw(Error, 'command requires');
    });

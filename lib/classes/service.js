...
class Service {
  ...
  loadServiceFileParam() {
    ...
    // copy over the baseName override from provider config (allows variable resolution rather than top-level prop)
    if (this.provider.baseName) {
      this.baseNameOverride = this.provider.baseName;
    }

    this.camelCaseNormalization = configurationInput.camelCaseNormalization || false; // just to be explicit of default
  ...
  reloadServiceFileParam() {
    ...
    this.camelCaseNormalization = configurationInput.camelCaseNormalization || false;
    if (configurationInput.package) {
      this.package = configurationInput.package;
    }
  ...
  setFunctionNames(rawOptions) {
    ...
    Object.entries(this.functions).forEach(([functionName, functionObj]) => {
      ...
      if (!functionObj.name) {
        // TODO 9604 how do we allow for an override here while simultaneously supporting the func-specific 'stage' option??
        // this will attempt to strip out a redundant `-{stage}` if it would be introduced in this formatting... we may not want this, though
        const baseName = this.getBaseName(stageNameForFunction);
        const namePrefix = baseName.endsWith(stageNameForFunction)
          ? baseName
          : `${baseName}-${stageNameForFunction}`;
        this.functions[functionName].name = `${namePrefix}-${functionName}`;
  ...
  getShortServiceName() {
    return this.baseNameOverride || this.service;
  }

  getBaseName(stageOverride = null) {
    return this.baseNameOverride || `${this.service}-${stageOverride || this.provider.stage}`;
  }

...
class Service {
  ...
  loadServiceFileParam() {
    ...
    // populate the baseName which is used for generating Export Name keys for CF Outputs
    if (configurationInput.baseName) {
      this.baseNameOverride = configurationInput.baseName;
    }
  ...
  getBaseName(stageOverride = null) {
    const stageName = stageOverride || this.provider.stage;
    return this.baseNameOverride || `${this.service}-${stageName}`;
  }

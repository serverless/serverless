...
const awsArnRegularExpressions = require('../plugins/aws/utils/arnRegularExpressions');
          ...
          if (func.events) {
            ...
            func.events.forEach((event) => {
              ...
              if (event.http && event.http.authorizer) {
                ...
                if ((typeof event.http.authorizer === 'string'
                  && event.http.authorizer.toUpperCase() !== 'AWS_IAM'
                  && !awsArnRegularExpressions.cognitoIdpArnExpr.test(event.http.authorizer))
                  || event.http.authorizer.name
                  || (event.http.authorizer.arn
                    && awsArnRegularExpressions.lambdaArnExpr.test(event.http.authorizer.arn))) {
                  hasCustomAuthorizer = true;
                }
                if ((typeof event.http.authorizer === 'string'
                  && awsArnRegularExpressions.cognitoIdpArnExpr.test(event.http.authorizer))
                  || (event.http.authorizer.arn
                    && awsArnRegularExpressions.cognitoIdpArnExpr.test(event.http.authorizer.arn))) {

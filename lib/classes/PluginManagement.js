...
class PluginManagement {
  ...
  runCommand(command) {
    ...
    // TODO: this code needs to be a bit more complex to support async hooks using Promises
    hooks.forEach((hook) => {
      hook();
    });
  }
  ...
  _loadCorePlugins() {
    ...
    corePlugins.forEach((corePlugin) => {
      // remove the .js from the plugin name to get the name of the corresponding directory
      const pluginDirectoryName = corePlugin.slice(0, -3);

      const Plugin = require(path.join(pluginsDirectoryPath, pluginDirectoryName, corePlugin));
...
module.exports = PluginManagement;

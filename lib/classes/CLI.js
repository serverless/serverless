...
class CLI {
  ...
  displayHelp(processedInput) {
    ...
    // if only "--interactive-help" was entered
    if (commands.length === 0 && options['interactive-help']) {
  ...
  generateInteractiveCliHelp() {
    ...
    this.consoleLog(chalk.yellow(
			`Run just serverless (or shortcut sls) to initialize an interactive setup of functionalities
related to given service or current environment Pass --help-interactive for contextual
help on interactive CLI options.`));

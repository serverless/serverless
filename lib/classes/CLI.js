...
const configIndependentCommands = new Set(['config credentials']);
...
class CLI {
  ...
  isConfigIndependentRequest(processedInput) {
    const commands = processedInput.commands.join(' ');

    if (configIndependentCommands.has(commands)) return true;

...
class CLI {
  ...
  logBreakingChanges() {
    ...
    if (breakingChanges.length !== 0 && !process.env.SLS_IGNORE_WARNING) {
      this.consoleLog('');
      this.consoleLog(chalk.yellow('  WARNING: You are running v{currentVersion}. v{nextVersion} will include the following breaking changes:')); // eslint-disable-line max-len
      breakingChanges
        .forEach(breakingChange => this.consoleLog(chalk.yellow(`  - ${breakingChange}`)));
      this.consoleLog('');
      this.consoleLog(chalk.yellow('  You can opt-out from these warnings by setting the "SLS_IGNORE_WARNING=*" environment variable.')); // eslint-disable-line max-len
      this.consoleLog('');
    }
  }
}

...
class CLI {
  ...
  printError(e) {
    ...
    console.log(chalk.yellow(` ${errorType} ${line}`));

    if (e.name !== 'ServerlessError') console.log(' ');

    console.log(chalk.yellow(`     ${e.message}`));

    if (e.name !== 'ServerlessError') console.log(' ');

    if(process.env.SERVERLESS_DEBUG) {
    ...
    console.log(chalk.yellow('  Get Support --------------------------------------------'));
    console.log(`${chalk.yellow('     Docs:          ')}${chalk.white('https://git.io/voP4S')}`);
    console.log(`${chalk.yellow('     Bugs:          ')}${chalk.white('https://git.io/voP45')}`);

    if (e.name !== 'ServerlessError') console.log(' ');
    if (e.name !== 'ServerlessError') console.log(chalk.red('     Please report this error. We think it might be a bug.'));
    console.log(' ');
  }

...
class CLI {
  ...
  logBreakingChanges() {
    ...
    if (breakingChanges.length !== 0 && !process.env.SLS_IGNORE_WARNING) {
      ...
      breakingChanges.forEach(breakingChange => this.consoleLog(chalk.yellow(breakingChange)));
      this.consoleLog('');
      this.consoleLog('You can opt-out from these warnings by setting the SLS_IGNORE_WARNING env var.'); // eslint-disable-line max-len
      this.consoleLog('');
    }
  }
}

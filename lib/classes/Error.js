...
module.exports.logError = (e) => {
  try {
    ...
    // Note sentry not installed for users who have opted out of tracking
    if (errorReporter && errorReporter.installed) {
      errorReporter.captureException(e, (sendErr, eventId) => { // eslint-disable-line
        process.exit(1); // process.exit(1) for CI systems to correctly fail
      });
    } else {
      process.exit(1); // process.exit(1) for CI systems to correctly fail
    }

...
const denormalizeUserConfig = (userConfig, { removedValuesMap }) => {
  for (const removedValueData of removedValuesMap) {
    // #7747, cannot remove _.set here, as paths in `removedValueData.path` are complex
    // and deep, though these paths in `userConfig` always seem to exist during testing
    // this thread may be useful in future: https://stackoverflow.com/q/6491463/5343977

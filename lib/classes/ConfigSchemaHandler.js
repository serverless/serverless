...
class ConfigSchemaHandler {
  ...
  defineProvider(name, options = {}) {
    ...
    if (options.function && options.function.properties) {
...
function addPropertiesToSchema(subSchema, extenstion = { properties: {}, required: [] }) {
  subSchema.properties = Object.assign(subSchema.properties, extenstion.properties);

  if (!subSchema.required) subSchema.required = [];

  if (Array.isArray(extenstion.required)) subSchema.required.push(...extenstion.required);

  if (Object.prototype.hasOwnProperty.call(extenstion, 'additionalProperties')) {
    subSchema.additionalProperties = extenstion.additionalProperties;
  }
}

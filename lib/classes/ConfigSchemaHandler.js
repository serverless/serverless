...
class ConfigSchemaHandler {
  ...
  defineProvider(name, options = {}) {
    ...
    if (options.function) {
      this.schema.properties.functions.patternProperties[functionNamePattern] = _.merge(
  ...
  defineCustomProperties(configSchemaParts) {
    const subSchema = this.schema.properties.custom;
    subSchema.properties = Object.assign(subSchema.properties, configSchemaParts.properties);
    if (Array.isArray(configSchemaParts.required)) {
      subSchema.required.push(...configSchemaParts.required);
    }
  }

...
class Service {
  ...
  loadServiceFileParam(serviceFilename, serverlessFileParam) {
    // console.log('servictFilename', JSON.stringify(serviceFilename))
    // console.log('severlessFileParam', JSON.stringify(serverlessFileParam))
    const that = this;
    ...
    if (
      typeof this.plugins === 'string' &&
      new RegExp(this.provider.variableSyntax).test(this.plugins)
    ) {
      const message = [
        'Using variables in plugins section is not allowed. ',
        `Used ${this.plugins} in ${serviceFilename}`,
      ].join();
    ...
    if (Array.isArray(this.plugins)) {
      for (const plugin of this.plugins) {
        if (new RegExp(this.provider.variableSyntax).test(plugin)) {
          const message = [
            'Using variables in plugins section is not allowed. ',
            `Used ${plugin} in ${serviceFilename}`,
          ].join();
          throw new ServerlessError(message);
        }
      }

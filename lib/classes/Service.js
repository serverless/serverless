...
const debug = require('debug')('Service.js');
...
class Service {
  ...
  load(opts) {
    ...
    debug(`Loading Service with options: ${util.inspect(opts, false, null)}`);
    ...
    return that.serverless.yamlParser
      ...
      .then(() => {
        ...
        debug(`Populating variables with stage: ${this.defaults.stage}`);
        debug(`Populating variables with region: ${this.defaults.region}`);
        debug(`Populating variables with variable syntax: ${varTemplateSyntax}`);

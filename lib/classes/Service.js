...
class Service {
  ...
  validate() {
    ...
    this._listDuplicateHandlers().forEach(arg => {
  ...
  _listDuplicateHandlers() {
    ...
    return _.entriesIn(groupedByHandler)
      .filter(arg => arg[1].length >= 2)
      .map(arg => {
        const handlerName = arg[0];
        const functionGroup = arg[1];
        return [handlerName, functionGroup.map(func => func.name)];
      });
  }

        ...
        traverse(this).forEach(function (valParam) {
          ...
          if (typeof(val) === 'string' && val.match(varTemplateSyntax)) {
            ...
            val.match(varTemplateSyntax).forEach((variableSyntax) => {
              ...
              // Populate
              if (!value && !value !== '') {
                throw new that.serverless.classes.Error(`Variable "${variableName}" doesn't exist in serverless.env.yaml.`);
              } else if (typeof value === 'string') {
                if (variableString.split('.').length > 1) {
                  throw new that.serverless.classes.Error('Trying to access sub properties of a string variable');
                }
                ...
                if (typeof val === 'object') {
                  ...
                  subProperties.splice(0, 1); // remove first element. It's the variableName
                  subProperties.forEach(subProperty => {
                    if (!val[subProperty]) {
                      throw new that.serverless.classes.Error(`Invalid sub property for variable "${variableName}"`);
                    }
                    val = val[subProperty];
                  });
                } else if (variableString.split('.').length > 1) {
                  throw new that.serverless.classes.Error('Trying to access sub properties of a non-object variable');
                }
              }
            });

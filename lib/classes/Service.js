              ...
              if (!value && !value !== '') {
                ...
                if (variableString.split('.').length > 1) {
                  const errorMessage = [
                    ...
                    ` is an object, otherwise you can't use the dot notation for that variable in serverless.yaml`
                  ].join('');
                  throw new that.serverless.classes
                    .Error(errorMessage);
                }
                ...
                if (typeof value === 'object') {
                  ...
                  subProperties.forEach(subProperty => {
                    if (!value[subProperty]) {
                      const errorMessage = [
                        ...
                        ` Please make sure the variable is the intended object in serverless.env.yaml,`,
                  ...
                  if (typeof value === 'string') {
                    ...
                    if (val !== variableSyntax) {
                      const errorMessage = [
                        `Trying to populate non string variables into a string for variable "${variableName}".`,
                  ...
                  const errorMessage = [
                    ...
                    ` otherwise, you can't use the dot notation for that variable in serverless.yaml`
                  ].join('');
                  throw new that.serverless.classes
                    .Error(errorMessage);
                } else {

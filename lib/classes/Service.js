...
class Service {
  ...
  load(opts) {
    ...
    let serverlessYamlPath = path.join(servicePath, 'serverless.yml');
    // change to serverless.yaml if the file could not be found
    if (!this.serverless.utils.fileExistsSync(serverlessYamlPath)) {
      serverlessYamlPath = path
        .join(this.serverless.config.servicePath, 'serverless.yaml');
    }

    let serverlessEnvYamlPath = path.join(servicePath, 'serverless.env.yml');
    // change to serverless.env.yaml if the file could not be found
    if (!this.serverless.utils.fileExistsSync(serverlessEnvYamlPath)) {
      serverlessEnvYamlPath = path
        .join(this.serverless.config.servicePath, 'serverless.env.yaml');
    }

    return that.serverless.yamlParser
      .parse(serverlessYamlPath)
      .then((serverlessYamlParam) => {
        ...
        if (serverlessYaml.package && serverlessYaml.package.artifact) {
          that.package.artifact = serverlessYaml.package.artifact;
        }
        if (serverlessYaml.package && serverlessYaml.package.exclude) {
          that.package.exclude = serverlessYaml.package.exclude;
        }
        if (serverlessYaml.package && serverlessYaml.package.include) {
          that.package.include = serverlessYaml.package.include;
        }

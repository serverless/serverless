    ...
    return that.serverless.yamlParser
      ...
      .then((serverlessFileParam) => {
        ...
        _.forEach(that.functions, (functionObj, functionName) => {
          const targetFunction = that.functions[functionName];
          if (!functionObj.events) {
            targetFunction.events = [];
          }
          ...
          // Translate HTTP events written in string notation to object notation
          _.forEach(functionObj.events, (event, index) => {
            if (event.http && typeof event.http === 'string') {

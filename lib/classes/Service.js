...
class Service {
  ...
  load(rawOptions) {
    ...
    return BbPromise.all([
      serverlessConfigFileUtils.getServerlessConfigFilePath(
        this.serverless.processedInput.options.config, this.serverless.config.servicePath),
      serverlessConfigFileUtils.getServerlessConfigFile(
        this.serverless.processedInput.options.config, this.serverless.config.servicePath),
  ...
  loadServiceFileParam([serviceFilename, serverlessFileParam]) {
    ...
    if (_.isObject(serverlessFile.service) && !serverlessFile.service.name) {
      throw new ServerlessError(`"service" is missing the "name" property in ${this.serviceFilename}`);   // eslint-disable-line max-len
    }
